...
{
public:
    vsomeip_sender(bool _use_tcp, const std::vector<vsomeip::byte_t>& _user_message,
          ...
          wait_for_answer_(true)
    {
        ...
        message_type_ =
            static_cast<vsomeip::message_type_e>(user_message_[VSOMEIP_MESSAGE_TYPE_POS]);
        ...
        app_->register_state_handler(
            std::bind(&vsomeip_sender::on_state, this, std::placeholders::_1));
                         ...
                         bool _is_available)
    {
        if (_is_available)
        {
            VSOMEIP_INFO << "Service [" << std::setw(4) << std::setfill('0') << std::hex << _service
    ...
    {
        VSOMEIP_INFO << "Received a response from Service [" << std::setfill('0') << std::hex
                     << std::setw(4) << _response->get_service() << "." << std::setw(4)
                     << _response->get_instance() << "]:";
        VSOMEIP_INFO << "########## begin message";
        VSOMEIP_INFO << std::hex << std::setw(4) << std::setfill('0') << _response->get_service()
                     << std::hex << std::setw(4) << std::setfill('0') << _response->get_method()
                     << " # service id / instance id";
        VSOMEIP_INFO << std::hex << std::setw(8) << std::setfill('0') << _response->get_length()
                     << " # length";
        VSOMEIP_INFO << std::hex << std::setw(4) << std::setfill('0') << _response->get_client()
                     << std::hex << std::setw(4) << std::setfill('0') << _response->get_session()
                     << " # client id / session id";
        VSOMEIP_INFO << std::hex << std::setw(2) << std::setfill('0')
                     << static_cast<std::uint16_t>(_response->get_protocol_version()) << std::hex
                     << std::setw(2) << std::setfill('0')
                     << static_cast<std::uint16_t>(_response->get_interface_version()) << std::hex
                     << std::setw(2) << std::setfill('0')
                     << static_cast<std::uint16_t>(_response->get_message_type()) << std::hex
                     << std::setw(2) << std::setfill('0')
    ...
    void send()
    {
        ...
        while (wait_service_available_)
        {
            if (std::cv_status::timeout == condition_.wait_for(its_lock, std::chrono::seconds(6)))
            {
                VSOMEIP_INFO << "Service [" << std::setw(4) << std::setfill('0') << std::hex
                             << service_id_ << "." << instance_ << "] isn't available. Exiting";
                stop(EXIT_FAILURE);
            }
        }
        ...
        its_payload->set_data(
            &user_message_[VSOMEIP_PAYLOAD_POS],
...
private:
    bool validate_message()
    {
        ...
        {
            VSOMEIP_ERROR << "Invalid message type 0x" << std::setw(2) << std::setfill('0')
                          << std::hex << static_cast<std::uint8_t>(message_type_) << ", exiting.";
            stop(EXIT_FAILURE);
        }

        if (!check_return_code())
        {
            VSOMEIP_ERROR << "Invalid return code 0x" << std::setw(2) << std::setfill('0')
                          << std::hex << static_cast<std::uint8_t>(return_code_) << ", exiting.";
            stop(EXIT_FAILURE);
        }
        ...
        {
            VSOMEIP_ERROR << "Provided length 0x" << std::setw(8) << std::setfill('0') << std::hex
                          << length_ << " doesn't match message size.";
            VSOMEIP_ERROR << "Assuming the same payload the length field should"
                             " be set to 0x"
                          << std::setw(8) << std::setfill('0') << std::hex
                          << user_message_.size() - 8 << " , exiting.";
            stop(EXIT_FAILURE);
        }

        if (use_tcp_ && user_message_.size() > VSOMEIP_MAX_TCP_MESSAGE_SIZE)
        {
            VSOMEIP_WARNING << "Max allowed message size for TCP is " << std::dec
        ...
        {
            VSOMEIP_WARNING << "Max allowed message size for UDP is " << std::dec
    ...
    bool check_return_code()
    {
        ...
        {
            VSOMEIP_ERROR << "Provided return code 0x" << std::setw(2) << std::setfill('0')
                          << std::hex << static_cast<std::uint8_t>(return_code_)
                          << " is out of range.";
            return false;
        }
        ...
        {
            VSOMEIP_ERROR << "Provided return code 0x" << std::setw(2) << std::setfill('0')
                          << std::hex << static_cast<std::uint8_t>(return_code_) << "is reserved.";
            return false;
        }
        switch (message_type_)
        {
        ...
        case vsomeip::message_type_e::MT_NOTIFICATION:
            if (return_code_ != vsomeip::return_code_e::E_OK)
            {
                VSOMEIP_ERROR << "Provided return code 0x" << std::setw(2) << std::setfill('0')
                              << std::hex << static_cast<std::uint8_t>(return_code_)
                              << "is invalid in combination with message type 0x" << std::setw(2)
...
int main(int argc, char** argv)
{
    if (argc < 2)
    {
        std::cerr << "To few arguments, please see the help with : " << argv[0] << " --help"
                  << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else if (arg == "--message" || arg == "-m")
        {
            ...
            for (unsigned int i = 0; i < message.length(); i += 2)
            {
                ...
                {
                    std::cerr << e.what() << ": Couldn't convert '" << message.substr(i, 2)
                              << "' to hex, exiting: " << std::endl;
                    exit(EXIT_FAILURE);
                }
        ...
        else if (arg == "--instance" || arg == "-i")
        {
            ...
            for (unsigned int i = 0; i < instance_str.length(); i += 2)
            {
                ...
                {
                    std::cerr << e.what() << ": Couldn't convert '" << instance_str.substr(i, 2)
                              << "' to hex, exiting: " << std::endl;
                    exit(EXIT_FAILURE);
                }
            }
