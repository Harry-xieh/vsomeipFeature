...
{
    {
        ...
        client_id_timer_.expires_from_now(
            std::chrono::milliseconds(VSOMEIP_MAX_CONNECT_TIMEOUT * 2));
        client_id_timer_.async_wait(
            std::bind(&routing_manager_stub::on_client_id_timer_expired,
...
void routing_manager_stub::stop()
{
    ...
    if (!is_socket_activated_)
    {
        ...
        if (is_local_routing)
        {
            ...
            its_endpoint_path << utility::get_base_path(configuration_->get_network()) << std::hex
                                      ...
                                      std::uint16_t                   _remote_port)
{
    ...
    {
        VSOMEIP_ERROR << __func__ << ": deserialization of command and client identifier failed ("
        ...
        && _bound_client != its_client)
    {
        VSOMEIP_WARNING << "vSomeIP Security: routing_manager_stub::on_message: "
                        << "Routing Manager received a message from client " << std::hex
                        << std::setw(4) << std::setfill('0') << its_client << " with command "
                        << (uint32_t)its_id << " which doesn't match the bound client "
    ...
    case protocol::id_e::REGISTER_APPLICATION_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
            update_registration(its_command.get_client(), registration_type_e::REGISTER,
                                _remote_address, its_command.get_port());
        else
            VSOMEIP_ERROR << __func__ << ": deserializing register application failed (" << std::dec
    ...
    case protocol::id_e::DEREGISTER_APPLICATION_ID: {
        ...
        its_command.deserialize(its_buffer, its_error);
        if (its_error == protocol::error_e::ERROR_OK)
            update_registration(its_command.get_client(), registration_type_e::DEREGISTER,
    ...
    case protocol::id_e::PONG_ID: {
        ...
        {
            on_pong(its_client);
            VSOMEIP_TRACE << "PONG(" << std::hex << std::setw(4) << std::setfill('0') << its_client
                          << ")";
        }
        else
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            else
            {
                VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << its_client
                                ...
                                << "the following service/instance " << its_service << "/"
                                << its_instance << " ~> Skip offer!";
            }
        }
        else
            VSOMEIP_ERROR << __func__ << ": deserializing offer service failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
        break;
    }
    ...
    case protocol::id_e::STOP_OFFER_SERVICE_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            VSOMEIP_ERROR << __func__ << ": deserializing stop offer service failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
        break;
    }
    ...
    case protocol::id_e::SUBSCRIBE_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            VSOMEIP_ERROR << __func__ << ": deserializing subscribe failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
        break;
    }
    ...
    case protocol::id_e::UNSUBSCRIBE_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            host_->unsubscribe(its_client, _sec_client, its_service, its_instance, its_eventgroup,
                               its_notifier);
        }
        else
            VSOMEIP_ERROR << __func__ << ": deserializing unsubscribe failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
        break;
    }
    ...
    case protocol::id_e::SUBSCRIBE_ACK_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            host_->on_subscribe_ack(its_subscriber, its_service, its_instance, its_eventgroup,
                                    its_notifier, its_subscription_id);
            ...
            VSOMEIP_ERROR << __func__ << ": deserializing subscribe ack failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
    ...
    case protocol::id_e::SUBSCRIBE_NACK_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            VSOMEIP_ERROR << __func__ << ": deserializing subscribe nack failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
    ...
    case protocol::id_e::UNSUBSCRIBE_ACK_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            host_->on_unsubscribe_ack(its_client, its_service, its_instance, its_eventgroup,
                                      its_subscription_id);

            VSOMEIP_INFO << "UNSUBSCRIBE ACK(" << std::hex << std::setfill('0') << std::setw(4)
                         << its_client << "): [" << std::setw(4) << its_service << "."
                         << std::setw(4) << its_instance << "." << std::setw(4) << its_eventgroup
                         << "]";
        }
        else
            VSOMEIP_ERROR << __func__ << ": deserializing unsubscribe ack failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
        break;
    }
    ...
    case protocol::id_e::SEND_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            if (its_message_data.size() > VSOMEIP_MESSAGE_TYPE_POS)
            {
                ...
                uint32_t its_contained_size =
                    bithelper::read_uint32_be(&its_message_data[VSOMEIP_LENGTH_POS_MIN]);
                if (its_message_data.size() != its_contained_size + VSOMEIP_SOMEIP_HEADER_SIZE)
                {
                    VSOMEIP_WARNING
                        << "Received a SEND command containing message with invalid size -> skip!";
                    break;
                }
                host_->on_message(its_service, its_instance, &its_message_data[0],
                                  length_t(its_message_data.size()), is_reliable, _bound_client,
                                  _sec_client, its_check_status, false);
            }
        }
        break;
    }
    ...
    case protocol::id_e::NOTIFY_ONE_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            if (its_message_data.size() > VSOMEIP_MESSAGE_TYPE_POS)
            {
                ...
                {
                    VSOMEIP_WARNING
                        << "Received a NOTIFY command containing message with invalid size -> skip!";
                    break;
                }

                host_->on_notification(its_client, its_service, its_instance, &its_message_data[0],
                                       length_t(its_message_data.size()),
    ...
    case protocol::id_e::REQUEST_SERVICE_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            {
                if (VSOMEIP_SEC_OK
                    == configuration_->get_security()->is_client_allowed_to_request(
            ...
            VSOMEIP_ERROR << __func__ << ": request service deserialization failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
    ...
    case protocol::id_e::RELEASE_SERVICE_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            host_->release_service(its_command.get_client(), its_command.get_service(),
                                   its_command.get_instance());
        }
        else
            VSOMEIP_ERROR << __func__ << ": release service deserialization failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
        break;
    }
    ...
    case protocol::id_e::REGISTER_EVENT_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            for (std::size_t i = 0; i < its_command.get_num_registrations(); i++)
            {
                ...
                host_->register_shadow_event(
                    its_client, its_service, its_instance, register_event.get_event(),
                    register_event.get_eventgroups(), register_event.get_event_type(),
                    register_event.get_reliability(), register_event.is_provided(),
                    register_event.is_cyclic());

                VSOMEIP_INFO << "REGISTER EVENT(" << std::hex << std::setfill('0') << std::setw(4)
                             << its_client << "): [" << std::setw(4) << its_service << "."
                             << std::setw(4) << its_instance << "." << std::setw(4)
                             << register_event.get_event() << ":eventtype=" << std::dec
                             << (int)register_event.get_event_type()
                             << ":is_provided=" << std::boolalpha << register_event.is_provided()
                             << ":reliable=" << (int)register_event.get_reliability() << "]";
            }
        }
        else
            VSOMEIP_ERROR << __func__ << ": register event deserialization failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
        break;
    }
    ...
    case protocol::id_e::UNREGISTER_EVENT_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            host_->unregister_shadow_event(its_command.get_client(), its_command.get_service(),
                                           its_command.get_instance(), its_command.get_event(),
                                           its_command.is_provided());

            VSOMEIP_INFO << "UNREGISTER EVENT(" << std::hex << std::setfill('0') << std::setw(4)
                         << its_command.get_client() << "): [" << std::setw(4)
                         << its_command.get_service() << "." << std::setw(4)
                         << its_command.get_instance() << "." << std::setw(4)
                         << its_command.get_event() << ":is_provider=" << std::boolalpha
                         << its_command.is_provided() << "]";
        }
        else
            VSOMEIP_ERROR << __func__ << ": unregister event deserialization failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
        break;
    }
    ...
    case protocol::id_e::REGISTERED_ACK_ID: {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            VSOMEIP_INFO << "REGISTERED_ACK(" << std::hex << std::setw(4) << std::setfill('0')
                         << its_command.get_client() << ")";
        }
        else
            VSOMEIP_ERROR << __func__ << ": registered ack deserialization failed (" << std::dec
                          << static_cast<int>(its_error) << ")";
        break;
    }
    ...
    default:
        VSOMEIP_WARNING << __func__ << ": Received an unhandled command (" << std::dec
                        << static_cast<int>(its_id) << ")";
    }
}
...
void routing_manager_stub::on_register_application(client_t _client)
{
    ...
    else
    {
        ...
        if (configuration_->is_local_routing())
        {
            ...
            bool has_mapping =
                configuration_->get_policy_manager()->get_client_to_sec_client_mapping(
...
{
    std::vector<std::tuple<service_t, instance_t, major_version_t, minor_version_t>>
        services_to_report;
    {
        std::lock_guard<std::mutex> its_lock(routing_info_mutex_);
            ...
            for (const auto& its_service : its_info->second.second)
            {
                for (const auto& its_instance : its_service.second)
                {
                    ...
                    services_to_report.push_back(
                        std::make_tuple(its_service.first, its_instance.first, its_version.first,
                                        its_version.second));
                }
            }
        }
    ...
    for (const auto& s : services_to_report)
    {
        ...
        host_->on_stop_offer_service(_client, std::get<0>(s), std::get<1>(s), std::get<2>(s),
                                     std::get<3>(s));
    }
}
        ...
        if (found_client.first != VSOMEIP_ROUTING_CLIENT)
        {
            for (const auto& s : found_client.second.second)
            {
                for (const auto& i : s.second)
                {
                    ...
                    bool has_port =
                        (its_reliable_port != ILLEGAL_PORT || its_unreliable_port != ILLEGAL_PORT);

                    if (_offer_type == offer_type_e::OT_ALL
                        || (_offer_type == offer_type_e::OT_LOCAL && !has_port)
                        || (_offer_type == offer_type_e::OT_REMOTE && has_port))
                    {
                        protocol::service its_service(s.first, i.first, i.second.first,
                                                      i.second.second);
...
void routing_manager_stub::client_registration_func(void)
{
    ...
    {
        std::stringstream s;
        s << std::hex << std::setw(4) << std::setfill('0') << host_->get_client() << "_client_reg";
                ...
                {
                    {
                        ...
                        if (find_connections != connection_matrix_.end())
                        {
                            ...
                            {
                                if (its_client != r.first && its_client != VSOMEIP_ROUTING_CLIENT
                                    && its_client != get_client())
                                {
                                    protocol::routing_info_entry its_entry;
                                    its_entry.set_type(
                                        protocol::routing_info_entry_type_e::RIE_DELETE_CLIENT);
                                    its_entry.set_client(r.first);
...
void routing_manager_stub::init_routing_endpoint()
{
...
#else
    {
        ...
        {
            VSOMEIP_WARNING
                << "Routing root creating (partially) failed. Please check your configuration.";
        }
#if defined(__linux__) || defined(ANDROID)
    }
    else
    {
        ...
        local_link_connector_ =
            std::make_shared<netlink_connector>(io_, its_host_address, boost::asio::ip::address(),
                                                false); // routing host doesn't need link up
        if (local_link_connector_)
        {
            local_link_connector_->register_net_if_changes_handler(
                std::bind(&routing_manager_stub::on_net_state_change, this, std::placeholders::_1,
...
void routing_manager_stub::on_net_state_change(bool _is_interface, const std::string& _name,
                                               bool _is_available)
{
    VSOMEIP_INFO << __func__ << "(" << std::hex << std::this_thread::get_id()
...
void routing_manager_stub::on_offer_service(client_t _client, service_t _service,
...
void routing_manager_stub::on_stop_offer_service(client_t _client, service_t _service,
                                                 instance_t _instance, major_version_t _major,
                                                 minor_version_t _minor)
{
    std::lock_guard<std::mutex> its_guard(routing_info_mutex_);
        ...
        if (found_service != found_client->second.second.end())
        {
            ...
            if (found_instance != found_service->second.end())
            {
                ...
                if (_major == found_version.first && _minor == found_version.second)
                {
                    ...
                    inform_requesters(
                        _client, _service, _instance, _major, _minor,
                                                   ...
                                                   std::set<std::pair<uid_t, gid_t>>& _credentials)
{
    ...
    if (its_endpoint)
    {
        ...
        VSOMEIP_ERROR << __func__ << ": Sending credentials to client [" << std::hex << std::setw(4)
                      << std::setfill('0') << _target << "] failed";
}
...
void routing_manager_stub::send_client_routing_info(
    const client_t _target, std::vector<protocol::routing_info_entry>&& _entries)
{
    ...
    if (its_target_endpoint)
    {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            VSOMEIP_ERROR << __func__ << ": routing info command serialization failed ("
                          << static_cast<int>(its_error) << ")";
    }
    else
        VSOMEIP_ERROR << __func__ << ": Sending routing info to client [" << std::hex
                      << std::setw(4) << std::setfill('0') << _target << "] failed";
}

void routing_manager_stub::distribute_credentials(client_t _hoster, service_t _service,
                                                  instance_t _instance)
{
    ...
    vsomeip_sec_client_t its_sec_client;
    if (configuration_->get_policy_manager()->get_client_to_sec_client_mapping(_hoster,
                                                                               its_sec_client))
    {
...
void routing_manager_stub::inform_requesters(client_t _hoster, service_t _service,
                                             instance_t _instance, major_version_t _major,
                                             minor_version_t                     _minor,
                                             protocol::routing_info_entry_type_e _type,
                                          ...
                                          remote_subscription_id_t                       _id)
{
    ...
    {
        VSOMEIP_WARNING << __func__ << " Couldn't send subscription to local client [" << std::hex
                        << std::setfill('0') << std::setw(4) << _service << "." << std::setw(4)
                        << _instance << "." << std::setw(4) << _eventgroup << "." << std::setw(4)
                        << _event << "]"
...
bool routing_manager_stub::send_unsubscribe(const std::shared_ptr<endpoint>& _target,
                                            client_t _client, service_t _service,
                                            instance_t _instance, eventgroup_t _eventgroup,
                                            event_t _event, remote_subscription_id_t _id)
{
    ...
    if (_target)
    {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            VSOMEIP_ERROR << __func__ << ": unsubscribe command serialization failed (" << std::dec
                          << int(its_error) << ")";
    }
    else
    {
        VSOMEIP_WARNING << __func__ << " Couldn't send unsubscription to local client [" << std::hex
                        << std::setfill('0') << std::setw(4) << _service << "." << std::setw(4)
                        << _instance << "." << std::setw(4) << _eventgroup << "." << std::setw(4)
                        << _event << "]"
                        << " subscriber: " << std::setw(4) << _client;
...
bool routing_manager_stub::send_expired_subscription(const std::shared_ptr<endpoint>& _target,
                                                     client_t _client, service_t _service,
                                                     instance_t _instance, eventgroup_t _eventgroup,
                                                     event_t _event, remote_subscription_id_t _id)
{
                                              ...
                                              event_t _event)
{
    ...
    if (its_target)
    {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            VSOMEIP_ERROR << __func__ << ": subscribe ack command serialization failed ("
...
bool routing_manager_stub::contained_in_routing_info(client_t _client, service_t _service,
                                                     instance_t _instance, major_version_t _major,
        ...
        std::list<client_t> lost;
        {
            ...
            for (const auto& i : routing_info_)
            {
                if (i.first > 0 && i.first != host_->get_client())
                {
                    ...
                    {
                        VSOMEIP_WARNING << "Lost contact to application " << std::hex
                                        << (int)i.first;
...
#ifdef __linux__
    else if (!configuration_->get_policy_manager()->check_credentials(get_client(),
                                                                      host_->get_sec_client()))
    {
...
#endif
    local_receiver_ = std::static_pointer_cast<endpoint_manager_base>(host_->get_endpoint_manager())
                          ->create_local_server(shared_from_this());
        ...
        else
        {
            ...
            if (ec)
            {
                VSOMEIP_ERROR << "routing_manager_stub::send_ping setting "
                                 "expiry time of timer failed: "
                              << ec.message();
            }
            pinged_clients_timer_.async_wait(std::bind(&routing_manager_stub::on_ping_timer_expired,
...
void routing_manager_stub::on_ping_timer_expired(boost::system::error_code const& _error)
{
    ...
    if (pinged_clients_remaining)
    {
        ...
        pinged_clients_timer_.async_wait(
            std::bind(&routing_manager_stub::on_ping_timer_expired, this, std::placeholders::_1));
    }
}
...
void routing_manager_stub::update_registration(client_t _client, registration_type_e _type,
...
void routing_manager_stub::handle_credentials(const client_t               _client,
                                              std::set<protocol::service>& _requests)
{
    if (!_requests.size())
    {
        return;
    ...
    vsomeip_sec_client_t              its_requester_sec_client;
    if (configuration_->get_policy_manager()->get_client_to_sec_client_mapping(
            _client, its_requester_sec_client))
    {
        ...
        for (auto its_offering_client : its_offering_clients)
        {
            vsomeip_sec_client_t its_sec_client;
            if (configuration_->get_policy_manager()->get_client_to_sec_client_mapping(
                    its_offering_client, its_sec_client))
            {
                if (its_sec_client.port == VSOMEIP_SEC_PORT_UNUSED
                    && !utility::compare(its_sec_client, its_requester_sec_client))
                {
                    its_credentials.insert(
                        std::make_pair(its_sec_client.user, its_sec_client.group));
                }
            }
        }
                                           ...
                                           std::set<protocol::service>& _requests)
{
    ...
    for (auto request : _requests)
    {
        ...
        {
            std::set<client_t> its_clients =
                host_->find_local_clients(request.service_, request.instance_);
            ...
            for (const client_t c : its_clients)
            {
                ...
                if (_client != VSOMEIP_ROUTING_CLIENT && _client != host_->get_client())
                {
                    ...
                    if (found_client != routing_info_.end())
                    {
                        const auto found_service =
                            found_client->second.second.find(request.service_);
                        if (found_service != found_client->second.second.end())
                        {
                    ...
                    if (found_instance != found_service->second.end())
                    {
                        if (c != VSOMEIP_ROUTING_CLIENT && c != host_->get_client())
                        {
                            if (!is_connected(c, _client))
                            {
                                ...
                                protocol::routing_info_entry its_entry;
                                its_entry.set_type(
                                    protocol::routing_info_entry_type_e::RIE_ADD_CLIENT);
                                its_entry.set_client(_client);
                                if (host_->get_guest(_client, its_address, its_port))
                                {
                                    its_entry.set_address(its_address);
...
void routing_manager_stub::on_client_id_timer_expired(boost::system::error_code const& _error)
{
    ...
    {
        VSOMEIP_WARNING << "Releasing client identifier " << std::hex << std::setw(4)
                        << std::setfill('0') << client << ". "
...
{
    {
        ...
        {
            VSOMEIP_INFO << __func__ << " Policy for UID: " << std::dec << _uid
...
bool routing_manager_stub::send_update_security_policy_request(
    client_t _client, pending_security_update_id_t _update_id, uid_t _uid,
    const std::shared_ptr<payload>& _payload)
{
    ...
    if (its_endpoint)
    {
        ...
        std::uint32_t its_size =
            uint32_t(sizeof(pending_security_update_id_t) + _payload->get_length());
...
bool routing_manager_stub::send_cached_security_policies(client_t _client)
{
    ...
    if (its_endpoint)
    {
        ...
        {
            VSOMEIP_INFO << __func__ << " Distributing [" << std::dec
                         << updated_security_policies_.size()
                         << "] security policy updates to registering client: " << std::hex
                         << _client;
            ...
            VSOMEIP_ERROR << __func__ << ": serializing distribute security policies ("
                          << static_cast<int>(its_error) << ")";
        }
    }
    else
...
bool routing_manager_stub::send_remove_security_policy_request(
    client_t _client, pending_security_update_id_t _update_id, uid_t _uid, gid_t _gid)
{
    ...
    if (its_error == protocol::error_e::ERROR_OK)
    {
        ...
        if (its_endpoint)
            ...
            VSOMEIP_ERROR << __func__ << ": cannot find local client endpoint for client "
                          << std::hex << std::setw(4) << std::setfill('0') << _client;
    }
    else
        VSOMEIP_ERROR << __func__ << ": remove security policy command serialization failed ("
                      << std::dec << static_cast<int>(its_error) << ")";

    return false;
}

bool routing_manager_stub::add_requester_policies(
    uid_t _uid, gid_t _gid, const std::set<std::shared_ptr<policy>>& _policies)
{
...
void routing_manager_stub::get_requester_policies(
    uid_t _uid, gid_t _gid, std::set<std::shared_ptr<policy>>& _policies) const
{
    std::lock_guard<std::mutex> its_lock(requester_policies_mutex_);
    auto                        found_uid = requester_policies_.find(_uid);
    if (found_uid != requester_policies_.end())
...
void routing_manager_stub::add_pending_security_update_handler(
...
void routing_manager_stub::add_pending_security_update_timer(pending_security_update_id_t _id)
{
    ...
    if (!ec)
    {
        its_timer->async_wait(std::bind(&routing_manager_stub::on_security_update_timeout,
                                        shared_from_this(), std::placeholders::_1, _id, its_timer));
    }
    else
    {
        VSOMEIP_ERROR << __func__ << "[" << std::dec << _id
                      << "]: timer creation: " << ec.message();
    }
    std::lock_guard<std::mutex> its_lock(security_update_timers_mutex_);
    security_update_timers_[_id] = its_timer;
}

bool routing_manager_stub::send_requester_policies(
    ...
    for (const auto& p : _policies)
    {
        ...
        if (p->serialize(its_policy_data))
        {
            ...
            uint32_t its_policy_size =
                static_cast<uint32_t>(its_policy_data.size() + sizeof(uint32_t));

            uint8_t new_its_policy_size[4] = {0};
            bithelper::write_uint32_le(its_policy_size, new_its_policy_size);
            its_message.insert(its_message.end(), new_its_policy_size,
                               new_its_policy_size + sizeof(new_its_policy_size));
            ...
            its_message.insert(its_message.end(), new_its_policy_id,
                               new_its_policy_id + sizeof(new_its_policy_id));
...
void routing_manager_stub::on_security_update_timeout(
    const boost::system::error_code& _error, pending_security_update_id_t _id,
    std::shared_ptr<boost::asio::steady_timer> _timer)
{
    ...
    }
    {
        ...
        if (!its_missing_clients.empty())
        {
            for (auto its_client : its_missing_clients)
            {
                VSOMEIP_INFO << __func__ << ": Client 0x" << std::hex << its_client
                             << " did not respond to the policy update / removal with ID: 0x"
                             << std::hex << _id;
                if (!host_->find_local(its_client))
                {
                    VSOMEIP_INFO
                        << __func__ << ": Client 0x" << std::hex << its_client
        ...
        {
            VSOMEIP_WARNING << __func__
                            << ": Callback not found for security update / removal with ID: 0x"
                            << std::hex << _id;
        }
    }
}
    ...
    const std::shared_ptr<payload>& _payload, const security_update_handler_t& _handler)
{
    ...
    if (!its_clients_to_inform.empty())
    {
        ...
        VSOMEIP_INFO << __func__ << ": Informing [" << std::dec << its_clients_to_inform.size()
                     << "] currently connected clients about policy update for UID: " << std::dec
                     << _uid << " with update ID: 0x" << std::hex << its_id;
        for (auto its_client : its_clients_to_inform)
        {
            if (!send_update_security_policy_request(its_client, its_id, _uid, _payload))
            {
                VSOMEIP_INFO << __func__ << ": Couldn't send update security policy " << std::hex
                             << std::setfill('0') << "request to client 0x" << std::setw(4)
                             << its_client << " policy UID: " << std::setw(4) << _uid
    ...
    if (is_policy_cached(_uid))
    {
        ...
        else
        {
            ...
            if (!its_clients_to_inform.empty())
            {
                ...
                uint32_t sent_counter(0);
                uint32_t its_tranche = uint32_t(
                    its_clients_to_inform.size() >= 10 ? (its_clients_to_inform.size() / 10) : 1);
                VSOMEIP_INFO << __func__ << ": Informing [" << std::dec
                             << its_clients_to_inform.size()
                ...
                for (auto its_client : its_clients_to_inform)
                {
                    ...
                    {
                        VSOMEIP_INFO
                            << __func__ << ": Couldn't send remove security policy " << std::hex
                            << std::setfill('0') << "request to client 0x" << std::setw(4)
                            << its_client << " policy UID: " << std::setw(4) << _uid
                            << " GID: " << std::setw(4) << _gid << " with update ID: 0x" << its_id
                            << " as client already disconnected";
...
routing_manager_stub::pending_security_update_add(const std::unordered_set<client_t>& _clients)
{
    std::lock_guard<std::mutex> its_lock(pending_security_updates_mutex_);
...
routing_manager_stub::pending_security_update_get(pending_security_update_id_t _id)
{
    std::lock_guard<std::mutex>  its_lock(pending_security_updates_mutex_);
...
bool routing_manager_stub::pending_security_update_remove(pending_security_update_id_t _id,
                                                          client_t                     _client)
{
    std::lock_guard<std::mutex> its_lock(pending_security_updates_mutex_);
    auto                        found_si = pending_security_updates_.find(_id);
...
void routing_manager_stub::on_security_update_response(pending_security_update_id_t _id,
                                                       client_t                     _client)
{
...
void routing_manager_stub::send_suspend() const
{
    ...
    if (its_error == protocol::error_e::ERROR_OK)
        ...
        VSOMEIP_ERROR << __func__ << ": suspend command serialization failed (" << std::dec
                      << int(its_error) << ")";
}
...
void routing_manager_stub::remove_subscriptions(port_t                          _local_port,
                                                const boost::asio::ip::address& _remote_address,
                                                port_t                          _remote_port)
{
    (void)_local_port;
