...
event::event(routing_manager* _routing, bool _is_shadow)
    : routing_(_routing),
      ...
      epsilon_change_func_(
          std::bind(&event::has_changed, this, std::placeholders::_1, std::placeholders::_2)),
...
void event::set_payload(const std::shared_ptr<payload>& _payload, bool _force)
{
    ...
    {
        VSOMEIP_INFO << __func__ << ":" << __LINE__ << " Cannot set payload for event [" << std::hex
                     << std::setw(4) << std::setfill('0') << current_->get_service() << "."
                     << current_->get_instance() << "." << current_->get_method()
                     << "]. It isn't provided";
    }
}
...
void event::notify(bool _force)
{
    ...
    {
        VSOMEIP_INFO << __func__ << ": Notifying " << std::hex << std::setw(4) << std::setfill('0')
...
void event::notify_one(client_t _client, const std::shared_ptr<endpoint_definition>& _target)
{
    ...
    {
        VSOMEIP_WARNING << __func__ << ": Notifying " << std::hex << std::setw(4)
                        << std::setfill('0') << get_service() << "." << get_instance() << "."
                        << get_event() << " failed. Target undefined";
    }
}
                                ...
                                const std::shared_ptr<endpoint_definition>& _target)
{
    if (_target)
    {
        ...
        {
            VSOMEIP_INFO << __func__ << ": Notifying " << std::hex << std::setw(4)
                         << std::setfill('0') << get_service() << "." << get_instance() << "."
                         << get_event() << " failed. Event payload not (yet) set!";
...
void event::notify_one_unlocked(client_t _client, bool _force)
{
    ...
    {
        VSOMEIP_INFO << __func__ << ": Initial value for [" << std::hex << std::setw(4)
                     << std::setfill('0') << get_service() << "." << get_instance() << "."
                     << get_event() << "] not yet set by the service/client."
...
bool event::add_subscriber(eventgroup_t                                   _eventgroup,
                           ...
                           bool _force)
{
        ...
        || is_cache_placeholder_)
    {
        if (_filter)
        {
            VSOMEIP_WARNING << "Using client [" << std::hex << std::setw(4) << std::setfill('0')
                            << _client << "] specific filter configuration for SOME/IP event "
                            << get_service() << "." << get_instance() << "." << get_event() << ".";
            std::stringstream its_filter_parameters;
            its_filter_parameters << "(on_change=" << std::boolalpha << _filter->on_change_
                                  << ", interval=" << std::dec << _filter->interval_
                                  << ", on_change_resets_interval=" << std::boolalpha
                                  << _filter->on_change_resets_interval_ << ", ignore=[ ";
            for (auto i : _filter->ignore_)
                its_filter_parameters << "(" << std::dec << i.first << ", " << std::hex
                                      << std::setw(2) << std::setfill('0') << (int)i.second << ") ";
            its_filter_parameters << "])";
            VSOMEIP_INFO << "Filter parameters: " << its_filter_parameters.str();
            {
                std::scoped_lock lk{filters_mutex_};
                filters_[_client] = [_filter](const std::shared_ptr<payload>& _old,
                    ...
                    if (_filter->interval_ > -1)
                    {
                        ...
                        int64_t elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                                              ...
                                              .count();
                        is_elapsed = (_filter->last_forwarded_
    ...
    {
        VSOMEIP_WARNING << __func__ << ": Didnt' insert client " << std::hex << std::setw(4)
                        << std::setfill('0') << _client << " to eventgroup 0x" << std::hex
                        << std::setw(4) << std::setfill('0') << get_service() << "."
                        << get_instance() << "." << _eventgroup;
    }
    return ret;
}
...
std::set<client_t> event::get_filtered_subscribers(bool _force)
{
    ...
    if (is_filters_empty)
    {
        bool must_forward = ((type_ != event_type_e::ET_FIELD && has_default_epsilon_change_func_)
                             || _force || epsilon_change_func_(its_payload, its_payload_update));
    ...
    else
    {
        ...
        for (const auto s : its_subscribers)
        {
            ...
            else
            {
                ...
                {
                    is_allowed =
                        ((type_ != event_type_e::ET_FIELD && has_default_epsilon_change_func_)
                                 || _force
...
std::set<client_t>
event::update_and_get_filtered_subscribers(const std::shared_ptr<payload>& _payload,
...
bool event::has_changed(const std::shared_ptr<payload>& _lhs,
