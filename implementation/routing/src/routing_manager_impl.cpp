...
bool routing_manager_impl::is_subscribe_to_any_event_allowed(
    const vsomeip_sec_client_t* _sec_client, client_t _client, service_t _service,
    instance_t _instance, eventgroup_t _eventgroup)
{
    return routing_manager_base::is_subscribe_to_any_event_allowed(_sec_client, _client, _service,
...
void routing_manager_impl::init()
{
    ...
    if (configuration_->is_sd_enabled())
    {
        ...
        auto its_plugin =
            plugin_manager::get()->get_plugin(plugin_type_e::SD_RUNTIME_PLUGIN, VSOMEIP_SD_LIBRARY);
        if (its_plugin)
        {
    ...
    if (configuration_->is_e2e_enabled())
    {
        ...
        auto its_plugin =
            plugin_manager::get()->get_plugin(plugin_type_e::APPLICATION_PLUGIN, plugin_name);
        if (its_plugin)
        {
    ...
    if (e2e_provider_)
    {
        std::map<e2exf::data_identifier_t, std::shared_ptr<cfg::e2e>> its_e2e_configuration =
            configuration_->get_e2e_configuration();
        for (auto& identifier : its_e2e_configuration)
        {
            if (!e2e_provider_->add_configuration(identifier.second))
            {
                VSOMEIP_INFO << "Unknown E2E profile: " << identifier.second->profile
                             << ", skipping ...";
            }
        }
    }
...
void routing_manager_impl::start()
{
    ...
    {
        VSOMEIP_ERROR << "Illegal multicast address \"" << configuration_->get_sd_multicast()
    ...
    netlink_connector_->register_net_if_changes_handler(
        std::bind(&routing_manager_impl::on_net_interface_or_route_state_changed, this,
    ...
    if (configuration_->log_version())
    {
        ...
        version_log_timer_.expires_from_now(std::chrono::seconds(0));
        version_log_timer_.async_wait(
            std::bind(&routing_manager_impl::log_version_timer_cbk, this, std::placeholders::_1));
    }
#if defined(__linux__) || defined(ANDROID) || defined(__QNX__)
    if (configuration_->log_memory())
    {
        ...
        memory_log_timer_.async_wait(
            std::bind(&routing_manager_impl::memory_log_timer_cbk, this, std::placeholders::_1));
    }
#endif
    if (configuration_->log_status())
    {
        ...
        status_log_timer_.async_wait(
            std::bind(&routing_manager_impl::status_log_timer_cbk, this, std::placeholders::_1));
    }
    ...
    if (configuration_->log_statistics())
    {
        ...
        statistics_log_timer_.async_wait(std::bind(&routing_manager_impl::statistics_log_timer_cbk,
...
void routing_manager_impl::stop()
{
    ...
    for (const auto& s : its_services)
    {
        for (const auto& i : s.second)
        {
            on_stop_offer_service(std::get<2>(i.second), s.first, i.first, std::get<0>(i.second),
                                  std::get<1>(i.second));
        }
    }
...
bool routing_manager_impl::insert_offer_command(service_t _service, instance_t _instance,
                ...
                else
                {
                    io_.post([&, its_command, _service, _instance]() {
                        stop_offer_service(std::get<1>(its_command), _service, _instance,
                                           std::get<2>(its_command), std::get<3>(its_command),
                                           false);
                    });
                }
            }
...
bool routing_manager_impl::offer_service(client_t _client, service_t _service, instance_t _instance,
                                         major_version_t _major, minor_version_t _minor)
{
                                         ...
                                         bool _must_queue)
{
    ...
    if (_must_queue)
    {
        if (!insert_offer_command(_service, _instance, uint8_t(protocol::id_e::OFFER_SERVICE_ID),
                                  _client, _major, _minor))
        {
            VSOMEIP_INFO << "rmi::" << __func__ << " (" << std::hex << std::setw(4)
                         << std::setfill('0') << _client << "): [" << std::hex << std::setw(4)
                         << std::setfill('0') << _service << "." << std::hex << std::setw(4)
                         << std::setfill('0') << _instance << ":" << std::dec << int(_major) << "."
                         << std::dec << _minor << "]"
    ...
    {
        if (VSOMEIP_SEC_OK
            != configuration_->get_security()->is_client_allowed_to_offer(get_sec_client(),
                                                                          _service, _instance))
        {
            VSOMEIP_WARNING << "routing_manager_impl::offer_service: " << std::hex
                            << "Security: Client 0x" << _client
                            << " isn't allowed to offer the following service/instance " << _service
                            << "/" << _instance << " ~> Skip offer!";
    ...
    {
        erase_offer_command(_service, _instance);
        VSOMEIP_INFO << __func__ << " (" << std::hex << std::setw(4) << std::setfill('0') << _client
...

    {
        ...
        for (auto& ps : pending_subscriptions_)
        {
            ...
            {
                insert_subscription(ps.service_, ps.instance_, ps.eventgroup_, ps.event_, nullptr,
    ...
    VSOMEIP_INFO << "OFFER(" << std::hex << std::setw(4) << std::setfill('0') << _client << "): ["
                 << std::hex << std::setw(4) << std::setfill('0') << _service << "." << std::hex
                 << std::setw(4) << std::setfill('0') << _instance << ":" << std::dec << int(_major)
                 << "." << std::dec << _minor << "]"
                 << " (" << std::boolalpha << _must_queue << ")";
...
void routing_manager_impl::stop_offer_service(client_t _client, service_t _service,
...
{
    VSOMEIP_INFO << "STOP OFFER(" << std::hex << std::setfill('0') << std::setw(4) << _client
    ...
    if (_must_queue)
    {
        if (!insert_offer_command(_service, _instance,
                                  uint8_t(protocol::id_e::STOP_OFFER_SERVICE_ID), _client, _major,
                                  _minor))
        {
            VSOMEIP_INFO << "rmi::" << __func__ << " (" << std::hex << std::setw(4)
                         << std::setfill('0') << _client << "): [" << std::hex << std::setw(4)
    ...
    {
        VSOMEIP_WARNING << __func__ << " received STOP_OFFER(" << std::hex << std::setfill('0')
                        << std::setw(4) << _client << "): [" << std::setw(4) << _service << "."
                        << std::setw(4) << _instance << ":" << std::dec << int(_major) << "."
                        << _minor << "] "
...
void routing_manager_impl::request_service(client_t _client, service_t _service,
                                           ...
                                           minor_version_t _minor)
{
    VSOMEIP_INFO << "REQUEST(" << std::hex << std::setfill('0') << std::setw(4) << _client << "): ["
        ...
        if (discovery_)
        {
            ...
            else
            {
                VSOMEIP_INFO << std::hex << "Avoid trigger SD find-service message"
                             << " for local service/instance/major/minor: " << _service << "/"
                             << _instance << std::dec << "/" << (uint32_t)_major << "/" << _minor;
            }
        }
    }
    else
    {
        if ((_major == its_info->get_major() || DEFAULT_MAJOR == its_info->get_major()
             || ANY_MAJOR == _major)
            && (_minor <= its_info->get_minor() || DEFAULT_MINOR == its_info->get_minor()
                || _minor == ANY_MINOR))
        {
...
void routing_manager_impl::release_service(client_t _client, service_t _service,
                                           instance_t _instance)
{
    VSOMEIP_INFO << "RELEASE(" << std::hex << std::setfill('0') << std::setw(4) << _client << "): ["
                 << std::setw(4) << _service << "." << std::setw(4) << _instance << "]";
...
void routing_manager_impl::subscribe(client_t _client, const vsomeip_sec_client_t* _sec_client,
                                     ...
                                     const std::shared_ptr<debounce_filter_impl_t>& _filter)
{
    ...
    VSOMEIP_INFO << "SUBSCRIBE(" << std::hex << std::setfill('0') << std::setw(4) << _client
                 << "): [" << std::setw(4) << _service << "." << std::setw(4) << _instance << "."
                 << std::setw(4) << _eventgroup << ":" << std::setw(4) << _event << ":" << std::dec
                 << (uint16_t)_major << "]";
                ...
                if (!_subscription_accepted)
                {
                    if (stub_)
                        stub_->send_subscribe_nack(_client, _service, _instance, _eventgroup,
                                                   _event);
                    VSOMEIP_INFO << "Subscription request from client: 0x" << std::hex << _client
                                 << std::dec << " for eventgroup: 0x" << _eventgroup
    ...
    else
    {
        if (discovery_)
        {
            ...
            bool inserted = insert_subscription(_service, _instance, _eventgroup, _event, _filter,
                                                _client, &its_already_subscribed_events);
            ...
            if (inserted)
            {
                if (0 == its_local_client)
                {
                    ...
                    if (its_info && (!subscriber_is_rm_host || find_service(_service, _instance)))
                    {
                        discovery_->subscribe(
                            _service, _instance, _eventgroup, _major, configured_ttl,
                ...
                else
                {
                    ...
                    {
                        stub_->send_subscribe(ep_mgr_->find_local(_service, _instance), _client,
                                              _service, _instance, _eventgroup, _major, _event,
                                              _filter, PENDING_SUBSCRIPTION_ID);
                    }
                }
            }
...
void routing_manager_impl::unsubscribe(client_t _client, const vsomeip_sec_client_t* _sec_client,
                                       service_t _service, instance_t _instance,
                                       eventgroup_t _eventgroup, event_t _event)
{
    ...
    if (discovery_)
    {
        host_->on_subscription(_service, _instance, _eventgroup, _client, _sec_client,
                               get_env(_client), false, [](const bool _subscription_accepted) {
        ...
        else
        {
            if (get_client() == _client)
            {
                ...
                remove_pending_subscription(_service, _instance, _eventgroup, _event);
                if (stub_)
                    stub_->send_unsubscribe(ep_mgr_->find_local(_service, _instance), _client,
...
bool routing_manager_impl::send(client_t _client, const byte_t* _data, length_t _size,
                                instance_t _instance, bool _reliable, client_t _bound_client,
                                const vsomeip_sec_client_t* _sec_client, uint8_t _status_check,
    ...
    if (_size > VSOMEIP_MESSAGE_TYPE_POS)
    {
...
#endif
                deliver_message(_data, _size, _instance, _reliable, _bound_client, _sec_client,
        ...
        if (its_target)
        {
            ...
            if ((is_request && its_client == get_client())
                ...
                || (is_notification
                    && find_local_client(its_service, _instance) == VSOMEIP_ROUTING_CLIENT))
            {
                trace::header its_header;
                if (its_header.prepare(its_target, true, _instance))
                    tc_->trace(its_header.data_, VSOMEIP_TRACE_HEADER_SIZE, _data, _size);
                ...
                if (e2e_provider_)
                {
                    if (!is_service_discovery)
                    {
                        ...
                        if (e2e_provider_->is_protected({its_service, its_method}))
                        {
                            ...
                            size_t its_base =
                                e2e_provider_->get_protection_base({its_service, its_method});

                            // Build a corresponding buffer
                            its_buffer.assign(_data + its_base, _data + _size);

                            e2e_provider_->protect({its_service, its_method}, its_buffer,
                                                   _instance);
                ...
                else
                {
                    ...
                    if (its_info || is_service_discovery)
                    {
                        if (is_notification && !is_service_discovery)
                        {
                            ...
                            std::shared_ptr<event> its_event =
                                find_event(its_service, _instance, its_method);
                            if (its_event)
                            {
#ifdef USE_DLT
                                bool has_sent(false);
                                ...
                                // and UDP
                                std::shared_ptr<endpoint> its_udp_server_endpoint =
                                    its_info->get_endpoint(false);
                                std::shared_ptr<endpoint> its_tcp_server_endpoint =
                                ...
                                if (its_udp_server_endpoint || its_tcp_server_endpoint)
                                {
                                    ...
                                    {
                                        auto its_eventgroup =
                                            find_eventgroup(its_service, _instance, its_group);
                                        if (its_eventgroup)
                                        {
                                            // Unicast targets
                                            for (const auto& its_remote :
                                                 its_eventgroup->get_unicast_targets())
                                            {
                                                if (its_remote->is_reliable()
                                                    && its_tcp_server_endpoint)
                                                {
                                                    if (its_reliability
                                                ...
                                                else if (its_udp_server_endpoint
                                            ...
                                            if (its_eventgroup->is_sending_multicast())
                                            {
                                                           ...
                                                           == reliability_type_e::RT_BOTH)
                                                {
                                                    ...
                                                    uint16_t                 its_port;
                                                    if (its_eventgroup->get_multicast(its_address,
                                                                                      its_port))
                                                    {
                                                        std::shared_ptr<endpoint_definition>
                                                            its_multicast_target;
                                                        its_multicast_target =
                                                            endpoint_definition::get(
                                                                its_address, its_port, false,
                        ...
                        else
                        {
                                ...
                                && !its_info->is_local())
                            {
                                ...
                                VSOMEIP_ERROR
                                    ...
                                    << std::hex << std::setfill('0') << std::setw(4) << its_client
                                    << "): [" << std::setw(4) << its_service << "." << std::setw(4)
                                    << _instance << "." << std::setw(4) << its_method << "] "
                                    << std::setw(4) << its_session;
                                return false;
                            }
                            its_target = is_service_discovery ?
                                             (sd_info_ ? sd_info_->get_endpoint(false) : nullptr) :
                            ...
                            else
                            {
                                const session_t its_session =
                                    bithelper::read_uint16_be(&_data[VSOMEIP_SESSION_POS_MIN]);
                                VSOMEIP_ERROR << "Routing error. Endpoint for service (" << std::hex
                                              << std::setfill('0') << std::setw(4) << its_client
                                              << "): [" << std::setw(4) << its_service << "."
                                              << std::setw(4) << _instance << "." << std::setw(4)
                                              << its_method << "] " << std::setw(4) << its_session
                                              << " could not be found!";
                    ...
                    else
                    {
                        if (!is_notification)
                        {
                            const session_t its_session =
                                bithelper::read_uint16_be(&_data[VSOMEIP_SESSION_POS_MIN]);
                            VSOMEIP_ERROR << "Routing error. Not hosting service (" << std::hex
                                          << std::setfill('0') << std::setw(4) << its_client
                                          << "): [" << std::setw(4) << its_service << "."
                                          << std::setw(4) << _instance << "." << std::setw(4)
                                          << its_method << "] " << std::setw(4) << its_session;
...
bool routing_manager_impl::send_to(const client_t                              _client,
...
{
    std::shared_ptr<endpoint> its_endpoint =
        ep_mgr_impl_->find_server_endpoint(_sd_port, _target->is_reliable());
...
void routing_manager_impl::register_event(
    client_t _client, service_t _service, instance_t _instance, event_t _notifier,
    ...
    bool _is_shadow, bool _is_cache_placeholder)
{
    ...
    if (is_first)
    {
        routing_manager_base::register_event(_client, _service, _instance, _notifier, _eventgroups,
                                             _type, _reliability, _cycle, _change_resets_cycle,
                                             _update_on_change, _epsilon_change_func, _is_provided,
                                             _is_shadow, _is_cache_placeholder);
    }
    VSOMEIP_INFO << "REGISTER EVENT(" << std::hex << std::setfill('0') << std::setw(4) << _client
                 << "): [" << std::setw(4) << _service << "." << std::setw(4) << _instance << "."
                 << std::setw(4) << _notifier << ":is_provider=" << std::boolalpha << _is_provided
                 << "]";
}

void routing_manager_impl::register_shadow_event(client_t _client, service_t _service,
                                                 ...
                                                 bool _is_cyclic)
{
    routing_manager_base::register_event(
        _client, _service, _instance, _notifier, _eventgroups, _type, _reliability,
        (_is_cyclic ? std::chrono::milliseconds(1) : std::chrono::milliseconds::zero()), false,
        true, nullptr, _is_provided, true);
}

void routing_manager_impl::unregister_shadow_event(client_t _client, service_t _service,
...
void routing_manager_impl::notify_one(service_t _service, instance_t _instance, event_t _event,
                                      std::shared_ptr<payload> _payload, client_t _client,
                                      bool _force
#ifdef VSOMEIP_ENABLE_COMPAT
                                      ,
...
)
{
    ...
    else
    {
        ...
        {
            VSOMEIP_WARNING << "Attempt to update the undefined event/field [" << std::hex
                                           ...
                                           minor_version_t _minor)
{
    ...
    if (_state == availability_state_e::AS_AVAILABLE)
    {
        if (discovery_)
        {
            ...
            {
                static const ttl_t                    configured_ttl(configuration_->get_sd_ttl());
                std::lock_guard<std::recursive_mutex> its_subscribed_lock(
                    discovery_->get_subscribed_mutex());
                std::lock_guard<std::mutex> its_lock(pending_subscription_mutex_);
                for (auto& ps : pending_subscriptions_)
                {
                    if (ps.service_ == _service && ps.instance_ == _instance && ps.major_ == _major)
                    {
                        ...
                        {
                            discovery_->subscribe(
                                _service, _instance, ps.eventgroup_, _major, configured_ttl,
...
bool routing_manager_impl::offer_service_remotely(service_t _service, instance_t _instance,
                                                  ...
                                                  bool _magic_cookies_enabled)
{
    ...
    {
        VSOMEIP_ERROR << __func__ << ": Service [" << std::hex << std::setfill('0') << std::setw(4)
    ...
    else
    {
        ...
        if (!configuration_->remote_offer_info_add(_service, _instance, _port, _reliable,
                                                   _magic_cookies_enabled))
        {
            ret = false;
        }
        else
        {
            ...
            {
                if (stub_
                    && !stub_->send_provided_event_resend_request(
                        its_offering_client, pending_remote_offer_add(_service, _instance)))
                {
                    VSOMEIP_ERROR << __func__ << ": Couldn't send event resend" << std::hex
                                  << std::setfill('0') << "request to client 0x" << std::setw(4)
                                  << its_offering_client << " providing service [" << std::setw(4)
                                  << _service << "." << std::setw(4) << _instance << "]";
...
bool routing_manager_impl::stop_offer_service_remotely(service_t _service, instance_t _instance,
                                                       std::uint16_t _port, bool _reliable,
                                                       bool _magic_cookies_enabled)
{
    bool ret = true;
    ...
    if (!configuration_->remote_offer_info_remove(_service, _instance, _port, _reliable,
                                                  _magic_cookies_enabled,
                                                  &service_still_offered_remote))
    {
        VSOMEIP_ERROR << __func__ << " couldn't remove remote offer info for service [" << std::hex
                      << std::setfill('0') << std::setw(4) << _service << "." << std::setw(4)
                      << _instance << "] from configuration";
        ret = false;
    }
    std::shared_ptr<serviceinfo> its_info = find_service(_service, _instance);
    ...
    if (!service_still_offered_remote)
    {
        ...
        routing_manager_base::stop_offer_service(its_offering_client, _service, _instance,
                                                 its_major, its_minor);
        // unregister events
    ...
    else
    {
        ...
        if (discovery_ && its_info)
        {
            ...
            VSOMEIP_INFO << __func__ << " only sending the StopOffer to [" << std::hex
                         << std::setw(4) << std::setfill('0') << _service << '.' << std::hex
                         << std::setw(4) << std::setfill('0') << _instance << ']'
...
void routing_manager_impl::on_message(const byte_t* _data, length_t _size, endpoint* _receiver,
                                      bool _is_multicast, client_t _bound_client,
                                      ...
                                      std::uint16_t                   _remote_port)
{
            ...
            if (discovery_ && its_method == sd::method)
            {
                if (configuration_->get_sd_port() == _remote_port)
                {
                    if (!_remote_address.is_unspecified())
                    {
                        ...
                        if (!is_acl_message_allowed(_receiver, its_service, ANY_INSTANCE,
                                                    _remote_address))
                        {
                            return;
                        }
        ...
        else
        {
            ...
            if (its_instance == 0xFFFF)
            {
                ...
                const client_t its_client =
                    bithelper::read_uint16_be(&_data[VSOMEIP_CLIENT_POS_MIN]);
                const session_t its_session =
                    bithelper::read_uint16_be(&_data[VSOMEIP_SESSION_POS_MIN]);
                boost::system::error_code ec;
                VSOMEIP_ERROR << "Received message on invalid port: [" << std::hex
                              ...
                              << its_session << "] from: " << _remote_address.to_string(ec) << ":"
                  ...
                  && utility::is_request_no_return(_data[VSOMEIP_MESSAGE_TYPE_POS])))
            {
                ...
                {
                    send_error(return_code, _data, _size, its_instance, _receiver->is_reliable(),
                               _receiver, _remote_address, _remote_port);
                    return;
                }
            }
                ...
                if (utility::is_request(_data[VSOMEIP_MESSAGE_TYPE_POS]))
                {
                    ...
                    if (!configuration_->is_offered_remote(its_service, its_instance))
                    {
                        VSOMEIP_WARNING << std::hex << "Security: Received a remote request "
                                        << "for service/instance " << its_service << "/"
                                        << its_instance
                    ...
                    if (!configuration_->is_remote_access_allowed())
                    {
                        ...
                        VSOMEIP_WARNING
                            << "routing_manager_impl::on_message: " << std::hex
                            << "Security: Remote client with client ID 0x" << requester
            ...
            if (e2e_provider_)
            {
                ...
                if (e2e_provider_->is_checked({its_service, its_method}))
                {
                    ...
                    e2e_provider_->check({its_service, its_method}, its_buffer, its_instance,
                                         its_check_status);

                    if (its_check_status != e2e::profile_interface::generic_check_status::E2E_OK)
                    {
                        VSOMEIP_INFO << "E2E protection: CRC check failed for service: " << std::hex
                                     << its_service << " method: " << its_method;
                    }
                }
#endif
            }
    ...
    if (is_forwarded)
    {
        ...
        trace::protocol_e its_protocol =
            _receiver->is_local() ?
                trace::protocol_e::local :
                _receiver->is_reliable() ? trace::protocol_e::tcp : trace::protocol_e::udp;
...
bool routing_manager_impl::on_message(service_t _service, instance_t _instance, const byte_t* _data,
                                      length_t _size, bool _reliable, client_t _bound_client,
                                      const vsomeip_sec_client_t* _sec_client,
                                      uint8_t _check_status, bool _is_from_remote)
{
    ...
    {
        is_forwarded =
            deliver_notification(_service, _instance, _data, _size, _reliable, _bound_client,
                                 _sec_client, _check_status, _is_from_remote);
    }
    else if (its_client == host_->get_client())
    {
        deliver_message(_data, _size, _instance, _reliable, _bound_client, _sec_client,
                        _check_status, _is_from_remote);
    }
    else
    {
        send(its_client, _data, _size, _instance, _reliable, _bound_client, _sec_client,
             _check_status, _is_from_remote, false); // send to proxy
    }
    return is_forwarded;
}

void routing_manager_impl::on_notification(client_t _client, service_t _service,
                                           instance_t _instance, const byte_t* _data,
                                           length_t _size, bool _notify_one)
{
    ...
    if (its_event)
    {
        ...
        std::shared_ptr<payload> its_payload =
            runtime::get()->create_payload(&_data[VSOMEIP_PAYLOAD_POS], its_length);
                                                 ...
                                                 minor_version_t _minor)
{
            ...
            if (found_instance != found_service->second.end())
            {
                    ...
                    || std::get<2>(found_instance->second) != _client)
                {
                    VSOMEIP_WARNING
                        ...
                        << _minor << "] by application: " << std::hex << std::setw(4) << _client
                        << ". Stored: [" << std::setw(4) << _service << "." << _instance << "."
                        << std::dec
                        << static_cast<std::uint32_t>(std::get<0>(found_instance->second)) << "."
                        << std::get<1>(found_instance->second) << "] by application: " << std::hex
    ...
    if (its_info)
    {
        ...
        auto ready_to_stop = std::make_shared<ready_to_stop_t>(its_reliable_endpoint == nullptr,
                                                               its_unreliable_endpoint == nullptr);
        auto ptr           = shared_from_this();

        auto callback = [this, ptr, its_info, its_reliable_endpoint, its_unreliable_endpoint,
                         ready_to_stop, _service, _instance, _major,
            ...
            if (utility::is_notification(its_message->get_message_type()))
            {
                                         ...
                                         its_message->get_instance(), its_message->get_method()))
                {
                    VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << get_client()
                                    << " : routing_manager_impl::deliver_message: " << std::hex
                ...
                else
                {
                    ...
                    {
                        VSOMEIP_WARNING
                            << "vSomeIP Security: Client 0x" << std::hex << get_client()
                            ...
                            << "/" << its_message->get_method() << " respectively from client 0x"
            ...
            else if (utility::is_request(its_message->get_message_type()))
            {
                    ...
                    && its_message->get_client() != _bound_client)
                {
                    VSOMEIP_WARNING
                        ...
                        << std::setw(4) << _bound_client << " ~> Skip message!";
                    return false;
                }
            ...
            else if (utility::is_notification(its_message->get_message_type()))
            {
                        ...
                        its_message->get_method()))
                {
                    VSOMEIP_WARNING
                        ...
                        << " isn't allowed to receive a notification from service/instance/event "
                        << its_message->get_service() << "/" << its_message->get_instance() << "/"
                        << its_message->get_method() << " respectively from remote client"
                        << " ~> Skip message!";
                    return false;
                }
            }
...
bool routing_manager_impl::deliver_notification(service_t _service, instance_t _instance,
                                                ...
                                                uint8_t _status_check, bool _is_from_remote)
{
    ...
    if (its_event)
    {
        if (!its_event->is_provided())
        {
            if (its_event->get_subscribers().size() == 0)
            {
                ...
                if (!cache_event)
                {
                    VSOMEIP_WARNING << __func__ << ": dropping [" << std::hex << std::setfill('0')
                                    << std::setw(4) << _service << "." << std::setw(4) << _instance
        ...
        if (its_event->get_type() != event_type_e::ET_SELECTIVE_EVENT)
        {
            for (const auto its_local_client : its_subscribers)
            {
                ...
                {
                    deliver_message(_data, _length, _instance, _reliable, _bound_client,
                                    _sec_client, _status_check, _is_from_remote);
                }
                else
                {
    ...
    else
    {
        ...
        if (has_subscribed_eventgroup(_service, _instance))
        {
            ...
            {
                VSOMEIP_WARNING << __func__ << ": Caching unregistered event [" << std::hex
                                << std::setw(4) << std::setfill('0') << _service << "." << std::hex
                                << std::setw(4) << std::setfill('0') << _instance << "." << std::hex
                                << std::setw(4) << std::setfill('0') << its_event_id << "]";
            }

            routing_manager_base::register_event(
                host_->get_client(), _service, _instance, its_event_id, {},
            ...
            if (its_event)
            {
                ...
                VSOMEIP_ERROR << __func__ << ": Event registration failed [" << std::hex
                              << std::setw(4) << std::setfill('0') << _service << "." << std::hex
                              << std::setw(4) << std::setfill('0') << _instance << "." << std::hex
                              << std::setw(4) << std::setfill('0') << its_event_id << "]";
        }
        ...
        {
            VSOMEIP_WARNING << __func__ << ": Dropping unregistered event [" << std::hex
                            << std::setw(4) << std::setfill('0') << _service << "." << std::hex
                            << std::setw(4) << std::setfill('0') << _instance << "." << std::hex
                            << std::setw(4) << std::setfill('0') << its_event_id << "] "
        ...
        if (!is_suppress_event(_service, _instance, its_event_id))
        {
            VSOMEIP_WARNING << __func__ << ": Event [" << std::hex << std::setw(4)
                            << std::setfill('0') << _service << "." << std::hex << std::setw(4)
                            << std::setfill('0') << _instance << "." << std::hex << std::setw(4)
                            << std::setfill('0') << its_event_id << "]"
...
bool routing_manager_impl::is_suppress_event(service_t _service, instance_t _instance,
...
routing_manager_impl::find_eventgroup(service_t _service, instance_t _instance,
...
routing_manager_impl::create_service_discovery_endpoint(const std::string& _address, uint16_t _port,
                                                        bool _reliable)
{
        ...
        try
        {
            ...
            if (its_service_endpoint)
            {
                sd_info_ = std::make_shared<serviceinfo>(
                    VSOMEIP_SD_SERVICE, VSOMEIP_SD_INSTANCE, ANY_MAJOR, ANY_MINOR, DEFAULT_TTL,
                ...
                if (!_reliable)
                {
                    auto its_server_endpoint =
                        std::dynamic_pointer_cast<udp_server_endpoint_impl>(its_service_endpoint);
                    if (its_server_endpoint)
                    {
                        its_server_endpoint->set_unicast_sent_callback(std::bind(
                            &sd::service_discovery::sent_messages, discovery_.get(),
                            std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
                        its_server_endpoint->set_receive_own_multicast_messages(true);
                        its_server_endpoint->set_sent_multicast_received_callback(std::bind(
                            &sd::service_discovery::sent_messages, discovery_.get(),
                            std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
services_t routing_manager_impl::get_offered_services() const
{
    ...
    for (const auto& s : get_services())
    {
        for (const auto& i : s.second)
        {
            ...
            {
                VSOMEIP_ERROR << __func__ << "Found instance with NULL ServiceInfo [" << std::hex
                              << std::setw(4) << std::setfill('0') << s.first << ":" << i.first
                              << "]";
            }
        }
    }
...
std::shared_ptr<serviceinfo> routing_manager_impl::get_offered_service(service_t  _service,
...
bool routing_manager_impl::is_acl_message_allowed(
    endpoint* _receiver, service_t _service, instance_t _instance,
    const boost::asio::ip::address& _remote_address) const
{
    if (message_acceptance_handler_ && _receiver)
    {
        ...
        message_acceptance_t message_acceptance{_remote_address.to_v4().to_uint(),
                                                _receiver->get_local_port(), is_local, _service,
                                                _instance};
        if (!message_acceptance_handler_(message_acceptance))
        {
            VSOMEIP_WARNING << "Message from " << _remote_address.to_string() << std::hex
                            << " with service/instance " << _instance << "/" << _instance
                            << " was rejected by the ACL check.";
            return false;
        }
    }
    return true;
...
// PRIVATE
///////////////////////////////////////////////////////////////////////////////
void routing_manager_impl::init_service_info(service_t _service, instance_t _instance,
                                             bool _is_local_service)
{
    std::shared_ptr<serviceinfo> its_info = find_service(_service, _instance);
    if (!its_info)
    {
    ...
    if (configuration_)
    {
        ...
        if (_is_local_service)
        {
            ...
            {
                std::shared_ptr<endpoint> its_reliable_endpoint =
                    ep_mgr_impl_->find_or_create_server_endpoint(its_reliable_port, true, is_someip,
                                                                 _service, _instance, _is_found);
            ...
            if (ILLEGAL_PORT != its_unreliable_port)
            {
                std::shared_ptr<endpoint> its_unreliable_endpoint =
                    ep_mgr_impl_->find_or_create_server_endpoint(
                        its_unreliable_port, false, is_someip, _service, _instance, _is_found);
            ...
            {
                VSOMEIP_INFO << "Port configuration missing for [" << std::hex << _service << "."
                             << _instance << "]. Service is internal.";
            }
        }
    }
    else
    ...
    const boost::asio::ip::address& _unreliable_address, uint16_t _unreliable_port)
{
    ...
    else if (its_info->is_local())
    {
        ...
        VSOMEIP_ERROR << "routing_manager_impl::add_routing_info: "
                      ...
                      << " is trying to offer [" << std::hex << std::setfill('0') << std::setw(4)
                      << _service << "." << std::setw(4) << _instance << "." << std::dec
                      << static_cast<std::uint32_t>(_major) << "." << _minor << "] on port "
                      << ((_reliable_port != ILLEGAL_PORT) ? _reliable_port : _unreliable_port)
                      << " offered previously on this node: [" << std::hex << std::setw(4)
                      << _service << "." << std::setw(4) << _instance << "." << std::dec
    ...
    ep_mgr_impl_->is_remote_service_known(_service, _instance, _major, _minor, _reliable_address,
                                          _reliable_port, &is_reliable_known, _unreliable_address,
                                          _unreliable_port, &is_unreliable_known);
    ...
    if (_reliable_port != ILLEGAL_PORT && !is_reliable_known)
    {
        ...
        {
            std::shared_ptr<endpoint_definition> endpoint_def_udp = endpoint_definition::get(
                _unreliable_address, _unreliable_port, false, _service, _instance);
        ...
        // check if service was requested and establish TCP connection if necessary
        {
            ...
            for (const client_t its_client :
                 get_requesters_unlocked(_service, _instance, _major, _minor))
            {
    ...
    else if (_reliable_port != ILLEGAL_PORT && is_reliable_known)
    {
        ...
        if (has_requester_unlocked(_service, _instance, _major, _minor))
        {
            ...
            if (ep)
            {
                if (ep->is_established() && stub_
                    && !stub_->contained_in_routing_info(VSOMEIP_ROUTING_CLIENT, _service,
                                                         _instance, its_info->get_major(),
                                                         its_info->get_minor()))
                {
                    on_availability(_service, _instance, availability_state_e::AS_AVAILABLE,
        ...
        {
            on_availability(_service, _instance, availability_state_e::AS_OFFERED,
                            its_info->get_major(), its_info->get_minor());
        }
    }
    ...
    if (_unreliable_port != ILLEGAL_PORT && !is_unreliable_known)
    {
        if (!udp_inserted)
        {
            std::shared_ptr<endpoint_definition> endpoint_def = endpoint_definition::get(
                _unreliable_address, _unreliable_port, false, _service, _instance);
        ...
        if (!is_reliable_known && !tcp_inserted)
        {
            ...
            if (has_requester_unlocked(_service, _instance, _major, _minor))
            {
                on_availability(_service, _instance, availability_state_e::AS_AVAILABLE, _major,
                                _minor);
                if (stub_)
                    stub_->on_offer_service(VSOMEIP_ROUTING_CLIENT, _service, _instance, _major,
                                            _minor);
            }
            else
            {
                on_availability(_service, _instance, availability_state_e::AS_OFFERED, _major,
                                _minor);
            }
        }
        if (discovery_)
    ...
    else if (_unreliable_port != ILLEGAL_PORT && is_unreliable_known)
    {
        ...
        if (has_requester_unlocked(_service, _instance, _major, _minor))
        {
            if (_reliable_port == ILLEGAL_PORT && !is_reliable_known && stub_
                && !stub_->contained_in_routing_info(VSOMEIP_ROUTING_CLIENT, _service, _instance,
                                                     its_info->get_major(), its_info->get_minor()))
            {
                on_availability(_service, _instance, availability_state_e::AS_AVAILABLE,
                                its_info->get_major(), its_info->get_minor());
                                            ...
                                            bool _has_reliable, bool _has_unreliable)
{
...

    {
        ...
        {
            if (found_service->second.erase(_instance) > 0 && !found_service->second.size())
            {
...

    {
        ...
        for (const auto& s : services_remote_)
        {
            for (const auto& i : s.second)
            {
                ...
                if (its_ttl < DEFAULT_TTL)
                { // do not touch "forever"
    ...
    for (const auto& s : its_expired_offers)
    {
        for (const auto& i : s.second)
        {
            ...
            VSOMEIP_INFO << "update_routing_info: elapsed=" << _elapsed.count()
                         << " : delete service/instance " << std::hex << std::setfill('0')
                                           ...
                                           bool                               _reliable)
{
    ...
    for (auto& s : get_services_remote())
    {
        for (auto& i : s.second)
        {
            ...
            {
                if ((expire_all
                     || (its_client_endpoint->get_remote_port() >= _range.first
    ...
    for (auto& s : its_expired_offers)
    {
        for (auto& i : s.second)
        {
            VSOMEIP_INFO << "expire_services for address: " << _address
                         << " : delete service/instance " << std::hex << std::setfill('0')
                         << std::setw(4) << s.first << "." << std::setw(4) << i << " port ["
                         << std::dec << _range.first << "," << _range.second
        ...
        for (const auto& its_instance : its_service.second)
        {
            for (const auto& its_eventgroup : its_instance.second)
            {
                ...
                for (auto its_subscription : its_info->get_remote_subscriptions())
                {
                    ...
                    {
                        VSOMEIP_WARNING
                            << __func__ << ": New remote subscription replaced expired ["
                            << std::hex << std::setw(4) << std::setfill('0') << its_service.first
                            << "." << std::hex << std::setw(4) << std::setfill('0')
                            << its_instance.first << "." << std::hex << std::setw(4)
                            << std::setfill('0') << its_eventgroup.first << "]";
                        continue;
                    }
                    ...
                    auto its_ep_definition = _reliable ? its_subscription->get_reliable() :
                                                         its_subscription->get_unreliable();

                    if (!its_ep_definition && expire_all)
                        its_ep_definition = (!_reliable) ? its_subscription->get_reliable() :
                                                           its_subscription->get_unreliable();
                                ...
                                && its_ep_definition->get_remote_port() <= _range.second)))
                    {
                        ...
                        VSOMEIP_INFO << __func__ << ": removing subscription to " << std::hex
                                     << its_info->get_service() << "." << std::hex
                                     << its_info->get_instance() << "." << std::hex
                                     << its_info->get_eventgroup() << " from target "
                                     << its_ep_definition->get_address() << ":" << std::dec
                                     << its_ep_definition->get_port()
                                     << " reliable=" << std::boolalpha
...
void routing_manager_impl::init_routing_info()
{
    ...
    {
        boost::asio::ip::address its_address(boost::asio::ip::address::from_string(
        ...
        {
            add_routing_info(i.first, i.second, DEFAULT_MAJOR, DEFAULT_MINOR, DEFAULT_TTL,
                             its_address, its_reliable_port, its_address, its_unreliable_port);
                                               ...
                                               const remote_subscription_callback_t& _callback)
{
    ...
    auto its_result = its_eventgroupinfo->update_remote_subscription(_subscription, its_expiration,
                                                                     its_added, its_id, true);
    if (its_result)
    {
        if (!_subscription->is_pending())
        { // resubscription without change
            its_update_lock.unlock();
            _callback(_subscription);
        }
        else if (!its_added.empty())
        { // new clients for a selective subscription
            const client_t its_offering_client = find_local_client(its_service, its_instance);
            send_subscription(its_offering_client, its_service, its_instance, its_eventgroup,
                              its_major, its_added, _subscription->get_id());
        }
        else
        { // identical subscription is not yet processed
            ...
            if (its_reliable && its_unreliable)
                its_warning << "[";
            if (its_reliable)
                its_warning << its_reliable->get_address().to_string() << ":" << std::dec
                            << its_reliable->get_port();
            if (its_reliable && its_unreliable)
                its_warning << ", ";
            if (its_unreliable)
                its_warning << its_unreliable->get_address().to_string() << ":" << std::dec
                            << its_unreliable->get_port();
            if (its_reliable && its_unreliable)
                its_warning << "]";
    ...
    { // new subscription
        ...
        const client_t its_offering_client = find_local_client(its_service, its_instance);
        send_subscription(its_offering_client, its_service, its_instance, its_eventgroup, its_major,
    ...
    std::shared_ptr<remote_subscription>& _subscription)
{
    ...
    auto                         its_result = its_info->update_remote_subscription(
        _subscription, std::chrono::steady_clock::now(), its_removed, its_id, false);

    if (its_result)
    {
        const client_t its_offering_client = find_local_client(its_service, its_instance);
        send_unsubscription(its_offering_client, its_service, its_instance, its_eventgroup,
                            its_major, its_removed, its_id);
    }
}
...
void routing_manager_impl::on_subscribe_ack(client_t _client, service_t _service,
                                            instance_t _instance, eventgroup_t _eventgroup,
    ...
    if (its_eventgroup)
    {
        ...
        if (its_subscription)
        {
            ...
            if (discovery_)
            {
                ...
                VSOMEIP_INFO << "REMOTE SUBSCRIBE(" << std::hex << std::setfill('0') << std::setw(4)
                             << _client << "): [" << std::setw(4) << _service << "." << std::setw(4)
                             << _instance << "." << std::setw(4) << _eventgroup << "]"
                             << " from " << its_subscription->get_subscriber()->get_address() << ":"
                             << std::dec << its_subscription->get_subscriber()->get_port()
                             << (its_subscription->get_subscriber()->is_reliable() ? " reliable" :
                                                                                     " unreliable")
                             << " was accepted";
        ...
        for (const auto& its_subscriber : subscribed_clients)
        {
            if (its_subscriber == get_client())
            {
                if (_event == ANY_EVENT)
                {
                    for (const auto& its_event : its_eventgroup->get_events())
                    {
                        host_->on_subscription_status(_service, _instance, _eventgroup,
                                                      its_event->get_event(), 0x0 /*OK*/);
                    }
                }
                else
                {
                    host_->on_subscription_status(_service, _instance, _eventgroup, _event,
                                                  0x0 /*OK*/);
                }
            }
            else if (stub_)
...
std::shared_ptr<endpoint> routing_manager_impl::find_or_create_remote_client(service_t  _service,
                                                                             instance_t _instance,
                                                                             bool       _reliable)
{
...
void routing_manager_impl::on_subscribe_nack(client_t _client, service_t _service,
                                             instance_t _instance, eventgroup_t _eventgroup,
                                                ...
                                                instance_t _instance)
{
    ...
    if (_size >= VSOMEIP_PAYLOAD_POS)
    {
            ...
            || utility::is_request_no_return(_data[VSOMEIP_MESSAGE_TYPE_POS]))
        {
            ...
            {
                VSOMEIP_WARNING
                    << "Received a message with unsupported protocol version for service 0x"
                    << std::hex << its_service;
                return return_code_e::E_WRONG_PROTOCOL_VERSION;
            }
            if (_instance == 0xFFFF)
            {
                VSOMEIP_WARNING << "Receiving endpoint is not configured for service 0x" << std::hex
                                << its_service;
            ...
            if (its_info)
            {
                ...
                {
                    VSOMEIP_WARNING
                        << "Received a message with unsupported interface version for service 0x"
                        << std::hex << its_service;
            ...
            if (_data[VSOMEIP_RETURN_CODE_POS] != static_cast<byte_t>(return_code_e::E_OK))
            {
                ...
                VSOMEIP_WARNING
                    << "Received a message with unsupported return code set for service 0x"
                    << std::hex << its_service;
    ...
    else
    {
        ...
        VSOMEIP_WARNING
            << "Received a message message which is shorter than vSomeIP message header!";
        return return_code_e::E_MALFORMED_MESSAGE;
    }
    return return_code_e::E_OK;
}

void routing_manager_impl::send_error(return_code_e _return_code, const byte_t* _data,
        ...
        if (its_serializer->serialize(error_message.get()))
        {
            if (_receiver)
            {
                auto its_endpoint_def = std::make_shared<endpoint_definition>(
                    _remote_address, _remote_port, _receiver->is_reliable());
                its_endpoint_def->set_remote_port(_receiver->get_local_port());
                std::shared_ptr<endpoint> its_endpoint = ep_mgr_impl_->find_server_endpoint(
                    its_endpoint_def->get_remote_port(), its_endpoint_def->is_reliable());
                if (its_endpoint)
                {
#ifdef USE_DLT
                    trace::header its_header;
                    ...
                    (void)_instance;
#endif
                    its_endpoint->send_error(its_endpoint_def, its_serializer->get_data(),
...
std::chrono::steady_clock::time_point routing_manager_impl::expire_subscriptions(bool _force)
{
    ...
    for (auto& its_service : its_eventgroups)
    {
        for (auto& its_instance : its_service.second)
        {
            ...
            {
                auto its_subscriptions = its_eventgroup.second->get_remote_subscriptions();
                for (auto& s : its_subscriptions)
                {
                    if (!s)
                    {
                        VSOMEIP_ERROR
                            << __func__ << ": Remote subscription is NULL for eventgroup ["
                            << std::hex << std::setfill('0') << std::setw(4) << its_service.first
                            << "." << std::setw(4) << its_instance.first << "." << std::setw(4)
    ...
    for (auto& s : its_expired_subscriptions)
    {
        ...
        if (its_info)
        {
            ...
            std::unique_lock<std::mutex> its_update_lock{update_remote_subscription_mutex_};
            auto                         its_result = its_info->update_remote_subscription(
                s.first, std::chrono::steady_clock::now(), s.second, its_id, false);
            if (its_result)
            {
                const client_t its_offering_client = find_local_client(its_service, its_instance);
                ...
                {
                    VSOMEIP_ERROR << __func__ << ": Unknown expired subscription " << std::dec
                                  << its_id << " for eventgroup [" << std::hex << std::setfill('0')
                                  << std::setw(4) << its_service << "." << std::setw(4)
                                  << its_instance << "." << std::setw(4) << its_eventgroup << "]";
                }
                send_expired_subscription(its_offering_client, its_service, its_instance,
                                          its_eventgroup, s.second, s.first->get_id());
            }

            if (s.first->get_unreliable())
            {
                VSOMEIP_INFO << (_force ? "Removed" : "Expired") << " subscription [" << std::hex
                             << std::setfill('0') << std::setw(4) << its_service << "."
                             << std::setw(4) << its_instance << "." << std::setw(4)
                             << its_eventgroup << "] unreliable from "
                             << s.first->get_unreliable()->get_address() << ":" << std::dec
            ...
            if (s.first->get_reliable())
            {
                VSOMEIP_INFO << (_force ? "Removed" : "Expired") << " subscription [" << std::hex
                             << std::setfill('0') << std::setw(4) << its_service << "."
                             << std::setw(4) << its_instance << "." << std::setw(4)
                             << its_eventgroup << "] reliable from "
                             << s.first->get_reliable()->get_address() << ":" << std::dec
            ...
            if (last_resume_ != std::chrono::steady_clock::time_point::min())
            {
                its_last_resume << " | " << std::dec
                                << std::chrono::duration_cast<std::chrono::seconds>(
                                       std::chrono::steady_clock::now() - last_resume_)
                                       .count()
                                << "s";
            }
        }
...
{
    {
        ...
        if (found_service != local_services_.end())
        {
            ...
            if (found_instance != found_service->second.end())
            {
                ...
                if (its_stored_major == _major && its_stored_minor == _minor
                    && its_stored_client == _client)
                {
                    VSOMEIP_WARNING << "routing_manager_impl::handle_local_offer_service: "
                                    ...
                                    << std::setw(4) << _client << " is offering: [" << std::setw(4)
                                    << _service << "." << std::setw(4) << _instance << "."
                                    << std::dec << static_cast<std::uint32_t>(_major) << "."
                                    << _minor << "] offered previously by itself.";
                    return false;
                }
                else if (its_stored_major == _major && its_stored_minor == _minor
                         && its_stored_client != _client)
                {
                    ...
                    if (!already_pinged)
                    {
                        ...
                        if (its_old_endpoint)
                        {
                            ...
                            if (stub_ && stub_->send_ping(its_stored_client))
                            {
                                pending_offers_[_service][_instance] =
                                    std::make_tuple(_major, _minor, _client, its_stored_client);
                                VSOMEIP_WARNING
                                    << "OFFER(" << std::hex << std::setfill('0') << std::setw(4)
                                    << _client << "): [" << std::setw(4) << _service << "."
                                    << std::setw(4) << _instance << ":" << std::dec << int(_major)
                                    << "." << std::dec << _minor
                        ...
                        else if (its_stored_client == host_->get_client())
                        {
                            VSOMEIP_ERROR
                                << "routing_manager_impl::handle_local_offer_service: "
                                << "rejecting service registration. Application: " << std::hex
                    ...
                    else
                    {
                        VSOMEIP_INFO << __func__ << std::hex << std::setw(4) << std::setfill('0')
                                     << _client << "): [" << std::hex << std::setw(4)
                                     << std::setfill('0') << _service << "." << std::hex
                                     << std::setw(4) << std::setfill('0') << _instance << ":"
                                     << std::dec << int(_major) << "." << std::dec << _minor << "]"
                ...
                else
                {
                    VSOMEIP_ERROR << "routing_manager_impl::handle_local_offer_service: "
                                  ...
                                  << std::setw(4) << its_stored_client << ": [" << std::setw(4)
                                  << _service << "." << std::setw(4) << _instance << "." << std::dec
                                  << static_cast<std::uint32_t>(its_stored_major) << "."
                                  << its_stored_minor << "]";
                    return false;
        ...
        else
        {
            VSOMEIP_ERROR << "routing_manager_impl::handle_local_offer_service: "
                          << "rejecting service registration. Application: " << std::hex
                          << std::setfill('0') << std::setw(4) << _client << " is trying to offer ["
                          << std::setw(4) << _service << "." << std::setw(4) << _instance << "."
    ...
    for (auto service_iter = pending_offers_.begin(); service_iter != pending_offers_.end();)
    {
             ...
             instance_iter != service_iter->second.end();)
        {
            if (std::get<3>(instance_iter->second) == _client)
            {
                ...
                VSOMEIP_WARNING << "OFFER(" << std::hex << std::setfill('0') << std::setw(4)
                                << std::get<2>(instance_iter->second) << "): [" << std::setw(4)
                                << service_iter->first << "." << std::setw(4)
                                << instance_iter->first << ":" << std::dec
                                << std::uint32_t(std::get<0>(instance_iter->second)) << "."
                                << std::get<1>(instance_iter->second)
                                << "] was rejected as application: " << std::hex << std::setw(4)
...
void routing_manager_impl::handle_client_error(client_t _client)
{
    ...
    std::forward_list<std::tuple<client_t, service_t, instance_t, major_version_t, minor_version_t>>
        its_offers;
    {
        std::lock_guard<std::mutex> its_lock(pending_offers_mutex_);
        if (pending_offers_.size() == 0)
                 ...
                 instance_iter != service_iter->second.end();)
            {
                if (std::get<3>(instance_iter->second) == _client)
                {
                    VSOMEIP_WARNING
                        << "OFFER(" << std::hex << std::setfill('0') << std::setw(4)
                        << std::get<2>(instance_iter->second) << "): [" << std::setw(4)
                        << service_iter->first << "." << std::setw(4) << instance_iter->first << ":"
                        << std::dec << std::uint32_t(std::get<0>(instance_iter->second)) << "."
                        << std::get<1>(instance_iter->second)
                        << "] is not pending anymore as application: " << std::hex << std::setw(4)
                        << std::get<3>(instance_iter->second) << " is dead. Offering again!";
                    its_offers.push_front(std::make_tuple(std::get<2>(instance_iter->second),
                                                          service_iter->first, instance_iter->first,
                                          ...
                                          const std::shared_ptr<debounce_filter_impl_t>& _filter)
{
    ...
    if (endpoint && stub_)
    {
        stub_->send_subscribe(endpoint, _client, _service, _instance, _eventgroup, _major, _event,
                              _filter, PENDING_SUBSCRIPTION_ID);
    }
}
...
void routing_manager_impl::set_routing_state(routing_state_e _routing_state)
{
    ...
    if (discovery_)
    {
        ...
        case routing_state_e::RS_SUSPENDED: {
            ...
            VSOMEIP_INFO << "rmi::" << __func__
                         << " Inform all applications that we are going to suspend";
            send_suspend();
            ...
            for (const auto& its_service : get_offered_services())
            {
                for (const auto& its_instance : its_service.second)
                {
                    ...
                    {
                        const client_t its_client(
                            find_local_client(its_service.first, its_instance.first));
                        if (its_client == VSOMEIP_ROUTING_CLIENT)
                        {
                            // Inconsistency between services_ and local_services_ table detected
                            // --> cleanup.
                            VSOMEIP_WARNING << "rmi::" << __func__
                                            << " Found table inconsistency for [" << std::hex
                                            << std::setw(4) << std::setfill('0')
                                            << its_service.first << "." << std::hex << std::setw(4)
                                            << std::setfill('0') << its_instance.first << "]";

                            // Remove the service from the offer_commands_ and
                            // prepare_stop_handlers_ to force the next offer to be processed
                            offer_commands_.erase(
                                std::make_pair(its_service.first, its_instance.first));
                            if (has_reliable)
                                its_instance.second->get_endpoint(true)->remove_stop_handler(
                                    its_service.first);
                            if (has_unreliable)
                                its_instance.second->get_endpoint(false)->remove_stop_handler(
                                    its_service.first);

                            del_routing_info(its_service.first, its_instance.first, has_reliable,
                                             has_unreliable);
                        ...
                        VSOMEIP_WARNING << "Service " << std::hex << std::setfill('0')
                                        << std::setw(4) << its_service.first << "." << std::setw(4)
                                        << its_instance.first << " still offered by "
            ...
            VSOMEIP_INFO << "rmi::" << __func__
                         << " Set routing to suspend mode done, diagnosis mode is "
                         << ((discovery_->get_diagnosis_mode() == true) ? "active." : "inactive.");
        ...
        case routing_state_e::RS_RESUMED: {
            ...
            VSOMEIP_INFO << "rmi::" << __func__
                         << " Set routing to resume mode done, diagnosis mode was "
                         << ((discovery_->get_diagnosis_mode() == true) ? "active." : "inactive.");
            break;
        }
        case routing_state_e::RS_DIAGNOSIS: {
            ...
            for (const auto& its_service : get_offered_services())
            {
                for (const auto& its_instance : its_service.second)
                {
                    if (host_->get_configuration()->is_someip(its_service.first,
                                                              its_instance.first))
                    {
        ...
        case routing_state_e::RS_RUNNING:
            ...
            VSOMEIP_INFO << "rmi::" << __func__
                         << " Set routing to running mode done, diagnosis mode was "
                         << ((discovery_->get_diagnosis_mode() == true) ? "active." : "inactive.");
            break;
        default:
            break;
...
void routing_manager_impl::on_net_interface_or_route_state_changed(bool               _is_interface,
                                                                   ...
                                                                   bool               _available)
{
    ...
    if (_is_interface)
    {
        if (if_state_running_ || (_available && !if_state_running_ && routing_running_))
        {
    ...
    {
        if (sd_route_set_ || (_available && !sd_route_set_ && routing_running_))
        {
            log_change_message(true);
        }
...
void routing_manager_impl::add_requested_service(client_t _client, service_t _service,
                                                 instance_t _instance, major_version_t _major,
                                                 minor_version_t _minor)
{
...
void routing_manager_impl::remove_requested_service(client_t _client, service_t _service,
                                                    instance_t _instance, major_version_t _major,
                                                    minor_version_t _minor)
{
    std::lock_guard<std::mutex> ist_lock(requested_services_mutex_);
    ...
    auto delete_client = [&_client](minor_map_t::iterator&       _minor_iter,
                                    const major_map_t::iterator& _parent_major_iter) {
        if (_minor_iter->second.erase(_client))
        { // client was requester
            ...
            { // there are still other requesters of this minor version
                ++_minor_iter;
            }
        }
        else
        { // client wasn't requester
            ++_minor_iter;
        }
    };

    auto handle_minor = [&_minor,
                         &delete_client](major_map_t::iterator&          _major_iter,
...
routing_manager_impl::get_requested_services(client_t _client)
{
    ...
    for (const auto& service : requested_services_)
    {
        for (const auto& instance : service.second)
        {
            ...
            if (requested)
            {
                its_requests.push_back(std::make_pair(service.first, instance.first));
                break;
            }
        }
    }
...
std::set<client_t> routing_manager_impl::get_requesters(service_t _service, instance_t _instance,
...
std::set<client_t> routing_manager_impl::get_requesters_unlocked(service_t       _service,
                                                                 instance_t      _instance,
                                                                 major_version_t _major,
                                                                 minor_version_t _minor)
{
    ...
    for (const auto& its_major : found_instance->second)
    {
        if (its_major.first == _major || _major == DEFAULT_MAJOR || its_major.first == ANY_MAJOR)
        {
            ...
            {
                if (its_minor.first <= _minor || _minor == DEFAULT_MINOR
...
bool routing_manager_impl::has_requester_unlocked(service_t _service, instance_t _instance,
                                                  major_version_t _major, minor_version_t _minor)
{
    auto found_service = requested_services_.find(_service);
    if (found_service == requested_services_.end())
...
std::set<eventgroup_t> routing_manager_impl::get_subscribed_eventgroups(service_t  _service,
                                                                        instance_t _instance)
{
...
void routing_manager_impl::clear_targets_and_pending_sub_from_eventgroups(service_t  _service,
                                                                          instance_t _instance)
{
    std::vector<std::shared_ptr<event>> its_events;
    {
...
void routing_manager_impl::clear_remote_subscriber(service_t _service, instance_t _instance)
{
    std::lock_guard<std::mutex> its_lock(remote_subscribers_mutex_);
    auto                        found_service = remote_subscribers_.find(_service);
    if (found_service != remote_subscribers_.end())
...
void routing_manager_impl::call_sd_endpoint_connected(
    const boost::system::error_code& _error, service_t _service, instance_t _instance,
    ...
    const std::shared_ptr<debounce_filter_impl_t>& _filter, client_t _client)
{
    ...
    if (its_local_client == host_->get_client())
    {
        ...
        register_event(host_->get_client(), _service, _instance, _event, its_eventgroups,
                       event_type_e::ET_UNKNOWN, reliability_type_e::RT_UNKNOWN,
    ...
    else if (its_local_client != VSOMEIP_ROUTING_CLIENT)
    {
        ...
        register_event(its_local_client, _service, _instance, _event, its_eventgroups,
                       event_type_e::ET_UNKNOWN, reliability_type_e::RT_UNKNOWN,
    ...
    else
    {
        ...
        if (its_info && !its_info->is_local())
        {
            ...
            register_event(_client, _service, _instance, _event, its_eventgroups,
                           event_type_e::ET_UNKNOWN, reliability_type_e::RT_UNKNOWN,
                           std::chrono::milliseconds::zero(), false, true, nullptr, false, true,
                           true);
        }
        else
        {
            VSOMEIP_WARNING << "routing_manager_impl::create_placeholder_event_and_subscribe("
                            << std::hex << std::setfill('0') << std::setw(4) << _client << "): ["
                            << std::setw(4) << _service << "." << std::setw(4) << _instance << "."
                            << std::setw(4) << _eventgroup << "." << std::setw(4) << _event << "]"
...
void routing_manager_impl::handle_subscription_state(client_t _client, service_t _service,
                                                     instance_t _instance, eventgroup_t _eventgroup,
                                                     event_t _event)
{
#if 0
...
void routing_manager_impl::sd_acceptance_enabled(const boost::asio::ip::address&    _address,
                                                 const configuration::port_range_t& _range,
                                                 bool                               _reliable)
{
...
void routing_manager_impl::memory_log_timer_cbk(boost::system::error_code const& _error)
{
    ...
    VSOMEIP_INFO << "memory usage: "
                 ...
                 << "| monotonic time: " << std::dec << monots.tv_sec << "." << std::dec
                 << monots.tv_nsec << " cpu time: " << std::dec << cputs.tv_sec << "." << std::dec
                 << cputs.tv_nsec;
#endif

    {
        std::lock_guard<std::mutex> its_lock(memory_log_timer_mutex_);
        boost::system::error_code   ec;
        memory_log_timer_.expires_from_now(
            std::chrono::seconds(configuration_->get_log_memory_interval()), ec);
        memory_log_timer_.async_wait(
            std::bind(&routing_manager_impl::memory_log_timer_cbk, this, std::placeholders::_1));
    }
}
...
void routing_manager_impl::status_log_timer_cbk(boost::system::error_code const& _error)
{
    ...
    ep_mgr_impl_->print_status();
    {
        std::lock_guard<std::mutex> its_lock(status_log_timer_mutex_);
        boost::system::error_code   ec;
        status_log_timer_.expires_from_now(
            std::chrono::seconds(configuration_->get_log_status_interval()), ec);
        status_log_timer_.async_wait(
            std::bind(&routing_manager_impl::status_log_timer_cbk, this, std::placeholders::_1));
    }
}
                                              ...
                                              remote_subscription_id_t _id)
{
    ...
    if (its_info)
    {
        ...
        {
            VSOMEIP_ERROR << __func__ << ": Unknown StopSubscribe " << std::dec << _id
                          << " for eventgroup [" << std::hex << std::setfill('0') << std::setw(4)
                          << _service << "." << std::setw(4) << _instance << "." << std::setw(4)
                          << _eventgroup << "]";
        }
    }
    else
    {
        VSOMEIP_ERROR << __func__ << ": Received StopSubscribe for unknown eventgroup: ("
                      << std::hex << std::setfill('0') << std::setw(4) << _client << "): ["
                      << std::setw(4) << _service << "." << std::setw(4) << _instance << "."
...
void routing_manager_impl::send_subscription(const client_t  _offering_client,
                                             const service_t _service, const instance_t _instance,
                                             const eventgroup_t             _eventgroup,
                                             const major_version_t          _major,
                                             ...
                                             const remote_subscription_id_t _id)
{
    if (host_->get_client() == _offering_client)
    {
        ...
        {
            host_->on_subscription(
                _service, _instance, _eventgroup, its_client, get_sec_client(), get_env(its_client),
                true,
                [this, self, _service, _instance, _eventgroup, its_client,
    ...
    { // service hosted by local client
        ...
        {
            if (stub_
                && !stub_->send_subscribe(find_local(_offering_client), its_client, _service,
                                          _instance, _eventgroup, _major, ANY_EVENT, nullptr, _id))
            {
                try
                {
                    const auto its_callback = std::bind(
...
void routing_manager_impl::cleanup_server_endpoint(service_t                        _service,
                                                   const std::shared_ptr<endpoint>& _endpoint)
{
    if (_endpoint)
    {
        ...
        {
            if (ep_mgr_impl_->remove_server_endpoint(_endpoint->get_local_port(),
...
pending_remote_offer_id_t routing_manager_impl::pending_remote_offer_add(service_t  _service,
                                                                         instance_t _instance)
{
...
std::pair<service_t, instance_t>
routing_manager_impl::pending_remote_offer_remove(pending_remote_offer_id_t _id)
{
    std::lock_guard<std::mutex>      its_lock(pending_remote_offers_mutex_);
...
void routing_manager_impl::service_endpoint_connected(service_t _service, instance_t _instance,
                                                      major_version_t                  _major,
                                                      minor_version_t                  _minor,
                                                      const std::shared_ptr<endpoint>& _endpoint,
    ...
    if (!ec)
    {
        its_timer->async_wait(
            std::bind(&routing_manager_impl::call_sd_endpoint_connected,
                      std::static_pointer_cast<routing_manager_impl>(shared_from_this()),
                      std::placeholders::_1, _service, _instance, _endpoint, its_timer));
    }
    else
    {
...
void routing_manager_impl::service_endpoint_disconnected(service_t _service, instance_t _instance,
                                                         major_version_t                  _major,
                                                         minor_version_t                  _minor,
                                                         const std::shared_ptr<endpoint>& _endpoint)
{
    ...
    VSOMEIP_WARNING << __func__ << ": lost connection to remote service: [" << std::hex
                    << std::setfill('0') << std::setw(4) << _service << "." << std::setw(4)
                    << _instance << "]";
}

void routing_manager_impl::send_unsubscription(client_t _offering_client, service_t _service,
                                               instance_t _instance, eventgroup_t _eventgroup,
                                               major_version_t           _major,
                                               ...
                                               remote_subscription_id_t  _id)
{
    ...
    if (host_->get_client() == _offering_client)
    {
        ...
        {
            host_->on_subscription(_service, _instance, _eventgroup, its_client, get_sec_client(),
    ...
    else
    {
        ...
        {
            if (stub_
                && !stub_->send_unsubscribe(find_local(_offering_client), its_client, _service,
                                            _instance, _eventgroup, ANY_EVENT, _id))
            {
                try
                {
                    const auto its_callback = std::bind(
...
void routing_manager_impl::send_expired_subscription(client_t _offering_client, service_t _service,
                                                     instance_t _instance, eventgroup_t _eventgroup,
                                                     const std::set<client_t>& _removed,
                                                     remote_subscription_id_t  _id)
{
    ...
    else
    {
        ...
        {
            if (stub_)
                stub_->send_expired_subscription(find_local(_offering_client), its_client, _service,
                                                 _instance, _eventgroup, ANY_EVENT, _id);
        }
    }
}
...
bool routing_manager_impl::update_security_policy_configuration(
    uid_t _uid, gid_t _gid, const std::shared_ptr<policy>& _policy,
...
void routing_manager_impl::statistics_log_timer_cbk(boost::system::error_code const& _error)
{
    if (!_error)
    {
            ...
            for (const auto& s : message_statistics_)
            {
                if (s.second.counter_ / (its_interval / 1000) > its_min_freq)
                {
                    ...
                    std::shared_ptr<event> its_event = find_event(
                        std::get<0>(s.first), std::get<1>(s.first), std::get<2>(s.first));
                    if (its_event)
                    {
                        if (!its_event->is_provided())
                        {
                            its_subscribed =
                                static_cast<std::uint16_t>(its_event->get_subscribers().size());
                        }
                    }
                    its_log << std::hex << std::setfill('0') << std::setw(4) << std::get<0>(s.first)
                            << "." << std::get<1>(s.first) << "." << std::get<2>(s.first)
                            << ": #=" << std::dec << s.second.counter_
                            << " L=" << s.second.avg_length_ << " S=" << std::dec << its_subscribed
                            << ", ";
                }
            }
...

        {
            ...
            statistics_log_timer_.async_wait(std::bind(
                &routing_manager_impl::statistics_log_timer_cbk, this, std::placeholders::_1));
        }
    }
}

bool routing_manager_impl::get_guest(client_t _client, boost::asio::ip::address& _address,
                                     port_t& _port) const
{
    return routing_manager_base::get_guest(_client, _address, _port);
}

void routing_manager_impl::add_guest(client_t _client, const boost::asio::ip::address& _address,
                                     port_t _port)
{
...
void routing_manager_impl::remove_subscriptions(port_t                          _local_port,
                                                const boost::asio::ip::address& _remote_address,
                                                port_t                          _remote_port)
{
    std::map<service_t,
