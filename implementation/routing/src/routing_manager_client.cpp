...
routing_manager_client::routing_manager_client(
    routing_manager_host* _host, bool _client_side_logging,
        ...
        else
        {
            ...
            if (local_link_connector_)
            {
                local_link_connector_->register_net_if_changes_handler(
                    std::bind(&routing_manager_client::on_net_state_change, this,
...
void routing_manager_client::stop()
{
    ...
    if (configuration_->is_local_routing())
    {
        ...
        its_client << utility::get_base_path(configuration_->get_network()) << std::hex
                   << get_client();
#ifdef _WIN32
        ...
        {
            VSOMEIP_ERROR << "routing_manager_proxy::stop unlink failed (" << its_client.str()
                          << "): " << std::strerror(errno);
        }
#endif
    }
...
void routing_manager_client::on_net_state_change(bool _is_interface, const std::string& _name,
                                                 bool _is_available)
{
    VSOMEIP_INFO << __func__ << ": " << std::boolalpha << _is_interface << " " << _name << " "
...
bool routing_manager_client::offer_service(client_t _client, service_t _service,
...
void routing_manager_client::send_offer_service(client_t _client, service_t _service,
                                                instance_t _instance, major_version_t _major,
                                                minor_version_t _minor)
{
    (void)_client;
    ...
    {
        VSOMEIP_ERROR << __func__ << ": offer_service serialization failed (" << std::dec
                      << static_cast<int>(its_error) << ")";
    }
}

void routing_manager_client::stop_offer_service(client_t _client, service_t _service,
                                                instance_t _instance, major_version_t _major,
                                                minor_version_t _minor)
{
    (void)_client;
...

    {
        ...
        if (state_ == inner_state_type_e::ST_REGISTERED)
        {
            ...
            {
                VSOMEIP_ERROR << __func__ << ": stop offer serialization failed (" << std::dec
                              << static_cast<int>(its_error) << ")";
            }
        }
...
void routing_manager_client::request_service(client_t _client, service_t _service,
                                             instance_t _instance, major_version_t _major,
                                             minor_version_t _minor)
{
    routing_manager_base::request_service(_client, _service, _instance, _major, _minor);
    {
        ...
        else
        {
            ...
            if (!request_debounce_timer_running_)
            {
                ...
                request_debounce_timer_.expires_from_now(
                    std::chrono::milliseconds(request_debouncing_time));
                request_debounce_timer_.async_wait(
                    std::bind(&routing_manager_client::request_debounce_timeout_cbk,
...
void routing_manager_client::release_service(client_t _client, service_t _service,
...
void routing_manager_client::register_event(
    ...
    bool _is_shadow, bool _is_cache_placeholder)
{
    ...
    const event_data_t registration = {_service,     _instance,    _notifier, _type,
                                       _reliability, _is_provided, is_cyclic, _eventgroups};
    bool               is_first(false);
    {
        std::lock_guard<std::mutex> its_lock(state_mutex_);
        is_first =
            pending_event_registrations_.find(registration) == pending_event_registrations_.end();
        ...
        if (is_first)
        {
            ...
            {
                if (iter->service_ == _service && iter->instance_ == _instance
    ...
    }
    {
        ...
        {
            send_register_event(get_client(), _service, _instance, _notifier, _eventgroups, _type,
                                _reliability, _is_provided, is_cyclic);
        }
    }
}

void routing_manager_client::unregister_event(client_t _client, service_t _service,
...
void routing_manager_client::subscribe(client_t _client, const vsomeip_sec_client_t* _sec_client,
...
void routing_manager_client::send_subscribe(client_t _client, service_t _service,
                                            ...
                                            const std::shared_ptr<debounce_filter_impl_t>& _filter)
{
    if (_event == ANY_EVENT)
    {
        if (!is_subscribe_to_any_event_allowed(get_sec_client(), _client, _service, _instance,
                                               _eventgroup))
        {
            VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << _client
                            << " : routing_manager_proxy::subscribe: "
                            << " isn't allowed to subscribe to service/instance/event " << _service
                            << "/" << _instance << "/ANY_EVENT"
    ...
    {
        VSOMEIP_ERROR << __func__ << ": subscribe command serialization failed (" << std::dec
                      << static_cast<int>(its_error) << ")";
    }
}

void routing_manager_client::send_subscribe_nack(client_t _subscriber, service_t _service,
                                                 instance_t _instance, eventgroup_t _eventgroup,
                                                 event_t _event, remote_subscription_id_t _id)
{
    ...
    {
        VSOMEIP_ERROR << __func__ << ": subscribe nack serialization failed (" << std::dec
                      << static_cast<int>(its_error) << ")";
    }
}

void routing_manager_client::send_subscribe_ack(client_t _subscriber, service_t _service,
                                                instance_t _instance, eventgroup_t _eventgroup,
                                                event_t _event, remote_subscription_id_t _id)
{
    ...
    {
        VSOMEIP_ERROR << __func__ << ": subscribe ack serialization failed (" << std::dec
                      << static_cast<int>(its_error) << ")";
    }
}

void routing_manager_client::unsubscribe(client_t _client, const vsomeip_sec_client_t* _sec_client,
                                         service_t _service, instance_t _instance,
                                         eventgroup_t _eventgroup, event_t _event)
{
...

    {
        ...
        if (state_ == inner_state_type_e::ST_REGISTERED)
        {
            ...
            {
                VSOMEIP_ERROR << __func__ << ": unsubscribe serialization failed (" << std::dec
                              << static_cast<int>(its_error) << ")";
            }
        }
    }
...
bool routing_manager_client::send(client_t _client, const byte_t* _data, length_t _size,
                                  ...
                                  bool _sent_from_remote, bool _force)
{
            ...
            if (client_side_logging_filter_.empty()
                || (1
                    == client_side_logging_filter_.count(
                        std::make_tuple(its_service, ANY_INSTANCE)))
                || (1
                    == client_side_logging_filter_.count(std::make_tuple(its_service, _instance))))
            {
                ...
                VSOMEIP_INFO << "routing_manager_client::send: (" << std::hex << std::setfill('0')
                             << std::setw(4) << get_client() << "): [" << std::setw(4)
                             << its_service << "." << std::setw(4) << _instance << "."
                             << std::setw(4) << its_method << ":" << std::setw(4) << its_session
                             << ":" << std::setw(4) << its_client << "] "
                             << "type=" << std::hex
                             << static_cast<std::uint32_t>(_data[VSOMEIP_MESSAGE_TYPE_POS])
                             << " thread=" << std::hex << std::this_thread::get_id();
            }
        }
        else
        {
            VSOMEIP_ERROR << "routing_manager_client::send: (" << std::hex << std::setw(4)
    ...
    if (_size > VSOMEIP_MESSAGE_TYPE_POS)
    {
        ...
        {
            // notify
            has_remote_subscribers = send_local_notification(get_client(), _data, _size, _instance,
...
#endif
                return send_local(its_target, get_client(), _data, _size, _instance, _reliable,
        ...
        {
            is_sent = send_local(
                its_target, (its_command == protocol::id_e::NOTIFY_ONE_ID ? _client : get_client()),
...
bool routing_manager_client::send_to(const std::shared_ptr<endpoint_definition>& _target,
...
void routing_manager_client::on_message(const byte_t* _data, length_t _size, endpoint* _receiver,
    ...
    if (its_error == protocol::error_e::ERROR_OK)
    {
        ...
        case protocol::id_e::SEND_ID: {
            ...
            if (its_error == protocol::error_e::ERROR_OK)
            {
                ...
                if (its_message)
                {
                    ...
                    if (!is_from_routing)
                    {
                        if (utility::is_notification(its_message->get_message_type()))
                        {
                                                     ...
                                                     its_message->get_method()))
                            {
                                VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex
                                                ...
                                                << " received a notification from client 0x"
                                                << _bound_client
                            ...
                            else
                            {
                                ...
                                {
                                    VSOMEIP_WARNING
                                        << "vSomeIP Security: Client 0x" << std::hex << std::setw(4)
                                        << std::setfill('0') << get_client()
                                        << " : routing_manager_client::on_message: "
                        ...
                        else if (utility::is_request(its_message->get_message_type()))
                        {
                            ...
                            {
                                VSOMEIP_WARNING
                                    << std::hex << "vSomeIP Security: Client 0x" << std::hex
                                    << std::setw(4) << std::setfill('0') << get_client()
                                    << " received a request from client 0x" << std::hex
                                    << std::setw(4) << std::setfill('0')
                                    << its_message->get_client() << " to service/instance/method "
                                    << its_message->get_service() << "/"
                                    << its_message->get_instance() << "/"
                                    << its_message->get_method()
                                    << " which doesn't match the bound client 0x" << std::hex
                                    << std::setw(4) << std::setfill('0') << _bound_client
                                    << " ~> skip message!";
                                return;
                            }
                                           ...
                                           its_message->get_instance(), its_message->get_method()))
                            {
                                VSOMEIP_WARNING
                                    << "vSomeIP Security: Client 0x" << std::hex << std::setw(4)
                                    << std::setfill('0') << its_message->get_client()
                                    << " : routing_manager_client::on_message: "
                        ...
                        { // response
                                                     ...
                                                     its_message->get_method()))
                            {
                                VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex
                                                << std::setw(4) << std::setfill('0') << get_client()
                                                << " : routing_manager_client::on_message: "
                                                << " received a response from client 0x"
                                                << _bound_client
                    ...
                    else
                    {
                        if (!configuration_->is_remote_access_allowed())
                        {
                            ...
                            VSOMEIP_WARNING
                                << "vSomeIP Security: Client 0x" << std::hex << std::setw(4)
                                << std::setfill('0') << get_client()
                                << " : routing_manager_client::on_message: " << std::hex
                                << "Security: Remote clients via routing manager with client ID 0x"
                                << its_client
                    ...
                    if (client_side_logging_
                        && (client_side_logging_filter_.empty()
                            || (1
                                == client_side_logging_filter_.count(
                                    std::make_tuple(its_message->get_service(), ANY_INSTANCE)))
                            || (1
                                    ...
                                    its_message->get_service(), its_message->get_instance())))))
                    {
                        ...
                        if (its_header.prepare(nullptr, false, its_send_command.get_instance()))
                        {
                            ...
                            if (its_message_size
                                >= uint32_t{vsomeip_v3::protocol::SEND_COMMAND_HEADER_SIZE})
                                its_message_size -=
                                    uint32_t{vsomeip_v3::protocol::SEND_COMMAND_HEADER_SIZE};
                            else
                                its_message_size = 0;
                            ...
                            tc_->trace(its_header.data_, VSOMEIP_TRACE_HEADER_SIZE,
                                       &_data[vsomeip_v3::protocol::SEND_COMMAND_HEADER_SIZE],
                                       its_message_size);
                        }
                    }
#endif
                ...
                VSOMEIP_ERROR << __func__ << ": send command deserialization failed (" << std::dec
                              << static_cast<int>(its_error) << ")";
            break;
        }
        ...
        case protocol::id_e::ROUTING_INFO_ID:
            ...
            else
            {
                VSOMEIP_WARNING
                    << "routing_manager_client::on_message: "
                    << "Security: Client 0x" << std::hex << std::setw(4) << std::setfill('0')
                    << get_client()
        ...
        case protocol::id_e::PING_ID: {
            ...
            {
                send_pong();
                VSOMEIP_TRACE << "PING(" << std::hex << std::setw(4) << std::setfill('0')
                              << get_client() << ")";
            }
            else
                VSOMEIP_ERROR << __func__ << ": ping command deserialization failed (" << std::dec
                              << int(its_error) << ")";
            break;
        }
            ...
            if (its_error == protocol::error_e::ERROR_OK)
            {
                             ...
                             its_major](const bool _subscription_accepted) {
                                ...
                                if (_subscription_accepted)
                                {
                                    ...
                                    bool              inserted = insert_subscription(
                                        ...
                                        its_filter, VSOMEIP_ROUTING_CLIENT,
                                ...
                                VSOMEIP_INFO << "SUBSCRIBE(" << std::hex << std::setw(4)
                                             << std::setfill('0') << its_client << "): ["
                                             << std::hex << std::setw(4) << std::setfill('0')
                                             << its_service << "." << std::hex << std::setw(4)
                                             << std::setfill('0') << its_instance << "." << std::hex
                                             << std::setw(4) << std::setfill('0') << its_eventgroup
                                             << ":" << std::hex << std::setw(4) << std::setfill('0')
                ...
                else if (is_client_known(its_client))
                {
                    ...
                    if (!is_from_routing)
                    {
                        ...
                        {
                            if (!is_subscribe_to_any_event_allowed(_sec_client, its_client,
                                                                   its_service, its_instance,
                                                                   its_eventgroup))
                            {
                                VSOMEIP_WARNING
                                    << "vSomeIP Security: Client 0x" << std::hex << its_client
                                    << " : routing_manager_client::on_message: "
                        ...
                        else
                        {
                                           ...
                                           _sec_client, its_service, its_instance, its_event))
                            {
                                VSOMEIP_WARNING
                                    << "vSomeIP Security: Client 0x" << std::hex << its_client
                                    << " : routing_manager_client::on_message: "
                                    << " subscribes to service/instance/event " << its_service
                    ...
                    else
                    {
                        if (!configuration_->is_remote_access_allowed())
                        {
                            VSOMEIP_WARNING
                                << "vSomeIP Security: Client 0x" << std::hex << its_client
                                << " : routing_manager_client::on_message: " << std::hex
                                << "Routing manager with client ID 0x" << its_client
                                << " isn't allowed to subscribe to service/instance/event "
                ...
                { // local subscription
                    VSOMEIP_INFO << "SUBSCRIBE(" << std::hex << std::setfill('0') << std::setw(4)
                                 ...
                                 << its_eventgroup << ":" << std::setw(4) << its_event << ":"
                                 << std::dec << (uint16_t)its_major << "]";
                }
            }
            else
            {
                VSOMEIP_ERROR << __func__ << ": subscribe command deserialization failed ("
        ...
        case protocol::id_e::UNSUBSCRIBE_ID: {
            ...
            if (its_error == protocol::error_e::ERROR_OK)
            {
                ...
                if (its_pending_id == PENDING_SUBSCRIPTION_ID)
                {
                    ...
                    routing_manager_base::unsubscribe(its_client, _sec_client, its_service,
                                                      its_instance, its_eventgroup, its_event);
                }
                else
                {
                ...
                VSOMEIP_INFO << "UNSUBSCRIBE(" << std::hex << std::setfill('0') << std::setw(4)
                             << its_client << "): [" << std::setw(4) << its_service << "."
                             << std::setw(4) << its_instance << "." << std::setw(4)
                             << its_eventgroup << "." << std::setw(4) << its_event << "] "
                             << (bool)(its_pending_id != PENDING_SUBSCRIPTION_ID) << " " << std::dec
                             << its_remote_subscriber_count;
            }
            else
                VSOMEIP_ERROR << __func__ << ": unsubscribe command deserialization failed ("
                              << std::dec << static_cast<int>(its_error) << ")";
            break;
        }
        ...
        case protocol::id_e::EXPIRE_ID: {
            ...
            if (its_error == protocol::error_e::ERROR_OK)
            {
                ...
                VSOMEIP_INFO << "EXPIRED SUBSCRIPTION(" << std::hex << std::setfill('0')
                             << std::setw(4) << its_client << "): [" << std::setw(4) << its_service
                             << "." << std::setw(4) << its_instance << "." << std::setw(4)
                             << its_eventgroup << "." << std::setw(4) << its_event << "] "
                             << (bool)(its_pending_id != PENDING_SUBSCRIPTION_ID) << " " << std::dec
                             << its_remote_subscriber_count;
        ...
        case protocol::id_e::SUBSCRIBE_NACK_ID: {
            ...
            if (its_error == protocol::error_e::ERROR_OK)
            {
                ...
                on_subscribe_nack(its_subscriber, its_service, its_instance, its_eventgroup,
                                  its_event);
                VSOMEIP_INFO << "SUBSCRIBE NACK(" << std::hex << std::setfill('0') << std::setw(4)
                             << its_client << "): [" << std::setw(4) << its_service << "."
                             << std::setw(4) << its_instance << "." << std::setw(4)
                             << its_eventgroup << "." << std::setw(4) << its_event << "]";
            }
            else
                VSOMEIP_ERROR << __func__ << ": subscribe nack command deserialization failed ("
                              << std::dec << static_cast<int>(its_error) << ")";
            break;
        }
        ...
        case protocol::id_e::SUBSCRIBE_ACK_ID: {
            ...
            if (its_error == protocol::error_e::ERROR_OK)
            {
                ...
                on_subscribe_ack(its_subscriber, its_service, its_instance, its_eventgroup,
                                 its_event);
                VSOMEIP_INFO << "SUBSCRIBE ACK(" << std::hex << std::setfill('0') << std::setw(4)
                             << its_client << "): [" << std::setw(4) << its_service << "."
                             << std::setw(4) << its_instance << "." << std::setw(4)
                             << its_eventgroup << "." << std::setw(4) << its_event << "]";
            }
            else
                VSOMEIP_ERROR << __func__ << ": subscribe ack command deserialization failed ("
                              << std::dec << static_cast<int>(its_error) << ")";
            break;
        }
        ...
        case protocol::id_e::RESEND_PROVIDED_EVENTS_ID: {
            ...
            if (its_error == protocol::error_e::ERROR_OK)
            {
                ...
                VSOMEIP_INFO << "RESEND_PROVIDED_EVENTS(" << std::hex << std::setw(4)
                             << std::setfill('0') << its_command.get_client() << ")";
            }
            else
                VSOMEIP_ERROR << __func__
            ...
            if (!configuration_->is_security_enabled() || is_from_routing)
            {
                ...
                if (its_error == protocol::error_e::ERROR_OK)
                {
                    ...
                    if (its_policy->get_uid_gid(its_uid, its_gid))
                    {
                        ...
                        {
                            its_policy_manager->update_security_policy(its_uid, its_gid,
                                                                       its_policy);
        ...
        VSOMEIP_ERROR << __func__ << ": dummy command deserialization failed (" << std::dec
                      << static_cast<int>(its_error) << ")";
}
...
void routing_manager_client::on_routing_info(const byte_t* _data, uint32_t _size)
{
    ...
    if (its_error != protocol::error_e::ERROR_OK)
    {
        VSOMEIP_ERROR << __func__ << ": deserializing routing info command failed ("
                      << static_cast<int>(its_error) << ")";
        return;
    }
    ...
    for (const auto& e : its_command.get_entries())
    {
        ...
        case protocol::routing_info_entry_type_e::RIE_ADD_CLIENT: {
            ...
            {
                VSOMEIP_INFO << "Application/Client " << std::hex << std::setw(4)
                             << std::setfill('0') << get_client() << " (" << host_->get_name()
                             << ") is registered.";
#if defined(__linux__) || defined(ANDROID) || defined(__QNX__)
        ...
        case protocol::routing_info_entry_type_e::RIE_ADD_SERVICE_INSTANCE: {
            ...
            for (const auto& s : e.get_services())
            {
                ...
                VSOMEIP_INFO << "ON_AVAILABLE(" << std::hex << std::setfill('0') << std::setw(4)
                             << get_client() << "): [" << std::setw(4) << its_service << "."
                             << std::setw(4) << its_instance << ":" << std::dec << int(its_major)
                             << "." << std::dec << its_minor << "]";
            }
            break;
        }
        ...
        case protocol::routing_info_entry_type_e::RIE_DELETE_SERVICE_INSTANCE: {
            for (const auto& s : e.get_services())
            {
                ...
                VSOMEIP_INFO << "ON_UNAVAILABLE(" << std::hex << std::setfill('0') << std::setw(4)
                             << get_client() << "): [" << std::setw(4) << its_service << "."
                             << std::setw(4) << its_instance << ":" << std::dec << int(its_major)
                             << "." << std::dec << its_minor << "]";
            }
        ...
        default:
            VSOMEIP_ERROR << __func__ << ": Unknown routing info entry type ("
...

    {
        ...
        if (pending_incoming_subscriptions_.size())
        {
                        ...
                        for (const auto& subscription : its_client->second)
                        {
                            subscription_actions.push_front(
                                {subscription.service_, subscription.instance_,
                                 subscription.eventgroup_, k.first, subscription.major_,
                                 subscription.event_, subscription.filter_,
            ...
            for (const subscription_info& si : subscription_actions)
            {
                ...
                auto self = shared_from_this();
                host_->on_subscription(
                    si.service_id_, si.instance_id_, si.eventgroup_id_, si.client_id_,
                    &si.sec_client_, si.env_, true,
                    [this, self, si](const bool _subscription_accepted) {
                        if (!_subscription_accepted)
                        {
                            send_subscribe_nack(si.client_id_, si.service_id_, si.instance_id_,
                                                si.eventgroup_id_, si.event_,
                                                PENDING_SUBSCRIPTION_ID);
                        }
                        else
                        {
                            send_subscribe_ack(si.client_id_, si.service_id_, si.instance_id_,
                                               si.eventgroup_id_, si.event_,
                                               PENDING_SUBSCRIPTION_ID);
                            routing_manager_base::subscribe(
                                si.client_id_, &si.sec_client_, si.service_id_, si.instance_id_,
                                si.eventgroup_id_, si.major_, si.event_, si.filter_);
#ifdef VSOMEIP_ENABLE_COMPAT
                            send_pending_notify_ones(si.service_id_, si.instance_id_,
                                                     si.eventgroup_id_, si.client_id_);
#endif
                        }
#ifdef VSOMEIP_ENABLE_COMPAT
                        routing_manager_base::erase_incoming_subscription_state(
                            si.client_id_, si.service_id_, si.instance_id_, si.eventgroup_id_,
                            si.event_);
#endif
                        {
                            std::lock_guard<std::recursive_mutex> its_lock2(
...
void routing_manager_client::reconnect(const std::map<client_t, std::string>& _clients)
{
    ...
    VSOMEIP_INFO << std::hex << "Application/Client " << std::hex << std::setw(4)
                 << std::setfill('0') << get_client() << ": Reconnecting to routing manager.";

#if defined(__linux__) || defined(ANDROID) || defined(__QNX__)
    if (!its_policy_manager->check_credentials(get_client(), get_sec_client()))
    {
        VSOMEIP_ERROR << "vSomeIP Security: Client 0x" << std::hex << std::setw(4)
                      << std::setfill('0') << get_client()
...
void routing_manager_client::assign_client()
{
    ...
    if (its_error != protocol::error_e::ERROR_OK)
    {
        VSOMEIP_ERROR << __func__ << ": command creation failed (" << std::dec
                      << static_cast<int>(its_error) << ")";
        return;
    }
    ...
    if (is_connected_)
    {
        ...
        if (sender_)
        {
            ...
            {
                VSOMEIP_WARNING << __func__ << ": (" << std::hex << std::setw(4)
                                << std::setfill('0') << get_client()
                                << ") Non-Deregistered State Set. Returning";
                return;
            }
            ...
            register_application_timer_.async_wait(
                std::bind(&routing_manager_client::assign_client_timeout_cbk,
                          std::dynamic_pointer_cast<routing_manager_client>(shared_from_this()),
                          std::placeholders::_1));
        }
        else
...
void routing_manager_client::register_application()
{
    ...
    {
        VSOMEIP_INFO << "Registering to routing manager @ " << its_configuration->get_network()
                     << "-0";
    }
    else
    {
        ...
        VSOMEIP_INFO << "Registering to routing manager @ " << its_routing_address.to_string()
                     << ":" << its_routing_port;
    }
    ...
    if (its_error == protocol::error_e::ERROR_OK)
    {
        if (is_connected_)
        {
            ...
            if (sender_)
            {
                ...
                register_application_timer_.expires_from_now(std::chrono::milliseconds(3000));
                register_application_timer_.async_wait(
                    std::bind(&routing_manager_client::register_application_timeout_cbk,
                              std::dynamic_pointer_cast<routing_manager_client>(shared_from_this()),
                              std::placeholders::_1));
...
void routing_manager_client::deregister_application()
{
    ...
    if (its_error == protocol::error_e::ERROR_OK)
    {
        ...
        VSOMEIP_ERROR << __func__ << ": deregister application command serialization failed("
                      << std::dec << int(its_error) << ")";
}
...
void routing_manager_client::send_request_services(const std::set<protocol::service>& _requests)
{
    ...
    {
        VSOMEIP_ERROR << __func__ << ": request service serialization failed (" << std::dec
                      << static_cast<int>(its_error) << ")";
    }
}
...
void routing_manager_client::send_pending_event_registrations(client_t _client)
{
    ...
    its_command.set_client(_client);
    std::set<vsomeip_v3::routing_manager_client::event_data_t>::iterator it =
        pending_event_registrations_.begin();
    ...
    while (it != pending_event_registrations_.end())
    {
        ...
        if (its_error == protocol::error_e::ERROR_OK)
        {
            ...
            VSOMEIP_ERROR << __func__ << ": register event command serialization failed ("
                          << std::dec << int(its_error) << ")";
    }
}

void routing_manager_client::send_register_event(client_t _client, service_t _service,
                                                 instance_t _instance, event_t _notifier,
                                                 ...
                                                 bool _is_cyclic)
{
    ...
    protocol::register_event reg(_service, _instance, _notifier, _type, _is_provided, _reliability,
                                 _is_cyclic, (uint16_t)_eventgroups.size(), _eventgroups);
    ...
    if (its_error == protocol::error_e::ERROR_OK)
    {
        ...
        if (_is_provided)
        {
            VSOMEIP_INFO << "REGISTER EVENT(" << std::hex << std::setfill('0') << std::setw(4)
                         << get_client() << "): [" << std::setw(4) << _service << "."
                         << std::setw(4) << _instance << "." << std::setw(4) << _notifier
                         << ":is_provider=" << std::boolalpha << _is_provided << "]";
        }
    }
    else
        VSOMEIP_ERROR << __func__ << ": register event command serialization failed (" << std::dec
                      << int(its_error) << ")";
}

void routing_manager_client::on_subscribe_ack(client_t _client, service_t _service,
                                              instance_t _instance, eventgroup_t _eventgroup,
                                              event_t _event)
{
    (void)_client;
...
void routing_manager_client::on_subscribe_nack(client_t _client, service_t _service,
                                               instance_t _instance, eventgroup_t _eventgroup,
                                               event_t _event)
{
    (void)_client;
    if (_event == ANY_EVENT)
    {
        ...
        if (its_eventgroup)
        {
            ...
            {
                host_->on_subscription_status(_service, _instance, _eventgroup,
                                              its_event->get_event(), 0x7 /*Rejected*/);
            }
        }
    }
    else
...
void routing_manager_client::cache_event_payload(const std::shared_ptr<message>& _message)
{
    ...
    else
    {
        ...
        routing_manager_base::register_event(
            host_->get_client(), its_service, its_instance, its_method, its_eventgroups,
...
void routing_manager_client::on_stop_offer_service(service_t _service, instance_t _instance,
...
void routing_manager_client::send_pending_commands()
{
    for (auto& po : pending_offers_)
        send_offer_service(get_client(), po.service_, po.instance_, po.major_, po.minor_);
    ...
    if (its_eventgroup)
    {
        ...
        for (const auto& e : its_eventgroup->get_events())
        {
            if (e->is_field() && e->is_set()
                && _events_to_exclude.find(e->get_event()) == _events_to_exclude.end())
            {
                ...
                {
                    {
                        ...
                        if (sender_)
                        {
                            send_local(sender_, VSOMEIP_ROUTING_CLIENT, its_serializer->get_data(),
                                       its_serializer->get_size(), _instance, false,
...
void routing_manager_client::assign_client_timeout_cbk(boost::system::error_code const& _error)
{
    if (!_error)
    {
        bool register_again(false);
        {
            ...
            {
                VSOMEIP_INFO << __func__ << ": Will not retry registry for Client [0x" << std::hex
                             << std::setw(4) << std::setfill('0') << get_client()
                             << "] : already registered ";
            }
        }
    ...
    { // ignore error when timer is deliberately cancelled
        VSOMEIP_WARNING << __func__ << ": Ignoring Client 0x" << std::hex << std::setw(4)
                        << std::setfill('0') << get_client()
    ...
    boost::system::error_code const& _error)
{
    ...
    {
        std::lock_guard<std::mutex> its_lock(sender_mutex_);
        VSOMEIP_WARNING << std::hex << "Client 0x" << std::hex << std::setw(4) << std::setfill('0')
                        << get_client() << " register timeout! Trying again...";
...
void routing_manager_client::send_registered_ack()
{
    ...
    if (its_error == protocol::error_e::ERROR_OK)
    {
        ...
        VSOMEIP_ERROR << __func__ << ": registered ack command serialization failed (" << std::dec
                      << int(its_error) << ")";
}
...
bool routing_manager_client::create_placeholder_event_and_subscribe(
    service_t _service, instance_t _instance, eventgroup_t _eventgroup, event_t _notifier,
    const std::shared_ptr<debounce_filter_impl_t>& _filter, client_t _client)
{
    ...
    if (find_service(_service, _instance))
    {
        ...
        routing_manager_base::register_event(
            host_->get_client(), _service, _instance, _notifier, its_eventgroups,
            event_type_e::ET_UNKNOWN, reliability_type_e::RT_UNKNOWN,
            std::chrono::milliseconds::zero(), false, true, nullptr, false, false, true);
            ...
            {
                {
                    ...
                    request_debounce_timer_.async_wait(std::bind(
...
void routing_manager_client::register_client_error_handler(
    client_t _client, const std::shared_ptr<endpoint>& _endpoint)
{
...
void routing_manager_client::handle_client_error(client_t _client)
{
    if (_client != VSOMEIP_ROUTING_CLIENT)
    {
        VSOMEIP_INFO << "rmc::handle_client_error"
                     << "Client 0x" << std::hex << std::setw(4) << std::setfill('0') << get_client()
                     << " handles a client error(" << std::hex << std::setw(4) << std::setfill('0')
...
void routing_manager_client::send_get_offered_services_info(client_t     _client,
                                                            offer_type_e _offer_type)
{
    ...
    if (its_error == protocol::error_e::ERROR_OK)
    {
        ...
        VSOMEIP_ERROR << __func__ << ": offered service request command serialization failed ("
                      << std::dec << int(its_error) << ")";
}

void routing_manager_client::send_unsubscribe_ack(service_t _service, instance_t _instance,
                                                  eventgroup_t             _eventgroup,
                                                  remote_subscription_id_t _id)
{
    ...
    if (its_error == protocol::error_e::ERROR_OK)
    {
        ...
        VSOMEIP_ERROR << __func__ << ": unsubscribe ack command serialization failed (" << std::dec
                      << int(its_error) << ")";
}
...
void routing_manager_client::resend_provided_event_registrations()
{
    ...
    for (const event_data_t& ed : pending_event_registrations_)
    {
        if (ed.is_provided_)
        {
            send_register_event(get_client(), ed.service_, ed.instance_, ed.notifier_,
                                ed.eventgroups_, ed.type_, ed.reliability_, ed.is_provided_,
                                ed.is_cyclic_);
        }
    }
}
...
void routing_manager_client::send_resend_provided_event_response(pending_remote_offer_id_t _id)
{
    ...
    if (its_error == protocol::error_e::ERROR_OK)
    {
        ...
        VSOMEIP_ERROR << __func__ << ": resend provided event command serialization failed ("
                      << std::dec << int(its_error) << ")";
}
    ...
    const protocol::update_security_credentials_command& _command)
{
    ...
    for (const auto& c : _command.get_credentials())
    {
        ...
        its_policy->credentials_ +=
            std::make_pair(boost::icl::interval<uid_t>::closed(its_uid, its_uid), its_gid_set);
...
void routing_manager_client::on_client_assign_ack(const client_t& _client)
{
    ...
    if (state_ == inner_state_type_e::ST_ASSIGNING)
    {
        if (_client != VSOMEIP_CLIENT_UNSET)
        {
            ...
            if (is_started_)
            {
                ...
                if (receiver_)
                {
                    ...
                    VSOMEIP_INFO << "Client " << std::hex << std::setw(4) << std::setfill('0')
                                 << get_client() << " (" << host_->get_name()
                ...
                {
                    VSOMEIP_WARNING << __func__ << ": (" << host_->get_name() << ":" << std::hex
        ...
        {
            VSOMEIP_ERROR << __func__ << ": (" << host_->get_name() << ":" << std::hex
                          << std::setw(4) << std::setfill('0') << _client << ") Invalid clientID";
        }
    }
    else
    {
...
{
    VSOMEIP_INFO << __func__ << ": Application " << std::hex << std::setw(4) << std::setfill('0')
                 << host_->get_client();
    ...
    for (const auto& s : remote_subscriber_count_)
    {
        for (const auto& i : s.second)
        {
            for (const auto& e : i.second)
                routing_manager_base::unsubscribe(VSOMEIP_ROUTING_CLIENT, nullptr, s.first, i.first,
                                                  e.first, ANY_EVENT);
        }
    }
