...
void routing_manager_base::register_debounce(const std::shared_ptr<debounce_filter_impl_t>& _filter,
                                             client_t                                       _client,
                                             const std::shared_ptr<vsomeip_v3::event>&      _event)
{
    if (_filter->send_current_value_after_ == true)
    {
        ...
        auto elem = debounce_clients_.emplace(
            timeout, std::make_tuple(_client, false,
                                     std::bind(&routing_manager_base::debounce_timeout_update_cbk,
                                               shared_from_this(), std::placeholders::_1, _event,
                                               _client, _filter),
                                     _event->get_event()));
...
void routing_manager_base::remove_debounce(client_t _client, event_t _event)
{
    ...
    {
        if ((std::get<0>(debounce_client.second) == _client)
            && (std::get<3>(debounce_client.second) == _event))
        {
...
void routing_manager_base::update_debounce_clients(const std::set<client_t>& _clients,
                                                   event_t                   _event)
{
    std::lock_guard<std::mutex> its_lock(debounce_mutex_);
    for (auto& debounce_client : debounce_clients_)
...
bool routing_manager_base::offer_service(client_t _client, service_t _service, instance_t _instance,
                                         major_version_t _major, minor_version_t _minor)
{
    ...
    if (its_info)
    {
        ...
        {
            VSOMEIP_ERROR << "rm_base::offer_service service property mismatch (" << std::hex
                          << std::setfill('0') << std::setw(4) << _client << "): [" << std::setw(4)
                                           ...
                                           minor_version_t _minor)
{
    ...
    if (its_info)
    {
        ...
        {
            VSOMEIP_ERROR << "rm_base::request_service service property mismatch (" << std::hex
                          << std::setfill('0') << std::setw(4) << _client << "): [" << std::setw(4)
                          << _service << "." << std::setw(4) << _instance << ":" << std::dec
                          << static_cast<std::uint32_t>(its_info->get_major()) << ":"
                          << its_info->get_minor()
...
void routing_manager_base::register_event(
    ...
    bool _is_shadow, bool _is_cache_placeholder)
{
    ...
    auto determine_event_reliability = [this, &_service, &_instance, &_notifier, &_reliability]() {
        ...
        { // automatic mode, user service' reliability
    ...
    if (its_event)
    {
        ...
        else
        {
            ...
            if (_eventgroups.size() == 0)
            { // No eventgroup specified
            ...
            if (its_debounce)
            {
                ...
                _epsilon_change_func = [its_debounce](const std::shared_ptr<payload>& _old,
                    ...
                    if (its_debounce->interval_ > -1)
                    {
                        ...
                        int64_t elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                                              its_current - its_debounce->last_forwarded_)
                                              .count();
                        is_elapsed = (its_debounce->last_forwarded_
...
void routing_manager_base::unregister_event(client_t _client, service_t _service,
                                            instance_t _instance, event_t _event, bool _is_provided)
{
    (void)_client;
...
std::set<std::shared_ptr<event>> routing_manager_base::find_events(service_t    _service,
                                                                   instance_t   _instance,
...
std::vector<event_t> routing_manager_base::find_events(service_t  _service,
                                               ...
                                               instance_t _instance, method_t _method)
{
    ...
    VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << get_client()
                    ...
                    << " which does not offer service/instance/method " << _service << "/"
        ...
        for (const auto& e : its_eventgroup->get_events())
        {
                    ...
                    _sec_client, _service, _instance, e->get_event()))
            {
                VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << _client
                                ...
                                << "subscribes to service/instance/event " << _service << "/"
...
void routing_manager_base::add_known_client(client_t _client, const std::string& _client_host)
{
    ...
    if (configuration_->is_security_enabled() && !configuration_->is_security_external())
    {
        ...
        if (policy_loaded == policy_manager_impl::policy_loaded_e::POLICY_PATH_FOUND_AND_NOT_LOADED)
        {
            if (configuration_->lazy_load_security(_client_host))
            {
                VSOMEIP_INFO << __func__ << " vSomeIP Security: Loaded security policies for host: "
                             << _client_host << " at UID/GID: " << std::dec << getuid() << "/"
                             << getgid();
            }
        }
        else if (policy_loaded == policy_manager_impl::policy_loaded_e::POLICY_PATH_INEXISTENT)
        {
            ...
            else if (configuration_->lazy_load_security(get_client_host()))
            { // necessary for lazy loading from inside android container
                VSOMEIP_INFO << __func__ << " vSomeIP Security: Loaded security policies for host: "
                             << get_client_host() << " at UID/GID: " << std::dec << getuid() << "/"
                             << getgid();
            }
        }
    }
...
void routing_manager_base::subscribe(client_t _client, const vsomeip_sec_client_t* _sec_client,
...
void routing_manager_base::unsubscribe(client_t _client, const vsomeip_sec_client_t* _sec_client,
                                       service_t _service, instance_t _instance,
...
void routing_manager_base::notify(service_t _service, instance_t _instance, event_t _event,
                                  std::shared_ptr<payload> _payload, bool _force)
{
    ...
    {
        VSOMEIP_WARNING << "Attempt to update the undefined event/field [" << std::hex << _service
                        << "." << _instance << "." << _event << "]";
    }
}

void routing_manager_base::notify_one(service_t _service, instance_t _instance, event_t _event,
...
)
{
    ...
    if (its_event)
    {
        ...
        subscription_state_e its_subscription_state(
        ...
        for (auto its_group : its_event->get_eventgroups())
        {
            ...
            if (its_eventgroup)
            {
                ...
                its_subscription_state = get_incoming_subscription_state(
                    _client, _service, _instance, valid_group, _event);
#endif
        ...
        if (found_eventgroup)
        {
            ...
            else
            {
                ...
                {
                    VSOMEIP_INFO << "routing_manager_base::notify_one(" << std::hex
                                 << std::setfill('0') << std::setw(4) << _client << "): ["
                                 << std::setw(4) << _service << "." << std::setw(4) << _instance
                                 << "." << std::setw(4) << valid_group << "." << std::setw(4)
                                 << _event << "]"
                                                    ...
                                                    bool _remote_subscriber)
{
    ...
    if (its_service != pending_notify_ones_.end())
    {
        ...
        if (its_instance != its_service->second.end())
        {
            ...
            {
                VSOMEIP_INFO << "routing_manager_base::send_pending_notify_ones(" << std::hex
                             << std::setfill('0') << std::setw(4) << _client << "): ["
                             << std::setw(4) << _service << "." << std::setw(4) << _instance << "."
                             << std::setw(4) << _eventgroup << "." << std::setw(4)
                             << its_group->second->get_method() << "]";
...
void routing_manager_base::notify_one_current_value(client_t _client, service_t _service,
                                                    instance_t _instance, eventgroup_t _eventgroup,
                                                    event_t                  _event,
...
bool routing_manager_base::send(client_t _client, std::shared_ptr<message> _message, bool _force)
{
    ...
    {
        is_sent = send(_client, its_serializer->get_data(), its_serializer->get_size(),
                       _message->get_instance(), _message->is_reliable(), get_client(),
                       get_sec_client(), 0, false, _force);
...
routing_manager_base::create_service_info(service_t _service, instance_t _instance,
                                          major_version_t _major, minor_version_t _minor,
                                          ttl_t _ttl, bool _is_local_service)
{
    std::shared_ptr<serviceinfo> its_info =
        std::make_shared<serviceinfo>(_service, _instance, _major, _minor, _ttl, _is_local_service);
    {
...
std::shared_ptr<serviceinfo> routing_manager_base::find_service(service_t  _service,
                                                                instance_t _instance) const
{
    std::shared_ptr<serviceinfo> its_info;
    std::lock_guard<std::mutex>  its_lock(services_mutex_);
...
void routing_manager_base::remove_local(
    client_t                                                         _client,
    ...
    bool                                                             _remove_sec_client)
{
    ...
    for (auto its_subscription : _subscribed_eventgroups)
    {
        host_->on_subscription(std::get<0>(its_subscription), std::get<1>(its_subscription),
                               std::get<2>(its_subscription), _client, &its_sec_client,
...
std::shared_ptr<event> routing_manager_base::find_event(service_t _service, instance_t _instance,
...
routing_manager_base::find_eventgroups(service_t _service, instance_t _instance) const
{
...
routing_manager_base::find_eventgroup(service_t _service, instance_t _instance,
                                      eventgroup_t _eventgroup) const
{
    std::lock_guard<std::mutex> its_lock(eventgroups_mutex_);
        ...
        if (found_instance != found_service->second.end())
        {
            ...
            if (found_eventgroup != found_instance->second.end())
            {
                ...
                if (its_service_info)
                {
                    ...
                    if (configuration_->get_multicast(_service, _instance, _eventgroup,
                                                      its_multicast_address, its_multicast_port))
                    {
                        try
                        {
                            its_info->set_multicast(
                                boost::asio::ip::address::from_string(its_multicast_address),
                                its_multicast_port);
                        } catch (...)
                        {
                            VSOMEIP_ERROR << "Eventgroup [" << std::hex << std::setw(4)
                                          << std::setfill('0') << _service << "." << _instance
                                          << "." << _eventgroup
                    ...
                    its_info->set_threshold(
                        configuration_->get_threshold(_service, _instance, _eventgroup));
                }
            }
        }
    }
...
void routing_manager_base::remove_eventgroup_info(service_t _service, instance_t _instance,
                                                  eventgroup_t _eventgroup)
{
    std::lock_guard<std::mutex> its_lock(eventgroups_mutex_);
    auto                        found_service = eventgroups_.find(_service);
...
bool routing_manager_base::send_local_notification(client_t _client, const byte_t* _data,
...
bool routing_manager_base::send_local(std::shared_ptr<endpoint>& _target, client_t _client,
                                      ...
                                      bool _reliable, protocol::id_e _command,
                                      uint8_t _status_check) const
{
    bool has_sent(false);
    ...
    std::set<event_t>* _already_subscribed_events)
{
    ...
    { // subscribe to specific event
        ...
        {
            is_inserted =
                its_event->add_subscriber(_eventgroup, _filter, _client, host_->is_routing());
        }
        else
        {
            VSOMEIP_WARNING << "routing_manager_base::insert_subscription(" << std::hex
                            << std::setfill('0') << std::setw(4) << _client << "): ["
                            << std::setw(4) << _service << "." << std::setw(4) << _instance << "."
            ...
            is_inserted = create_placeholder_event_and_subscribe(_service, _instance, _eventgroup,
                                                                 _event, _filter, _client);
        }
    }
    else
    { // subscribe to all events of the eventgroup
        ...
        if (its_eventgroup)
        {
            ...
            else
            {
                for (const auto& e : its_events)
                {
                    ...
                    is_inserted =
                        e->add_subscriber(_eventgroup, _filter, _client, host_->is_routing())
                        || is_inserted;
                }
            }
        }
...
std::shared_ptr<serializer> routing_manager_base::get_serializer()
{
    ...
    {
        VSOMEIP_INFO << __func__ << ": Client " << std::hex << std::setw(4) << std::setfill('0')
                     << get_client() << " has no available serializer. Waiting...";
        serializer_condition_.wait(its_lock, [this] {
            return !serializers_.empty();
        });
        VSOMEIP_INFO << __func__ << ": Client " << std::hex << std::setw(4) << std::setfill('0')
                     << get_client() << " now checking for available serializer.";
    }
...
std::shared_ptr<deserializer> routing_manager_base::get_deserializer()
{
    ...
    while (deserializers_.empty())
    {
        ...
        deserializer_condition_.wait(its_lock, [this] {
...
void routing_manager_base::send_pending_subscriptions(service_t _service, instance_t _instance,
                                                      major_version_t _major)
{
    for (auto& ps : pending_subscriptions_)
    {
        ...
        {
            send_subscribe(get_client(), ps.service_, ps.instance_, ps.eventgroup_, ps.major_,
                           ps.event_, ps.filter_);
        }
    }
}

void routing_manager_base::remove_pending_subscription(service_t _service, instance_t _instance,
                                                       eventgroup_t _eventgroup, event_t _event)
{
    ...
    else
    {
        ...
        {
            if (it->service_ == _service && it->instance_ == _instance
                && it->eventgroup_ == _eventgroup && it->event_ == _event)
            {
                it = pending_subscriptions_.erase(it);
                break;
            }
    ...
    for (const auto& its_service : events_)
    {
        for (const auto& its_instance : its_service.second)
        {
            for (const auto& its_event : its_instance.second)
            {
                ...
                {
                    result.insert(std::make_tuple(its_service.first, its_instance.first, e));
                }
            }
        }
    }
...
bool routing_manager_base::get_guest(client_t _client, boost::asio::ip::address& _address,
                                     port_t& _port) const
{
...
void routing_manager_base::add_guest(client_t _client, const boost::asio::ip::address& _address,
                                     port_t _port)
{
    std::lock_guard<std::mutex> its_lock(guests_mutex_);
...
void routing_manager_base::remove_subscriptions(port_t                          _local_port,
...
#ifdef VSOMEIP_ENABLE_COMPAT
void routing_manager_base::set_incoming_subscription_state(client_t _client, service_t _service,
                                                           instance_t   _instance,
                                                           eventgroup_t _eventgroup, event_t _event,
                                                           subscription_state_e _state)
{
...
void routing_manager_base::erase_incoming_subscription_state(client_t _client, service_t _service,
                                                             instance_t   _instance,
                                                             eventgroup_t _eventgroup,
                                                             event_t      _event)
{
