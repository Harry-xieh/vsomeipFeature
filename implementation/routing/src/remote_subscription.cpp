...
bool remote_subscription::address_equals(const std::shared_ptr<remote_subscription>& _other) const
{
    ...
    if (unreliable_ && (*_other).unreliable_)
        unrelibale_address_equals =
            (unreliable_->get_address() == (*_other).unreliable_->get_address());
    return (relibale_address_equals || unrelibale_address_equals);
}

void remote_subscription::reset(const std::set<client_t>& _clients)
{
    auto its_client_state = std::make_pair(remote_subscription_state_e::SUBSCRIPTION_PENDING,
                            ...
                            const bool                                   _is_subscribe)
{
    ...
    for (const auto& its_client : its_changed)
    {
        ...
        {
            clients_[its_client] =
                std::make_pair(remote_subscription_state_e::SUBSCRIPTION_PENDING, _timepoint);
        }
        else
        {
