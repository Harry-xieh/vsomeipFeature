...
eventgroupinfo::eventgroupinfo(const service_t _service, const instance_t _instance,
...
bool eventgroupinfo::update_remote_subscription(
...

    {
        ...
        for (const auto& its_item : subscriptions_)
        {
            if (its_item.second->equals(_subscription))
            {
                ...
                _changed = its_item.second->update(_subscription->get_clients(), _expiration,
                                                   _is_subscribe);
...
bool eventgroupinfo::is_remote_subscription_limit_reached(
    const std::shared_ptr<remote_subscription>& _subscription)
{
        ...
        if (find_address != remote_subscribers_count_.end())
        {
            if (find_address->second > max_remote_subscribers_)
            {
                VSOMEIP_WARNING << ": remote subscriber limit [" << std::dec
                                << (uint32_t)max_remote_subscribers_ << "] to [" << std::hex
                                << std::setfill('0') << std::setw(4) << service_ << "."
                                << std::setw(4) << instance_ << "." << std::setw(4) << eventgroup_
                                << "]"
...
eventgroupinfo::add_remote_subscription(const std::shared_ptr<remote_subscription>& _subscription)
{
    if (_subscription == nullptr)
    {
        VSOMEIP_ERROR << __func__ << ": Received ptr is null";
...
eventgroupinfo::get_remote_subscription(const remote_subscription_id_t _id)
{
    std::lock_guard<std::mutex> its_lock(subscriptions_mutex_);
...
void eventgroupinfo::send_initial_events(
        ...
        for (const auto& its_event : events_)
        {
            if (its_event && its_event->get_type() == event_type_e::ET_FIELD)
            {
                ...
                case reliability_type_e::RT_BOTH:
                    ...
                    break;
                default:
                    VSOMEIP_WARNING << __func__ << "Event reliability unknown: [" << std::hex
                                    << std::setfill('0') << std::setw(4) << service_ << "."
                                    << std::setw(4) << instance_ << "." << std::setw(4)
                                    << eventgroup_ << "." << std::setw(4) << its_event->get_event()
                                    << "]";
                }
            }
        }
