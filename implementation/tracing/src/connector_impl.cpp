...
{
    channels_[VSOMEIP_TC_DEFAULT_CHANNEL_ID] = std::make_shared<channel_impl>(
    ...
    DLT_REGISTER_CONTEXT_LL_TS(*(its_default_context.get()), VSOMEIP_TC_DEFAULT_CHANNEL_ID,
...
void connector_impl::configure(const std::shared_ptr<cfg::trace>& _configuration)
{
    ...
    if (is_enabled_)
    { // No need to create filters if tracing is disabled!
    ...
    VSOMEIP_INFO << "vsomeip tracing " << (is_enabled_ ? "enabled." : "not enabled.")
...
void connector_impl::trace(const byte_t* _header, uint16_t _header_size, const byte_t* _data,
                           uint32_t _data_size)
{
#if USE_DLT
    if (!is_enabled_)
        ...
        if (ftype.first)
        {
            ...
            if (its_context != contexts_.end())
            {
                try
                {
                    ...
                    {
                        // Positive Filter
                        DLT_TRACE_NETWORK_SEGMENTED(
                            *(its_context->second.get()), DLT_NW_TRACE_IPC, _header_size,
                    ...
                    {
                        // Header-Only Filter
                        DLT_TRACE_NETWORK_TRUNCATED(
                            *(its_context->second.get()), DLT_NW_TRACE_IPC, _header_size,
                            static_cast<void*>(const_cast<byte_t*>(_header)),
