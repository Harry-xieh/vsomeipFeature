...
namespace vsomeip_v3 { namespace trace {
...
{
    filter_type_e its_filter_type =
        (_is_positive ? filter_type_e::POSITIVE : filter_type_e::NEGATIVE);
...
filter_id_t channel_impl::add_filter(const match_t& _match, filter_type_e _type)
{
    ...
    if (std::get<0>(_match) != ANY_SERVICE)
    {
        ...
        else
        {
            ...
            {
                its_filter_func = [_match](service_t _s, instance_t, method_t) {
    ...
    else
    {
        if (std::get<1>(_match) != ANY_INSTANCE)
        {
            ...
            {
                its_filter_func = [_match](service_t, instance_t _i, method_t) {
        ...
        else
        {
            if (std::get<2>(_match) != ANY_METHOD)
            {
                its_filter_func = [_match](service_t, instance_t, method_t _m) {
                    return (std::get<2>(_match) == _m);
                };
            }
            else
            {
                its_filter_func = [](service_t, instance_t, method_t) {
                    return true;
                };
            }
        }
...
filter_id_t channel_impl::add_filter(const match_t& _from, const match_t& _to, filter_type_e _type)
{
    ...
    if (std::get<0>(_from) == ANY_SERVICE || std::get<1>(_from) == ANY_INSTANCE
        || std::get<2>(_from) == ANY_METHOD || std::get<0>(_to) == ANY_SERVICE
        || std::get<1>(_to) == ANY_INSTANCE || std::get<2>(_to) == ANY_METHOD)
    {
    ...
    std::function<bool(service_t, instance_t, method_t)> its_filter_func =
        [_from, _to](service_t _s, instance_t _i, method_t _m) {
            return (std::get<0>(_from) <= _s && _s <= std::get<0>(_to) && std::get<1>(_from) <= _i
                    && _i <= std::get<1>(_to) && std::get<2>(_from) <= _m
                    && _m <= std::get<2>(_to));
        };
...
std::pair<bool, bool> channel_impl::matches(service_t _service, instance_t _instance,
                                            method_t _method)
{
    std::lock_guard<std::mutex> its_lock(mutex_);
