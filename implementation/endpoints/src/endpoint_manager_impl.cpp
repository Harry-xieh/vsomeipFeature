      ...
      options_thread_(std::bind(&endpoint_manager_impl::process_multicast_options, this))
{
    ...
    if (!is_local_routing_)
    {
        VSOMEIP_INFO << __func__ << ": Connecting to other clients from "
                     << configuration_->get_routing_host_address().to_string() << ":" << std::dec
                     << local_port_;
    }
}
...
std::shared_ptr<endpoint> endpoint_manager_impl::find_or_create_remote_client(service_t  _service,
        ...
        if (found_instance != found_service->second.end())
        {
            ...
            if (_reliable_port != ILLEGAL_PORT)
            {
                ...
                if (found_reliable != found_instance->second.end())
                {
                    ...
                    {
                        VSOMEIP_WARNING << "Reliable service endpoint has changed: [" << std::hex
                                        << std::setfill('0') << std::setw(4) << _service << "."
                                        << std::setw(4) << _instance << "." << std::dec
                                        << static_cast<std::uint32_t>(_major) << "." << _minor
                                        << "] old: " << its_definition->get_address().to_string()
                                        << ":" << its_definition->get_port()
                                        << " new: " << _reliable_address.to_string() << ":"
                                        << _reliable_port;
                    }
                }
            }
            if (_unreliable_port != ILLEGAL_PORT)
            {
                ...
                if (found_unreliable != found_instance->second.end())
                {
                    ...
                    {
                        VSOMEIP_WARNING << "Unreliable service endpoint has changed: [" << std::hex
                                        << std::setfill('0') << std::setw(4) << _service << "."
                                        << std::setw(4) << _instance << "." << std::dec
                                        << static_cast<std::uint32_t>(_major) << "." << _minor
                                        << "] old: " << its_definition->get_address().to_string()
                                        << ":" << its_definition->get_port()
                                        << " new: " << _unreliable_address.to_string() << ":"
                                        << _unreliable_port;
                    }
                }
            }
    ...
    const std::shared_ptr<endpoint_definition>& _ep_definition)
{
    ...
    if (must_report)
        static_cast<routing_manager_impl*>(rm_)->service_endpoint_connected(
            _service, _instance, its_info->get_major(), its_info->get_minor(), its_endpoint, false);
}
    ...
    const std::shared_ptr<endpoint_definition>& _ep_definition_unreliable)
{
    ...
    {
        static_cast<routing_manager_impl*>(rm_)->service_endpoint_connected(
            _service, _instance, its_info->get_major(), its_info->get_minor(), its_unreliable,
            false);
        static_cast<routing_manager_impl*>(rm_)->service_endpoint_connected(
            _service, _instance, its_info->get_major(), its_info->get_minor(), its_reliable, false);
    }
}

void endpoint_manager_impl::clear_remote_service_info(service_t _service, instance_t _instance,
                                                      bool _reliable)
{
    std::lock_guard<std::recursive_mutex> its_lock(endpoint_mutex_);
...
std::shared_ptr<endpoint> endpoint_manager_impl::create_server_endpoint(uint16_t _port,
                                                                        bool _reliable, bool _start)
{
    ...
    if (_start)
    {
        if (_reliable)
        {
            auto its_tmp{std::make_shared<tcp_server_endpoint_impl>(
        ...
        {
            auto its_tmp{std::make_shared<udp_server_endpoint_impl>(
                shared_from_this(), rm_->shared_from_this(), io_, configuration_)};
            if (its_tmp)
            {
    ...
    {
        its_server_endpoint =
            std::make_shared<virtual_server_endpoint_impl>(its_unicast_str, _port, _reliable, io_);
    }
    ...
    else
    {
        VSOMEIP_ERROR << __func__ << " Server endpoint creation failed."
                      << " Reason: " << its_error.message() << " Port: " << _port << " ("
                      << _reliable << ")";
    }
...
std::shared_ptr<endpoint> endpoint_manager_impl::find_server_endpoint(uint16_t _port,
...
endpoint_manager_impl::find_or_create_server_endpoint(uint16_t _port, bool _reliable, bool _start,
                                                      service_t _service, instance_t _instance,
...
void endpoint_manager_impl::find_or_create_multicast_endpoint(
    service_t _service, instance_t _instance, const boost::asio::ip::address& _sender,
...
bool endpoint_manager_impl::supports_selective(service_t _service, instance_t _instance) const
{
    ...
    if (its_service != remote_service_info_.end())
    {
        ...
        if (its_instance != its_service->second.end())
        {
            ...
            {
                supports_selective |= configuration_->supports_selective_broadcasts(
                                                ...
                                                const std::shared_ptr<routing_host>& _host)
{
    ...
    if (configuration_->is_security_enabled() && get_client() != its_routing_host_id)
    {
        VSOMEIP_ERROR << "endpoint_manager_impl::" << __func__ << ": "
                      << "Client [" << std::hex << std::setw(4) << std::setfill('0') << get_client()
                      << "] does not match the configured routing manager client identifier ["
                      << std::hex << std::setw(4) << std::setfill('0') << its_routing_host_id
                      << "]";
    ...
    if (configuration_->is_local_routing())
    {
        ...
        {
            VSOMEIP_ERROR
                << "Too many file descriptors received by systemd socket activation! num_fd: "
                << num_fd;
        }
        else if (num_fd == 1)
        {
            its_socket = SD_LISTEN_FDS_START + 0;
            VSOMEIP_INFO << "Using native socket created by systemd socket activation! fd: "
                         << its_socket;
            if (is_local_routing_)
            {
                try
                {
                    ...
                    if (its_root)
                    {
                        boost::asio::local::stream_protocol::endpoint its_endpoint(
                            its_endpoint_path);
                        boost::system::error_code its_error;
                        ...
                        {
                            VSOMEIP_ERROR
                                << "Routing endpoint creation failed. Client ID: " << std::hex
                                << std::setw(4) << std::setfill('0') << VSOMEIP_ROUTING_CLIENT
                                << ": " << its_error.message();
        ...
        else
        {
            if (is_local_routing_)
            {
                try
                {
                    ...
                    {
                        VSOMEIP_ERROR
                            << "endpoint_manager_impl::create_local_server unlink failed ("
                    ...
                    if (its_root)
                    {
                        ...
                        {
                            VSOMEIP_ERROR
                                << "Local routing endpoint creation failed. Client ID: " << std::hex
                                << std::setw(4) << std::setfill('0') << VSOMEIP_ROUTING_CLIENT
                                << ": " << its_error.message();
        ...
        try
        {
            ...
            auto its_root{std::make_shared<local_tcp_server_endpoint_impl>(
                shared_from_this(), _host, io_, configuration_, true)};
            if (its_root)
            {
    ...
    else
    {
        try
        {
            ...
            VSOMEIP_INFO << __func__ << ": Routing root @ " << its_address.to_string() << ":"
                         << std::dec << its_port;
                ...
                do
                {
                    ...
                    if (its_error)
                    {
                        VSOMEIP_ERROR << "endpoint_manager_impl::create_routing_root: "
                                      << "Remote routing root endpoint creation failed ("
                                      << its_retry << ") "
...
instance_t
endpoint_manager_impl::find_instance_multicast(service_t                       _service,
        ...
        for (auto& its_service : remote_services_)
        {
            for (auto& its_instance : its_service.second)
            {
                ...
                if (found_endpoint != its_instance.second.end())
                {
                    ...
                    {
                        std::shared_ptr<serviceinfo> its_info(
                            rm_->find_service(its_service.first, its_instance.first));
                        if (!its_info)
                        {
                        ...
                        const auto its_other_endpoint =
                            its_info->get_endpoint(!endpoint_is_reliable);

                        if (!its_other_endpoint
                            || (its_other_endpoint
                                && its_other_endpoint->is_established_or_connected()))
                        {
                            services_to_report_.push_front(
                                {its_service.first, its_instance.first, its_info->get_major(),
                ...
                if (found_endpoint->second == _endpoint)
                {
                    ...
                    if (!is_reliable)
                    {
                        static_cast<routing_manager_impl*>(rm_)->on_availability(
                            ...
                            availability_state_e::AS_UNAVAILABLE, its_info->get_major(),
                            its_info->get_minor());
                    }
        ...
        for (auto& its_instance : its_service.second)
        {
            ...
            if (found_endpoint != its_instance.second.end())
            {
                if (found_endpoint->second == _endpoint)
                {
                    ...
                    if (configuration_->get_client_port(its_service.first, its_instance.first,
...
void endpoint_manager_impl::on_error(const byte_t* _data, length_t _length,
                                     ...
                                     std::uint16_t                   _remote_port)
{
    ...
    static_cast<routing_manager_impl*>(rm_)->send_error(
        return_code_e::E_MALFORMED_MESSAGE, _data, _length, its_instance, _receiver->is_reliable(),
        _receiver, _remote_address, _remote_port);
}

void endpoint_manager_impl::get_used_client_ports(const boost::asio::ip::address&   _remote_address,
                                                  port_t                            _remote_port,
...
void endpoint_manager_impl::request_used_client_port(
    const boost::asio::ip::address& _remote_address, port_t _remote_port, bool _reliable,
    port_t _local_port)
{
...
void endpoint_manager_impl::release_used_client_port(
    const boost::asio::ip::address& _remote_address, port_t _remote_port, bool _reliable,
    port_t _local_port)
{
    std::lock_guard<std::mutex> its_lock(used_client_ports_mutex_);
...
std::shared_ptr<endpoint>
endpoint_manager_impl::find_remote_client(service_t _service, instance_t _instance, bool _reliable)
{
    std::shared_ptr<endpoint> its_endpoint;
...
std::shared_ptr<endpoint> endpoint_manager_impl::create_remote_client(service_t  _service,
                                                                      instance_t _instance,
                                                                      bool       _reliable)
{
    std::shared_ptr<endpoint>            its_endpoint;
    ...
    if (its_remote_port != ILLEGAL_PORT)
    {
                                            ...
                                            its_used_client_ports, its_local_port))
        {
            if (its_endpoint_def)
            {
                its_endpoint = create_client_endpoint(its_remote_address, its_local_port,
                                                      its_remote_port, _reliable);
            }

            if (its_endpoint)
            {
                request_used_client_port(its_remote_address, its_remote_port, _reliable,
                                         its_local_port);
                ...
                client_endpoints_[its_endpoint_def->get_address()][its_endpoint_def->get_port()]
                                 [_reliable][its_partition] = its_endpoint;
                ...
                VSOMEIP_INFO << "endpoint_manager_impl::create_remote_client: "
                             << its_endpoint_def->get_address().to_string(ec) << ":" << std::dec
...
std::shared_ptr<endpoint>
endpoint_manager_impl::create_client_endpoint(const boost::asio::ip::address& _address,
...
void endpoint_manager_impl::log_client_states() const
{
    ...
    client_endpoints_t its_client_endpoints;
    std::vector<std::pair<std::tuple<boost::asio::ip::address, uint16_t, bool>, size_t>>
                ...
                for (const auto& its_partition : its_reliability.second)
                {
                    ...
                    if (its_queue_size > VSOMEIP_DEFAULT_QUEUE_WARN_SIZE)
                        its_client_queue_sizes.push_back(
                            std::make_pair(std::make_tuple(its_address.first, its_port.first,
    ...
    std::sort(
        its_client_queue_sizes.begin(), its_client_queue_sizes.end(),
        [](const std::pair<std::tuple<boost::asio::ip::address, uint16_t, bool>, size_t>& _a,
           const std::pair<std::tuple<boost::asio::ip::address, uint16_t, bool>, size_t>& _b) {
    ...
    for (size_t i = 0; i < its_max; i++)
    {
        its_log << std::hex << std::setw(4) << std::setfill('0')
                << std::get<0>(its_client_queue_sizes[i].first).to_string() << ":" << std::dec
                << std::get<1>(its_client_queue_sizes[i].first) << "("
...
void endpoint_manager_impl::log_server_states() const
{
    ...
    for (const auto& its_port : its_server_endpoints)
    {
        for (const auto& its_reliability : its_port.second)
        {
            ...
            if (its_queue_size > VSOMEIP_DEFAULT_QUEUE_WARN_SIZE)
                its_client_queue_sizes.push_back(std::make_pair(
                    std::make_pair(its_port.first, its_reliability.first), its_queue_size));
        }
    }
    ...
    for (size_t i = 0; i < its_max; i++)
    {
        its_log << std::dec << its_client_queue_sizes[i].first.first << "("
                << (its_client_queue_sizes[i].first.second ? "tcp" : "udp") << "):" << std::dec
    ...
    while (is_processing_options_)
    {
        if (options_queue_.size() > 0)
        {
            ...
            if (its_udp_server_endpoint)
            {
                ...
                boost::system::error_code its_error;
                its_udp_server_endpoint->set_multicast_option(its_front.address_,
                                                              its_front.is_join_, its_error);
