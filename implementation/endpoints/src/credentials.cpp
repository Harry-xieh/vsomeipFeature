...
boost::optional<credentials::received_t> credentials::receive_credentials(const int _fd)
{
    ...
    if (nr == -1)
    {
        VSOMEIP_ERROR << __func__
                      << ": vSomeIP Security: Receiving credentials failed. No data. errno: "
                      << std::strerror(errno);
        return boost::none;
    }
    ...
    {
        VSOMEIP_ERROR << __func__
                      << ": vSomeIP Security: Receiving credentials failed. Invalid data.";
        return boost::none;
    }
    ...
    {
        VSOMEIP_ERROR << __func__
                      << ": vSomeIP Security: Receiving client host failed. No data. errno: "
                      << std::strerror(errno);
        return boost::none;
    }
...
void credentials::send_credentials(const int _fd, client_t _client, std::string _client_host)
{
    ...
    if (ns == -1)
    {
        VSOMEIP_ERROR << __func__ << ": vSomeIP Security: Sending credentials failed. errno: "
                      << std::strerror(errno);
    }
}
