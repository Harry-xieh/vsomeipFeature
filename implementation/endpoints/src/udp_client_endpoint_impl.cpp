...
void udp_client_endpoint_impl::connect()
{
    ...
    if (!its_error || its_error == boost::asio::error::already_open)
    {
        ...
        socket_->set_option(boost::asio::socket_base::receive_buffer_size(
                                static_cast<int>(udp_receive_buffer_size_)),
                            its_error);
        if (its_error)
        {
            VSOMEIP_WARNING << "udp_client_endpoint_impl::connect: couldn't set "
                            << "SO_RCVBUF: " << its_error.message() << " to: " << std::dec
                            << udp_receive_buffer_size_ << " local port:" << std::dec
        ...
        {
            its_error.assign(setsockopt(socket_->native_handle(), SOL_SOCKET, SO_RCVBUFFORCE,
        ...
        if (its_error)
        {
            VSOMEIP_WARNING << "udp_client_endpoint_impl::connect: couldn't get "
                            << "SO_RCVBUF: " << its_error.message() << " local port:" << std::dec
                            << local_.port() << " remote:" << get_address_port_remote();
        }
        else
        {
            VSOMEIP_INFO << "udp_client_endpoint_impl::connect: SO_RCVBUF is: " << std::dec
        ...
        if (its_device != "")
        {
            if (setsockopt(socket_->native_handle(), SOL_SOCKET, SO_BINDTODEVICE,
                           its_device.c_str(), socklen_t(its_device.size()))
                == -1)
            {
        ...
        if (local_.port() != ILLEGAL_PORT)
        {
            ...
            if (its_bind_error)
            {
                VSOMEIP_WARNING << "udp_client_endpoint::connect: "
                                ...
                                << " local: " << local_.address().to_string() << ":" << std::dec
                                << local_.port() << " remote:" << get_address_port_remote();
                ...
                {
                    VSOMEIP_ERROR << "udp_client_endpoint_impl::connect: " << e.what()
                                  << " remote:" << get_address_port_remote();
                }
                return;
            }
        ...
        socket_->async_connect(remote_,
                               strand_.wrap(std::bind(&udp_client_endpoint_base_impl::connect_cbk,
    ...
    else
    {
        ...
        strand_.post(
            std::bind(&udp_client_endpoint_base_impl::connect_cbk, shared_from_this(), its_error));
    }
}
...
void udp_client_endpoint_impl::receive()
{
    ...
    message_buffer_ptr_t its_buffer =
        std::make_shared<message_buffer_t>(VSOMEIP_MAX_UDP_MESSAGE_SIZE);
    socket_->async_receive_from(
        boost::asio::buffer(*its_buffer), const_cast<endpoint_type&>(remote_),
        strand_.wrap(
            std::bind(&udp_client_endpoint_impl::receive_cbk,
...
void udp_client_endpoint_impl::receive_cbk(boost::system::error_code const& _error,
                                           std::size_t                      _bytes,
    ...
    if (!_error && 0 < _bytes && its_host)
    {
        ...
        do
        {
            uint64_t read_message_size =
                utility::get_message_size(&(*_recv_buffer)[i], remaining_bytes);
                ...
                && current_message_size <= remaining_bytes)
            {
                if (remaining_bytes - current_message_size > remaining_bytes)
                {
                    ...
                    return;
                         ...
                         && ((*_recv_buffer)[i + VSOMEIP_PROTOCOL_VERSION_POS]
                ...
                {
                    if ((*_recv_buffer)[i + VSOMEIP_PROTOCOL_VERSION_POS]
                        != VSOMEIP_PROTOCOL_VERSION)
                    {
                        VSOMEIP_ERROR
                            << "uce: Wrong protocol version: 0x" << std::hex << std::setw(2)
                            << std::setfill('0')
                            << std::uint32_t((*_recv_buffer)[i + VSOMEIP_PROTOCOL_VERSION_POS])
                            << " local: " << get_address_port_local()
                            << " remote: " << get_address_port_remote();
                        // ensure to send back a message w/ wrong protocol version
                        its_host->on_message(&(*_recv_buffer)[i], VSOMEIP_SOMEIP_HEADER_SIZE + 8,
                                             this, false, VSOMEIP_ROUTING_CLIENT, nullptr,
                    ...
                    {
                        VSOMEIP_ERROR
                            << "uce: Invalid message type: 0x" << std::hex << std::setw(2)
                            << std::setfill('0')
                            ...
                            << " remote: " << get_address_port_remote();
                                 ...
                                 (*_recv_buffer)[i + VSOMEIP_RETURN_CODE_POS])))
                    {
                        VSOMEIP_ERROR << "uce: Invalid return code: 0x" << std::hex << std::setw(2)
                                      << std::setfill('0')
                ...
                else if (tp::tp::tp_flag_is_set((*_recv_buffer)[i + VSOMEIP_MESSAGE_TYPE_POS]))
                {
                    const auto res = tp_reassembler_->process_tp_message(
                        &(*_recv_buffer)[i], current_message_size, remote_address_, remote_port_);
                    if (res.first)
                    {
                        its_host->on_message(
                            &res.second[0], static_cast<std::uint32_t>(res.second.size()), this,
                            false, VSOMEIP_ROUTING_CLIENT, nullptr, remote_address_, remote_port_);
                    }
                }
                else
                {
                    its_host->on_message(&(*_recv_buffer)[i], current_message_size, this, false,
            ...
            else
            {
                VSOMEIP_ERROR << "Received a unreliable vSomeIP message with bad "
                              ...
                              << current_message_size << " Bytes. From: " << remote_.address()
...
void udp_client_endpoint_impl::print_status()
{
    ...
    VSOMEIP_INFO << "status uce: " << local << " -> " << get_address_port_remote()
                                        ...
                                        const message_buffer_ptr_t& _sent_msg)
{
        ...
        bool stopping(false);
        {
            ...
            else
            {
                ...
                VSOMEIP_WARNING << "uce::send_cbk received error: " << _error.message() << " ("
                                ...
                                << " " << std::dec << queue_.size() << " " << std::dec
                                << queue_size_ << " (" << std::hex << std::setfill('0')
                                << std::setw(4) << its_client << "): [" << std::setw(4)
                                << its_service << "." << std::setw(4) << its_method << "."
    ...
    {
        VSOMEIP_WARNING << "uce::send_cbk received error: " << _error.message() << " (" << std::dec
...
bool udp_client_endpoint_impl::tp_segmentation_enabled(service_t _service, instance_t _instance,
