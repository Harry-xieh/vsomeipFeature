                                                  ...
                                                  service_t                               _service)
{
    ...
    if (_service == ANY_SERVICE)
    {
                        ...
                        }))
        {
            ...
            endpoint_impl<Protocol>::io_.post([ptr, _handler]() {
                _handler(ptr);
            });
        }
        else
    ...
    else
    {
        ...
        { // no messages of the to be stopped service are or have been queued
            auto ptr = this->shared_from_this();
            endpoint_impl<Protocol>::io_.post([ptr, _handler]() {
                _handler(ptr);
            });
...
bool server_endpoint_impl<Protocol>::send(const uint8_t* _data, uint32_t _size)
{
    ...
    if (VSOMEIP_SESSION_POS_MAX < _size)
    {
        ...
        if (found_client != clients_.end())
        {
            ...
            {
                VSOMEIP_WARNING << "server_endpoint::send: session_id 0x" << std::hex << its_session
    ...
    if (!prepare_stop_handlers_.empty())
    {
        ...
        if (prepare_stop_handlers_.find(its_service) != prepare_stop_handlers_.end())
        {
            ...
            VSOMEIP_WARNING << "server_endpoint::send: Service is stopping, ignoring message: ["
                            ...
                            << std::setw(4) << its_method << "." << std::setw(4) << its_client
                                                       ...
                                                       endpoint_data_type& _endpoint_data) const
{
    ...
    if (_endpoint_data.queue_size_ + _size > endpoint_impl<Protocol>::queue_limit_
        || _endpoint_data.queue_size_ + _size < _size)
    { // overflow protection
                                              ...
                                              std::size_t                      _bytes)
{
    ...
    auto check_if_all_msgs_for_stopped_service_are_sent = [&]() {
             ...
             stp_hndlr_iter != prepare_stop_handlers_.end();)
        {
            ...
            { // all messages of the to be stopped service have been sent
    ...
    else
    {
        ...
        VSOMEIP_WARNING << "sei::send_cbk received error: " << _error.message() << " (" << std::dec
                        ...
                        << its_data.queue_.size() << " " << its_data.queue_size_ << " (" << std::hex
                        << std::setfill('0') << std::setw(4) << its_client << "): [" << std::setw(4)
                        << its_service << "." << std::setw(4) << its_method << "." << std::setw(4)
                        << its_session << "]";
        cancel_dispatch_timer(it);
        targets_.erase(it);
        if (!prepare_stop_handlers_.empty())
