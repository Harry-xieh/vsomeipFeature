...
endpoint_manager_base::create_local_server(const std::shared_ptr<routing_host>& _routing_host)
{
    std::shared_ptr<endpoint> its_local_server;
    std::stringstream         its_path;
    its_path << utility::get_base_path(configuration_->get_network()) << std::hex
             << rm_->get_client();
    ...
    if (is_local_routing_)
    {
        try
        {
            ...
            {
                VSOMEIP_ERROR << "Local UDS server endpoint creation failed. Client " << std::hex
        ...
        try
        {
            ...
            if (its_tmp)
            {
                while (get_local_server_port(its_port, its_used_ports) && !its_local_server)
                {
                    ...
                    if (!its_error)
                    {
                        VSOMEIP_INFO << __func__ << ": Listening @ " << its_address.to_string()
                                     << ":" << std::dec << its_port;
                ...
                else
                {
                    VSOMEIP_ERROR << __func__
                                  << ": Local TCP server endpoint initialization failed. "
                                  << "Client " << std::hex << std::setw(4) << std::setfill('0')
                                  << its_client << " Reason: No local port available!";
                }
            }
            else
            {
...
void endpoint_manager_base::on_error(const byte_t* _data, length_t _length,
...
void endpoint_manager_base::log_client_states() const
{
    ...
    std::sort(its_client_queue_sizes.begin(), its_client_queue_sizes.end(),
              [](const std::pair<client_t, size_t>& _a, const std::pair<client_t, size_t>& _b) {
    ...
    {
        its_log << std::hex << std::setw(4) << its_client_queue_sizes[i].first << ":" << std::dec
...
std::shared_ptr<endpoint> endpoint_manager_base::create_local_unlocked(client_t _client)
{
    ...
    if (is_local_routing_)
    {
        ...
        VSOMEIP_INFO << "Client [" << std::hex << rm_->get_client() << "] is connecting to ["
                     << std::hex << _client << "] at " << its_path.str() << " endpoint > "
                     << its_endpoint;
    }
    else
    {
#else
    {
        ...
        if (is_guest)
        {
            try
            {
                ...
                its_endpoint      = std::make_shared<local_tcp_client_endpoint_impl>(
                    ...
                    boost::asio::ip::tcp::endpoint(its_remote_address, its_remote_port), io_,
                    configuration_);

                VSOMEIP_INFO << "Client [" << std::hex << std::setw(4) << std::setfill('0')
                             << rm_->get_client() << "] @ " << its_local_address.to_string() << ":"
                             << std::dec << local_port_ << " is connecting to [" << std::hex
        ...
        {
            VSOMEIP_ERROR << __func__ << ": Cannot get guest address of client [" << std::hex
                          << std::setw(4) << std::setfill('0') << _client << "]";
        }
    }
...
bool endpoint_manager_base::get_local_server_port(port_t&                 _port,
...
#define SERVER_PORT_OFFSET 2
    ...
    {
        VSOMEIP_WARNING << __func__ << ": No configured port ranges for uid/gid=" << std::dec
                        << its_uid << '/' << its_gid;
    }
