...
#ifndef __QNX__
    if (chmod(_local.path().c_str(), static_cast<mode_t>(configuration_->get_permissions_uds()))
        == -1)
    {
...
void local_uds_server_endpoint_impl::start()
{
    ...
    if (acceptor_.is_open())
    {
        connection::ptr new_connection = connection::create(
            std::dynamic_pointer_cast<local_uds_server_endpoint_impl>(shared_from_this()),
...
bool local_uds_server_endpoint_impl::send_to(const std::shared_ptr<endpoint_definition> _target,
...
bool local_uds_server_endpoint_impl::send_error(const std::shared_ptr<endpoint_definition> _target,
                                                const byte_t* _data, uint32_t _size)
{
    (void)_target;
    (void)_data;
...
void local_uds_server_endpoint_impl::accept_cbk(const connection::ptr&           _connection,
                                                boost::system::error_code const& _error)
{
    ...
    {
        VSOMEIP_ERROR << "local_usd_server_endpoint_impl::accept_cbk: " << _error.message() << " ("
    ...
    if (!_error)
    {
        ...
        {
            VSOMEIP_WARNING
                << "vSomeIP Security: Client 0x" << std::hex << its_host->get_client()
        ...
        if (its_host && configuration_->is_security_enabled())
        {
            ...
            {
                VSOMEIP_WARNING
                    << "vSomeIP Security: Rejecting new connection with routing manager client ID 0x"
                    << std::hex << its_client << " uid/gid= " << std::dec << its_sec_client.user
            ...
            {
                {
                    ...
                    if (found_client != connections_.end())
                    {
                        VSOMEIP_WARNING
                            ...
                            << " uid/gid= " << std::dec << its_sec_client.user << "/"
                            << its_sec_client.group
                ...
                if (!configuration_->get_policy_manager()->check_credentials(its_client,
                                                                             &its_sec_client))
                {
                    VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex
                                    << its_host->get_client()
                                    << " received client credentials from client 0x" << its_client
                                    << " which violates the security policy : uid/gid=" << std::dec
        ...
        {
            configuration_->get_policy_manager()->store_client_to_sec_client_mapping(
                its_client, &its_sec_client);
            configuration_->get_policy_manager()->store_sec_client_to_client_mapping(
...
local_uds_server_endpoint_impl::connection::connection(
    const std::shared_ptr<local_uds_server_endpoint_impl>& _server, std::uint32_t _max_message_size,
    std::uint32_t _initial_recv_buffer_size, std::uint32_t _buffer_shrink_threshold,
...
local_uds_server_endpoint_impl::connection::ptr local_uds_server_endpoint_impl::connection::create(
    const std::shared_ptr<local_uds_server_endpoint_impl>& _server, std::uint32_t _max_message_size,
    std::uint32_t _buffer_shrink_threshold, boost::asio::io_context& _io)
{
    const std::uint32_t its_initial_buffer_size = static_cast<std::uint32_t>(
        protocol::COMMAND_HEADER_SIZE + sizeof(instance_t) + sizeof(bool) + sizeof(bool));
...
void local_uds_server_endpoint_impl::connection::start()
{
    ...
    if (socket_.is_open())
    {
        ...
        auto its_storage = std::make_shared<local_endpoint_receive_op::storage>(
            socket_,
            std::bind(&local_uds_server_endpoint_impl::connection::receive_cbk, shared_from_this(),
...
client_t local_uds_server_endpoint_impl::assign_client(const byte_t* _data, uint32_t _size)
{
    ...
    {
        VSOMEIP_ERROR << __func__ << ": assign client command deserialization failed (" << std::dec
...
void local_uds_server_endpoint_impl::connection::receive_cbk(
    boost::system::error_code const& _error, std::size_t _bytes, std::uint32_t const& _uid,
    std::uint32_t const& _gid)
{
    std::shared_ptr<local_uds_server_endpoint_impl> its_server(server_.lock());
    if (!its_server)
    {
    ...
    if (!_error && 0 < _bytes)
    {
        ...
        do
        {
            ...
            while (its_start + 3 < recv_buffer_size_ + its_iteration_gap
                   && (recv_buffer_[its_start] != 0x67 || recv_buffer_[its_start + 1] != 0x37
            ...
            if (!message_is_empty)
            {
                ...
                {
                    its_command_size = bithelper::read_uint32_le(
                ...
                while (its_end + 3 < recv_buffer_size_ + its_iteration_gap
                       && (recv_buffer_[its_end] != 0x07 || recv_buffer_[its_end + 1] != 0x6d
                ...
                if (recv_buffer_size_ + its_iteration_gap < its_end + 4
                    || recv_buffer_[its_end] != 0x07 || recv_buffer_[its_end + 1] != 0x6d
                    || recv_buffer_[its_end + 2] != 0x37 || recv_buffer_[its_end + 3] != 0x67)
                {
                        ...
                        > recv_buffer_size_)
                    {
                        missing_capacity_ =
                            ...
                            + its_command_size
                            + std::uint32_t(protocol::COMMAND_HEADER_SIZE + protocol::TAG_SIZE)
                    ...
                    else
                    {
                        ...
                        VSOMEIP_ERROR << "lse::c<" << this
                                      ...
                                      << " its_iteration_gap: " << std::dec << its_iteration_gap
                                      << " bound client: 0x" << std::hex << bound_client_
            ...
            if (!message_is_empty && its_end + 3 < recv_buffer_size_ + its_iteration_gap)
            {
                ...
                {
                    client_t its_client = its_server->assign_client(&recv_buffer_[its_start],
                                                                    uint32_t(its_end - its_start));
                    ...
                    if (its_config && its_config->is_security_enabled())
                    {
                        ...
                        else if (!its_server->configuration_->get_policy_manager()
                                      ->check_credentials(its_client, &sec_client_))
                        {
                            VSOMEIP_WARNING
                                << std::hex << "Client 0x" << its_host->get_client()
                                << " received client credentials from client 0x" << its_client
                ...
                else if (!its_server->is_routing_endpoint_ || assigned_client_)
                {
                    ...
                    its_host->on_message(&recv_buffer_[its_start], uint32_t(its_end - its_start),
    ...
    const std::exception& _e)
{
    ...
    its_message << "local_uds_server_endpoint_impl::connection catched exception" << _e.what()
                << " local: " << get_path_local() << " remote: " << get_path_remote()
...
void local_uds_server_endpoint_impl::print_status()
{
    ...
    VSOMEIP_INFO << "status lse: " << its_local_path << " connections: " << std::dec
    ...
    for (const auto& c : its_connections)
    {
        ...
        VSOMEIP_INFO << "status lse: client: " << its_remote_path << " recv_buffer: " << std::dec
                     << its_recv_size;
    }
}

std::string
local_uds_server_endpoint_impl::get_remote_information(const target_data_iterator_type _it) const
{
    (void)_it;
    return "local";
}
...
std::string
local_uds_server_endpoint_impl::get_remote_information(const endpoint_type& _remote) const
{
    (void)_remote;
    return "local";
}
...
bool local_uds_server_endpoint_impl::check_packetizer_space(target_data_iterator_type _it,
...
void local_uds_server_endpoint_impl::send_client_identifier(const client_t& _client)
{
    ...
    if (its_error != protocol::error_e::ERROR_OK)
    {
        VSOMEIP_ERROR << __func__ << ": assign client ack command serialization failed ("
