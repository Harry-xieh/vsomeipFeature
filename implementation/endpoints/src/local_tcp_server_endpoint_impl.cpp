...
void local_tcp_server_endpoint_impl::start()
{
    ...
    if (acceptor_.is_open())
    {
        connection::ptr new_connection = connection::create(
            std::dynamic_pointer_cast<local_tcp_server_endpoint_impl>(shared_from_this()),
...
bool local_tcp_server_endpoint_impl::send_to(const std::shared_ptr<endpoint_definition> _target,
...
bool local_tcp_server_endpoint_impl::send_error(const std::shared_ptr<endpoint_definition> _target,
                                                const byte_t* _data, uint32_t _size)
{
    (void)_target;
    (void)_data;
...
bool local_tcp_server_endpoint_impl::get_default_target(
    service_t, local_tcp_server_endpoint_impl::endpoint_type&) const
{
    return false;
}
...
void local_tcp_server_endpoint_impl::accept_cbk(const connection::ptr&           _connection,
                                                boost::system::error_code const& _error)
{
        ...
        endpoint_type             remote;
        {
            ...
            if (its_error)
            {
                VSOMEIP_WARNING << "ltsei::accept_cbk: couldn't disable "
                                << "Nagle algorithm: " << its_error.message() << " endpoint > "
                                << this;
            }
    ...
    {
        VSOMEIP_ERROR << "ltsei::accept_cbk: " << _error.message() << " (" << std::dec
...
local_tcp_server_endpoint_impl::connection::connection(
    const std::shared_ptr<local_tcp_server_endpoint_impl>& _server, std::uint32_t _max_message_size,
...
local_tcp_server_endpoint_impl::connection::ptr local_tcp_server_endpoint_impl::connection::create(
    const std::shared_ptr<local_tcp_server_endpoint_impl>& _server, std::uint32_t _max_message_size,
    ...
    if (socket_.is_open())
    {
        ...
        if (recv_buffer_size_ > its_capacity)
        {
            VSOMEIP_ERROR << __func__ << "Received buffer size is greater than the buffer capacity!"
                          ...
                          << " its_capacity: " << its_capacity << " endpoint > " << this;
            return;
        }
                                                          ...
                                                          std::size_t                      _bytes)
{
    ...
    (void)_bytes;
    if (_error)
        VSOMEIP_WARNING << "ltsei::send_cbk received error: " << _error.message() << " endpoint > "
                        << this;
}
            ...
            if (!message_is_empty && its_end + 3 < recv_buffer_size_ + its_iteration_gap)
            {
                ...
                else if (!its_server->is_routing_endpoint_ || assigned_client_)
                {
                    ...
                    if (!ec)
                    {
                        ...
                        {
                            sec_client_.host = htonl(uint32_t(its_address.to_v4().to_ulong()));
                        }
                        ...
                        its_host->on_message(&recv_buffer_[its_start],
                                             uint32_t(its_end - its_start), its_server.get(), false,
    ...
    const std::exception& _e)
{
    ...
    its_message << "local_tcp_server_endpoint_impl::connection catched exception" << _e.what()
...
void local_tcp_server_endpoint_impl::print_status()
{
    ...
    VSOMEIP_INFO << "status lse: " << its_local_path << " connections: " << std::dec
                 << its_connections.size() << " queues: " << std::dec << targets_.size();
...
std::string
local_tcp_server_endpoint_impl::get_remote_information(const target_data_iterator_type _it) const
{
    boost::system::error_code ec;
...
std::string
local_tcp_server_endpoint_impl::get_remote_information(const endpoint_type& _remote) const
{
    boost::system::error_code ec;
...
bool local_tcp_server_endpoint_impl::check_packetizer_space(target_data_iterator_type _it,
...
void local_tcp_server_endpoint_impl::send_client_identifier(const client_t& _client)
{
    ...
    if (its_error != protocol::error_e::ERROR_OK)
    {
        VSOMEIP_ERROR << "ltsei::" << __func__
                      << ": assign client ack command serialization failed (" << std::dec
                      << static_cast<int>(its_error) << ")"
                      << " endpoint > " << this;
        return;
    }
