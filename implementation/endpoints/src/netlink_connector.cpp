...
void netlink_connector::start()
{
    ...
    if (socket_.is_open())
    {
        socket_.bind(nl_endpoint<nl_protocol>(RTMGRP_LINK | RTMGRP_IPV4_IFADDR | RTMGRP_IPV6_IFADDR
        ...
        socket_.async_receive(boost::asio::buffer(&recv_buffer_[0], recv_buffer_size),
...
void netlink_connector::send_ifa_request(std::uint32_t _retry)
{
    ...
    socket_.async_send(boost::asio::buffer(&get_address_msg, get_address_msg.nlhdr.nlmsg_len),
                       std::bind(&netlink_connector::send_cbk, shared_from_this(),
                                 std::placeholders::_1, std::placeholders::_2));
}
...
void netlink_connector::send_ifi_request(std::uint32_t _retry)
{
    ...
    {
        std::lock_guard<std::mutex> its_lock(socket_mutex_);
        socket_.async_send(boost::asio::buffer(&get_link_msg, get_link_msg.nlhdr.nlmsg_len),
                           std::bind(&netlink_connector::send_cbk, shared_from_this(),
                                     std::placeholders::_1, std::placeholders::_2));
    }
}
...
void netlink_connector::send_rt_request(std::uint32_t _retry)
{
    ...
    {
        std::lock_guard<std::mutex> its_lock(socket_mutex_);
        socket_.async_send(boost::asio::buffer(&get_route_msg, get_route_msg.nlhdr.nlmsg_len),
                           std::bind(&netlink_connector::send_cbk, shared_from_this(),
                                     std::placeholders::_1, std::placeholders::_2));
    }
}
...
void netlink_connector::handle_netlink_error(struct nlmsgerr* _error_msg)
{
    ...
    if (retry >= max_retries_)
    {
        VSOMEIP_ERROR << "netlink_connector::receive_cbk received "
                         "error message: "
                      << strerror(-_error_msg->error) << " type " << std::dec
                      << _error_msg->msg.nlmsg_type << " seq " << _error_msg->msg.nlmsg_seq;
        return;
    }
...
bool netlink_connector::has_address(struct ifaddrmsg* ifa_struct, size_t length,
...
bool netlink_connector::check_sd_multicast_route_match(struct rtmsg* _routemsg, size_t _length,
                                                       std::string* _routename) const
{
    ...
    while (RTA_OK(retrta, _length))
    {
        if (retrta->rta_type == RTA_DST)
        {
            ...
            if (rtattr_length == 4 && multicast_address_.is_v4())
            { // IPv4 route
                ...
                const std::uint32_t sd_addr =
                    static_cast<std::uint32_t>(multicast_address_.to_v4().to_ulong());
                const std::uint32_t sd_net = (sd_addr & netmask);
                matches_sd_multicast       = !(dst_net ^ sd_net);
            }
            else if (rtattr_length == 16 && multicast_address_.is_v6())
            { // IPv6 route
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    const std::uint32_t dst =
                        ntohl((*(struct in6_addr*)RTA_DATA(retrta)).__in6_u.__u6_addr32[i]);
#else
                    const std::uint32_t dst =
                        ntohl((*(struct in6_addr*)RTA_DATA(retrta)).in6_u.u6_addr32[i]);
#endif
                    const std::uint32_t sd      = ntohl(reinterpret_cast<std::uint32_t*>(
                        multicast_address_.to_v6().to_bytes().data())[i]);
    ...
    else if (if_index > 0 && net_if_index_for_address_ == if_index && _routemsg->rtm_dst_len == 0)
    {
