...
void tcp_server_endpoint_impl::init(const endpoint_type& _local, boost::system::error_code& _error)
{
    ...
    this->max_message_size_ =
        configuration_->get_max_message_size_reliable(_local.address().to_string(), _local.port());
    this->queue_limit_ =
                            ...
                            {
                                {
                                    std::lock_guard<std::mutex> its_lock(socket_mutex_);
                                    VSOMEIP_ERROR
                                        << "Detected Magic Cookie within message data. Resyncing."
                                        << " local: " << get_address_port_local()
                                        << " remote: " << get_address_port_remote();
                                }
                                if (!is_magic_cookie(its_iteration_gap))
                ...
                if (!has_full_message)
                {
                                ...
                                recv_buffer_[its_iteration_gap + VSOMEIP_RETURN_CODE_POS]))))
                    {
                        ...
                        {
                            {
                                std::lock_guard<std::mutex> its_lock(socket_mutex_);
                                VSOMEIP_ERROR
                                    << "tse: Wrong protocol version: 0x" << std::hex << std::setw(2)
                                    << std::setfill('0')
                                     ...
                                     recv_buffer_[its_iteration_gap + VSOMEIP_MESSAGE_TYPE_POS])))
                        {
                            std::lock_guard<std::mutex> its_lock(socket_mutex_);
                            VSOMEIP_ERROR
                                << "tse: Invalid message type: 0x" << std::hex << std::setw(2)
                                << std::setfill('0')
                                << std::uint32_t(
                                       recv_buffer_[its_iteration_gap + VSOMEIP_MESSAGE_TYPE_POS])
                                << " local: " << get_address_port_local()
                                     ...
                                     recv_buffer_[its_iteration_gap + VSOMEIP_RETURN_CODE_POS])))
                        {
                            std::lock_guard<std::mutex> its_lock(socket_mutex_);
                            VSOMEIP_ERROR
                                << "tse: Invalid return code: 0x" << std::hex << std::setw(2)
                                << std::setfill('0')
                                << std::uint32_t(
                                       recv_buffer_[its_iteration_gap + VSOMEIP_RETURN_CODE_POS])
                                << " local: " << get_address_port_local()
                    ...
                    {
                        missing_capacity_ =
                            current_message_size - static_cast<std::uint32_t>(recv_buffer_size_);
                    }
                    ...
                    {
                        {
                            std::lock_guard<std::mutex> its_lock(socket_mutex_);
                            VSOMEIP_ERROR
                                << "tse::c<" << this << ">rcb: recv_buffer_size is: " << std::dec
                                << recv_buffer_size_
                                << " but couldn't read "
...
void tcp_server_endpoint_impl::connection::wait_until_sent(const boost::system::error_code& _error)
{
    ...
    if (it != its_server->targets_.end())
    {
        ...
        if (its_data.is_sending_ && _error)
        {
            ...
            its_data.sent_timer_.async_wait(std::bind(
                &tcp_server_endpoint_impl::connection::wait_until_sent,
