      ...
      sent_timer_(_io)
{
    ...
    this->max_message_size_ = _configuration->get_max_message_size_reliable(
        _remote.address().to_string(), _remote.port());
    this->queue_limit_ =
        _configuration->get_endpoint_queue_limit(_remote.address().to_string(), _remote.port());
}
...
{
    strand_.dispatch(std::bind(&client_endpoint_impl::connect, this->shared_from_this()));
}
    ...
    auto restart_func = [self, _force] {
        if (!_force && self->state_ == cei_state_e::CONNECTING)
        {
            ...
            std::int64_t                          its_connect_duration =
                std::chrono::duration_cast<std::chrono::milliseconds>(its_current
                                                                      - self->connect_timepoint_)
                    .count();
        ...
        std::string address_port_local;
        {
            ...
            self->recv_buffer_ =
                std::make_shared<message_buffer_t>(self->recv_buffer_size_initial_, 0);
        }
        ...
        self->reconnect_counter_ = 0;
        {
            ...
            for (const auto& q : self->queue_)
            {
                ...
                const method_t its_method =
                    bithelper::read_uint16_be(&(*q.first)[VSOMEIP_METHOD_POS_MIN]);
                const client_t its_client =
                    bithelper::read_uint16_be(&(*q.first)[VSOMEIP_CLIENT_POS_MIN]);
                const session_t its_session =
                    bithelper::read_uint16_be(&(*q.first)[VSOMEIP_SESSION_POS_MIN]);
                VSOMEIP_WARNING << "tce::restart: dropping message: "
                                << "remote:" << self->get_address_port_remote() << " (" << std::hex
...
void tcp_client_endpoint_impl::connect()
{
    ...
    if (!its_error || its_error == boost::asio::error::already_open)
    {
        ...
        if (local_.port() != ILLEGAL_PORT)
        {
            ...
            if (its_bind_error)
            {
                ...
                if (operations_cancelled != 0)
                {
                    ...
                    {
                        VSOMEIP_ERROR << "tcp_client_endpoint_impl::connect: " << e.what()
                                      << " local: " << get_address_port_local()
                                      << " remote:" << get_address_port_remote();
                    }
                }
        ...
        socket_->async_connect(
            remote_, strand_.wrap(std::bind(&tcp_client_endpoint_base_impl::cancel_and_connect_cbk,
    ...
    else
    {
        ...
        if (operations_cancelled != 0)
        {
            VSOMEIP_WARNING << "tce::" << __func__ << "Error opening socket: ("
                            << its_error.message()
                            << "): conneting to local:" << this->get_address_port_local()
                            << " remote: " << this->get_address_port_remote();
            strand_.post(std::bind(&tcp_client_endpoint_base_impl::connect_cbk, shared_from_this(),
                                   its_error));
        }
    }
}
...
void tcp_client_endpoint_impl::receive(message_buffer_ptr_t _recv_buffer,
                                       std::size_t _recv_buffer_size, std::size_t _missing_capacity)
{
    std::lock_guard<std::mutex> its_lock(socket_mutex_);
    if (socket_->is_open())
    {
        ...
        try
        {
            if (_missing_capacity)
            {
                ...
                if (its_capacity < its_required_capacity)
                {
                    ...
                    {
                        VSOMEIP_INFO << "tce: recv_buffer size is: " << _recv_buffer->size()
                                     << " local: " << get_address_port_local()
                                     << " remote: " << get_address_port_remote();
                    }
                }
...
{
    const service_t its_service =
        bithelper::read_uint16_be(&(*_entry.first)[VSOMEIP_SERVICE_POS_MIN]);
    ...
    const session_t its_session =
        bithelper::read_uint16_be(&(*_entry.first)[VSOMEIP_SESSION_POS_MIN]);
...
#endif
    {
        ...
        if (socket_->is_open())
        {
            boost::asio::async_write(
                *socket_, boost::asio::buffer(*_entry.first),
                std::bind(&tcp_client_endpoint_impl::write_completion_condition,
                          ...
                          std::placeholders::_1, std::placeholders::_2, _entry.first->size(),
                          its_service, its_method, its_client, its_session,
                          std::chrono::steady_clock::now()),
                strand_.wrap(std::bind(&tcp_client_endpoint_base_impl::send_cbk, shared_from_this(),
...
{
    configuration_->get_configured_timing_requests(_service, remote_address_.to_string(),
                                                   remote_port_, _method, _debouncing,
                                                   _maximum_retention);
}
    ...
    session_t _session, const std::chrono::steady_clock::time_point _start)
{
    if (_error)
    {
        VSOMEIP_ERROR << "tce::write_completion_condition: " << _error.message() << "(" << std::dec
                      ...
                      << _service << "." << std::setw(4) << _method << "." << std::setw(4)
                      << _session << "]";
        return 0;
    }
    ...
    if (passed > send_timeout_warning_)
    {
        ...
        {
            VSOMEIP_ERROR << "tce::write_completion_condition: " << _error.message() << "("
        ...
        {
            VSOMEIP_WARNING << "tce::write_completion_condition: " << _error.message() << "("
                            << std::dec << _error.value() << ") took longer than " << std::dec
                            ...
                            << std::setw(4) << _service << "." << std::setw(4) << _method << "."
                            << std::setw(4) << _session << "]";
        }
    }
...
void tcp_client_endpoint_impl::receive_cbk(boost::system::error_code const& _error,
                                           std::size_t                      _bytes,
    ...
    if (its_host)
    {
            ...
            do
            {
                ...
                if (has_full_message)
                {
                    ...
                    else
                    {
                        if (has_enabled_magic_cookies_)
                        {
                            uint32_t its_offset = find_magic_cookie(
                                &(*_recv_buffer)[its_iteration_gap], (uint32_t)_recv_buffer_size);
                    ...
                    if (needs_forwarding)
                    {
                        if (!has_enabled_magic_cookies_)
                        {
                            ...
                            its_host->on_message(&(*_recv_buffer)[its_iteration_gap],
                                                 current_message_size, this, false,
                ...
                {
                    const uint32_t its_offset =
                        find_magic_cookie(&(*_recv_buffer)[its_iteration_gap], _recv_buffer_size);
                ...
                if (!has_full_message)
                {
                    if (_recv_buffer_size > VSOMEIP_RETURN_CODE_POS)
                    {
                        ...
                        {
                            VSOMEIP_ERROR
                                << "TCP Client receive_cbk is trying to access invalid vector position."
                        ...
                        else if ((*recv_buffer_)[its_iteration_gap + VSOMEIP_PROTOCOL_VERSION_POS]
                                 != VSOMEIP_PROTOCOL_VERSION)
                        {
                            invalid_parameter_detected = true;
                            VSOMEIP_ERROR
                                << "tce: Wrong protocol version: 0x" << std::hex << std::setw(2)
                                << std::setfill('0')
                            ...
                            its_lock.unlock();
                            its_host->on_message(&(*_recv_buffer)[its_iteration_gap],
                                                 VSOMEIP_SOMEIP_HEADER_SIZE + 8, this, false,
                                                 VSOMEIP_ROUTING_CLIENT, nullptr, remote_address_,
                                                 remote_port_);
                            its_lock.lock();
                        }
                                     ...
                                     *recv_buffer_)[its_iteration_gap + VSOMEIP_MESSAGE_TYPE_POS])))
                        {
                            invalid_parameter_detected = true;
                            VSOMEIP_ERROR
                                << "tce: Invalid message type: 0x" << std::hex << std::setw(2)
                                << std::setfill('0')
                                << std::uint32_t((
                                       *recv_buffer_)[its_iteration_gap + VSOMEIP_MESSAGE_TYPE_POS])
                                << " local: " << get_address_port_local()
                                     ...
                                     (*recv_buffer_)[its_iteration_gap + VSOMEIP_RETURN_CODE_POS])))
                        {
                            invalid_parameter_detected = true;
                            VSOMEIP_ERROR
                                << "tce: Invalid return code: 0x" << std::hex << std::setw(2)
                                << std::setfill('0')
                                << std::uint32_t(
                                       (*recv_buffer_)[its_iteration_gap + VSOMEIP_RETURN_CODE_POS])
                                << " local: " << get_address_port_local()
                    ...
                    {
                        its_missing_capacity =
                            current_message_size - static_cast<std::uint32_t>(_recv_buffer_size);
                    }
                    ...
                    else if (has_enabled_magic_cookies_ && _recv_buffer_size > 0)
                    {
                        ...
                        VSOMEIP_ERROR
                            << "tce::c<" << this
                            << ">rcb: recv_buffer_capacity: " << _recv_buffer->capacity()
                            << " local: " << get_address_port_local()
                            << " remote: " << get_address_port_remote()
                    ...
                    {
                        VSOMEIP_ERROR
                            << "tce::c<" << this << ">rcb: recv_buffer_size is: " << std::dec
                            << _recv_buffer_size
                            << " but couldn't read "
        ...
        else
        {
            VSOMEIP_WARNING << "tcp_client_endpoint receive_cbk: " << _error.message() << "("
                            << std::dec << _error.value() << ") local: " << get_address_port_local()
                            << " remote: " << get_address_port_remote();
            if (_error == boost::asio::error::eof || _error == boost::asio::error::timed_out
...
void tcp_client_endpoint_impl::handle_recv_buffer_exception(
    const std::exception& _e, const message_buffer_ptr_t& _recv_buffer,
    std::size_t _recv_buffer_size)
{
    std::stringstream its_message;
    its_message << "tcp_client_endpoint_impl::connection catched exception" << _e.what()
                << " local: " << get_address_port_local()
                << " remote: " << get_address_port_remote()
                << " shutting down connection. Start of buffer: " << std::setfill('0') << std::hex;
...
void tcp_client_endpoint_impl::print_status()
{
    ...
    VSOMEIP_INFO << "status tce: " << local << " -> " << get_address_port_remote()
...
{
    boost::system::error_code ec;
    return remote_.address().to_string(ec) + ":" + std::to_string(remote_.port());
}
        ...
        if (queue_.size() > 0)
        {
            ...
            else
            {
                ...
                if (its_entry.first)
                {
                    auto self =
                        std::dynamic_pointer_cast<tcp_client_endpoint_impl>(shared_from_this());
                    strand_.dispatch([self, &its_entry]() {
                        self->send_queued(its_entry);
                    });
                }
            }
        }
    ...
    else
    {
        ...
        if (_error == boost::system::errc::destination_address_required)
        {
            VSOMEIP_WARNING << "tce::send_cbk received error: " << _error.message() << " ("
                            << std::dec << _error.value() << ") " << get_remote_information();
        ...
        else
        {
            ...
            VSOMEIP_WARNING << "tce::send_cbk received error: " << _error.message() << " ("
                            << std::dec << _error.value() << ") " << get_remote_information() << " "
                            << std::dec << queue_.size() << " " << std::dec << queue_size_ << " ("
...
void tcp_client_endpoint_impl::wait_until_sent(const boost::system::error_code& _error)
{
    ...
    else
    {
        ...
        sent_timer_.async_wait(
            std::bind(&tcp_client_endpoint_impl::wait_until_sent,
                      std::dynamic_pointer_cast<tcp_client_endpoint_impl>(shared_from_this()),
                      std::placeholders::_1));
    }
}
