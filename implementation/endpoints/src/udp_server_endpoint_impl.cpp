...
udp_server_endpoint_impl::udp_server_endpoint_impl(
    ...
    : server_endpoint_impl<ip::udp>(_endpoint_host, _routing_host, _io, _configuration),
      ...
      is_stopped_(true),
      on_unicast_sent_{nullptr},
      receive_own_multicast_messages_(false),
      on_sent_multicast_received_{nullptr}
{
...
void udp_server_endpoint_impl::init(const endpoint_type& _local, boost::system::error_code& _error)
{
    ...
    unicast_socket_->set_option(
        boost::asio::socket_base::receive_buffer_size(static_cast<int>(its_udp_recv_buffer_size)),
        _error);
                    ...
                    && current_message_size <= remaining_bytes)
                {
                    ...
                    if (utility::is_request(_buffer[i + VSOMEIP_MESSAGE_TYPE_POS]))
                    {
                        ...
                        if (its_client != MAGIC_COOKIE_CLIENT)
                        {
                            const session_t its_session =
                                bithelper::read_uint16_be(&_buffer[i + VSOMEIP_SESSION_POS_MIN]);
                            clients_mutex_.lock();
                    ...
                    if (tp::tp::tp_flag_is_set(_buffer[i + VSOMEIP_MESSAGE_TYPE_POS]))
                    {
                        ...
                        const auto res = tp_reassembler_->process_tp_message(
                            &_buffer[i], current_message_size, its_remote_address, its_remote_port);
                        if (res.first)
                        {
                            if (utility::is_request(res.second[VSOMEIP_MESSAGE_TYPE_POS]))
                            {
                                const client_t its_client =
                                    bithelper::read_uint16_be(&res.second[VSOMEIP_CLIENT_POS_MIN]);
                                if (its_client != MAGIC_COOKIE_CLIENT)
                                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            VSOMEIP_ERROR
                                << "Received an unreliable vSomeIP SD message with too short length field"
                                << " local: " << get_address_port_local()
                                << " remote: " << its_remote_address << ":" << std::dec
                                << its_remote_port;
                        }
                ...
                else
                {
                    ...
                    if (remaining_bytes > VSOMEIP_SERVICE_POS_MAX)
                    {
                        ...
                        if (its_service != VSOMEIP_SD_SERVICE)
                        {
                            ...
                            {
                                VSOMEIP_ERROR
                                    << "Ignoring unreliable vSomeIP message with SomeIP message length 0!";
                            }
                            else
                            {
                                auto its_endpoint_host = endpoint_host_.lock();
                                                    ...
                                                    boost::system::error_code&      _error)
{
    ...
    if (_is_join)
    {
        ...
        if (!multicast_socket_)
        {
            ...
            {
                _error =
                    boost::system::errc::make_error_code(boost::system::errc::not_enough_memory);
                return;
            }
        }
