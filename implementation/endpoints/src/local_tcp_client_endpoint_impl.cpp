...
void local_tcp_client_endpoint_impl::connect()
{
    ...
    if (!its_error || its_error == boost::asio::error::already_open)
    {
        ...
        socket_->async_connect(
            remote_, strand_.wrap(std::bind(&local_tcp_client_endpoint_impl::cancel_and_connect_cbk,
                                            shared_from_this(), std::placeholders::_1)));
    }
    else
    {
        VSOMEIP_WARNING << "ltcei::connect: Error opening socket: " << its_error.message() << " ("
                        << std::dec << its_error.value() << ")"
                        << " endpoint > " << this;
...
bool local_tcp_client_endpoint_impl::send(const uint8_t* _data, uint32_t _size)
{
    std::lock_guard<std::recursive_mutex> its_lock(mutex_);
    bool                                  ret(true);
    if (endpoint_impl::sending_blocked_ || check_message_size(nullptr, _size) != cms_ret_e::MSG_OK
        || !check_packetizer_space(_size) || !check_queue_limit(_data, _size))
    {
        ret = false;
    }
    else
...
void local_tcp_client_endpoint_impl::send_queued(std::pair<message_buffer_ptr_t, uint32_t>& _entry)
{
...

    {
        ...
        boost::asio::async_write(
            *socket_, bufs,
            std::bind(&client_endpoint_impl::send_cbk,
                      std::dynamic_pointer_cast<local_tcp_client_endpoint_impl>(shared_from_this()),
    ...
    std::chrono::nanoseconds* _maximum_retention) const
{
    ...
    VSOMEIP_ERROR << "ltcei::get_configured_times_from_endpoint called."
                  << " endpoint > " << this;
}

void local_tcp_client_endpoint_impl::send_magic_cookie() {}

void local_tcp_client_endpoint_impl::receive_cbk(boost::system::error_code const& _error,
                                                 std::size_t                      _bytes)
{
    if (_error)
    {
        VSOMEIP_INFO << "ltcei::" << __func__ << " Error: " << _error.message() << " endpoint > "
                     << this << " state_ > " << static_cast<int>(state_.load());
    ...
    else
    {
        ...
        if (_bytes == VSOMEIP_LOCAL_CLIENT_ENDPOINT_RECV_BUFFER_SIZE && recv_buffer_[0] == 0x67
            && recv_buffer_[1] == 0x37 && recv_buffer_[2] == 0x6d && recv_buffer_[3] == 0x07
            && recv_buffer_[4] == byte_t(protocol::id_e::ASSIGN_CLIENT_ACK_ID)
            && recv_buffer_[15] == 0x07 && recv_buffer_[16] == 0x6d && recv_buffer_[17] == 0x37
            && recv_buffer_[18] == 0x67)
        {
...
void local_tcp_client_endpoint_impl::print_status()
{
    ...
    VSOMEIP_INFO << "status lce: " << its_path << " queue: " << its_queue_size
                 << " data: " << its_data_size;
}
...
{
    VSOMEIP_ERROR << "ltcei::max_allowed_reconnects_reached: " << get_remote_information()
                  << " endpoint > " << this;
    error_handler_t handler;
    {
        std::lock_guard<std::mutex> its_lock(error_handler_mutex_);
