...
client_endpoint_impl<Protocol>::client_endpoint_impl(
    ...
    : endpoint_impl<Protocol>(_endpoint_host, _routing_host, _io, _configuration),
      socket_{std::make_unique<socket_type>(_io)},
      remote_{_remote},
      flush_timer_{_io},
      connect_timer_{_io},
...
template <typename Protocol>
void client_endpoint_impl<Protocol>::prepare_stop(const endpoint::prepare_stop_handler_t& _handler,
                                                  service_t                               _service)
{
    (void)_handler;
    (void)_service;
...
template <typename Protocol>
bool client_endpoint_impl<Protocol>::send_to(const std::shared_ptr<endpoint_definition> _target,
...
template <typename Protocol>
bool client_endpoint_impl<Protocol>::send_error(const std::shared_ptr<endpoint_definition> _target,
                                                const byte_t* _data, uint32_t _size)
{
    (void)_target;
    (void)_data;
...
template <typename Protocol>
bool client_endpoint_impl<Protocol>::tp_segmentation_enabled(service_t /*_service*/,
...
template <typename Protocol>
void client_endpoint_impl<Protocol>::send_segments(const tp::tp_split_messages_t& _segments,
                                                   std::uint32_t                  _separation_time)
{
    ...
    { // no writing in progress
        ...
        if (its_entry.first)
        {
            is_sending_ = true;
            strand_.dispatch(
                std::bind(&client_endpoint_impl::send_queued, this->shared_from_this(), its_entry));
        }
    }
}
    ...
    if (its_host)
    {
        if (_error && _error != boost::asio::error::already_connected)
        {
            VSOMEIP_WARNING << "cei::" << __func__ << ": restarting socket due to"
                            << "(" << _error.value() << "):" << _error.message() << " endpoint > "
    ...
    if (operations_cancelled != 0)
    {
        if (_error)
        {
            VSOMEIP_WARNING << "cei::" << __func__ << ": cancelled " << operations_cancelled
                            ...
                            << "): msg: " << _error.message() << " endpoint > " << this
...
template <typename Protocol>
void client_endpoint_impl<Protocol>::send_cbk(boost::system::error_code const& _error,
                                              std::size_t                      _bytes,
                                              const message_buffer_ptr_t&      _sent_msg)
{
        ...
        bool stopping(false);
        {
            ...
            else
            {
                ...
                VSOMEIP_WARNING << "cei::send_cbk received error: " << _error.message() << " ("
                                ...
                                << queue_size_ << " (" << std::hex << std::setw(4)
    ...
    {
        VSOMEIP_WARNING << "cei::send_cbk received error: " << _error.message() << " endpoint > "
                        << this << " socket state > " << static_cast<int>(state_.load());
...
void client_endpoint_impl<Protocol>::start_connect_timer()
{
    ...
    connect_timer_.async_wait(std::bind(&client_endpoint_impl<Protocol>::wait_connect_cbk,
...
void client_endpoint_impl<Protocol>::start_connecting_timer()
{
    ...
    connecting_timer_.async_wait(std::bind(&client_endpoint_impl<Protocol>::wait_connecting_cbk,
                                           this->shared_from_this(), std::placeholders::_1));
}

template <typename Protocol>
typename endpoint_impl<Protocol>::cms_ret_e
client_endpoint_impl<Protocol>::check_message_size(const std::uint8_t* const _data,
                                                   std::uint32_t             _size)
{
        ...
        && _size > endpoint_impl<Protocol>::max_message_size_)
    {
            ...
            if (its_instance != ANY_INSTANCE)
            {
                if (tp_segmentation_enabled(its_service, its_instance, its_method))
                {
                    ...
                    this->configuration_->get_tp_configuration(
                        its_service, its_instance, its_method, true, its_max_segment_length,
                        its_separation_time);
        ...
        VSOMEIP_ERROR << "cei::check_message_size: Dropping to big message (" << std::dec << _size
...
template <typename Protocol>
bool client_endpoint_impl<Protocol>::check_queue_limit(const uint8_t* _data,
                                                       std::uint32_t  _size) const
{
    ...
    { // overflow protection
        ...
        VSOMEIP_ERROR << "cei::check_queue_limit: queue size limit (" << std::dec
                      ...
                      << std::hex << std::setw(4) << std::setfill('0') << its_method << "."
                      << std::hex << std::setw(4) << std::setfill('0') << its_session << "] "
                      << "queue_size: " << std::dec << queue_size_ << " data size: " << std::dec
                      << _size;
        return false;
    }
    return true;
...
#endif
    dispatch_timer_.async_wait(std::bind(&client_endpoint_impl<Protocol>::flush_cbk,
                                         this->shared_from_this(), std::placeholders::_1));
}
