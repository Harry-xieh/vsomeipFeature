                                   ...
                                   const message_impl::options_t& _options) const
{
    if (service_ == _other.service_ && instance_ == _other.instance_
        && eventgroup_ == _other.eventgroup_ && major_version_ == _other.major_version_
        && counter_ == _other.counter_)
    {
        // Check, whether options are identical
        if (index1_ == _other.index1_ && index2_ == _other.index2_
                ...
                if (its_options_size > option_index)
                {
                    ...
                    case option_type_e::IP4_ENDPOINT:
                        its_options_current.push_back(
                            std::static_pointer_cast<ipv4_option_impl>(_options[option_index]));
                        break;
                    case option_type_e::IP6_ENDPOINT:
                        its_options_current.push_back(
                            std::static_pointer_cast<ipv6_option_impl>(_options[option_index]));
                        break;
                    default:
                        break;
                    }
...
std::shared_ptr<selective_option_impl> eventgroupentry_impl::get_selective_option() const
{
    for (const auto i : {0, 1})
    {
        ...
        {
            auto its_option =
                std::dynamic_pointer_cast<selective_option_impl>(owner_->get_option(j));
            if (its_option)
                return its_option;
        }
    }
