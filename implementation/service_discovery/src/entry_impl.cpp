...
void entry_impl::assign_option(const std::shared_ptr<option_impl>& _option)
{
    ...
    if (i > -1 && i < 256)
    {
        ...
        if (options_[0].empty() || options_[0][0] == its_index + 1
        ...
        else if (options_[1].empty() || options_[1][0] == its_index + 1
        ...
        {
            VSOMEIP_WARNING
                << "Option is not referenced by entries array, maximum number of endpoint options reached!";
        }
    }
    else
    {
...
bool entry_impl::serialize(vsomeip_v3::serializer* _to) const
{
    ...
    uint8_t number_of_options =
        uint8_t((((uint8_t)options_[0].size()) << 4) | (((uint8_t)options_[1].size()) & 0x0F));
