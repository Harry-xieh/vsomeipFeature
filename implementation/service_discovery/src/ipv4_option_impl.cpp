...
ipv4_option_impl::ipv4_option_impl(const boost::asio::ip::address& _address, const uint16_t _port,
                                   const bool _is_reliable)
...
void ipv4_option_impl::set_address(const ipv4_address_t& _address)
{
    ...
    type_ =
        (its_address.is_multicast() ? option_type_e::IP4_MULTICAST : option_type_e::IP4_ENDPOINT);
}
...
{
    bool is_successful =
        option_impl::deserialize(_from) && length_ == VSOMEIP_SD_IPV4_OPTION_LENGTH;
