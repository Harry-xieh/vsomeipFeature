...
service_discovery_impl::service_discovery_impl(service_discovery_host*               _host,
    ...
    : io_(_host->get_io()),
      ...
      deserializer_(std::make_shared<deserializer>(configuration_->get_buffer_shrink_threshold())),
...
void service_discovery_impl::init()
{
    ...
    try
    {
        std::random_device                           r;
        std::mt19937                                 e(r());
        std::uniform_int_distribution<std::uint32_t> distribution(initial_delay_min,
                                                                  initial_delay_max);
    ...
    repetitions_base_delay_ =
        std::chrono::milliseconds(configuration_->get_sd_repetitions_base_delay());
                                       ...
                                       const std::shared_ptr<eventgroupinfo>& _info)
{
            ...
            if (found_eventgroup != found_instance->second.end())
            {
                ...
                else if (its_subscription->is_selective())
                {
                    if (its_subscription->add_client(_client))
                    {
                        ...
                        send_subscription(its_subscription, _service, _instance, _eventgroup,
                                          _client);
                    }
                }
                return;
    ...
    std::shared_ptr<subscription> its_subscription =
        create_subscription(_major, _ttl, its_reliable, its_unreliable, _info);
...
void service_discovery_impl::send_subscription(const std::shared_ptr<subscription>& _subscription,
                                               ...
                                               const client_t     _client)
{
    ...
    if (!its_address.is_unspecified())
    {
        ...
        if (its_reliability_type == reliability_type_e::RT_UNRELIABLE && its_unreliable)
        {
            ...
            {
                its_data = create_eventgroup_entry(_service, _instance, _eventgroup, _subscription,
        ...
        else if (its_reliability_type == reliability_type_e::RT_BOTH && its_reliable
                 && its_unreliable)
        {
        ...
        {
            VSOMEIP_WARNING << "sd::" << __func__
                            << ": couldn't determine reliability type for subscription to ["
...
void service_discovery_impl::get_subscription_endpoints(
    service_t _service, instance_t _instance, std::shared_ptr<endpoint>& _reliable,
...
void service_discovery_impl::unsubscribe(service_t _service, instance_t _instance,
        ...
        if (found_service != subscribed_.end())
        {
            ...
            if (found_instance != found_service->second.end())
            {
                ...
                if (found_eventgroup != found_instance->second.end())
                {
                    ...
                    const reliability_type_e its_reliability_type = get_eventgroup_reliability(
                        _service, _instance, _eventgroup, its_subscription);
                    auto its_data = create_eventgroup_entry(_service, _instance, _eventgroup,
                                                            its_subscription, its_reliability_type);
                    if (its_data.entry_)
...
{
    std::map<boost::asio::ip::address, std::vector<std::shared_ptr<message_impl>>>
        its_stopsubscribes;
            ...
            for (auto its_instance : its_service.second)
            {
                for (auto& its_eventgroup : its_instance.second)
                {
                    ...
                    else
                    {
                        VSOMEIP_WARNING
                            << __func__
                            << ": Failed to create StopSubscribe entry for: " << std::hex
                            << std::setfill('0') << std::setw(4) << its_service.first << "."
                            << std::setw(4) << its_instance.first << "." << std::setw(4)
                            << its_eventgroup.first << " address: " << its_address.to_string();
                    }
                }
...
service_discovery_impl::get_session(const boost::asio::ip::address& _address)
{
...
bool service_discovery_impl::is_reboot(const boost::asio::ip::address& _sender, bool _is_multicast,
                                       bool _reboot_flag, session_t _session)
{
    ...
    if (its_received == sessions_received_.end())
    {
        sessions_received_[_sender] =
            std::make_tuple(its_multicast_session, its_unicast_session, its_multicast_reboot_flag,
                            its_unicast_reboot_flag);
    }
    else
    {
...
bool service_discovery_impl::check_session_id_sequence(const boost::asio::ip::address& _sender,
...
void service_discovery_impl::insert_find_entries(
    std::vector<std::shared_ptr<message_impl>>& _messages, const requests_t& _requests)
{
    entry_data_t its_data;
...
void service_discovery_impl::insert_offer_entries(
    std::vector<std::shared_ptr<message_impl>>& _messages, const services_t& _services,
    bool _ignore_phase)
{
    for (const auto& its_service : _services)
    {
        for (const auto& its_instance : its_service.second)
        {
            if ((!is_suspended_)
                && ((!is_diagnosis_)
                    || (is_diagnosis_
                        && !configuration_->is_someip(its_service.first, its_instance.first))))
            {
...
entry_data_t service_discovery_impl::create_eventgroup_entry(
    service_t _service, instance_t _instance, eventgroup_t _eventgroup,
    const std::shared_ptr<subscription>& _subscription, reliability_type_e _reliability_type)
{
    entry_data_t its_data;
                                        ...
                                        const boost::asio::ip::address& _sender, bool _is_multicast)
{
    ...
    if (its_message)
    {
        ...
        if (is_reboot(_sender, _is_multicast, its_message->get_reboot_flag(),
                      its_message->get_session()))
        {
        ...
        if (!check_session_id_sequence(_sender, _is_multicast, its_message->get_session(),
        ...
        for (auto iter = its_entries.begin(); iter != its_end; iter++)
        {
            ...
            if ((*iter)->is_service_entry())
            {
                ...
                process_serviceentry(its_service_entry, its_options, its_unicast_flag,
                                     its_resubscribes, _is_multicast, accept_state);
            }
            else
            {
                std::shared_ptr<eventgroupentry_impl> its_eventgroup_entry =
                ...
                if (must_process)
                {
                    ...
                    process_eventgroupentry(its_eventgroup_entry, its_options, its_acknowledgement,
                                            _sender, _is_multicast, is_stop_subscribe_subscribe,
                                            force_initial_events, accept_state);
                }
            }
        }
...
// Entry processing
void service_discovery_impl::check_sent_offers(
    const message_impl::entries_t& _entries, const boost::asio::ip::address& _remote_address) const
{
    ...
    const sd_acceptance_state_t& _sd_ac_state)
{
             ...
             && (_sd_ac_state.sd_acceptance_required_ || _sd_ac_state.accept_entries_))
    {
        ...
        remove_remote_offer_type(its_service, its_instance, its_reliable_address, its_reliable_port,
                                 its_unreliable_address, its_unreliable_port);
        remove_subscriptions(its_service, its_instance);
        if (!is_diagnosis_ && !is_suspended_)
        {
            host_->del_routing_info(its_service, its_instance, (its_reliable_port != ILLEGAL_PORT),
    ...
    const sd_acceptance_state_t& _sd_ac_state)
{
    ...
    {
        VSOMEIP_WARNING << __func__ << ": Ignoring offer of [" << std::hex << std::setfill('0')
    ...
    {
        VSOMEIP_WARNING << __func__ << ": Unknown remote offer type [" << std::hex
    ...
    if (_sd_ac_state.sd_acceptance_required_)
    {
        auto expire_subscriptions_and_services =
            [this, &_sd_ac_state](const boost::asio::ip::address& _address, std::uint16_t _port,
                                  bool _reliable) {
                    ...
                    == _sd_ac_state.expired_ports_.end())
                {
                    VSOMEIP_WARNING << "service_discovery_impl::" << __func__
                                    << ": Do not accept offer from " << _address.to_string() << ":"
        ...
        for (auto eg : its_eventgroups)
        {
            ...
            if (its_info)
            {
                if (its_info->is_reliability_auto_mode())
                {
                    ...
                    {
                        VSOMEIP_WARNING
                            << "sd::" << __func__ << ": eventgroup reliability type changed ["
                            << std::hex << std::setfill('0') << std::setw(4) << _service << "."
                            << std::setw(4) << _instance << "." << std::setw(4) << eg << "]"
                            << " using reliability type:  " << std::setw(4)
                            << static_cast<uint16_t>(offer_type);
...
void service_discovery_impl::process_findservice_serviceentry(service_t       _service,
                    ...
                    for (const auto& its_eventgroup : found_instance->second)
                    {
                        ...
                        if (its_subscription)
                        {
                                ...
                                || !its_subscription->is_udp_connection_established())
                            {
                                ...
                                if (its_reliable_endpoint
                                    && its_reliable_endpoint->is_established())
                                {
                                        ...
                                        && its_subscription->is_udp_connection_established()))
                                {
                                    ...
                                    get_subscription_endpoints(_service, _instance, its_reliable,
                                                               its_unreliable);
                                    get_subscription_address(its_reliable, its_unreliable,
                                                             its_address);
...
service_discovery_impl::create_ip_option(const boost::asio::ip::address& _address, uint16_t _port,
...
void service_discovery_impl::process_eventgroupentry(
    ...
    const boost::asio::ip::address& _sender, bool _is_multicast, bool _is_stop_subscribe_subscribe,
    bool _force_initial_events, const sd_acceptance_state_t& _sd_ac_state)
{
    ...
    if (!its_info)
    {
        if (entry_type_e::SUBSCRIBE_EVENTGROUP == its_type)
        {
            ...
            its_info = std::make_shared<eventgroupinfo>(its_service, its_instance, its_eventgroup,
                                                        its_major, its_ttl,
                                                        VSOMEIP_DEFAULT_MAX_REMOTE_SUBSCRIBERS);
            boost::system::error_code ec;
            VSOMEIP_ERROR << __func__
                          << ": Received a SubscribeEventGroup entry for unknown eventgroup "
                          << " from: " << its_sender.to_string(ec) << " for: [" << std::hex
    ...
    if (_entry->get_owning_message()->get_return_code() != return_code)
    {
        ...
        VSOMEIP_ERROR << __func__ << ": Invalid return code in SOMEIP/SD header "
                      << its_sender.to_string(ec) << " session: " << std::hex << std::setw(4)
                      << std::setfill('0') << its_session;
        if (its_ttl > 0)
        {
            insert_subscription_ack(_acknowledgement, its_info, 0, nullptr, its_clients);
        }
    ...
    if (its_type == entry_type_e::SUBSCRIBE_EVENTGROUP)
    {
            ...
            && its_ttl > 0)
        {
            ...
            VSOMEIP_ERROR << __func__
                          ...
                          << static_cast<uint32_t>(_entry->get_owning_message()->get_length())
                          << "] bytes. " << its_sender.to_string(ec) << " session: " << std::hex
                          << std::setw(4) << std::setfill('0') << its_session;
            return;
        }
    }
            ...
            } catch (const std::out_of_range&)
            {
                ...
                VSOMEIP_ERROR << __func__
                              << ": Fewer options in SD message than "
                                 ...
                                 "option run number: "
                              << i << " " << its_sender.to_string(ec) << " session: " << std::hex
                              << std::setw(4) << std::setfill('0') << its_session;
            ...
            case option_type_e::IP4_ENDPOINT: {
                if (entry_type_e::SUBSCRIBE_EVENTGROUP == its_type)
                {
                    ...
                    if (!check_layer_four_protocol(its_ipv4_option))
                    {
                        if (its_ttl > 0)
                        {
                            insert_subscription_ack(_acknowledgement, its_info, 0, nullptr,
                                                    its_clients);
                        }
                        return;
                    }
                    ...
                    if (its_first_port == ILLEGAL_PORT)
                    {
                            ...
                            && its_second_port != ILLEGAL_PORT)
                        {
                            ...
                            VSOMEIP_ERROR
                                ...
                                << its_sender.to_string(ec) << " session: " << std::hex
                                << std::setw(4) << std::setfill('0') << its_session
            ...
            case option_type_e::IP6_ENDPOINT: {
                if (entry_type_e::SUBSCRIBE_EVENTGROUP == its_type)
                {
                    ...
                    if (!check_layer_four_protocol(its_ipv6_option))
                    {
                        ...
                        boost::system::error_code ec;
                        VSOMEIP_ERROR
                            << "Invalid layer 4 protocol type in IPv6 endpoint option specified! "
                            << its_sender.to_string(ec) << " session: " << std::hex << std::setw(4)
                            << std::setfill('0') << its_session;
                        return;
                    }
            ...
            case option_type_e::IP4_MULTICAST:
                if (entry_type_e::SUBSCRIBE_EVENTGROUP_ACK == its_type)
                {
                    ...
                    if (its_first_port != ILLEGAL_PORT && its_second_port != ILLEGAL_PORT)
                    {
                        boost::system::error_code ec;
                        VSOMEIP_ERROR
                            << __func__ << ": Multiple IPv4 multicast options referenced! "
                            << its_sender.to_string(ec) << " session: " << std::hex << std::setw(4)
                            << std::setfill('0') << its_session;
                        return;
                    }
    ...
    if (entry_type_e::SUBSCRIBE_EVENTGROUP == its_type)
    {
        handle_eventgroup_subscription(
            its_service, its_instance, its_eventgroup, its_major, its_ttl, 0, 0, its_first_address,
            its_first_port, is_first_reliable, its_second_address, its_second_port,
            is_second_reliable, _acknowledgement, _is_stop_subscribe_subscribe,
            _force_initial_events, its_clients, _sd_ac_state, its_info, _sender);
    }
    else
    {
        if (entry_type_e::SUBSCRIBE_EVENTGROUP_ACK == its_type)
        { // this type is used for ACK and NACK messages
            if (its_ttl > 0)
            {
                handle_eventgroup_subscription_ack(its_service, its_instance, its_eventgroup,
                                                   its_major, its_ttl, 0, its_clients, _sender,
                                                   its_first_address, its_first_port);
            }
            else
            {
                handle_eventgroup_subscription_nack(its_service, its_instance, its_eventgroup, 0,
                                                    its_clients);
            }
        }
    }
...
void service_discovery_impl::handle_eventgroup_subscription(
    ...
    bool _is_second_reliable, std::shared_ptr<remote_subscription_ack>& _acknowledgement,
    ...
    const std::shared_ptr<eventgroupinfo>& _info, const boost::asio::ip::address& _sender)
{
    ...
    if (_ttl > 0)
    {
        ...
        {
            if (!its_info->is_accepting_remote_subscriptions())
            { // offer not sent to multicast ip
                auto its_remote_ips =
    ...
    if (_major != _info->get_major())
    {
        ...
        auto its_info = std::make_shared<eventgroupinfo>(_service, _instance, _eventgroup, _major,
                                                         0, VSOMEIP_DEFAULT_MAX_REMOTE_SUBSCRIBERS);
        boost::system::error_code ec;
        // TODO: Add session id
        VSOMEIP_ERROR << __func__ << ": Requested major version:[" << static_cast<uint32_t>(_major)
                      ...
                      << "] subscriber: " << _first_address.to_string(ec) << ":" << std::dec
                      << _first_port;
        if (_ttl > 0)
        {
    ...
    else
    {
        ...
        if (ILLEGAL_PORT != _first_port)
        {
            ...
            its_subscriber = endpoint_definition::get(_first_address, _first_port,
                                                      _is_first_reliable, _service, _instance);
            if (!_is_first_reliable && _info->get_multicast(its_first_address, its_first_port)
                && _info->is_sending_multicast())
            { // udp multicast
                its_unreliable = endpoint_definition::get(its_first_address, its_first_port, false,
                                                          _service, _instance);
            }
            ...
            { // tcp unicast
                ...
                if (_ttl > 0 && !is_tcp_connected(_service, _instance, its_reliable))
                {
                    ...
                    VSOMEIP_ERROR << "TCP connection to target1: ["
                                  ...
                                  << "] not established for subscription to: [" << std::hex
                                  << std::setfill('0') << std::setw(4) << _service << "."
                                  << std::setw(4) << _instance << "." << std::setw(4) << _eventgroup
                                  << "] ";
                    return;
                }
        ...
        if (ILLEGAL_PORT != _second_port)
        {
            ...
            its_subscriber = endpoint_definition::get(_second_address, _second_port,
                                                      _is_second_reliable, _service, _instance);
            if (!_is_second_reliable && _info->get_multicast(its_second_address, its_second_port)
                && _info->is_sending_multicast())
            { // udp multicast
                its_unreliable = endpoint_definition::get(its_second_address, its_second_port,
                                                          false, _service, _instance);
            }
            else if (_is_second_reliable)
            { // tcp unicast
                its_reliable = its_subscriber;
                // check if TCP connection is established by client
                if (_ttl > 0 && !is_tcp_connected(_service, _instance, its_reliable))
    ...
    if (_ttl > 0 && _sd_ac_state.sd_acceptance_required_)
    {
        ...
        if (!insert_nack && _second_port != ILLEGAL_PORT && !_sd_ac_state.accept_entries_
            && configuration_->is_protected_port(_second_address, _second_port,
                                                 _is_second_reliable))
        {
            insert_nack = true;
        }
                ...
                for (const auto its_client : _clients)
                {
                        ...
                        == subscription_state_e::ST_NOT_ACKNOWLEDGED)
                    {
                        found_eventgroup->second->set_state(its_client,
                                                            subscription_state_e::ST_ACKNOWLEDGED);
                        host_->on_subscribe_ack(its_client, _service, _instance, _eventgroup,
                                                ANY_EVENT, PENDING_SUBSCRIPTION_ID);
                    }
                }
                if (_address.is_multicast())
                {
                    host_->on_subscribe_ack_with_multicast(_service, _instance, _sender, _address,
                                                           _port);
                }
            }
        }
...
bool service_discovery_impl::is_tcp_connected(
    ...
    const std::shared_ptr<endpoint_definition>& its_endpoint)
{
    ...
    if (its_info)
    {
        ...
        auto its_reliable_server_endpoint =
            std::dynamic_pointer_cast<tcp_server_endpoint_impl>(its_info->get_endpoint(true));
...
bool service_discovery_impl::serialize_and_send(
    ...
    if (!_address.is_unspecified())
    {
        ...
        for (const auto& m : _messages)
        {
            if (m->has_entry())
            {
                ...
                {
                    if (host_->send_via_sd(endpoint_definition::get(_address, port_, reliable_,
                                                                    m->get_service(),
                                                                    m->get_instance()),
...
void service_discovery_impl::start_ttl_timer(int _shift)
{
    ...
    ttl_timer_.async_wait(
        std::bind(&service_discovery_impl::check_ttl, shared_from_this(), std::placeholders::_1));
}
...
void service_discovery_impl::start_subscription_expiration_timer_unlocked()
{
    ...
    subscription_expiration_timer_.async_wait(std::bind(
        &service_discovery_impl::expire_subscriptions, shared_from_this(), std::placeholders::_1));
}
    ...
    bool       new_finds(false);
    {
        ...
        for (const auto& its_service : requested_)
        {
            for (const auto& its_instance : its_service.second)
            {
                ...
                {
                    repetition_phase_finds[its_service.first][its_instance.first] =
                        its_instance.second;
                }
            }
        }
    ...
    const boost::system::error_code& _error)
{
    ...
    its_timer->async_wait(std::bind(&service_discovery_impl::on_repetition_phase_timer_expired,
                                    this, std::placeholders::_1, its_timer, its_repetitions,
                                    its_delay.count()));
    start_offer_debounce_timer(false);
}
    ...
    std::uint32_t _last_delay)
{
    ...
    else
    {
        ...
        if (its_timer_pair != repetition_phase_timers_.end())
        {
            ...
            if (ec)
            {
                VSOMEIP_ERROR << "service_discovery_impl::on_repetition_phase_timer_expired "
                                 "setting expiry time of timer failed: "
                              << ec.message();
            }
            its_timer_pair->first->async_wait(std::bind(
                &service_discovery_impl::on_repetition_phase_timer_expired, this,
    ...
    std::uint32_t _last_delay)
{
    ...
    if (its_timer_pair != find_repetition_phase_timers_.end())
    {
        ...
        if (ec)
        {
            VSOMEIP_ERROR << __func__ << "setting expiry time of timer failed: " << ec.message();
        }
        its_timer_pair->first->async_wait(
            std::bind(&service_discovery_impl::on_find_repetition_phase_timer_expired, this,
                      std::placeholders::_1, its_timer_pair->first, repetition, new_delay.count()));
    }
}
...
bool service_discovery_impl::stop_offer_service(const std::shared_ptr<serviceinfo>& _info,
                                                bool                                _send)
{
    std::lock_guard<std::mutex> its_lock(offer_mutex_);
    _info->set_ttl(0);
...
bool service_discovery_impl::send_collected_stop_offers(
    const std::vector<std::shared_ptr<serviceinfo>>& _infos)
{
    std::vector<std::shared_ptr<message_impl>> its_messages;
    auto its_current_message = std::make_shared<message_impl>();
    its_messages.push_back(its_current_message);
...
void service_discovery_impl::start_main_phase_timer()
{
    ...
    main_phase_timer_.async_wait(std::bind(&service_discovery_impl::on_main_phase_timer_expired,
...
void service_discovery_impl::update_acknowledgement(
    const std::shared_ptr<remote_subscription_ack>& _acknowledgement)
{
    if (_acknowledgement->is_complete() && !_acknowledgement->is_pending()
            ...
            if (e && e->get_type() == entry_type_e::SUBSCRIBE_EVENTGROUP_ACK && e->get_ttl())
            {
                const std::chrono::steady_clock::time_point its_expiration =
                    ...
                    + std::chrono::seconds(e->get_ttl()
                                           * get_ttl_factor(e->get_service(), e->get_instance(),
...
bool service_discovery_impl::check_stop_subscribe_subscribe(
    message_impl::entries_t::const_iterator _iter, message_impl::entries_t::const_iterator _end,
...
bool service_discovery_impl::has_opposite(message_impl::entries_t::const_iterator _iter,
...
bool service_discovery_impl::has_same(message_impl::entries_t::const_iterator _iter,
                                      message_impl::entries_t::const_iterator _end,
                                      const message_impl::options_t&          _options) const
{
    const auto its_entry = std::dynamic_pointer_cast<eventgroupentry_impl>(*_iter);
    ...
    if (its_info)
    {
        ...
        for (const auto& o : _options)
        {
                ...
                if (its_endpoint_option)
                {
                        ...
                        == layer_four_protocol_e::TCP)
                    {
                        its_reliable = endpoint_definition::get(
                            boost::asio::ip::address_v4(its_endpoint_option->get_address()),
            ...
            else if (o->get_type() == option_type_e::IP6_ENDPOINT)
            {
                ...
                {
                    its_reliable = endpoint_definition::get(
                        boost::asio::ip::address_v6(its_endpoint_option->get_address()),
                        its_endpoint_option->get_port(), true, its_service, its_instance);
                }
                else if (its_endpoint_option->get_layer_four_protocol()
                         == layer_four_protocol_e::UDP)
...
service_discovery_impl::get_ttl_factor(service_t _service, instance_t _instance,
    ...
    const boost::system::error_code& _error)
{
    if (!_error)
    {
        ...
        VSOMEIP_WARNING << "Didn't receive a multicast SD message for " << std::dec
                        << last_msg_received_timer_timeout_.count() << "ms.";
...
reliability_type_e service_discovery_impl::get_remote_offer_type(service_t  _service,
...
reliability_type_e service_discovery_impl::get_remote_offer_type(
    const std::shared_ptr<subscription>& _subscription) const
{
    ...
    return (
        has_reliable ?
            (has_unreliable ? reliability_type_e::RT_BOTH : reliability_type_e::RT_RELIABLE) :
...
void service_discovery_impl::remove_remote_offer_type(
    service_t _service, instance_t _instance, const boost::asio::ip::address& _reliable_address,
    ...
    bool _received_via_multicast)
{
    ...
    auto check_offer_info = [this, &was_unicast, _received_via_multicast](
                                const boost::asio::ip::address& address, bool reliable, port_t port,
...
void service_discovery_impl::send_subscription_ack(
    const std::shared_ptr<remote_subscription_ack>& _acknowledgement)
{
    ...
    for (const auto& its_subscription : _acknowledgement->get_subscriptions())
    {
        ...
        if (its_subscription->get_ttl() > 0 && its_subscription->is_initial())
        {
            ...
            if (its_info)
            {
                its_info->send_initial_events(its_subscription->get_reliable(),
...
void service_discovery_impl::add_entry_data(std::vector<std::shared_ptr<message_impl>>& _messages,
                                            const entry_data_t&                         _data)
{
    auto       its_current_message = _messages.back();
    const auto is_fitting =
        its_current_message->add_entry_data(_data.entry_, _data.options_, _data.other_);
    if (!is_fitting)
    {
...
void service_discovery_impl::add_entry_data_to_remote_subscription_ack_msg(
    const std::shared_ptr<remote_subscription_ack>& _acknowledgement, const entry_data_t& _data)
{
    ...
    const std::shared_ptr<subscription>& _subscription)
{
    ...
    if (its_info)
    {
            ...
            && its_info->is_reliability_auto_mode())
        {
            ...
            VSOMEIP_WARNING << "sd::" << __func__
                            << ": couldn't determine eventgroup reliability type for [" << std::hex
                            << std::setfill('0') << std::setw(4) << _service << "." << std::setw(4)
                            << _instance << "." << std::setw(4) << _eventgroup << "]"
                            << " using reliability type:  " << std::setw(4)
                            << static_cast<uint16_t>(its_reliability);
            its_info->set_reliability(its_reliability);
        }
    }
    else
    {
        VSOMEIP_WARNING << "sd::" << __func__ << ": couldn't lock eventgroupinfo [" << std::hex
                        << std::setfill('0') << std::setw(4) << _service << "." << std::setw(4)
                        << _instance << "." << std::setw(4) << _eventgroup << "] ";
