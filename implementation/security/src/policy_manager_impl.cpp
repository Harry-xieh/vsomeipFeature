...
bool policy_manager_impl::check_credentials(client_t                    _client,
    ...
    for (const auto& p : any_client_policies_)
    {
        ...
        if ((has_id && p->allow_who_) || (!has_id && !p->allow_who_))
        {
            ...
            if (!store_client_to_sec_client_mapping(_client, _sec_client))
            {
                ...
                VSOMEIP_INFO
                    << "vSomeIP Security: Client 0x" << std::hex << _client
...
bool policy_manager_impl::is_client_allowed(const vsomeip_sec_client_t* _sec_client,
                                            ...
                                            method_t _method, bool _is_request_service) const
{
    ...
    else
    {
        ...
        VSOMEIP_INFO << "vSomeIP Security: uid/gid " << std::dec << its_uid << "/" << its_gid
                     << " is not valid."
                     << "Therefore it isn't allowed to communicate to service/instance " << _service
                     << "/" << _instance << security_mode_text;
    ...
    for (const auto& p : any_client_policies_)
    {
        ...
        if ((has_uid && has_gid && p->allow_who_) || ((!has_uid || !has_gid) && !p->allow_who_))
        {
            if (p->allow_what_)
            {
                ...
                if (is_matching)
                {
                    boost::unique_lock<boost::shared_mutex> its_cache_lock(
                        is_client_allowed_cache_mutex_);
...
bool policy_manager_impl::is_offer_allowed(const vsomeip_sec_client_t* _sec_client,
                                           service_t _service, instance_t _instance) const
{
    ...
    else
    {
        ...
        VSOMEIP_INFO << "vSomeIP Security: uid/gid " << std::dec << its_uid << "/" << its_gid
                     << " is not valid."
                     << "Therefore it isn't allowed to offer service/instance " << _service << "/"
                     << _instance << security_mode_text;
    ...
    VSOMEIP_INFO << "vSomeIP Security: UID/GID=" << std::dec << its_uid << "/" << its_gid
                 << " isn't allowed to offer service/instance " << std::hex << _service << "/"
                 << _instance << security_mode_text;
...
void policy_manager_impl::update_security_policy(uid_t _uid, gid_t _gid,
                                                 const std::shared_ptr<policy>& _policy)
{
    ...
    if (its_matching_policy)
    {
        ...
        for (const auto& r : _policy->requests_)
        {
            ...
            {
                boost::icl::discrete_interval<service_t> its_service(
                    s, s, boost::icl::interval_bounds::closed());
...
void policy_manager_impl::add_security_credentials(uid_t _uid, gid_t _gid,
                                                   const std::shared_ptr<policy>& _policy,
                                                   client_t                       _client)
{
    ...
    {
        any_client_policies_.push_back(_policy);
        VSOMEIP_INFO << __func__ << " Added security credentials at client: 0x" << std::hex
                     << _client << std::dec << " with UID: " << _uid << " GID: " << _gid;
    }
}

bool policy_manager_impl::is_policy_update_allowed(uid_t                    _uid,
                                                   std::shared_ptr<policy>& _policy) const
{
    ...
    if (is_uid_allowed && _policy)
    {
        ...
        for (const auto& its_request : _policy->requests_)
        {
            ...
            for (its_service = its_request.first.lower(); its_service <= its_request.first.upper();
                 its_service++)
            {
            ...
            if (!has_service)
            {
                ...
                {
                    VSOMEIP_INFO
                        << "vSomeIP Security: Policy update requesting service ID: " << std::hex
                        << its_service
                ...
                else
                {
                    VSOMEIP_WARNING
                        << "vSomeIP Security: Policy update requesting service ID: " << std::hex
                        << its_service << " is not allowed! -> ignore update";
                }
    ...
    else
    {
        ...
        {
            VSOMEIP_INFO
                << "vSomeIP Security: Policy update for UID: " << std::dec << _uid
                << " is not allowed, but will be allowed due to whitelist audit mode is active!";
        }
        else
        {
...
bool policy_manager_impl::is_policy_removal_allowed(uid_t _uid) const
{
    ...
    {
        VSOMEIP_INFO
            << "vSomeIP Security: Policy removal for UID: " << std::dec << _uid
            << " is not allowed, but will be allowed due to whitelist audit mode is active!";
    }
    else
    {
...
bool policy_manager_impl::parse_policy(const byte_t*& _buffer, uint32_t& _buffer_size, uid_t& _uid,
                                       gid_t& _gid, const std::shared_ptr<policy>& _policy) const
{
...
bool policy_manager_impl::exist_in_any_client_policies_unlocked(std::shared_ptr<policy>& _policy)
{
    ...
    {
        std::lock_guard<std::mutex> its_policy_lock(p->mutex_);
        if (p->credentials_ == _policy->credentials_ && p->requests_ == _policy->requests_
            && p->offers_ == _policy->offers_ && p->allow_what_ == _policy->allow_what_
...
void policy_manager_impl::load_policies(const configuration_element& _element)
{
    ...
    try
    {
        ...
        for (auto its_security = found_policy.begin(); its_security != found_policy.end();
             ++its_security)
        {
...
void policy_manager_impl::load_policy(const boost::property_tree::ptree& _tree)
{
    ...
    for (auto i = _tree.begin(); i != _tree.end(); ++i)
    {
                ...
                if (its_key == "uid")
                {
                    ...
                    else
                    {
                        if (its_value != "any")
                        {
                            ...
                            its_uid_interval =
                                boost::icl::construct<boost::icl::discrete_interval<uid_t>>(
                ...
                else if (its_key == "gid")
                {
                    ...
                    else
                    {
                        if (its_value != "any")
                        {
                            ...
                            its_gid_interval =
                                boost::icl::construct<boost::icl::discrete_interval<gid_t>>(
        ...
        else if (i->first == "allow")
        {
            ...
            {
                VSOMEIP_WARNING
                    << "vSomeIP Security: Security configuration: \"allow\" tag overrides "
        ...
        else if (i->first == "deny")
        {
            ...
            {
                VSOMEIP_WARNING
                    << "vSomeIP Security: Security configuration: \"deny\" tag overrides "
...
void policy_manager_impl::load_policy_body(std::shared_ptr<policy>& _policy,
    ...
    for (auto l = _tree->second.begin(); l != _tree->second.end(); ++l)
    {
        if (l->first == "requests")
        {
            for (auto n = l->second.begin(); n != l->second.end(); ++n)
            {
                ...
                boost::icl::interval_map<instance_t, boost::icl::interval_set<method_t>>
                ...
                for (auto k = n->second.begin(); k != n->second.end(); ++k)
                {
                    ...
                    { // legacy definition for instances
                        boost::icl::interval_set<instance_t>      its_instance_interval_set;
                        boost::icl::interval_set<method_t>        its_method_interval_set;
                        boost::icl::discrete_interval<instance_t> all_instances(
                            0x01, 0xFFFF, boost::icl::interval_bounds::closed());
                        boost::icl::discrete_interval<method_t> all_methods(
                            0x01, 0xFFFF, boost::icl::interval_bounds::closed());
                    ...
                    else if (k->first == "instances")
                    { // new instances definition
                    ...
                    { // legacy definition for instances
                        ...
                        else
                        {
                            its_instance_interval_set.insert(
                                boost::icl::discrete_interval<instance_t>(0x0001, 0xFFFF));
                        }
                    }
                    else if (k->first == "instances")
                    { // new instances definition
...
void policy_manager_impl::load_credential(
...
void policy_manager_impl::load_security_update_whitelist(const configuration_element& _element)
{
    ...
    try
    {
        ...
        for (auto its_whitelist = found_whitelist.begin(); its_whitelist != found_whitelist.end();
             ++its_whitelist)
        {
...
void policy_manager_impl::load_security_policy_extensions(const configuration_element& _element)
{
    ...
    try
    {
        ...
        for (auto i = found_policy_extensions.begin(); i != found_policy_extensions.end(); ++i)
        {
            ...
            {
                VSOMEIP_DEBUG << __func__ << ": The path " << its_filesystem_path
                              << " is not valid";
            }
            std::map<std::string, bool> empty_map;
            policy_extension_paths_[its_client_host] =
                std::make_pair(its_filesystem_path, empty_map);
...
template <typename T_>
void policy_manager_impl::load_interval_set(const boost::property_tree::ptree& _tree,
                                            boost::icl::interval_set<T_>&      _intervals,
...
void policy_manager_impl::get_requester_policies(
    const std::shared_ptr<policy> _policy, std::set<std::shared_ptr<policy>>& _requesters) const
{
                            ...
                            for (const auto& k : j.second)
                            {
                                ...
                                if (its_i_lower <= its_k_upper && its_k_lower <= its_i_upper)
                                {
                                    ...
                                    boost::icl::interval_map<instance_t,
                                                             boost::icl::interval_set<method_t>>
                                        its_instances_methods;
                                    its_instances_methods +=
                                        std::make_pair(boost::icl::interval<instance_t>::closed(
                                      ...
                                      std::unordered_set<client_t>& _clients) const
{
    ...
    {
        if (i.second.port == VSOMEIP_SEC_PORT_UNUSED && i.second.user == _uid
            && i.second.group == _gid)
...
policy_manager_impl::is_policy_extension_loaded(const std::string& _client_host) const
{
    ...
    if (found_host != policy_extension_paths_.end())
    {
        auto found_complete_path =
            found_host->second.second.find(get_security_config_folder(found_host->second.first));
...
void policy_manager_impl::set_is_policy_extension_loaded(const std::string& _client_host,
                                                         const bool         _loaded)
{
...
bool policy_manager_impl::parse_uid_gid(const byte_t*& _buffer, uint32_t& _buffer_size, uid_t& _uid,
                                        gid_t& _gid) const
{
...
bool policy_manager_impl::store_client_to_sec_client_mapping(
        ...
        if (found_client != ids_.end())
        {
            if (!utility::compare(found_client->second, *_sec_client))
            {
                ...
                VSOMEIP_WARNING << "vSomeIP Security: Client 0x" << std::hex << _client
                                << " with UID/GID=" << std::dec << its_new_uid << "/" << its_new_gid
                                << " : Overwriting existing credentials UID/GID=" << std::dec
                                << its_old_uid << "/" << its_old_gid;
...
bool policy_manager_impl::get_client_to_sec_client_mapping(client_t              _client,
...
void policy_manager_impl::store_sec_client_to_client_mapping(
...
bool policy_manager_impl::get_sec_client_to_clients_mapping(const vsomeip_sec_client_t* _sec_client,
