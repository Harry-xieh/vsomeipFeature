...
bool policy::deserialize_uid_gid(const byte_t*& _data, uint32_t& _size, uid_t& _uid,
                                 gid_t& _gid) const
{
    bool its_result;
...
bool policy::deserialize(const byte_t*& _data, uint32_t& _size)
{
    ...
    while (0 < its_requests_length)
    {
        ...
        {
            VSOMEIP_WARNING << "vSomeIP Security: Policy with service ID: 0x" << std::hex
                            << its_service << " is not allowed!";
            return false;
        }
...
bool policy::deserialize_id_item_list(const byte_t*& _data, uint32_t& _size,
...
bool policy::deserialize_id_item(const byte_t*& _data, uint32_t& _size, uint16_t& _low,
                                 uint16_t& _high) const
{
...
bool policy::serialize(std::vector<byte_t>& _data) const
{
    ...
    {
        for (auto its_service = its_request.first.lower(); its_service <= its_request.first.upper();
             its_service++)
        {
...
void policy::serialize_interval_set(const boost::icl::interval_set<uint16_t>& _intervals,
...
void policy::serialize_interval(const boost::icl::discrete_interval<uint16_t>& _interval,
                                std::vector<byte_t>&                           _data) const
{
...
void policy::print() const
{
    for (auto its_credential : credentials_)
    {
        ...
        {
            VSOMEIP_INFO << "policy::print Security configuration: UID: " << std::dec
                         << its_uid_interval.lower();
        }
        for (auto its_gid_interval : its_credential.second)
        {
            ...
            {
                VSOMEIP_INFO << "    policy::print Security configuration: GID: " << std::dec
                             << its_gid_interval.lower();
            }
        }
    }

    VSOMEIP_INFO << "policy::print Security configuration: REQUESTS POLICY SIZE: " << std::dec
                 << requests_.size();
    for (auto its_request : requests_)
    {
        ...
        for (auto its_instance : its_request.second)
        {
            ...
            VSOMEIP_INFO << "policy::print          first: 0x" << std::hex
                         << its_instance.first.lower() << " last: 0x" << its_instance.first.upper();
            VSOMEIP_INFO << "policy::print     Methods: ";
            for (auto its_method : its_instance.second)
            {
                VSOMEIP_INFO << "policy::print          first: 0x" << std::hex << its_method.lower()
                             << " last: 0x" << its_method.upper();
            }
        }
    }

    VSOMEIP_INFO << "policy::print Security configuration: OFFER POLICY SIZE: " << std::dec
                 << offers_.size();
    for (auto its_offer : offers_)
    {
        ...
        {
            VSOMEIP_INFO << "policy::print     Instances: ";
            VSOMEIP_INFO << "policy::print          first: 0x" << std::hex << its_instance.lower()
