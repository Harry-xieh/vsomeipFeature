...
void routing_info_entry::serialize(std::vector<byte_t>& _buffer, size_t& _index,
                                   error_e& _error) const
{
    ...
    if (!address_.is_unspecified())
    {
        ...
        {
            its_client_size +=
                uint32_t(sizeof(boost::asio::ip::address_v4::bytes_type) + sizeof(port_));
        }
        else
        {
            its_client_size +=
                uint32_t(sizeof(boost::asio::ip::address_v6::bytes_type) + sizeof(port_));
        }
    }
    ...
    if (type_ > routing_info_entry_type_e::RIE_DELETE_CLIENT)
    {
        its_size = (services_.size()
                    * (sizeof(service_t) + sizeof(instance_t) + sizeof(major_version_t)
                       + sizeof(minor_version_t)));
...
void routing_info_entry::deserialize(const std::vector<byte_t>& _buffer, size_t& _index,
                                     error_e& _error)
{
    uint32_t its_size;
