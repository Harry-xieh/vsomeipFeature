...
namespace vsomeip_v3 { namespace cfg {
...
bool configuration_impl::lazy_load_security(const std::string& _client_host)
{
    ...
    if (!its_folder.empty())
    {
        ...
        std::string its_security_config_folder =
            policy_manager_->get_security_config_folder(its_folder);
...
bool configuration_impl::remote_offer_info_add(service_t _service, instance_t _instance,
    ...
    else
    {
...

        {
            ...
            if (found_service != services_.end())
            {
                ...
                {
                    VSOMEIP_INFO << "Updating remote configuration for service [" << std::hex
                                 << std::setw(4) << std::setfill('0') << its_service->service_
            ...
            if (!updated)
            {
                ...
                VSOMEIP_INFO << "Added new remote configuration for service [" << std::hex
                             << std::setfill('0') << std::setw(4) << its_service->service_ << "."
...
bool configuration_impl::remote_offer_info_remove(service_t _service, instance_t _instance,
                                                  std::uint16_t _port, bool _reliable,
                                                  bool  _magic_cookies_enabled,
                                                  bool* _still_offered_remote)
{
    ...
    else
    {
        ...
        if (found_service != services_.end())
        {
            ...
            if (found_instance != found_service->second.end())
            {
                VSOMEIP_INFO << "Removing remote configuration for service [" << std::hex
                ...
                *_still_offered_remote = (found_instance->second->unreliable_ != ILLEGAL_PORT
                                          || found_instance->second->reliable_ != ILLEGAL_PORT);
                ret                    = true;
            }
        }
...
bool configuration_impl::load_routing(const configuration_element& _element)
{
    try
    {
        ...
        else
        {
            ...
            bool is_loaded =
                load_routing_host(its_routing, _element.name_) && load_routing_guests(its_routing);
...
bool configuration_impl::load_routing_host(const boost::property_tree::ptree& _tree,
...
configuration_impl::load_routing_guest_port_range(const boost::property_tree::ptree& _tree) const
{
    std::stringstream                   its_converter;
    std::set<std::pair<port_t, port_t>> its_ranges;
    ...
    for (auto its_range = _tree.begin(); its_range != _tree.end(); ++its_range)
    {
        ...
        for (auto its_element = its_range->second.begin(); its_element != its_range->second.end();
             ++its_element)
        {
...
void configuration_impl::load_application_data(const boost::property_tree::ptree& _tree,
...
std::map<plugin_type_e, std::set<std::string>>
configuration_impl::load_plugins(const boost::property_tree::ptree& _tree,
...
void configuration_impl::load_trace_filter_expressions(const boost::property_tree::ptree& _tree,
...
void configuration_impl::load_trace_filter_match(const boost::property_tree::ptree& _data,
...
std::set<event_t>
configuration_impl::load_suppress_multiple_events(const boost::property_tree::ptree& _tree)
{
...
void configuration_impl::insert_suppress_events(service_t _service, instance_t _instance,
                                                event_t _event)
{
...
void configuration_impl::print_suppress_events(void) const
{
    ...
    {
        VSOMEIP_INFO << std::hex << std::setfill('0') << "+[" << std::setw(4) << its_log.service
                     << "." << std::setw(4) << its_log.instance << "." << its_log.event << "]";
    }
}
    ...
    try
    {
        ...
        for (auto i = its_service_discovery.begin(); i != its_service_discovery.end(); ++i)
        {
            ...
            else if (its_key == "repetitions_base_delay")
            {
                ...
                {
                    VSOMEIP_WARNING
                        << "Multiple definitions for service_discovery.repetition_base_delay."
                           " Ignoring definition from "
                        << _element.name_;
                }
                else
            ...
            else if (its_key == "ttl")
            {
                ...
                else
                {
                    ...
                    if (sd_ttl_ == 0)
                    {
                        VSOMEIP_WARNING << "TTL=0 is not allowed. Using default (" << std::dec
                                        << VSOMEIP_SD_DEFAULT_TTL << ")";
                        sd_ttl_ = VSOMEIP_SD_DEFAULT_TTL;
                    }
                    else
            ...
            else if (its_key == "cyclic_offer_delay")
            {
                ...
                {
                    VSOMEIP_WARNING
                        << "Multiple definitions for service_discovery.cyclic_offer_delay."
                           " Ignoring definition from "
                        << _element.name_;
                }
                else
            ...
            else if (its_key == "request_response_delay")
            {
                ...
                {
                    VSOMEIP_WARNING
                        << "Multiple definitions for service_discovery.request_response_delay."
                           " Ignoring definition from "
                        << _element.name_;
                }
                else
            ...
            else if (its_key == "ttl_factor_subscriptions")
            {
                ...
                {
                    VSOMEIP_WARNING
                        << "Multiple definitions for service_discovery.ttl_factor_subscriptions."
                           " Ignoring definition from "
                        << _element.name_;
                }
                else
            ...
            else if (its_key == "max_remote_subscribers")
            {
                ...
                {
                    VSOMEIP_WARNING
                        << "Multiple definitions for service_discovery.max_remote_subscribers."
                           " Ignoring definition from "
                        << _element.name_;
                }
                else
...
void configuration_impl::load_npdu_default_timings(const configuration_element& _element)
{
    ...
    try
    {
        if (_element.tree_.get_child_optional(ndt))
        {
            ...
            {
                VSOMEIP_WARNING << "Multiple definitions of " << ndt << " Ignoring definition from "
                                << _element.name_;
            }
            else
            {
    ...
    try
    {
        ...
        if (is_loaded)
        {
            ...
            if (its_service->unicast_address_ == default_unicast_)
            {
                ...
                if (its_service->unreliable_ != ILLEGAL_PORT)
                {
                    ...
                    services_by_ip_port_[boost::asio::ip::address_v4::any().to_string()]
                                        [its_service->unreliable_][its_service->service_] =
                                            its_service;
            ...
            else
            {
                ...
                if (its_service->reliable_ != ILLEGAL_PORT)
                {
                    services_by_ip_port_[its_service->unicast_address_][its_service->reliable_]
                                        [its_service->service_] = its_service;
                }
                if (its_service->unreliable_ != ILLEGAL_PORT)
                {
        ...
        if (its_event_id > 0)
        {
            ...
            else
            {
                ...
                if (its_reliability == reliability_type_e::RT_UNKNOWN)
                {
                    ...
                    VSOMEIP_WARNING
                        << "Reliability type for event [" << std::hex << _service->service_ << "."
                        << _service->instance_ << "." << its_event_id
                        << "] was not configured Using : "
                        << ((its_reliability == reliability_type_e::RT_RELIABLE) ? "RT_RELIABLE" :
                                                                                   "RT_UNRELIABLE");
                }

                auto its_event =
                    std::make_shared<event>(its_event_id, its_is_field, its_reliability, its_cycle,
...
void configuration_impl::load_eventgroup(std::shared_ptr<service>&          _service,
                                         const boost::property_tree::ptree& _tree)
{
    for (auto i = _tree.begin(); i != _tree.end(); ++i)
    {
...
configuration_impl::load_client_port_range(const boost::property_tree::ptree& _tree)
{
    ...
    {
        VSOMEIP_WARNING << "Port range invalid: first: " << std::dec << its_first_port
                        << " last: " << its_last_port;
...
void configuration_impl::load_payload_sizes(const configuration_element& _element)
{
    ...
    {
        for (const auto& s :
             {max_local_payload_size, max_reliable_payload_size, max_unreliable_payload_size})
        {
            if (_element.tree_.get_child_optional(s))
            {
                ...
                try
                {
                    ...
                    const auto its_size =
                        static_cast<std::uint32_t>(std::stoul(size_str.c_str(), NULL, 10) + 16);
        ...
        if (_element.tree_.get_child_optional(payload_sizes))
        {
            ...
            for (auto i = its_ps.begin(); i != its_ps.end(); ++i)
            {
                     ...
                     j != i->second.get_child(ports).end(); ++j)
                {
                    ...
                    try
                    {
                        ...
                        // add 16 Byte for the SOME/IP header
                        its_message_size =
                            static_cast<std::uint32_t>(std::stoul(s.c_str(), NULL, 10) + 16);
                    } catch (const std::exception& e)
                    {
                ...
                && max_configured_message_size_ > max_local_message_size_)
            {
                VSOMEIP_WARNING << max_local_payload_size
                                ...
                                << " for external communication. " << max_local_payload_size
                                << " will be increased to " << max_configured_message_size_ - 16
                                << " to ensure "
            ...
            {
                VSOMEIP_WARNING << max_local_payload_size << " (" << max_local_message_size_ - 16
                                << ") is configured"
        ...
        if (!its_partition_members.empty())
        {
            ...
            for (const auto& p : its_partition_members)
            {
                for (const auto& m : p.second)
                {
                    ...
                    its_log << "<" << std::setfill('0') << std::hex << std::setw(4) << p.first
                            << "." << std::setw(4) << m << ">";
                }
            }
...
void configuration_impl::set_magic_cookies_unicast_address()
{
    ...
    its_magic_cookies_[get_unicast_address().to_string()].insert(
        magic_cookies_[default_unicast_].begin(), magic_cookies_[default_unicast_].end());
...
bool configuration_impl::is_internal_service(service_t _service, instance_t _instance) const
{
    ...
    {
        if (_service >= its_range.first_service_ && _service <= its_range.last_service_
            && _instance >= its_range.first_instance_ && _instance <= its_range.last_instance_)
        {
            return true;
        }
    }
...
void configuration_impl::get_configured_timing_requests(
    service_t _service, const std::string& _ip_target, std::uint16_t _port_target, method_t _method,
    std::chrono::nanoseconds* _debounce_time, std::chrono::nanoseconds* _max_retention_time) const
{
...
void configuration_impl::get_configured_timing_responses(
    service_t _service, const std::string& _ip_service, std::uint16_t _port_service,
...
bool configuration_impl::get_client_port(service_t _service, instance_t _instance,
                                         ...
                                         uint16_t&                           _client_port) const
{
    ...
    VSOMEIP_ERROR << "Cannot find free client port for communication to service [" << std::hex
                  << std::setw(4) << std::setfill('0') << _service << "." << std::hex
                  << std::setw(4) << std::setfill('0') << _instance << "." << std::dec
...
std::set<std::pair<port_t, port_t>> configuration_impl::get_routing_guest_ports(uid_t _uid,
                                                                                gid_t _gid) const
{
...
bool configuration_impl::is_local_routing() const
{
    ...
    {
        is_local =
            routing_.host_.unicast_.is_unspecified() || routing_.host_.unicast_.is_multicast();
    } catch (...)
    {}
...
bool configuration_impl::get_multicast(service_t _service, instance_t _instance,
                                       eventgroup_t _eventgroup, std::string& _address,
                                       uint16_t& _port) const
{
...
uint8_t configuration_impl::get_threshold(service_t _service, instance_t _instance,
...
void configuration_impl::get_event_update_properties(service_t _service, instance_t _instance,
                                                     event_t                    _event,
        ...
        while (it != its_client->ports_[_reliable].end())
        {
            if (_used_client_ports[_reliable].find(*it) == _used_client_ports[_reliable].end())
            {
                ...
                VSOMEIP_INFO << "configuration_impl:find_specific_port #1:"
                             << " service: " << std::hex << _service << " instance: " << _instance
            ...
            for (auto its_port : _used_client_ports[_reliable])
            {
                    ...
                    == _used_client_ports[_reliable].end())
                {
                    ...
                    VSOMEIP_INFO << "configuration_impl:find_specific_port #2:"
                                 ...
                                 << _reliable << " return specific port: " << (uint32_t)_port;
                    return true;
                }
            }
        }
...
reliability_type_e configuration_impl::get_event_reliability(service_t  _service,
...
reliability_type_e configuration_impl::get_service_reliability(service_t  _service,
                                                               instance_t _instance) const
{
    std::lock_guard<std::mutex> its_lock(services_mutex_);
    reliability_type_e          its_reliability(reliability_type_e::RT_UNKNOWN);
...
std::shared_ptr<eventgroup> configuration_impl::find_eventgroup(service_t    _service,
                                                                instance_t   _instance,
                                                                eventgroup_t _eventgroup) const
{
...
std::uint32_t configuration_impl::get_max_message_size_local() const
{
    if (max_local_message_size_ == 0
        && (VSOMEIP_MAX_LOCAL_MESSAGE_SIZE == 0 || VSOMEIP_MAX_TCP_MESSAGE_SIZE == 0))
    {
...
std::uint32_t configuration_impl::get_max_message_size_reliable(const std::string& _address,
                                                                std::uint16_t      _port) const
{
...
std::map<plugin_type_e, std::set<std::string>>
configuration_impl::get_plugins(const std::string& _name) const
{
    std::map<plugin_type_e, std::set<std::string>> its_plugins;
...
configuration_impl::get_additional_data(const std::string& _application_name,
...
void configuration_impl::load_e2e_protected(const boost::property_tree::ptree& _tree)
{
    ...
    e2e_configuration_[std::make_pair(service_id, event_id)] = std::make_shared<cfg::e2e>(
        variant, profile, service_id, event_id, std::move(custom_parameters));
}
...
void configuration_impl::load_ttl_factors(const boost::property_tree::ptree& _tree,
                                          ttl_map_t*                         _target)
{
...
configuration_impl::get_endpoint_queue_limit(const std::string& _address, std::uint16_t _port) const
{
...
void configuration_impl::load_endpoint_queue_sizes(const configuration_element& _element)
{
    ...
    try
    {
        if (_element.tree_.get_child_optional(endpoint_queue_limit_external))
        {
            ...
            {
                VSOMEIP_WARNING << "Multiple definitions for " << endpoint_queue_limit_external
                                << " Ignoring definition from " << _element.name_;
            }
            else
            {
        ...
        if (_element.tree_.get_child_optional(endpoint_queue_limit_local))
        {
            ...
            {
                VSOMEIP_WARNING << "Multiple definitions for " << endpoint_queue_limit_local
                                << " Ignoring definition from " << _element.name_;
            }
            else
            {
...
void configuration_impl::load_service_debounce(const boost::property_tree::ptree& _tree,
                                               debounce_configuration_t&          _debounces)
{
    ...
    if (its_service > 0 && its_instance > 0 && !its_debounces.empty())
    {
        ...
        if (find_service != debounces_.end())
        {
            ...
            if (find_instance != find_service->second.end())
            {
                VSOMEIP_ERROR << "Multiple debounce configurations for service " << std::hex
...
void configuration_impl::load_event_debounce_ignore(const boost::property_tree::ptree& _tree,
                                                    std::map<std::size_t, byte_t>&     _ignore)
{
    ...
    for (auto i = _tree.begin(); i != _tree.end(); ++i)
    {
        ...
        if (!its_value.empty() && std::find_if(its_value.begin(), its_value.end(), [](char _c) {
                    ...
                    { // range
                        ...
                        if (its_type != port_type_e::PT_UNKNOWN)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    its_ports.operator[](is_reliable)
                                        .second.insert(boost::icl::interval<std::uint16_t>::closed(
                                            its_first, its_last));
                                }
                            }
                        }
...
bool configuration_impl::load_npdu_debounce_times_configuration(
    const std::shared_ptr<service>& _service, const boost::property_tree::ptree& _tree)
{
    bool is_loaded(true);
    try
    {
    ...
    try
    {
        ...
        for (const auto& i : _tree)
        {
            ...
            if (its_method_str.size())
            {
                ...
                {
                    _service->debounce_times_responses_[its_method] = {its_debounce_time,
                                                                       its_retention_time};
                }
            }
        }
...
void configuration_impl::load_someip_tp(const std::shared_ptr<service>&    _service,
                                        const boost::property_tree::ptree& _tree)
{
    try
    {
...
void configuration_impl::load_someip_tp_for_service(const std::shared_ptr<service>&    _service,
                                                    const boost::property_tree::ptree& _tree,
                                                    bool                               _is_request)
{
    try
    {
        ...
        for (const auto& method : _tree)
        {
                ...
                for (const auto& its_data : method.second)
                {
                    ...
                    if (!its_value.empty())
                    {
                        ...
                        else if (its_data.first == "max-segment-length")
                        {
                            ...
                            {
                                VSOMEIP_WARNING
                                    << "SOMEIP/TP: max-segment-length must be multiple of 16. Corrected "
                ...
                else
                {
                    ...
                    else
                    {
                        VSOMEIP_WARNING
                            ...
                            << std::hex << std::setfill('0') << std::setw(4) << _service->service_
                            << "." << std::setw(4) << _service->instance_ << "." << std::setw(4)
                            << its_method << "]:"
                            << " using (" << std::dec << its_entry->second.first << ", " << std::dec
                            << its_entry->second.second << ")";
                    }
                }
            }
            ...
            {
                VSOMEIP_ERROR
                    << "SOME/IP-TP configuration contains invalid entry. No valid method specified!";
            }
        }
    } catch (...)
    {
...
bool configuration_impl::is_protected_port(const boost::asio::ip::address& _address,
...
bool configuration_impl::is_secure_port(const boost::asio::ip::address& _address,
                                        std::uint16_t _port, bool _reliable) const
{
...
void configuration_impl::set_sd_acceptance_rule(const boost::asio::ip::address& _address,
                                                port_range_t _port_range, port_type_e _type,
                                                const std::string& _path, bool _reliable,
                                                bool _enable, bool _default)
{
    (void)_port_range;
    (void)_type;
    ...
    const bool rules_active =
        (sd_acceptance_rules_active_.find(_address) != sd_acceptance_rules_active_.end());

    const auto found_address = sd_acceptance_rules_.find(_address);
    if (found_address != sd_acceptance_rules_.end())
    {
        if (found_address->second.first.length() > 0 && found_address->second.first != _path)
        {
            VSOMEIP_WARNING << __func__ << ": activation path for IP: " << _address
                            << " differ: " << found_address->second.first << " vs. " << _path
        ...
        else if (_enable)
        {
            ...
            const auto found_reliability = found_address->second.second.find(_reliable);
            VSOMEIP_INFO << "ipsec:acceptance:" << _address << ":" << (_reliable ? "tcp" : "udp")
                         << ": using default ranges " << found_reliability->second.first << " "
                         << found_reliability->second.second;
        }
    }
    else if (_enable)
...
void configuration_impl::set_sd_acceptance_rules_active(const boost::asio::ip::address& _address,
                                                        bool                            _enable)
{
    std::lock_guard<std::mutex> its_lock(sd_acceptance_required_ips_mutex_);
    if (_enable)
                                      ...
                                      method_t _method) const
{
    ...
    {
        ret =
            (its_service->tp_client_config_.find(_method) != its_service->tp_client_config_.end());
    }
...
void configuration_impl::get_tp_configuration(service_t _service, instance_t _instance,
                                              method_t _method, bool _is_client,
                                              std::uint16_t& _max_segment_length,
                                              std::uint32_t& _separation_time) const
{
...
reliability_type_e configuration_impl::get_reliability_type(
    const boost::asio::ip::address& _reliable_address, const uint16_t& _reliable_port,
    const boost::asio::ip::address& _unreliable_address, const uint16_t& _unreliable_port) const
{
    if (_reliable_port != ILLEGAL_PORT && _unreliable_port != ILLEGAL_PORT
        && !_reliable_address.is_unspecified() && !_unreliable_address.is_unspecified())
    {
