...
bool utility::is_routing_manager(const std::string& _network)
{
    ...
    if (GetTempPathW(MAX_PATH, its_tmp_folder))
    {
        ...
        r.first->second.lock_handle_ =
            CreateFileW(its_lockfile.c_str(), GENERIC_READ, 0, NULL, CREATE_NEW, 0, NULL);
    ...
    {
        VSOMEIP_ERROR << __func__ << ": Could not open " << its_lockfile << ": "
                      << std::strerror(errno);
    }
...
void utility::remove_lockfile(const std::string& _network)
{
    ...
    if (r->second.lock_handle_ != INVALID_HANDLE_VALUE)
    {
        ...
        {
            VSOMEIP_ERROR << __func__ << ": Could not get temp folder." << std::hex
                          << GetLastError();
        }
    }
#else
...
client_t utility::request_client_id(const std::shared_ptr<configuration>& _config,
                                    const std::string& _name, client_t _client)
{
    ...
    { // predefined client identifier
        ...
        if (its_iterator == r->second.used_clients_.end())
        { // unused identifier
        ...
        { // already in use
            ...
            VSOMEIP_WARNING << "Requested client identifier " << std::setw(4) << std::setfill('0')
                            << std::hex << _client << " is already used by application \""
                            << its_iterator->second << "\".";
    ...
    do
    {
        r->second.next_client_ =
            (r->second.next_client_
             & static_cast<std::uint16_t>(~its_client_mask)) // save diagnosis address bits
            | (static_cast<std::uint16_t>(
                   (r->second.next_client_ // set all diagnosis address bits to one
...
void utility::set_thread_niceness(int _nice) noexcept
{
    ...
    {
        VSOMEIP_WARNING << "failed to set niceness for thread " << std::this_thread::get_id()
