...
application_impl::application_impl(const std::string& _name, const std::string& _path)
    : runtime_(runtime::get()),
      ...
      dispatcher_counter_(0),
      max_detached_thread_wait_time(VSOMEIP_MAX_WAIT_TIME_DETACHED_THREADS),
      stopped_(false),
...
application_impl::~application_impl()
{
    ...
    if (configuration_)
    {
        auto its_plugin = plugin_manager::get()->get_plugin(plugin_type_e::CONFIGURATION_PLUGIN,
                                                            VSOMEIP_CFG_LIBRARY);
        if (its_plugin)
        {
            ...
            if (its_configuration_plugin)
            {
                ...
                if (!its_removed)
                {
                    VSOMEIP_WARNING << __func__
                                    << ": Unable to remove configuration entry stored for "
                                    << name_;
                }
            }
        }
...
bool application_impl::init()
{
    ...
    if (its_configuration)
    {
        ...
        if (is_routing_manager_host_)
        {
            ...
            {
                client_ = static_cast<client_t>((configuration_->get_diagnosis_address() << 8)
        ...
        else
        {
            ...
            routing_ = std::make_shared<routing_manager_client>(this, client_side_logging_,
                                                                client_side_logging_filter_);
        }
        ...
        VSOMEIP_INFO << "Application(" << (name_ != "" ? name_ : "unnamed") << ", " << std::hex
                     << std::setfill('0') << std::setw(4) << client_ << ") is initialized ("
...
void application_impl::start()
{
    ...
    const int    io_thread_nice_level = configuration_->get_io_thread_nice_level(name_);
    {
        ...
        stopped_        = false;
        stopped_called_ = false;
        VSOMEIP_INFO << "Starting vsomeip application \"" << name_ << "\" (" << std::hex
                     << std::setw(4) << std::setfill('0') << client_ << ") using " << std::dec
                     << io_thread_count << " threads"
    ...
    if (its_app_plugin_info != its_plugins.end())
    {
        ...
        {
            auto its_application_plugin =
                plugin_manager::get()->get_plugin(plugin_type_e::APPLICATION_PLUGIN, its_library);
            if (its_application_plugin)
            {
                std::dynamic_pointer_cast<application_plugin>(its_application_plugin)
...
{
    VSOMEIP_INFO << "Stopping vsomeip application \"" << name_ << "\" (" << std::hex << std::setw(4)
                 << std::setfill('0') << client_ << ").";
    ...
    if (configuration_)
    {
        ...
        if (its_app_plugin_info != its_plugins.end())
        {
            for (const auto& its_library : its_app_plugin_info->second)
            {
                ...
                {
                    std::dynamic_pointer_cast<application_plugin>(its_application_plugin)
                        ->on_application_state_change(name_,
                                                      application_plugin_state_e::STATE_STOPPED);
                }
            }
        }
    }
    ...
    if (block)
    {
        ...
        block_stop_cv_.wait_for(block_stop_lock, std::chrono::milliseconds(1000), [this] {
            return block_stopping_.load();
        });
        block_stopping_ = false;
    }
}
                                 ...
                                 major_version_t _major, event_t _event)
{
    if (routing_)
    {
        ...
        check_send_back_cached_event(_service, _instance, _event, _eventgroup, &send_back_cached,
                                     &send_back_cached_group);
...
availability_state_e application_impl::is_available_unlocked(service_t       _service,
                                                             ...
                                                             minor_version_t _minor) const
{
    ...
    else if (_service == ANY_SERVICE)
    {
        for (const auto& found_service : available_)
        {
            ...
            {
                for (auto it = found_service.second.cbegin(); it != found_service.second.cend();
                     it++)
                {
                    check_major_minor(it);
                    if (its_state != availability_state_e::AS_UNKNOWN)
...
bool application_impl::are_available(available_t& _available, service_t _service,
...
availability_state_e application_impl::are_available_unlocked(available_t&    _available,
                                                              service_t       _service,
                                                              instance_t      _instance,
                                                              major_version_t _major,
                                                              minor_version_t _minor) const
{
    ...
    if (_service == ANY_SERVICE)
    {
        ...
        for (auto its_available_services_it = available_.begin();
             its_available_services_it != available_.end(); ++its_available_services_it)
        {
    ...
    for (auto its_available_services_it = _available.begin();
         its_available_services_it != _available.end(); ++its_available_services_it)
    {
         ...
         its_available_services_it != _available.end(); ++its_available_services_it)
    {
        ...
        if (found_available_service != available_.end())
        {
                 ...
                 ++its_available_instances_it)
            {
                ...
                auto found_available_instance =
                    found_available_service->second.find(its_available_instances_it->first);
                 ...
                 ++its_available_instances_it)
            {
                ...
                if (found_available_instance != found_available_service->second.end())
                {
                         ...
                         ++its_available_major_it)
                    {
                        ...
                        auto found_available_major =
                            found_available_instance->second.find(its_available_major_it->first);
...
void application_impl::send(std::shared_ptr<message> _message)
{
    ...
    if (client_side_logging_
        && (client_side_logging_filter_.empty()
            ...
            || (1
                == client_side_logging_filter_.count(
                    std::make_tuple(_message->get_service(), _message->get_instance())))))
    {
        VSOMEIP_INFO << "application_impl::send: (" << std::hex << std::setfill('0') << std::setw(4)
                     << client_ << "): [" << std::setw(4) << _message->get_service() << "."
                     << std::setw(4) << _message->get_instance() << "." << std::setw(4)
                     << _message->get_method() << ":" << std::setw(4)
                     << (is_request ? session_ : _message->get_session()) << ":" << std::setw(4)
...
void application_impl::register_availability_handler(service_t _service, instance_t _instance,
                                                     const availability_handler_t& _handler,
                                                     major_version_t _major, minor_version_t _minor)
{
    ...
    auto its_handler_ext = [_handler](service_t _service, instance_t _instance,
        ...
        _handler(_service, _instance, (_state == availability_state_e::AS_AVAILABLE));
    };
...
void application_impl::register_availability_handler(service_t _service, instance_t _instance,
                                                     const availability_state_handler_t& _handler,
                                                     major_version_t _major, minor_version_t _minor)
{
    std::lock_guard<std::mutex> availability_lock(availability_mutex_);
...
void application_impl::invoke_availability_handler(service_t _service, instance_t _instance,
                                                   major_version_t _major, minor_version_t _minor)
{
    std::lock_guard<std::mutex> availability_lock(availability_mutex_);
            ...
            if (found_major != found_instance->second.end())
            {
                ...
                if (found_minor != found_major->second.end())
                {
                    ...
                    if (availability_state_e::AS_UNKNOWN != its_state
                        && get_availability_state(found_minor->second.second, _service, _instance,
                                                  _major, _minor)
                               != its_state)
                    {
...
void application_impl::register_availability_handler_unlocked(
    service_t _service, instance_t _instance, const availability_state_handler_t& _handler,
    major_version_t _major, minor_version_t _minor)
{
    auto its_state{is_available_unlocked(_service, _instance, _major, _minor)};
...
void application_impl::unregister_availability_handler(service_t _service, instance_t _instance,
                                                       major_version_t _major,
...
void application_impl::register_subscription_handler(service_t _service, instance_t _instance,
                                                     eventgroup_t                  _eventgroup,
                                                     const subscription_handler_t& _handler)
{
    subscription_handler_ext_t its_handler_ext = [_handler](client_t _client, uid_t _uid,
                                                            gid_t _gid, const std::string& _env,
                                                            bool _is_subscribed) {
...
void application_impl::on_subscription_status(service_t _service, instance_t _instance,
                                              eventgroup_t _eventgroup, event_t _event,
                                              uint16_t _error)
{
...
void application_impl::register_message_handler(service_t _service, instance_t _instance,
                                                method_t _method, const message_handler_t& _handler)
{
    register_message_handler_ext(_service, _instance, _method, _handler,
                                 handler_registration_type_e::HRT_REPLACE);
}

void application_impl::unregister_message_handler(service_t _service, instance_t _instance,
                                                  method_t _method)
{
...
void application_impl::offer_event(service_t _service, instance_t _instance, event_t _notifier,
                                   ...
                                   reliability_type_e           _reliability)
{
    if (routing_)
    {
        if (_cycle == std::chrono::milliseconds::zero() && _change_resets_cycle == false
            && _update_on_change == true)
        {
            configuration_->get_event_update_properties(_service, _instance, _notifier, _cycle,
                                                        _change_resets_cycle, _update_on_change);

            VSOMEIP_INFO << __func__ << std::hex << std::setfill('0') << ": Event [" << std::setw(4)
                         << _service << "." << std::setw(4) << _instance << "." << std::setw(4)
                         << _notifier << "] uses configured cycle time " << std::dec
                         << _cycle.count() << "ms";
        }

        routing_->register_event(client_, _service, _instance, _notifier, _eventgroups, _type,
                                 _reliability, _cycle, _change_resets_cycle, _update_on_change,
...
{
    if (routing_)
        routing_->register_event(client_, _service, _instance, _event, _eventgroups, _type,
                                 _reliability, std::chrono::milliseconds::zero(), false, true,
                                 nullptr, false);
}
                                       ...
                                       minor_version_t _minor)
{
    std::vector<availability_state_handler_t> its_handlers;
    {
        ...
        }
        {
            ...
            {
                auto its_sync_handler =
                    std::make_shared<sync_handler>([handler, _service, _instance, _state]() {
...
const std::deque<message_handler_t>&
application_impl::find_handlers(service_t _service, instance_t _instance, method_t _method) const
{
...
void application_impl::on_message(std::shared_ptr<message>&& _message)
{
    ...
    if (_message->get_message_type() == message_type_e::MT_NOTIFICATION)
    {
        ...
        {
            VSOMEIP_INFO << "application_impl::on_message [" << std::hex << std::setfill('0')
...

    {
        ...
        if (!its_handlers.empty())
        {
            ...
            for (const auto& handler : its_handlers)
            {
                auto its_sync_handler = std::make_shared<sync_handler>([handler, _message]() {
                    handler(_message);
                });
...
void application_impl::main_dispatch()
{
    ...
    {
        std::stringstream s;
        s << std::hex << std::setw(4) << std::setfill('0') << client_ << "_m_dispatch";
        pthread_setname_np(pthread_self(), s.str().c_str());
    }
#endif
...
void application_impl::dispatch()
{
    ...
    {
        std::stringstream s;
        s << std::hex << std::setw(4) << std::setfill('0') << client_ << "_dispatch";
        pthread_setname_np(pthread_self(), s.str().c_str());
    }
#endif
    const std::thread::id its_id = std::this_thread::get_id();
    ...
    while (!handlers_.empty() && !its_next_handler)
    {
        ...
        if (its_next_handler->handler_type_ == handler_type_e::AVAILABILITY)
        {
            const std::pair<service_t, instance_t> its_si_pair =
                std::make_pair(its_next_handler->service_id_, its_next_handler->instance_id_);
            auto found_si = availability_handlers_.find(its_si_pair);
            if (found_si != availability_handlers_.end() && !found_si->second.empty()
...
void application_impl::invoke_handler(std::shared_ptr<sync_handler>& _handler)
{
    ...
    auto its_sync_handler = std::make_shared<sync_handler>(
        _handler->service_id_, _handler->instance_id_, _handler->method_id_, _handler->session_id_,
        _handler->eventgroup_id_, _handler->handler_type_);
    ...
    its_dispatcher_timer.async_wait(
        [this, its_sync_handler](const boost::system::error_code& _error) {
            if (!_error)
            {
    ...
    if (client_side_logging_
        && (client_side_logging_filter_.empty()
            || (1
                == client_side_logging_filter_.count(
                    std::make_tuple(its_sync_handler->service_id_, ANY_INSTANCE)))
            || (1
                == client_side_logging_filter_.count(std::make_tuple(
                    its_sync_handler->service_id_, its_sync_handler->instance_id_)))))
    {
        VSOMEIP_INFO << "Invoking handler: (" << std::hex << std::setfill('0') << std::setw(4)
                     << client_ << "): [" << std::setw(4) << its_sync_handler->service_id_ << "."
                     << std::setw(4) << its_sync_handler->instance_id_ << "." << std::setw(4)
...
void application_impl::shutdown()
{
    ...
    {
        std::stringstream s;
        s << std::hex << std::setw(4) << std::setfill('0') << client_ << "_shutdown";
        pthread_setname_np(pthread_self(), s.str().c_str());
    }
#endif
...
void application_impl::send_back_cached_event(service_t _service, instance_t _instance,
                                              event_t _event)
{
    ...
    if (its_event && its_event->is_field() && its_event->is_set())
    {
        ...
        VSOMEIP_INFO << "Sending back cached event (" << std::hex << std::setfill('0')
                     << std::setw(4) << client_ << "): [" << std::setw(4) << _service << "."
                     << std::setw(4) << _instance << "." << std::setw(4) << _event << "]";
    }
}

void application_impl::send_back_cached_eventgroup(service_t _service, instance_t _instance,
                                                   eventgroup_t _eventgroup)
{
...
void application_impl::check_send_back_cached_event(service_t _service, instance_t _instance,
...
void application_impl::remove_subscription(service_t _service, instance_t _instance,
                                           eventgroup_t _eventgroup, event_t _event)
{
    {
        std::lock_guard<std::mutex> its_lock(subscriptions_state_mutex_);
...
bool application_impl::check_for_active_subscription(service_t _service, instance_t _instance,
                                                     event_t _event)
{
    std::lock_guard<std::mutex> its_lock(subscriptions_mutex_);
    auto                        found_service = subscriptions_.find(_service);
...
void application_impl::print_blocking_call(const std::shared_ptr<sync_handler>& _handler)
{
    ...
    case handler_type_e::AVAILABILITY:
        VSOMEIP_WARNING << "BLOCKING CALL AVAILABILITY(" << std::hex << std::setfill('0')
                        << std::setw(4) << get_client() << "): [" << std::setw(4)
                        << _handler->service_id_ << "." << std::setw(4) << _handler->instance_id_
                        << "]";
        break;
    case handler_type_e::MESSAGE:
        VSOMEIP_WARNING << "BLOCKING CALL MESSAGE(" << std::hex << std::setfill('0') << std::setw(4)
                        << get_client() << "): [" << std::setw(4) << _handler->service_id_ << "."
                        << std::setw(4) << _handler->instance_id_ << "." << std::setw(4)
                        << _handler->method_id_ << ":" << std::setw(4) << _handler->session_id_
                        << "]";
        break;
    case handler_type_e::STATE:
        VSOMEIP_WARNING << "BLOCKING CALL STATE(" << std::hex << std::setw(4) << std::setfill('0')
                        << get_client() << ")";
        break;
    case handler_type_e::SUBSCRIPTION:
        VSOMEIP_WARNING << "BLOCKING CALL SUBSCRIPTION(" << std::hex << std::setfill('0')
                        << std::setw(4) << get_client() << "): [" << std::setw(4)
                        << _handler->service_id_ << "." << std::setw(4) << _handler->instance_id_
                        << "." << std::setw(4) << _handler->eventgroup_id_ << ":" << std::setw(4)
                        << _handler->method_id_ << "]";
        break;
    case handler_type_e::OFFERED_SERVICES_INFO:
        VSOMEIP_WARNING << "BLOCKING CALL OFFERED_SERVICES_INFO(" << std::hex << std::setw(4)
    ...
    case handler_type_e::UNKNOWN:
        VSOMEIP_WARNING << "BLOCKING CALL UNKNOWN(" << std::hex << std::setw(4) << std::setfill('0')
                        << get_client() << ")";
        break;
    }
}
...
void application_impl::watchdog_cbk(boost::system::error_code const& _error)
{
    if (!_error)
    {
        watchdog_handler_t handler = nullptr;
        {
            ...
            if (handler && std::chrono::seconds::zero() != watchdog_interval_)
            {
                ...
                watchdog_timer_.async_wait(
                    std::bind(&application_impl::watchdog_cbk, this, std::placeholders::_1));
            }
        }
        ...
        {
            std::lock_guard<std::mutex> its_lock(handlers_mutex_);
            auto its_sync_handler           = std::make_shared<sync_handler>([handler]() {
                handler();
            });
    ...
    const async_subscription_handler_t& _handler)
{
    async_subscription_handler_ext_t its_handler_ext =
        [_handler](client_t _client, uid_t _uid, gid_t _gid, const std::string& _env,
                                                  ...
                                                  const std::string& _path, bool _enable)
{
    ...
    const boost::asio::ip::address its_address(
        _remote.ip_.is_v4_ ? static_cast<boost::asio::ip::address>(
    ...
    configuration_->set_sd_acceptance_rule(its_address, its_range, port_type_e::PT_UNKNOWN, _path,
...
bool application_impl::update_service_configuration(service_t _service, instance_t _instance,
                                                    ...
                                                    bool _magic_cookies_enabled, bool _offer)
{
    ...
    else
    {
        ...
        if (rm_impl)
        {
            if (_offer)
            {
                ret = rm_impl->offer_service_remotely(_service, _instance, _port, _reliable,
                                                      _magic_cookies_enabled);
            }
            else
            {
                ret = rm_impl->stop_offer_service_remotely(_service, _instance, _port, _reliable,
                                                           _magic_cookies_enabled);
            }
        }
    }
                                               ...
                                               event_t _event, const debounce_filter_t& _filter)
{
    if (routing_)
    {
        ...
        if (check_subscription_state(_service, _instance, _eventgroup, _event))
        {
            ...
            routing_->subscribe(client_, get_sec_client(), _service, _instance, _eventgroup, _major,
                                _event, its_filter);
        }
    }
}
...
void application_impl::register_message_handler_ext(service_t _service, instance_t _instance,
                                                    method_t                    _method,
