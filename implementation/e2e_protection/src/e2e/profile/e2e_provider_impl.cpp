...
value_t read_value_from_config(const std::shared_ptr<vsomeip_v3::cfg::e2e>& _config,
                               const std::string& _name, value_t _default_value = value_t())
{
...
namespace vsomeip_v3 { namespace e2e {
...
bool e2e_provider_impl::add_configuration(std::shared_ptr<cfg::e2e> config)
{
    ...
    {
        process_e2e_profile<profile01::profile_config, profile01::profile_01_checker,
                            profile01::protector>(config);
        return true;
    }

    if (config->profile == "CRC32" || config->profile == "CSTM")
    {
        process_e2e_profile<profile_custom::profile_config, profile_custom::profile_custom_checker,
    ...
    {
        process_e2e_profile<profile04::profile_config, profile04::profile_04_checker,
                            profile04::protector>(config);
        return true;
    }

    if (config->profile == "P05")
    {
        process_e2e_profile<profile05::profile_config, profile05::profile_05_checker,
                            profile05::protector>(config);
        return true;
    }

    if (config->profile == "P07")
    {
        process_e2e_profile<profile07::profile_config, profile07::profile_07_checker,
                            profile07::protector>(config);
        return true;
    }
...
e2e_provider_impl::make_e2e_profile_config(const std::shared_ptr<cfg::e2e>& _config)
{
    ...
    uint16_t data_id_nibble_offset =
        read_value_from_config<uint16_t>(_config, "data_id_nibble_offset", 12);

    e2e::profile01::p01_data_id_mode data_id_mode = static_cast<e2e::profile01::p01_data_id_mode>(
        read_value_from_config<uint16_t>(_config, "data_id_mode"));

    return e2e::profile01::profile_config(crc_offset, data_id, data_id_mode, data_length,
                                          counter_offset, data_id_nibble_offset);
}
...
e2e_provider_impl::make_e2e_profile_config(const std::shared_ptr<cfg::e2e>& _config)
{
    ...
    size_t max_data_length =
        read_value_from_config<size_t>(_config, "max_data_length", size_t(0xffff));

    uint16_t max_delta_counter =
        read_value_from_config<uint16_t>(_config, "max_delta_counter", uint16_t(0xffff));

    return e2e::profile04::profile_config(data_id, offset, min_data_length, max_data_length,
                                          max_delta_counter);
}
...
e2e_provider_impl::make_e2e_profile_config(const std::shared_ptr<cfg::e2e>& _config)
{
    ...
    size_t max_data_length =
        read_value_from_config<size_t>(_config, "max_data_length", size_t(0xffffffff));

    uint32_t max_delta_counter =
        read_value_from_config<uint32_t>(_config, "max_delta_counter", uint32_t(0xffffffff));

    return e2e::profile07::profile_config(data_id, offset, min_data_length, max_data_length,
                                          max_delta_counter);
}

}} // namespace vsomeip_v3::e2e
