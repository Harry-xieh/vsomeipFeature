...
uint8_t profile_01::compute_crc(const profile_config& _config, const e2e_buffer& _buffer)
{
    ...
    case p01_data_id_mode::E2E_P01_DATAID_BOTH: // CRC over 2 bytes
        ...
        computed_crc = e2e_crc::calculate_profile_01(
            buffer_view(data_id_buffer, 0, 1), computed_crc); // CRC over high byte of Data ID (MSB)
    ...
    if (_config.crc_offset_ >= 1)
    {
        ...
        computed_crc = e2e_crc::calculate_profile_01(buffer_view(_buffer, 0, _config.crc_offset_),
                                                     computed_crc);
    }
    ...
    if ((_config.crc_offset_) < (_config.data_length_ / 8) - 1)
    {
        ...
        computed_crc = e2e_crc::calculate_profile_01(
            buffer_view(_buffer, static_cast<size_t>(_config.crc_offset_ + 1), _buffer.size()),
            computed_crc);
    }
