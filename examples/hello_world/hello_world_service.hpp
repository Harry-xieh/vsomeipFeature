...
#define LOG_INF(...)                                                                               \
    std::fprintf(stdout, __VA_ARGS__), std::fprintf(stdout, "\n"),                                 \
...
public:
    ...
    hello_world_service()
        : rtm_(vsomeip::runtime::get()), app_(rtm_->create_application()), stop_(false)
    {
    ...
    void on_message_cbk(const std::shared_ptr<vsomeip::message>& _request)
    {
        ...
        str.append(reinterpret_cast<const char*>(_request->get_payload()->get_data()), 0,
