...
public:
    ...
    bool init()
    {
        ...
        std::cout << "Client settings [protocol=" << (use_tcp_ ? "TCP" : "UDP")
                  << ":quiet=" << (be_quiet_ ? "true" : "false") << ":cycle=" << cycle_ << "]"
                  << std::endl;
    ...
    {
        std::cout << "Received a response from Service [" << std::setfill('0') << std::hex
        ...
        {
            {
                ...
                if (is_available_)
                {
                    ...
                    std::cout << "Client/Session [" << std::setfill('0') << std::hex << std::setw(4)
                              << request_->get_client() << "/" << std::setw(4)
                              << request_->get_session() << "] sent a request to Service ["
                              << std::setw(4) << request_->get_service() << "." << std::setw(4)
