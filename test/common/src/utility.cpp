...
void utility::load_policy_data(std::string                                     _input,
...
void utility::read_data(const std::set<std::string>&                    _input,
                        std::vector<vsomeip_v3::configuration_element>& _elements,
                        std::set<std::string>&                          _failed)
{
    for (auto i : _input)
...
std::set<std::string> utility::get_all_files_in_dir(const std::string&              _dir_path,
        ...
        if (boost::filesystem::exists(_dir_path) && boost::filesystem::is_directory(_dir_path))
        {
            ...
            while (iter != end)
            {
                ...
                if (boost::filesystem::is_directory(iter->path())
                    && (std::find(_dir_skip_list.begin(), _dir_skip_list.end(),
                                  iter->path().filename())
                        != _dir_skip_list.end()))
                {
...
{
    return boost::filesystem::canonical(boost::filesystem::current_path()).string()
...
void utility::force_check_credentials(
    std::vector<vsomeip_v3::configuration_element>& _policy_elements, std::string _value)
{
    ...
    try
    {
        ...
        for (auto policy_node : policy_tree)
        {
            ...
            if (allow_requests)
            {
                ...
                {
                    auto optional_service =
                        request_node.second.get_child("service").get_value_optional<std::string>();
                    if (optional_service)
                    {
