...
server::server()
    : applet{"server"}, counter_event_sent{}, counter_method_request{}, counter_method_response{}
{}
...
void server::init()
{
            ...
            if (me)
            {
                ...
                case vsomeip_v3::message_type_e::MT_REQUEST:
                    ...
                    me->counter_method_request++;
                    {
                        ...
                        me->application->notify(config::SERVICE_ID, config::INSTANCE_ID,
                                                config::EVENT_ID, payload, true);
    ...
    this->application->offer_event(config::SERVICE_ID, config::INSTANCE_ID, config::EVENT_ID,
                                   {config::EVENTGROUP_ID}, vsomeip_v3::event_type_e::ET_FIELD, {},
                                   false, true, nullptr,
    ...
    std::thread{[its_me] {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            me->application->notify(config::SERVICE_ID, config::INSTANCE_ID, config::EVENT_ID,
                                    payload, true);
            me->counter_event_sent++;
