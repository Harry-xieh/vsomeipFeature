...
big_payload_test_client::big_payload_test_client(bool                        _use_tcp,
                                                 big_payload_test::test_mode _test_mode)
...
bool big_payload_test_client::init()
{
    ...
    app_->register_state_handler(
        std::bind(&big_payload_test_client::on_state, this, std::placeholders::_1));
...
void big_payload_test_client::on_message(const std::shared_ptr<vsomeip::message>& _response)
{
    ...
    {
        ASSERT_LT(_response->get_payload()->get_length(),
                  big_payload_test::BIG_PAYLOAD_SIZE_RANDOM);
    }
    else if (test_mode_ == big_payload_test::test_mode::UDP)
    {
...
void big_payload_test_client::run()
{
    ...
    for (unsigned int i = 0; i < number_of_messages_to_send_; i++)
    {
        if (test_mode_ == big_payload_test::test_mode::RANDOM)
        {
            unsigned int datasize(static_cast<unsigned int>(std::rand())
                                  % big_payload_test::BIG_PAYLOAD_SIZE_RANDOM);
