...
bool big_payload_test_service::init()
{
    ...
    app_->register_message_handler(
        vsomeip::ANY_SERVICE, big_payload_test::TEST_SERVICE_INSTANCE_ID,
        big_payload_test::TEST_SERVICE_METHOD_ID,
        std::bind(&big_payload_test_service::on_message, this, std::placeholders::_1));

    app_->register_state_handler(
        std::bind(&big_payload_test_service::on_state, this, std::placeholders::_1));
    return true;
}
...
void big_payload_test_service::on_message(const std::shared_ptr<vsomeip::message>& _request)
{
    VSOMEIP_INFO << "Received a message with Client/Session [" << std::setw(4) << std::setfill('0')
                 ...
                 << std::hex << _request->get_session() << "] size: " << std::dec
...
{
    {
        ...
        while (!blocked_ || !incoming_requests_.empty())
        {
            ...
            {
                EXPECT_LT(_request->get_payload()->get_length(),
                          big_payload_test::BIG_PAYLOAD_SIZE_RANDOM);
            }
            else if (test_mode_ == big_payload_test::test_mode::UDP)
            {
                EXPECT_EQ(big_payload_test::BIG_PAYLOAD_SIZE_UDP,
                          _request->get_payload()->get_length());
            }
            else
            {
                EXPECT_EQ(big_payload_test::BIG_PAYLOAD_SIZE,
                          _request->get_payload()->get_length());
            }
            bool              check(true);
            ...
            if (test_mode_ == big_payload_test::test_mode::RANDOM)
            {
                its_payload_data.assign(static_cast<unsigned int>(std::rand())
                                            % big_payload_test::BIG_PAYLOAD_SIZE_RANDOM,
