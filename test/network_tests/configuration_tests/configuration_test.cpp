...
::testing::AssertionResult check(const T& _is, const T& _expected, const std::string& _test)
{
    ...
    else
    {
        return ::testing::AssertionFailure()
               << "Test \"" << _test << "\" failed! (" << _is << " != " << _expected << ")";
    }
}
...
void check_file(
    const std::string& _config_file, const std::string& _expected_unicast_address,
    bool _expected_has_console, bool _expected_has_file, bool _expected_has_dlt,
    bool _expected_version_logging_enabled, uint32_t _expected_version_logging_interval,
    ...
    uint16_t _expected_reliable_port_4466_0321, uint16_t _expected_unreliable_port_4466_0321,
    bool _expected_enabled, const std::string& _expected_protocol,
    const std::string& _expected_multicast, uint16_t _expected_port,
    uint32_t _expected_initial_delay_min, uint32_t _expected_initial_delay_max,
    int32_t _expected_repetitions_base_delay, uint8_t _expected_repetitions_max,
    vsomeip::ttl_t _expected_ttl, vsomeip::ttl_t _expected_cyclic_offer_delay,
    vsomeip::ttl_t _expected_request_response_delay)
{
    ...
    if (its_plugin)
    {
        ...
        if (its_configuration_plugin)
            its_configuration =
                its_configuration_plugin->get_configuration(EXPECTED_ROUTING_MANAGER_HOST, "");
    }
    ...
    EXPECT_TRUE(
        its_configuration->check_suppress_events(0x0023, 0x0001, 0x8002)); // Multiple values
    ...
    EXPECT_TRUE(
        its_configuration->check_suppress_events(0x1111, 0x00f2, 0x8001)); // "ANY" Service/Event
    EXPECT_TRUE(its_configuration->check_suppress_events(0x0102, 0x0010, 0x8005)); // "ANY" Instance
    EXPECT_FALSE(
        its_configuration->check_suppress_events(0x0102, 0x0010, 0x8007)); // "ANY" Instance

    EXPECT_TRUE(its_configuration->check_suppress_events(0x0024, 0x5555, 0x8011)); // "ANY" INSTANCE
    EXPECT_FALSE(
        its_configuration->check_suppress_events(0x0024, 0x5555, 0x8016)); // "ANY" INSTANCE
    ...
    EXPECT_TRUE(check<std::string>(its_host_unicast_address.to_string(), _expected_unicast_address,
                                   "UNICAST ADDRESS"));
    ...
    EXPECT_TRUE(
        check<bool>(has_version_logging, _expected_version_logging_enabled, "VERSION LOGGING"));
    EXPECT_TRUE(check<uint32_t>(version_logging_interval, _expected_version_logging_interval,
    ...
    for (const auto& f : its_trace->filters_)
    {
        ...
        EXPECT_TRUE(its_channel_name == std::string("testname")
                    || its_channel_name == std::string("testname2")
                    || its_channel_name == std::string("testname3")
                    || its_channel_name == std::string("testname4"));
    ...
    // Applications
    std::size_t max_dispatchers =
        its_configuration->get_max_dispatchers(EXPECTED_ROUTING_MANAGER_HOST);
    std::size_t max_dispatch_time =
        its_configuration->get_max_dispatch_time(EXPECTED_ROUTING_MANAGER_HOST);
    std::size_t max_detached_thread_wait_time =
        its_configuration->get_max_detached_thread_wait_time(EXPECTED_ROUTING_MANAGER_HOST);
    std::size_t io_threads = its_configuration->get_io_thread_count(EXPECTED_ROUTING_MANAGER_HOST);
    std::size_t request_time =
        its_configuration->get_request_debouncing(EXPECTED_ROUTING_MANAGER_HOST);

    EXPECT_TRUE(check<std::size_t>(max_dispatchers, _expected_application_max_dispatcher,
                                   "MAX DISPATCHERS"));
    EXPECT_TRUE(check<std::size_t>(max_dispatch_time, _expected_application_max_dispatch_time,
                                   "MAX DISPATCH TIME"));
    EXPECT_TRUE(check<std::size_t>(max_detached_thread_wait_time,
                                   _expected_application_max_detached_thread_wait_time,
                                   "MAX DETACHED THREADS WAIT TIME"));
    EXPECT_TRUE(check<std::size_t>(io_threads, _expected_application_threads, "IO THREADS"));
    EXPECT_TRUE(check<std::size_t>(request_time, _expected_application_request_debounce_time,
    ...
    EXPECT_TRUE(check<std::string>(its_unicast_address, _expected_unicast_address_1234_0022,
                                   "UNICAST_ADDRESS_1234_0022"));
    EXPECT_TRUE(check<uint16_t>(its_reliable_port, _expected_reliable_port_1234_0022,
                                "RELIABLE_PORT_1234_0022"));
    EXPECT_TRUE(check<uint16_t>(its_unreliable_port, _expected_unreliable_port_1234_0022,
    ...
    EXPECT_TRUE(check<std::string>(its_unicast_address, _expected_unicast_address_1234_0023,
                                   "UNICAST_ADDRESS_1234_0023"));
    EXPECT_TRUE(check<uint16_t>(its_reliable_port, _expected_reliable_port_1234_0023,
                                "RELIABLE_PORT_1234_0023"));
    EXPECT_TRUE(check<uint16_t>(its_unreliable_port, _expected_unreliable_port_1234_0023,
    ...
    EXPECT_TRUE(check<std::string>(its_unicast_address, _expected_unicast_address_2277_0022,
                                   "UNICAST_ADDRESS_2277_0022"));
    EXPECT_TRUE(check<uint16_t>(its_reliable_port, _expected_reliable_port_2277_0022,
                                "RELIABLE_PORT_2277_0022"));
    EXPECT_TRUE(check<uint16_t>(its_unreliable_port, _expected_unreliable_port_2277_0022,
    ...
    EXPECT_TRUE(check<std::string>(its_unicast_address, _expected_unicast_address_2266_0022,
                                   "UNICAST_ADDRESS_2266_0022"));
    EXPECT_TRUE(check<uint16_t>(its_reliable_port, _expected_reliable_port_2266_0022,
                                "RELIABLE_PORT_2266_0022"));
    EXPECT_TRUE(check<uint16_t>(its_unreliable_port, _expected_unreliable_port_2266_0022,
    ...
    EXPECT_TRUE(check<std::string>(its_unicast_address, _expected_unicast_address_4466_0321,
                                   "UNICAST_ADDRESS_4466_0321"));
    EXPECT_TRUE(check<uint16_t>(its_reliable_port, _expected_reliable_port_4466_0321,
                                "RELIABLE_PORT_4466_0321"));
    EXPECT_TRUE(check<uint16_t>(its_unreliable_port, _expected_unreliable_port_4466_0321,
                                "UNRELIABLE_PORT_4466_0321"));

    std::string   its_multicast_address;
    std::uint16_t its_multicast_port;
    its_configuration->get_multicast(0x7809, 0x1, 0x1111, its_multicast_address,
                                     its_multicast_port);
    ...
    EXPECT_TRUE(its_configuration->get_client_port(0x8888, 0x1, vsomeip::ILLEGAL_PORT, true,
                                                   used_ports, port_to_use));
    EXPECT_EQ(0x10, port_to_use);
    EXPECT_TRUE(its_configuration->get_client_port(0x8888, 0x1, vsomeip::ILLEGAL_PORT, false,
                                                   used_ports, port_to_use));
    EXPECT_EQ(0x11, port_to_use);

    used_ports[true].insert(0x10);
    used_ports[false].insert(0x11);
    EXPECT_FALSE(its_configuration->get_client_port(0x8888, 0x1, vsomeip::ILLEGAL_PORT, true,
                                                    used_ports, port_to_use));
    EXPECT_EQ(vsomeip::ILLEGAL_PORT, port_to_use);
    EXPECT_FALSE(its_configuration->get_client_port(0x8888, 0x1, vsomeip::ILLEGAL_PORT, false,
                                                    used_ports, port_to_use));
    EXPECT_EQ(vsomeip::ILLEGAL_PORT, port_to_use);
    ...
    used_ports.clear();
    EXPECT_TRUE(
        its_configuration->get_client_port(0x8888, 0x12, 0x7725, true, used_ports, port_to_use));
    ...
    EXPECT_TRUE(
        its_configuration->get_client_port(0x8888, 0x12, 0x7B0D, true, used_ports, port_to_use));
    ...
    EXPECT_TRUE(
        its_configuration->get_client_port(0x8888, 0x12, 0x7EF4, true, used_ports, port_to_use));
    ...
    std::uint32_t max_local_message_size(17000u + 16u
                                         + vsomeip::protocol::SEND_COMMAND_HEADER_SIZE);
    ...
    _policy->requests_ += std::make_pair(
        boost::icl::discrete_interval<vsomeip::service_t>(its_second_service, its_second_service,
    ...
    _policy->requests_ += std::make_pair(
        boost::icl::discrete_interval<vsomeip::service_t>(its_third_service, its_third_service,
                                                          boost::icl::interval_bounds::closed()),
        its_instances_methods);
    ...
    EXPECT_TRUE(
        check<uint32_t>(initial_delay_min, _expected_initial_delay_min, "SD INITIAL DELAY MIN"));
    EXPECT_TRUE(
        check<uint32_t>(initial_delay_max, _expected_initial_delay_max, "SD INITIAL DELAY MAX"));
    EXPECT_TRUE(check<int32_t>(repetitions_base_delay, _expected_repetitions_base_delay,
                               "SD REPETITION BASE DELAY"));
    ...
    EXPECT_TRUE(check<int32_t>(cyclic_offer_delay,
                               static_cast<int32_t>(_expected_cyclic_offer_delay),
                               "SD CYCLIC OFFER DELAY"));
    EXPECT_TRUE(check<int32_t>(request_response_delay,
                               static_cast<int32_t>(_expected_request_response_delay),
                               "SD RESPONSE REQUEST DELAY"));
    EXPECT_EQ(1000u, its_configuration->get_sd_offer_debounce_time());

    ASSERT_TRUE(vsomeip::plugin_manager::get()->unload_plugin(
...
TEST(configuration_test, check_config_file)
{
    ...
    check_file(
        CONFIGURATION_FILE, EXPECTED_UNICAST_ADDRESS, EXPECTED_HAS_CONSOLE, EXPECTED_HAS_FILE,
        EXPECTED_HAS_DLT, EXPECTED_VERSION_LOGGING_ENABLED, EXPECTED_VERSION_LOGGING_INTERVAL,
        EXPECTED_APPLICATION_MAX_DISPATCHERS, EXPECTED_APPLICATION_MAX_DISPATCH_TIME,
        EXPECTED_APPLICATION_MAX_DETACHED_THREAD_WAIT_TIME, EXPECTED_APPLICATION_THREADS,
        EXPECTED_APPLICATION_REQUEST_DEBOUNCE_TIME, EXPECTED_LOGFILE, EXPECTED_LOGLEVEL,
        EXPECTED_UNICAST_ADDRESS_1234_0022, EXPECTED_RELIABLE_PORT_1234_0022,
        EXPECTED_UNRELIABLE_PORT_1234_0022, EXPECTED_UNICAST_ADDRESS_1234_0023,
        EXPECTED_RELIABLE_PORT_1234_0023, EXPECTED_UNRELIABLE_PORT_1234_0023,
        EXPECTED_UNICAST_ADDRESS_2277_0022, EXPECTED_RELIABLE_PORT_2277_0022,
        EXPECTED_UNRELIABLE_PORT_2277_0022, EXPECTED_UNICAST_ADDRESS_2266_0022,
        EXPECTED_RELIABLE_PORT_2266_0022, EXPECTED_UNRELIABLE_PORT_2266_0022,
        EXPECTED_UNICAST_ADDRESS_4466_0321, EXPECTED_RELIABLE_PORT_4466_0321,
        EXPECTED_UNRELIABLE_PORT_4466_0321, EXPECTED_SD_ENABLED, EXPECTED_SD_PROTOCOL,
        EXPECTED_SD_MULTICAST, EXPECTED_SD_PORT, EXPECTED_INITIAL_DELAY_MIN,
        EXPECTED_INITIAL_DELAY_MAX, EXPECTED_REPETITIONS_BASE_DELAY, EXPECTED_REPETITIONS_MAX,
...
TEST(configuration_test, check_deprecated_config_file)
{
    ...
    check_file(DEPRECATED_CONFIGURATION_FILE, EXPECTED_UNICAST_ADDRESS, EXPECTED_HAS_CONSOLE,
               EXPECTED_HAS_FILE, EXPECTED_HAS_DLT, EXPECTED_VERSION_LOGGING_ENABLED,
               EXPECTED_VERSION_LOGGING_INTERVAL, EXPECTED_APPLICATION_MAX_DISPATCHERS,
               ...
               EXPECTED_UNICAST_ADDRESS_4466_0321, EXPECTED_RELIABLE_PORT_4466_0321,
               EXPECTED_UNRELIABLE_PORT_4466_0321, EXPECTED_SD_ENABLED, EXPECTED_SD_PROTOCOL,
               EXPECTED_SD_MULTICAST, EXPECTED_SD_PORT, EXPECTED_DEPRECATED_INITIAL_DELAY_MIN,
               EXPECTED_DEPRECATED_INITIAL_DELAY_MAX, EXPECTED_DEPRECATED_REPETITIONS_BASE_DELAY,
               EXPECTED_DEPRECATED_REPETITIONS_MAX, EXPECTED_DEPRECATED_TTL,
               EXPECTED_CYCLIC_OFFER_DELAY, EXPECTED_DEPRECATED_REQUEST_RESPONSE_DELAY);
}
