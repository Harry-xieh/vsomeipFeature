...
bool e2e_test_client::init()
{
    ...
    app_->register_state_handler(
        std::bind(&e2e_test_client::on_state, this, std::placeholders::_1));
    ...
    app_->register_availability_handler(
        ...
        std::bind(&e2e_test_client::on_availability, this, std::placeholders::_1,
...
void e2e_test_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance,
                                      bool _is_available)
{
    VSOMEIP_INFO << std::hex << "Client 0x" << app_->get_client() << " : Service [" << std::setw(4)
                 << std::setfill('0') << std::hex << _service << "." << _instance << "] is "
        ...
        && vsomeip_test::TEST_SERVICE_INSTANCE_ID == _instance)
    {
        ...
        else if (_is_available && !is_available_)
        {
            ...
            app_->subscribe(vsomeip_test::TEST_SERVICE_SERVICE_ID,
                            vsomeip_test::TEST_SERVICE_INSTANCE_ID, 0x01);
            app_->subscribe(vsomeip_test::TEST_SERVICE_SERVICE_ID,
                            vsomeip_test::TEST_SERVICE_INSTANCE_ID, 0x02);
            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
...
{
    VSOMEIP_INFO << "Received a response from Service [" << std::setw(4) << std::setfill('0')
                 << std::hex << _response->get_service() << "." << std::setw(4) << std::setfill('0')
                 << std::hex << _response->get_instance() << "] to Client/Session [" << std::setw(4)
                 << std::setfill('0') << std::hex << _response->get_client() << "/" << std::setw(4)
        ...
        && vsomeip_test::TEST_SERVICE_METHOD_ID == _response->get_method())
    {
        ...
        VSOMEIP_INFO << "Method ID 0x8421 -> IS_VALID_CRC 8 = " << std::hex
                     << _response->is_valid_crc();
             ...
             && 0x8001 == _response->get_method())
    {
        ...
        VSOMEIP_INFO << "Event ID 0x8001 -> IS_VALID_CRC 8 = " << std::hex
                     << _response->is_valid_crc();
        EXPECT_EQ(true, _response->is_valid_crc());
        ...
        {
            EXPECT_EQ(dataptr[i],
                      event_payloads_profile_01_[received_responses_counters_[0x8001]
                                                 % vsomeip_test::NUMBER_OF_MESSAGES_TO_SEND][i]);
        }
    ...
    {
        // check for calculated CRC status OK for the predefined fixed payload sent by service
        VSOMEIP_INFO << "Method ID 0x6543 -> IS_VALID_CRC 32 = " << std::hex
                     << _response->is_valid_crc();
        EXPECT_EQ(true, _response->is_valid_crc());
        ...
        {
            EXPECT_EQ(dataptr[i],
                      payloads_custom_profile_[received_responses_counters_[0x6543]
                                               % vsomeip_test::NUMBER_OF_MESSAGES_TO_SEND][i]);
        }
    ...
    {
        VSOMEIP_INFO << "Event ID 0x8002 -> IS_VALID_CRC 32 = " << std::hex
                     << _response->is_valid_crc();
        EXPECT_EQ(true, _response->is_valid_crc());
        ...
        for (uint32_t i = 0; i < pl->get_length(); i++)
        {
            EXPECT_EQ(
                dataptr[i],
                event_payloads_custom_profile_[received_responses_counters_[0x8002]
                                               % vsomeip_test::NUMBER_OF_MESSAGES_TO_SEND][i]);
        }
...
int main(int argc, char** argv)
{
    ...
    "data_id" : "0xA73"
     */
    payloads_profile_01_.push_back(
        {{0x82, 0xa4, 0xe3, 0xff, 0xff, 0xff, 0xff, 0xff}}); // initial event
