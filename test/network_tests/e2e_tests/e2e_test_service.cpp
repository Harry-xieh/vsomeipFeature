...
bool e2e_test_service::init()
{
    ...
    app_->register_message_handler(
        vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID, 0x6543,
        std::bind(&e2e_test_service::on_message, this, std::placeholders::_1));
    ...
    app_->register_state_handler(
        std::bind(&e2e_test_service::on_state, this, std::placeholders::_1));
...
void e2e_test_service::on_message(const std::shared_ptr<vsomeip::message>& _request)
{
    ...
    VSOMEIP_INFO << "Received a message with Client/Session [" << std::setw(4) << std::setfill('0')
                 ...
                 << std::hex << _request->get_session() << "] method: " << _request->get_method();
    ...
    if (_request->get_method() == vsomeip_test::TEST_SERVICE_METHOD_ID)
    {
        its_vsomeip_payload->set_data(
            payloads_profile_01_[received_requests_counters_[vsomeip_test::TEST_SERVICE_METHOD_ID]
        ...
        vsomeip::byte_t its_data[8] = {
            0x00, 0x00, (uint8_t)received_requests_counters_[vsomeip_test::TEST_SERVICE_METHOD_ID],
            0xff, 0xff, 0xff,
            0xff, 0xff};
    ...
    else if (_request->get_method() == 0x6543)
    {
        ...
        its_vsomeip_payload->set_data(
            payloads_custom_profile_[received_requests_counters_[0x6543]
                                     % vsomeip_test::NUMBER_OF_MESSAGES_TO_SEND]);
        its_response->set_payload(its_vsomeip_payload);
        app_->send(its_response);
        ...
        vsomeip::byte_t its_data[8] = {
            0x00, 0x00, 0x00, 0x00, 0xff, 0xff, (uint8_t)received_requests_counters_[0x6543], 0x32};
        its_event_payload->set_data(its_data, 8);
        app_->notify(vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
