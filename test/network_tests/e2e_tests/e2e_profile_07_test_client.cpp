...
bool e2e_profile_07_test_client::init()
{
    ...
    app_->register_state_handler(
        std::bind(&e2e_profile_07_test_client::on_state, this, std::placeholders::_1));
    ...
    app_->register_availability_handler(PROFILE_07_SERVICE, PROFILE_07_INSTANCE,
                                        std::bind(&e2e_profile_07_test_client::on_availability,
...
void e2e_profile_07_test_client::on_state(vsomeip::state_type_e _state)
{
    if (_state == vsomeip::state_type_e::ST_REGISTERED)
    {
        ...
        app_->request_event(PROFILE_07_SERVICE, PROFILE_07_INSTANCE, PROFILE_07_EVENT,
                            {PROFILE_07_EVENTGROUP}, vsomeip::event_type_e::ET_FIELD,
                            vsomeip::reliability_type_e::RT_UNRELIABLE);
    }
}

void e2e_profile_07_test_client::on_availability(vsomeip::service_t  _service,
                                                 vsomeip::instance_t _instance, bool _is_available)
{
    VSOMEIP_INFO << __func__ << ": Client " << std::hex << std::setw(4) << std::setfill('0')
                 << app_->get_client() << " : Service [" << _service << "." << _instance << "] is "
    ...
    if (PROFILE_07_SERVICE == _service && PROFILE_07_INSTANCE == _instance)
    {
        ...
        else if (_is_available && !is_available_)
        {
            ...
            app_->subscribe(PROFILE_07_SERVICE, PROFILE_07_INSTANCE, PROFILE_07_EVENTGROUP,
                            PROFILE_07_MAJOR);
            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
...
{
    VSOMEIP_INFO << __func__ << ": Received a message from Service [" << std::setw(4)
                 << std::setfill('0') << std::hex << _message->get_service() << "."
                 << _message->get_instance() << "] to Client/Session [" << _message->get_client()
        ...
        && PROFILE_07_METHOD == _message->get_method())
    {
        ...
        VSOMEIP_INFO << "Method ID 0x0001 -> IS_VALID_CRC = " << std::boolalpha
                     << _message->is_valid_crc();
        ...
        for (size_t i = 0; i < its_payload->get_length(); i++)
            EXPECT_EQ(its_data[i],
                      responses_[counters_[PROFILE_07_METHOD] % PROFILE_07_NUM_MESSAGES][i]);
             ...
             && PROFILE_07_EVENT == _message->get_method())
    {
        ...
        VSOMEIP_INFO << __func__ << ": Event 0x8001 -> IS_VALID_CRC = " << std::boolalpha
                     << _message->is_valid_crc();
        EXPECT_EQ(true, _message->is_valid_crc());
        ...
        for (size_t i = 0; i < its_payload->get_length(); i++)
            EXPECT_EQ(its_data[i],
                      events_[counters_[PROFILE_07_EVENT] % PROFILE_07_NUM_MESSAGES][i]);
    ...
    {
        VSOMEIP_WARNING << __func__ << ": Client" << std::setw(4) << std::setfill('0') << std::hex
                        << app_->get_client() << " received all messages ~> going down!";
    }
}
...
int main(int argc, char** argv)
{
    ...
    responses_ = {
        {0xf7, 0xa1, 0x9e, 0x0e, 0x9d, 0xbd, 0xa7, 0xca, 0x00, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00,
         0x00, 0x00, 0x00, 0x00, 0x2d, 0x00, 0x01, 0x06, 0xfe, 0x01, 0x3e, 0x4c, 0xcc, 0xcd, 0x80,
         0x3f, 0xb2, 0x3d, 0x83, 0x3e, 0xba, 0x68, 0xed, 0x3f, 0xb3, 0x7a, 0xf2, 0xbd, 0x96, 0xc1,
         0x42, 0x3d, 0x25, 0x1a, 0x62, 0xbd, 0xae, 0x77, 0xf3, 0x3f, 0x80, 0x00, 0x00, 0xfc, 0x01,
         0x01, 0x3c, 0x1d, 0xbd, 0x4e, 0x01, 0x01, 0x3c, 0x2b, 0x87, 0xed, 0x00},
        {0xca, 0xb7, 0xae, 0x78, 0xe2, 0xa5, 0xdc, 0xab, 0x00, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00,
         0x01, 0x00, 0x00, 0x00, 0x2d, 0x00, 0x01, 0x06, 0xfe, 0x01, 0x3e, 0x4c, 0xcc, 0xcd, 0x80,
         0x3f, 0xb2, 0x3c, 0x2f, 0x3e, 0xba, 0x46, 0x81, 0x3f, 0xb3, 0x73, 0x8d, 0xbd, 0x93, 0xcb,
         0xae, 0x3c, 0xf7, 0xd2, 0x58, 0xbd, 0xa2, 0x6e, 0xcd, 0x3f, 0x80, 0x00, 0x00, 0xfc, 0x01,
         0x01, 0x3c, 0x1c, 0x89, 0x24, 0x01, 0x01, 0x3c, 0x2b, 0x24, 0x45, 0x00},
        {0xbe, 0x5c, 0xe8, 0xd0, 0xa8, 0x3a, 0xca, 0x41, 0x00, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00,
         0x02, 0x00, 0x00, 0x00, 0x2d, 0x00, 0x01, 0x06, 0xfe, 0x01, 0x3e, 0x4c, 0xcc, 0xcd, 0x80,
         0x3f, 0xb2, 0x3e, 0xf3, 0x3e, 0xba, 0x97, 0x45, 0x3f, 0xb3, 0x86, 0x81, 0xbd, 0x8a, 0xda,
         0xc2, 0x3c, 0xf6, 0x00, 0x7a, 0xbd, 0xb4, 0xf9, 0xb9, 0x3f, 0x80, 0x00, 0x00, 0xfc, 0x01,
         0x01, 0x3c, 0x1c, 0x1b, 0x72, 0x01, 0x01, 0x3c, 0x2a, 0x9e, 0x1f, 0x00}};

    events_ = {
        {0x23, 0xfc, 0x1f, 0x7f, 0x13, 0x58, 0x95, 0xc0, 0x00, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00,
         0x00, 0x00, 0x00, 0x00, 0x2d, 0x00, 0x01, 0x06, 0xfe, 0x01, 0x3e, 0x4c, 0xcc, 0xcd, 0x80,
         0x3f, 0xb2, 0x3d, 0x83, 0x3e, 0xba, 0x68, 0xed, 0x3f, 0xb3, 0x7a, 0xf2, 0xbd, 0x96, 0xc1,
         0x42, 0x3d, 0x25, 0x1a, 0x62, 0xbd, 0xae, 0x77, 0xf3, 0x3f, 0x80, 0x00, 0x00, 0xfc, 0x01,
         0x01, 0x3c, 0x1d, 0xbd, 0x4e, 0x01, 0x01, 0x3c, 0x2b, 0x87, 0xed, 0x00},
        {0xd0, 0x18, 0xa8, 0x8d, 0x18, 0xcf, 0x43, 0x6e, 0x00, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00,
         0x01, 0x00, 0x00, 0x00, 0x2d, 0x00, 0x01, 0x06, 0xfe, 0x01, 0x3e, 0x4c, 0xcc, 0xcd, 0x80,
         0x3f, 0xb2, 0x3c, 0x2f, 0x3e, 0xba, 0x46, 0x81, 0x3f, 0xb3, 0x73, 0x8d, 0xbd, 0x93, 0xcb,
         0xae, 0x3c, 0xf7, 0xd2, 0x58, 0xbd, 0xa2, 0x6e, 0xcd, 0x3f, 0x80, 0x00, 0x00, 0xfc, 0x01,
         0x01, 0x3c, 0x1c, 0x89, 0x24, 0x01, 0x01, 0x3c, 0x2b, 0x24, 0x45, 0x00},
        {0xe1, 0x5d, 0x93, 0x59, 0x7e, 0x2a, 0xce, 0xc1, 0x00, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00,
         0x02, 0x00, 0x00, 0x00, 0x2d, 0x00, 0x01, 0x06, 0xfe, 0x01, 0x3e, 0x4c, 0xcc, 0xcd, 0x80,
         0x3f, 0xb2, 0x3e, 0xf3, 0x3e, 0xba, 0x97, 0x45, 0x3f, 0xb3, 0x86, 0x81, 0xbd, 0x8a, 0xda,
         0xc2, 0x3c, 0xf6, 0x00, 0x7a, 0xbd, 0xb4, 0xf9, 0xb9, 0x3f, 0x80, 0x00, 0x00, 0xfc, 0x01,
         0x01, 0x3c, 0x1c, 0x1b, 0x72, 0x01, 0x01, 0x3c, 0x2a, 0x9e, 0x1f, 0x00}};
