...
public:
          ...
          sender_(std::bind(&cpu_load_test_client::run, this))
    {
        ...
        app_->register_state_handler(
            std::bind(&cpu_load_test_client::on_state, this, std::placeholders::_1));
...
private:
    ...
    void run()
    {
        ...
        VSOMEIP_INFO << "Sent: " << number_of_sent_messages_total_
                     ...
                     << std::fixed << std::setprecision(2) << average_load
        ...
        VSOMEIP_INFO << "Sent: " << number_of_sent_messages_total_
                     ...
                     << "% load in average, if measured "
                     << "cpu load was greater zero (average of " << results_no_zero.size()
                     << " measurements).";
    ...
    void send_messages_sync(std::unique_lock<std::mutex>& lk, std::uint32_t _messages_to_send)
    {
        ...
        VSOMEIP_DEBUG << "Synchronously sent " << std::setw(4) << std::setfill('0')
                      << number_of_sent_messages_ << " messages. CPU load [%]: " << std::fixed
                      << std::setprecision(2)
    ...
    void send_service_start_measuring(bool _start_measuring)
    {
        std::shared_ptr<vsomeip::message> m =
            vsomeip::runtime::get()->create_request(protocol_ == protocol_e::PR_TCP);
...
{
    cpu_load_test_client test_client_(protocol, number_of_calls, payload_size, call_service_sync,
                                      shutdown_service);
}
...
int main(int argc, char** argv)
{
    ...
    while (i < argc)
    {
                 ...
                 || std::string("-c") == std::string(argv[i]))
        {
            ...
            {
                number_of_calls =
                    static_cast<std::uint32_t>(std::stoul(std::string(argv[i + 1]), nullptr, 10));
