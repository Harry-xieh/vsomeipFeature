...
public:
    ...
    bool init()
    {
        ...
        app_->register_message_handler(
            cpu_load_test::service_id, cpu_load_test::instance_id, cpu_load_test::method_id,
            std::bind(&cpu_load_test_service::on_message, this, std::placeholders::_1));

        app_->register_message_handler(
            cpu_load_test::service_id, cpu_load_test::instance_id,
            cpu_load_test::method_id_shutdown,
            std::bind(&cpu_load_test_service::on_message_shutdown, this, std::placeholders::_1));
        app_->register_message_handler(cpu_load_test::service_id, cpu_load_test::instance_id,
                                       cpu_load_test::method_id_cpu_measure_start,
                                       std::bind(&cpu_load_test_service::on_message_start_measuring,
                                                 this, std::placeholders::_1));
        app_->register_message_handler(cpu_load_test::service_id, cpu_load_test::instance_id,
                                       cpu_load_test::method_id_cpu_measure_stop,
                                       std::bind(&cpu_load_test_service::on_message_stop_measuring,
                                                 this, std::placeholders::_1));
        app_->register_state_handler(
            std::bind(&cpu_load_test_service::on_state, this, std::placeholders::_1));
        return true;
    }
    ...
    void on_message_stop_measuring(const std::shared_ptr<vsomeip::message>& _request)
    {
        ...
        VSOMEIP_DEBUG << "Received " << std::setw(4) << std::setfill('0')
                      << number_of_received_messages_ << " messages. CPU load [%]: " << std::fixed
                      << std::setprecision(2)
                      << (std::isfinite(load_measurer_.get_cpu_load()) ?
                              load_measurer_.get_cpu_load() :
                              0.0);
        results_.push_back(
    ...
    void on_message_shutdown(const std::shared_ptr<vsomeip::message>& _request)
    {
        ...
        const double average_load(std::accumulate(results_.begin(), results_.end(), 0.0)
                                  / static_cast<double>(results_.size()));
        VSOMEIP_INFO << "Received: " << number_of_received_messages_total_
                     << " in total (excluding control messages). This caused: " << std::fixed
                     << std::setprecision(2) << average_load << "% load in average (average of "
        ...
        const double average_load_no_zero(
            std::accumulate(results_no_zero.begin(), results_no_zero.end(), 0.0)
        ...
        VSOMEIP_INFO << "Sent: " << number_of_received_messages_total_
                     ...
                     << std::fixed << std::setprecision(2) << average_load_no_zero
                     << "% load in average, if measured cpu load "
                     << "was greater zero (average of " << results_no_zero.size()
                     << " measurements).";
        stop();
    }
