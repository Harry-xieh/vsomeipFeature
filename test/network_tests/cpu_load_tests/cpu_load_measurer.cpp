...
void cpu_load_measurer::stop()
{
    ...
    if (jiffies_complete_stop_ < jiffies_complete_start_
        || jiffies_passed_pid_stop_ < jiffies_passed_pid_start_)
    {
        std::cerr << "Overflow of values in procfs occured, can't calculate load" << std::endl;
        exit(0);
    }
    cpu_load_pid_ = 100.0
                    * static_cast<double>(jiffies_passed_pid_stop_ - jiffies_passed_pid_start_)
    ...
    cpu_load_overall_ = 100.0
                        * static_cast<double>((jiffies_complete_stop_ - jiffies_idle_stop_)
                        ...
                        / static_cast<double>(jiffies_complete_stop_ - jiffies_complete_start_);
    cpu_load_pid_wo_idle_ =
        100.0 * static_cast<double>(jiffies_passed_pid_stop_ - jiffies_passed_pid_start_)
        / static_cast<double>((jiffies_complete_stop_ - jiffies_idle_stop_)
                              - (jiffies_complete_start_ - jiffies_idle_start_));
}
...
void cpu_load_measurer::print_cpu_load() const
{
    ...
    std::cout << "Used Jiffies of pid " << pid_ << ": "
              << jiffies_passed_pid_stop_ - jiffies_passed_pid_start_ << std::endl;
    std::cout << "Cpu load pid " << pid_ << " [%]: " << cpu_load_pid_ << std::endl;
    std::cout << "Overall cpu load[%]: " << cpu_load_overall_ << std::endl;
    std::cout << "Load caused by pid " << pid_
              << " of overall cpu load [%]:" << cpu_load_pid_wo_idle_ << std::endl;
}
...
std::uint64_t cpu_load_measurer::read_proc_stat(std::uint64_t* _idle)
{
    ...
    if (std::fscanf(f, "%*s %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu", &user, &nice, &system, &idle,
