...
enum class payloadsize : std::uint8_t
{
    UDS,
    TCP,
    UDP
...
npdu_test_client::npdu_test_client(
    bool _use_tcp, bool _call_service_sync, std::uint32_t _sliding_window_size,
...
void npdu_test_client::init()
{
    ...
    app_->register_state_handler(
        std::bind(&npdu_test_client::on_state, this, std::placeholders::_1));
...
void npdu_test_client::register_availability_handler()
{
    app_->register_availability_handler(
        npdu_test::service_ids[service_idx], npdu_test::instance_ids[service_idx],
...
void npdu_test_client::register_message_handler()
{
    app_->register_message_handler(npdu_test::service_ids[service_idx],
                                   ...
                                   std::bind(&npdu_test_client::on_message<service_idx, method_idx>,
...
void npdu_test_client::stop()
{
    ...
    for (unsigned int i = 0; i < npdu_test::service_ids.size(); i++)
    {
        ...
        {
            app_->unregister_message_handler(npdu_test::service_ids[i], npdu_test::instance_ids[i],
...
template <int service_idx>
void npdu_test_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance,
                                       bool _is_available)
{
...
void npdu_test_client::on_message(const std::shared_ptr<vsomeip::message>& _response)
{
    ...
    VSOMEIP_DEBUG << "Received reply from:" << std::setw(4) << std::setfill('0') << std::hex
                  << npdu_test::service_ids[service_idx] << ":" << std::setw(4) << std::setfill('0')
                  << std::hex << npdu_test::instance_ids[service_idx] << ":" << std::setw(4)
    ...
    else
    {
        std::lock_guard<std::mutex> its_lock(
            number_of_acknowledged_messages_mutexes_[service_idx][method_idx]);
        ...
        if (number_of_acknowledged_messages_[service_idx][method_idx]
        ...
        else if (number_of_acknowledged_messages_[service_idx][method_idx] % sliding_window_size
                 == 0)
        {
            std::lock_guard<std::mutex> lk(all_msg_acknowledged_mutexes_[service_idx][method_idx]);
            all_msg_acknowledged_[service_idx][method_idx] = true;
...
void npdu_test_client::run()
{
    ...
    while (current_payload_size_[service_idx] <= max_allowed_payload)
    {
        ...
        for (int var = 0; var < 4; ++var)
        {
            ...
            payload_data_[service_idx][var].assign(current_payload_size_[service_idx],
                                                   static_cast<vsomeip::byte_t>(0x11 * (var + 1)));
        ...
        if (wait_for_replies_)
        {
            call_service_sync_ ? send_messages_sync<service_idx>() :
                                 send_messages_async<service_idx>();
        }
        else
        {
...
std::thread npdu_test_client::start_send_thread_sync()
{
    return std::thread([&]() {
        ...
        std::shared_ptr<vsomeip::message> request =
            vsomeip::runtime::get()->create_request(use_tcp);
...
void npdu_test_client::wait_for_all_senders()
{
    ...
    while (!all_finished)
    {
        {
            std::lock_guard<std::mutex> its_lock(finished_mutex_);
            if (std::all_of(finished_.begin(), finished_.end(), [](bool i) {
                    return i;
                }))
            {
                all_finished = true;
    ...
    for (int service_id = 0; service_id < 4; service_id++)
    {
        for (int method_id = 0; method_id < 4; method_id++)
        {
            ...
            its_configuration->get_configured_timing_requests(
                ...
                its_configuration->get_unreliable_port(npdu_test::service_ids[service_id],
                                                       npdu_test::instance_ids[service_id]),
            ...
            if (debounce == std::chrono::nanoseconds(VSOMEIP_DEFAULT_NPDU_DEBOUNCING_NANO)
                && retention
                       == std::chrono::nanoseconds(VSOMEIP_DEFAULT_NPDU_MAXIMUM_RETENTION_NANO))
            {
            ...
            else
            {
                ...
                applicative_debounce[service_id][method_id] =
                    std::chrono::duration_cast<std::chrono::milliseconds>(debounce + retention)
                    + std::chrono::milliseconds(1);
            }
        }
    }

    npdu_test_client test_client_(use_tcp, call_service_sync, sliding_window_size, wait_for_replies,
