...
void npdu_test_service::init()
{
    ...
    app_->register_message_handler(
        service_id_, instance_id_, npdu_test::NPDU_SERVICE_SHUTDOWNMETHOD_ID,
        std::bind(&npdu_test_service::on_message_shutdown, this, std::placeholders::_1));

    app_->register_state_handler(
        std::bind(&npdu_test_service::on_state, this, std::placeholders::_1));
}
...
{
    app_->register_message_handler(
        service_id_, instance_id_, method_ids_[method_idx],
...
void npdu_test_service::stop()
{
    ...
    if (!undershot_debounce_times_.empty())
    {
        ...
        double average = static_cast<double>(sum.count())
                         / static_cast<double>(undershot_debounce_times_.size());
        VSOMEIP_INFO << "[" << std::setw(4) << std::setfill('0') << std::hex << service_id_ << "."
                     << std::setw(4) << std::setfill('0') << std::hex << instance_id_ << "]: "
                     << " Debounce time was undershot " << std::dec
                     << undershot_debounce_times_.size() << "/" << number_of_received_messages_
                     << "(" << std::setprecision(2)
                     << (static_cast<double>(undershot_debounce_times_.size())
                         / static_cast<double>(number_of_received_messages_))
                            * 100.00
    ...
    app_->unregister_message_handler(service_id_, instance_id_,
...
void npdu_test_service::check_times()
{
    ...
    {
        const auto time_undershot = std::chrono::duration_cast<std::chrono::microseconds>(
            debounce_times_[method_idx] - time_since_last_message);
...
void npdu_test_service::on_message(const std::shared_ptr<vsomeip::message>& _request)
{
    ...
    VSOMEIP_DEBUG << __func__ << " 0x" << std::setw(4) << std::setfill('0') << std::hex
                  << method_ids_[method_idx] << " payload size: " << std::dec
...
TEST(someip_npdu_test, offer_service_and_check_debounce_times)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        its_configuration->get_configured_timing_requests(
            ...
            its_configuration->get_unreliable_port(npdu_test::service_ids[SERVICE_NUMBER],
    ...
    npdu_test_service test_service(
        npdu_test::service_ids[SERVICE_NUMBER], npdu_test::instance_ids[SERVICE_NUMBER],
