...
public:
          ...
          offer_thread_(std::bind(&offer_test_big_sd_msg_service::run, this))
    {
        ...
        app_->register_state_handler(
            std::bind(&offer_test_big_sd_msg_service::on_state, this, std::placeholders::_1));
        ...
        {
            app_->offer_event(s, 0x1, offer_test::big_msg_event_id, its_eventgroups,
                              vsomeip::event_type_e::ET_EVENT, std::chrono::milliseconds::zero(),
                              false, true, nullptr, vsomeip::reliability_type_e::RT_UNKNOWN);
            app_->register_subscription_handler(
                s, 0x1, offer_test::big_msg_eventgroup_id,
                std::bind(&offer_test_big_sd_msg_service::on_subscription, this,
                          std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                          std::placeholders::_4, s));
            subscriptions_[s] = 0;
        }
    ...
    bool on_subscription(vsomeip::client_t _client, std::uint32_t _uid, std::uint32_t _gid,
                         bool _subscribed, vsomeip::service_t _service)
    {
        (void)_client;
        (void)_uid;
        (void)_gid;
        if (_subscribed)
        {
            ...
            if (std::all_of(subscriptions_.begin(), subscriptions_.end(),
                            [&](const subscriptions_t::value_type& v) {
                                return v.second == 1;
                            }))
            {
                std::lock_guard<std::mutex> its_lock(mutex_);
