...
public:
          ...
          send_thread_(std::bind(&offer_test_big_sd_msg_client::send, this))
    {
        ...
        app_->register_state_handler(
            std::bind(&offer_test_big_sd_msg_client::on_state, this, std::placeholders::_1));
        ...
        for (std::uint16_t s = 1; s <= offer_test::big_msg_number_services; s++)
        {
            ...
            app_->request_event(s, 0x1, offer_test::big_msg_event_id, its_eventgroups,
            ...
            app_->register_subscription_status_handler(
                s, 0x1, offer_test::big_msg_eventgroup_id, offer_test::big_msg_event_id,
            ...
            if (found_service != services_available_subribed_.end())
            {
                ...
                if (found_service->second.second > 1)
                {
                    ADD_FAILURE() << "Registered subscription status handler was "
                                     "called "
                                  << std::dec << found_service->second.second
                                  << " times for service: " << std::hex << found_service->first;
                }
                if (std::all_of(services_available_subribed_.cbegin(),
                                services_available_subribed_.cend(),
                                [](const services_available_subribed_t::value_type& v) {
...
private:
    ...
    typedef std::map<vsomeip::service_t, std::pair<bool, std::uint32_t>>
                                  services_available_subribed_t;
