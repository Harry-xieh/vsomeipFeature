...
{
public:
    virtual void on_availability(service_t _service_id, instance_t _instance_id,
                                 bool _is_available);
...
{
public:
    client(service_t _service_id, instance_t _instance_id, major_version_t _major_version,
           minor_version_t _minor_version)
    {
        ...
        app_->register_message_handler(service_id_, instance_id_, ANY_METHOD,
    ...
    bool was_message_received()
    {
        ...
        condition_message_received_.wait(lock, [=] {
    ...
    {
        std::unique_lock<std::mutex> lock(mutex_);
        condition_availability_.wait(lock, [=] {
    ...
    void send_message(const std::vector<uint8_t>& _outgoing_payload)
    {
        ...
        std::unique_lock<std::mutex> lock(mutex_);
        condition_message_sent_.wait(lock, [=] {
            return msg_sent_.load();
        });
    }
...
private:
    ...
    void on_message(const std::shared_ptr<message>& _message)
    {
        ...
        VSOMEIP_INFO << "[TEST] Got message from " << std::hex << std::setw(4) << std::setfill('0')
                     << _message->get_service() << "." << std::hex << std::setw(4)
                     << std::setfill('0') << _message->get_instance() << " length " << std::dec
                     << len << " and payload " << msg.str();
        {
...
public:
    ...
    bool is_available()
    {
        return app_->is_available(service_id_, instance_id_);
    }
...
public:
    ...
    void set_routing_state(routing_state_e state)
    {
        app_->set_routing_state(state);
    }
