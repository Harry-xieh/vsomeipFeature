...
TEST(someip_offer_test, send_offer_service_sd_message)
{
    try
    {
        ...
        boost::asio::ip::udp::socket::endpoint_type target_sd(
            boost::asio::ip::address::from_string(std::string(passed_address)), 30490);
        boost::asio::ip::udp::socket udp_socket(
            io, boost::asio::ip::udp::endpoint(boost::asio::ip::udp::v4(), 30490));
        std::uint8_t its_offer_service_message[] = {
            0xff, 0xff, 0x81, 0x00, 0x00, 0x00, 0x00, 0x3c, 0x00, 0x00, 0x00, 0x01, 0x01,
            ...
            0x00, 0x06, 0x9c, 0x41, 0x00, 0x09, 0x04, 0x00, 0x0a, 0x00, 0x03, 0x7D, // slave address
            0x00, 0x11, 0x75, 0x31};
        ...
        std::uint8_t shutdown_call[] = {0x11, 0x11, 0x14, 0x04, 0x00, 0x00, 0x00, 0x08,
                                        0x22, 0x22, 0x00, 0x01, 0x01, 0x00, 0x01, 0x00};
        boost::asio::ip::udp::socket::endpoint_type target_service(
            boost::asio::ip::address::from_string(std::string(passed_address)), 30001);
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cout << "Please pass an target IP address to this binary like: " << argv[0]
                  << " 10.0.3.1" << std::endl;
        exit(1);
    }
