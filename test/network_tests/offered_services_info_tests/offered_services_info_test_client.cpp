...
{
public:
    offered_services_info_test_client(struct offer_test::service_info _service_info,
                                      offer_test::service_info        _remote_service_info,
        ...
        : vsomeip_utilities::base_logger("OFIC", "OFFERED SERVICES INFO TEST CLIENT"),
          ...
          test_offered_services_thread_(
              std::bind(&offered_services_info_test_client::test_offered_services, this))
    {
        if (!app_->init())
        {
        ...
        local_offered_services[service_info_.service_id].insert(
            (vsomeip::instance_t)(service_info_.instance_id + 1));
        all_offered_services[service_info_.service_id].insert(
            (vsomeip::instance_t)(service_info_.instance_id + 1));
        ...
        remote_offered_services[remote_service_info_.service_id].insert(
            remote_service_info_.instance_id);
        all_offered_services[remote_service_info_.service_id].insert(
            remote_service_info_.instance_id);
        ...
        app_->register_state_handler(
            std::bind(&offered_services_info_test_client::on_state, this, std::placeholders::_1));
    ...
    void on_response(const std::shared_ptr<vsomeip::message>& _message)
    {
        ...
        ASSERT_LT(std::chrono::duration_cast<std::chrono::milliseconds>(
                      std::chrono::steady_clock::now() - last_received_response_)
                      .count(),
    ...
    void test_offered_services()
    {
        ...
        app_->get_offered_services_async(
            ...
            std::bind(&offered_services_info_test_client::on_offered_services_local, this,
                      std::placeholders::_1));

        // send shutdown command to service
        if (std::future_status::timeout
            == all_callbacks_received_.get_future().wait_for(std::chrono::seconds(15)))
        {
    ...
    void on_offered_services_local(
        const std::vector<std::pair<vsomeip::service_t, vsomeip::instance_t>>& _services)
    {
        ...
        for (auto its_pair : _services)
        {
            ...
            std::cout << "CALLBACK VALUE -> Service: " << std::hex << std::get<0>(its_pair)
                      << " instance: " << std::get<1>(its_pair) << std::endl;
        ...
        app_->get_offered_services_async(
            ...
            std::bind(&offered_services_info_test_client::on_offered_services_remote, this,
                      std::placeholders::_1));
    }

    void on_offered_services_remote(
        const std::vector<std::pair<vsomeip::service_t, vsomeip::instance_t>>& _services)
    {
        ...
        app_->get_offered_services_async(
            ...
            std::bind(&offered_services_info_test_client::on_offered_services_all, this,
                      std::placeholders::_1));
    }

    void on_offered_services_all(
        const std::vector<std::pair<vsomeip::service_t, vsomeip::instance_t>>& _services)
    {
...
{
    offered_services_info_test_client its_sample(offer_test::service, offer_test::remote_service,
                                                 passed_mode);
}
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr << "Please specify a operation mode, like: " << argv[0] << " SUBSCRIBE"
                  << std::endl;
