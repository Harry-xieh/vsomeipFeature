    ...
    std::thread receive_thread([&]() {
        ...
        while (keep_receiving)
        {
            ...
            else
            {
                ...
                while (bytes_transferred > 0)
                {
                    ...
                    if (its_service == vsomeip::sd::service && its_method == vsomeip::sd::method)
                    {
                        ...
                        for (const auto& e : sd_msg.get_entries())
                        {
                            ...
                            EXPECT_EQ(pending_subscription_test::service.service_id,
                                      e->get_service());
                            EXPECT_EQ(pending_subscription_test::service.instance_id,
                                      e->get_instance());
                            if (e->get_type()
                                == vsomeip::sd::entry_type_e::SUBSCRIBE_EVENTGROUP_ACK)
                            {
                                ...
                                EXPECT_TRUE(
                                    its_casted_entry->get_eventgroup()
                                        == pending_subscription_test::service.eventgroup_id
                                    || its_casted_entry->get_eventgroup()
                                           == pending_subscription_test::service.eventgroup_id + 1);
                    ...
                    { // non-sd-message
                        ...
                        if (msg.get_message_type() == vsomeip::message_type_e::MT_RESPONSE)
                        {
                            ...
                            EXPECT_EQ(pending_subscription_test::service.service_id,
                                      msg.get_service());
                            EXPECT_EQ(pending_subscription_test::service.shutdown_method_id,
                                      msg.get_method());
                        ...
                        else if (msg.get_message_type() == vsomeip::message_type_e::MT_NOTIFICATION)
                        {
                            ...
                            {
                                EXPECT_EQ(pending_subscription_test::service.event_id,
                                          its_received_events[0]);
                                EXPECT_EQ(static_cast<vsomeip::event_t>(
                                              pending_subscription_test::service.event_id + 1u),
                ...
                if (subscribe_acks_receiveid == 30)
                { // all subscribeAcks received
    ...
    std::thread send_thread([&]() {
        try
        {
            ...
            boost::asio::ip::udp::socket::endpoint_type target_sd(
                boost::asio::ip::address::from_string(std::string(remote_address)), 30490);
            ...
            if (std::future_status::timeout
                == trigger_notifications.get_future().wait_for(std::chrono::seconds(10)))
            {
            ...
            {
                // call notify method
                std::uint8_t trigger_notifications_call[] = {0x11, 0x22, 0x42, 0x42, 0x00, 0x00,
                                                             0x00, 0x08, 0x22, 0x22, 0x00, 0x01,
                                                             0x01, 0x00, 0x01, 0x00};
                boost::asio::ip::udp::socket::endpoint_type target_service(
                    boost::asio::ip::address::from_string(std::string(remote_address)), 30001);
...
TEST_F(pending_subscription, send_alternating_subscribe_nack_unsubscribe)
{
    ...
    std::thread receive_thread([&]() {
        ...
        while (keep_receiving)
        {
            ...
            if (nacks_received == expected_nacks && acks_received == expected_acks
                && notifications_received == expected_notifications
                && responses_received == expected_responses)
            {
                keep_receiving = false;
...
TEST_F(pending_subscription, send_alternating_subscribe_unsubscribe_same_port)
{
    ...
    boost::asio::ip::tcp::socket tcp_socket(
        io_, boost::asio::ip::tcp::endpoint(boost::asio::ip::tcp::v4(), 30490));
    ...
    std::thread receive_thread([&]() {
        ...
        tcp_socket.connect(boost::asio::ip::tcp::endpoint(
                               boost::asio::ip::address::from_string(remote_address), 40001),
                           ec);
    ...
    std::thread send_thread([&]() {
        if (std::future_status::timeout
            == tcp_connected.get_future().wait_for(std::chrono::seconds(10)))
        {
        ...
        try
        {
            std::uint8_t its_subscribe_message[] = {0xff, 0xff, 0x81, 0x00, 0x00, 0x00, 0x00,
                                                    ...
                                                    0x00, 0x20, // length entries array
                                                    ...
                                                    0xff, // ip address
                                                    0x00, 0x11, 0x77, 0x1a, 0x00, 0x09, 0x04,
                                                    0x00, 0xff, 0xff, 0xff, 0xff, // ip address
                                                    0x00, 0x06, 0x77, 0x1a};
...
TEST_F(pending_subscription, subscribe_resubscribe_mixed)
{
                ...
                while (bytes_transfered > 0)
                {
                    ...
                    { // non-sd-message
                        ...
                        else if (msg.get_message_type() == vsomeip::message_type_e::MT_NOTIFICATION)
                        {
                            ...
                            if (its_received_events.size() == 2)
                            {
                                EXPECT_TRUE(its_received_events.find(static_cast<vsomeip::event_t>(
                                                pending_subscription_test::service.event_id + 1u))
                                            != its_received_events.end());
                                EXPECT_TRUE(its_received_events.find(
                                                pending_subscription_test::service.event_id)
                                            != its_received_events.end());
                            }
                            EXPECT_EQ(1u, msg.get_payload()->get_length());
                            EXPECT_EQ(0xDD, *msg.get_payload()->get_data());
                    ...
                    if (!second_initial_event_checked && notifications_received == 2)
                    { // events were received as well
                        ...
                        EXPECT_TRUE(its_received_events.find(static_cast<vsomeip::event_t>(
                                    ...
                                    != its_received_events.end());
                        EXPECT_TRUE(
                            its_received_events.find(pending_subscription_test::service.event_id)
                            != its_received_events.end());
    ...
    std::thread send_thread([&]() {
        try
        {
            ...
            if (std::future_status::timeout
                == first_initial_event_received.get_future().wait_for(std::chrono::seconds(10)))
            {
                ADD_FAILURE()
                    << "Didn't receive all SubscribeAck of first subscription within time";
            }
            ...
            std::uint8_t its_subscribe_resubscribe_message[] = {
                ...
                0x00, 0x09, 0x04, 0x00, 0xff, 0xff, 0xff, 0xff, // ip address
                0x00, 0x11, 0x77, 0x1a};
            std::memcpy(&its_subscribe_resubscribe_message[64],
                        &its_local_address.to_v4().to_bytes()[0], 4);
            udp_socket.send_to(boost::asio::buffer(its_subscribe_resubscribe_message), target_sd);

            if (std::future_status::timeout
                == second_initial_event_received.get_future().wait_for(std::chrono::seconds(10)))
            {
                ADD_FAILURE()
                    << "Didn't receive all SubscribeAck of second subscription within time";
            }
            // call shutdown method
            std::uint8_t shutdown_call[] = {0x11, 0x22, 0x14, 0x04, 0x00, 0x00, 0x00, 0x08,
                                            0x22, 0x22, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00};
...
TEST_F(pending_subscription, send_subscribe_stop_subscribe_subscribe)
{
                                ...
                                == vsomeip::sd::entry_type_e::SUBSCRIBE_EVENTGROUP_ACK)
                            {
                                std::shared_ptr<vsomeip::sd::eventgroupentry_impl>
                                    its_casted_entry =
                                        std::static_pointer_cast<vsomeip::sd::eventgroupentry_impl>(
                                            e);
                                EXPECT_TRUE(its_casted_entry->get_eventgroup()
                                            == pending_subscription_test::service.eventgroup_id);
                            }
                        }
                        EXPECT_EQ(0u, sd_msg.get_options().size());
    ...
    std::thread send_thread([&]() {
        ...
        try
        {
            ...
            std::memcpy(&its_normal_subscribe_message[48], &its_local_address.to_v4().to_bytes()[0],
                        4);
...
TEST_F(pending_subscription, send_request_to_sd_port)
{
        ...
        while (keep_receiving)
        {
            ...
            else
            {
                ...
                while (bytes_transferred > 0)
                {
                    ...
                    { // response to shutdown method was received as well
                        keep_receiving = false;
                    }
                    else if (offers_received == 3)
                    { // all multiple offers received
                        try
                        {
    ...
    std::thread send_thread([&]() {
        try
        {
            ...
            if (std::future_status::timeout
                == all_offers_received.get_future().wait_for(std::chrono::seconds(10)))
            {
...
int main(int argc, char** argv)
{
    ...
    {
        std::cerr
            << "Please pass an target and local IP address and test mode to this binary like: "
