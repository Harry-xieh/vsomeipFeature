...
public:
          ...
          offer_thread_(std::bind(&pending_subscription_test_service::run, this))
    {
        ...
        app_->register_state_handler(
            std::bind(&pending_subscription_test_service::on_state, this, std::placeholders::_1));

        // offer field
        std::set<vsomeip::eventgroup_t> its_eventgroups;
        its_eventgroups.insert(_service_info.eventgroup_id);
        app_->offer_event(service_info_.service_id, 0x1, service_info_.event_id, its_eventgroups,
        ...
        its_eventgroups.insert(
            static_cast<vsomeip::eventgroup_t>(_service_info.eventgroup_id + 1u));
        ...
        app_->register_message_handler(
            vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, service_info_.notify_method_id,
        ...
        app_->register_subscription_handler(
            service_info_.service_id, 0x1,
            static_cast<vsomeip::eventgroup_t>(service_info_.eventgroup_id + 1u),
            std::bind(&pending_subscription_test_service::subscription_handler, this,
                      std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                      std::placeholders::_4));
        app_->start();
    }
                                    ...
                                    const std::function<void(const bool)>& _cbk)
    {
        ...
        else if (testmode_ == pending_subscription_test::test_mode_e::SUBSCRIBE_UNSUBSCRIBE_NACK)
        {
            ...
            if (count_subscribe == 8 || count_unsubscribe == 7)
            {
                subscription_accepted_asynchronous_ = true;
                 ...
                 == pending_subscription_test::test_mode_e::SUBSCRIBE_UNSUBSCRIBE_SAME_PORT)
        {
            static int count_subscribe   = 0;
            static int count_unsubscribe = 0;
            _subscribed ? count_subscribe++ : count_unsubscribe++;
        ...
        else if (testmode_ == pending_subscription_test::test_mode_e::SUBSCRIBE_RESUBSCRIBE_MIXED)
        {
            ...
            subscription_accepted_asynchronous_ = true;
                 ...
                 == pending_subscription_test::test_mode_e::SUBSCRIBE_STOPSUBSCRIBE_SUBSCRIBE)
        {
            static int was_called = 0;
            was_called++;
            EXPECT_EQ(1, was_called);
    ...
    bool subscription_handler(vsomeip::client_t _client, std::uint32_t _uid, std::uint32_t _gid,
                              bool _subscribed)
    {
        (void)_subscribed;
        (void)_uid;
...
pending_subscription_test::test_mode_e
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr << "Please pass a test mode to this binary like: " << argv[0] << " SUBSCRIBE"
                  << std::endl;
        std::cerr
            << "Testmodes are [SUBSCRIBE, SUBSCRIBE_UNSUBSCRIBE, UNSUBSCRIBE, SUBSCRIBE_UNSUBSCRIBE_NACK, SUBSCRIBE_UNSUBSCRIBE_SAME_PORT]"
            << std::endl;
        exit(1);
    }
