...
{
    if (MEMORY_SERVICE == message_->get_service()
        && message_->get_method() <= MEMORY_EVENT + TEST_EVENT_NUMBER
    ...
    : vsomeip_utilities::base_vsip_app(app_name_, app_id_), map_events(map_events_)
{
    ...
    {
        _app->request_event(MEMORY_SERVICE, MEMORY_INSTANCE, MEMORY_EVENT + i, {MEMORY_EVENTGROUP},
...
TEST(memory_tests, receive_messages)
{
    ...
    memory_checker_thread = std::thread([&stop_checking] {
        ...
        double memory_average =
            static_cast<double>(sum) / static_cast<double>(test_memory_array.size());
