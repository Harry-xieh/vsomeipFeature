...
payload_test_client::payload_test_client(bool _use_tcp, bool _call_service_sync,
...
bool payload_test_client::init()
{
    ...
    app_->register_state_handler(
        std::bind(&payload_test_client::on_state, this, std::placeholders::_1));
    ...
    app_->register_availability_handler(
        ...
        std::bind(&payload_test_client::on_availability, this, std::placeholders::_1,
...
void payload_test_client::print_throughput()
{
    ...
    std::double_t           calls_per_sec =
        number_of_sent_messages_ * (usec_per_sec / static_cast<double>(time_needed));
    std::double_t mbyte_per_sec = ((number_of_sent_messages_ * current_payload_size_)
                                   / (static_cast<double>(time_needed) / usec_per_sec))
                                  / (1024 * 1024);

    VSOMEIP_INFO << "[ Payload Test ] : :"
                 << "Payload size [byte]: " << std::dec << std::setw(8) << std::setfill('0')
                 << current_payload_size_ << " Messages sent: " << std::dec << std::setw(8)
                 << std::setfill('0') << number_of_sent_messages_
                 << " Meantime/message [usec]: " << std::dec << std::setw(8) << std::setfill('0')
                 << time_per_message << " Calls/sec: " << std::dec << std::setw(8)
                 << std::setfill('0') << calls_per_sec << " MiB/sec: " << std::dec << std::setw(8)
                 << std::setfill('0') << mbyte_per_sec;
}
