...
public:
    subscribe_notify_test_one_event_two_eventgroups_client(
        ...
        : vsomeip_utilities::base_logger("SNC1",
                                         "SUBSCRIBE NOTIFY TEST ONE EVENT TWO EVENTGROUPS CLIENT"),
          app_(vsomeip::runtime::get()->create_application("subscribe_notify_test_client")),
          info_(_info),
          use_tcp_(_use_tcp),
    ...
    bool init()
    {
        ...
        app_->register_state_handler(
            std::bind(&subscribe_notify_test_one_event_two_eventgroups_client::on_state, this,
                      std::placeholders::_1));

        app_->register_message_handler(
            vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, vsomeip::ANY_METHOD,
            std::bind(&subscribe_notify_test_one_event_two_eventgroups_client::on_message, this,
                      std::placeholders::_1));

        app_->register_availability_handler(
            info_.service_id, info_.instance_id,
            std::bind(&subscribe_notify_test_one_event_two_eventgroups_client::on_availability,
        ...
        app_->request_event(info_.service_id, info_.instance_id, info_.event_id, its_groups,
                            vsomeip::event_type_e::ET_FIELD,
                            (use_tcp_ ? vsomeip::reliability_type_e::RT_RELIABLE :
                                        vsomeip::reliability_type_e::RT_UNRELIABLE));
        app_->request_event(info_.service_id, info_.instance_id,
                            static_cast<vsomeip::event_t>(info_.event_id + 2), its_groups,
                            vsomeip::event_type_e::ET_FIELD,
                            (use_tcp_ ? vsomeip::reliability_type_e::RT_RELIABLE :
                                        vsomeip::reliability_type_e::RT_UNRELIABLE));
        ...
        app_->request_event(info_.service_id, info_.instance_id,
                            static_cast<vsomeip::event_t>(info_.event_id + 1), its_groups,
                            vsomeip::event_type_e::ET_FIELD,
                            (use_tcp_ ? vsomeip::reliability_type_e::RT_RELIABLE :
                                        vsomeip::reliability_type_e::RT_UNRELIABLE));
        app_->request_event(info_.service_id, info_.instance_id,
    ...
    void stop()
    {
        ...
        app_->unsubscribe(info_.service_id, info_.instance_id,
                          static_cast<vsomeip::eventgroup_t>(info_.eventgroup_id + 1));
        app_->release_event(info_.service_id, info_.instance_id, info_.event_id);
        app_->release_event(info_.service_id, info_.instance_id,
    ...
    void on_message(const std::shared_ptr<vsomeip::message>& _response)
    {
        ...
        its_message << "Received a message [" << std::setw(4) << std::setfill('0') << std::hex
                    << _response->get_service() << "." << std::setw(4) << std::setfill('0')
                    << std::hex << _response->get_instance() << "." << std::setw(4)
                    << std::setfill('0') << std::hex << _response->get_method()
                    << "] from Client/Session [" << std::setw(4) << std::setfill('0') << std::hex
                 ...
                 && _response->get_method() <= static_cast<vsomeip::event_t>(info_.event_id + 3))
        {
            ...
            {
                ADD_FAILURE() << "Received too many events [" << std::hex << _response->get_method()
        ...
        {
            ADD_FAILURE() << "Received unknown method id: " << std::setw(4) << std::setfill('0')
    ...
    void set_field_at_service(vsomeip::byte_t _value)
    {
        ...
        std::shared_ptr<vsomeip::payload> its_payload =
            its_runtime->create_payload(&_value, sizeof(_value));
    ...
    void wait_on_condition(std::unique_lock<std::mutex>&& _lock, bool* _predicate,
                           std::condition_variable&& _condition, std::uint32_t _timeout)
    {
        while (*_predicate)
        {
            if (std::cv_status::timeout
                == _condition.wait_for(_lock, std::chrono::seconds(_timeout)))
            {
                ADD_FAILURE() << "Condition variable wasn't notified within time (" << _timeout
                              << "sec)";
            }
        }
        *_predicate = true;
    ...
    void subscribe_at_service()
    {
        ...
        app_->subscribe(info_.service_id, info_.instance_id,
                        static_cast<vsomeip::eventgroup_t>(info_.eventgroup_id + 1));
    }
    ...
    void
    check_received_events_number(std::set<std::pair<vsomeip::event_t, std::uint32_t>> _expected)
    {
    ...
    void run()
    {
        ...
        wait_on_condition(std::move(its_availability_lock), &wait_availability_,
                          std::move(availability_condition_), 300);
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            set_field_at_service(0x1);
            {
                ...
                wait_on_condition(std::move(its_set_value_lock), &wait_set_value_,
                                  std::move(set_value_condition_), 30);
            }
        ...
        wait_on_condition(std::move(its_shutdown_lock), &wait_shutdown_response_,
                          std::move(shutdown_response_condition_), 30);
        stop();
    }
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr << "Please specify a offer type of the service, like: " << argv[0] << " UDP"
                  << std::endl;
