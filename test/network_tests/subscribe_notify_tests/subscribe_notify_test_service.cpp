...
public:
          ...
          reliability_type_(_reliability_type)
    {
        ...
        app_->register_state_handler(
            std::bind(&subscribe_notify_test_service::on_state, this, std::placeholders::_1));
        ...
        app_->offer_event(service_info_.service_id, service_info_.instance_id,
                          service_info_.event_id, its_eventgroups, vsomeip::event_type_e::ET_FIELD,
        ...
        for (const auto& i : service_infos_)
        {
            ...
            auto handler =
                std::bind(&subscribe_notify_test_service::on_subscription_state_change, this,
                          std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
                                      ...
                                      const vsomeip::event_t _event, const uint16_t _error)
    {
        ...
        if (!_error)
        {
            subscription_state_handler_called_.insert(
                std::make_tuple(_service, _instance, _eventgroup));
        }
        else
        {
                         ...
                         bool _subscribed)
    {
        ...
        VSOMEIP_DEBUG << "[" << std::setw(4) << std::setfill('0') << std::hex
                      ...
                      << "Client: " << std::setw(4) << std::setfill('0') << std::hex << _client
                      << (_subscribed ? " subscribed" : " unsubscribed") << ", now have "
...
private:
    ...
    std::set<std::tuple<vsomeip::service_t, vsomeip::instance_t, vsomeip::eventgroup_t>>
...
TEST(someip_subscribe_notify_test, send_ten_notifications_to_service)
{
    ...
    else
    {
        subscribe_notify_test_service its_sample(
            ...
            subscribe_notify_test::service_infos, reliability_type);
    }
}
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr << "Please specify a service number and event reliability type, like: " << argv[0]
                  << " 2 UDP SAME_SERVICE_ID" << std::endl;
        ...
        std::cerr
            << "If SAME_SERVICE_ID is specified as third parameter the test is run w/ multiple instances of the same service"
            << std::endl;
        return 1;
    }
