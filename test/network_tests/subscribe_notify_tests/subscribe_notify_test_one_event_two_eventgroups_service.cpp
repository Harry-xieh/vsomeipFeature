...
class subscribe_notify_test_one_event_two_eventgroups_service
    : public vsomeip_utilities::base_logger
{
public:
    subscribe_notify_test_one_event_two_eventgroups_service(
        subscribe_notify_test::service_info _info, bool _use_tcp)
        : vsomeip_utilities::base_logger("SNS1",
                                         "SUBSCRIBE NOTIFY TEST ONE EVENT TWO EVENTGROUPS SERVICE"),
          app_(vsomeip::runtime::get()->create_application()),
          wait_for_shutdown_(true),
          info_(_info),
          notify_thread_(std::bind(
              &subscribe_notify_test_one_event_two_eventgroups_service::wait_for_shutdown, this)),
          use_tcp_(_use_tcp)
    {}
    ...
    bool init()
    {
        ...
        app_->offer_event(info_.service_id, info_.instance_id, info_.event_id, its_groups,
    ...
    void on_set(const std::shared_ptr<vsomeip::message>& _message)
    {
        ...
        app_->notify(info_.service_id, info_.instance_id,
                     static_cast<vsomeip::event_t>(info_.event_id + 1), payload_);
        app_->notify(info_.service_id, info_.instance_id,
                     static_cast<vsomeip::event_t>(info_.event_id + 2), payload_);
    }
