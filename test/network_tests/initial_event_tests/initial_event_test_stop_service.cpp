...
{
public:
    initial_event_test_stop_service(struct initial_event_test::service_info _service_info,
                                    bool                                    _is_master)
          ...
          called_other_node_(false)
    {
        ...
        app_->register_state_handler(
            std::bind(&initial_event_test_stop_service::on_state, this, std::placeholders::_1));
        ...
        if (is_master_)
        {
            app_->request_service(initial_event_test::stop_service_slave.service_id,
                                  initial_event_test::stop_service_slave.instance_id);
            app_->register_availability_handler(
                initial_event_test::stop_service_slave.service_id,
                initial_event_test::stop_service_slave.instance_id,
                std::bind(&initial_event_test_stop_service::on_availability, this,
                          std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        }
        else
        {
            app_->request_service(initial_event_test::stop_service_master.service_id,
    ...
    void offer()
    {
        if (is_master_)
        {
            app_->offer_service(initial_event_test::stop_service_master.service_id,
                                initial_event_test::stop_service_master.instance_id);
        }
        else
        {
            app_->offer_service(initial_event_test::stop_service_slave.service_id,
                                initial_event_test::stop_service_slave.instance_id);
        }
    }
    ...
    void stop_offer()
    {
        ...
        {
            app_->stop_offer_service(initial_event_test::stop_service_slave.service_id,
                                     initial_event_test::stop_service_slave.instance_id);
        }
    }
    ...
    void wait_for_stop()
    {
        ...
        VSOMEIP_INFO << "(" << std::dec << its_call_number << ") [" << std::setw(4)
                     << std::setfill('0') << std::hex << service_info_.service_id
...
TEST(someip_initial_event_test, wait_for_stop_method_to_be_called)
{
    if (is_master)
    {
        initial_event_test_stop_service its_sample(initial_event_test::stop_service_master,
                                                   is_master);
    }
    else
    {
        initial_event_test_stop_service its_sample(initial_event_test::stop_service_slave,
                                                   is_master);
    }
}
