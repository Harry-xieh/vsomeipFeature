...
public:
          ...
          reliability_type_(_reliability_type)
    {
        ...
        app_->register_state_handler(
            std::bind(&initial_event_test_service::on_state, this, std::placeholders::_1));
        ...
        vsomeip::byte_t                   its_data[2] = {
            static_cast<vsomeip::byte_t>((service_info_.service_id & 0xFF00) >> 8),
...
TEST(someip_initial_event_test, set_field_once)
{
    if (use_same_service_id)
    {
        initial_event_test_service its_sample(
            ...
            offer_multiple_events, reliability_type);
    }
    else
    {
        initial_event_test_service its_sample(initial_event_test::service_infos[service_number],
                                              offer_multiple_events, reliability_type);
    }
}
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr << "Please specify a service number and subscription type, like: " << argv[0]
        ...
        std::cerr
            ...
            << std::endl;
        std::cerr
            << " - MULTIPLE_EVENTS flag. If set the test will offer to multiple events in the eventgroup, default false"
            << std::endl;
        return 1;
    }
