...
public:
    initial_event_test_client(int _client_number, bool _service_offered_tcp_and_udp,
                              ...
                              bool _initial_event_strict_checking, bool _dont_exit,
                              bool                        _subscribe_only_one,
          ...
          client_subscribes_twice_(_client_subscribes_twice)
    {
        ...
        app_->register_state_handler(
            std::bind(&initial_event_test_client::on_state, this, std::placeholders::_1));
        ...
        for (const auto& i : service_infos_)
        {
            ...
            for (std::uint32_t j = 0; j < events_to_subscribe_; j++)
            {
                app_->request_event(i.service_id, i.instance_id,
                                    static_cast<vsomeip::event_t>(i.event_id + j), its_eventgroups,
            ...
            if (!subscribe_on_available_)
            {
                if (events_to_subscribe_ == 1)
                {
                    ...
                    other_services_received_notification_[std::make_pair(i.service_id,
                                                                         i.event_id)] = 0;
                }
                else if (events_to_subscribe_ > 1)
                {
                    if (!subscribe_only_one_)
                    {
                        for (std::uint32_t j = 0; j < events_to_subscribe_; j++)
                        {
                            ...
                            std::lock_guard<std::mutex> its_lock(received_notifications_mutex_);
                            other_services_received_notification_[std::make_pair(
                                i.service_id, i.event_id + j)] = 0;
                        }
                    }
                    else
                    {
            ...
            if (its_service != other_services_available_.end())
            {
                if (its_service->second != _is_available)
                {
                    ...
                    VSOMEIP_DEBUG << "[" << std::setw(4) << std::setfill('0') << std::hex
                                  << client_number_ << "] Service [" << std::setw(4)
                                  << std::setfill('0') << std::hex << _service << "." << _instance
                                  << "] is available.";
                }
            }

            if (std::all_of(other_services_available_.cbegin(), other_services_available_.cend(),
                            [](const std::map<std::pair<vsomeip::service_t, vsomeip::instance_t>,
                                              bool>::value_type& v) {
                                return v.second;
                            }))
            {
        ...
        {
            {
                ...
                VSOMEIP_DEBUG << "[" << std::setw(4) << std::setfill('0') << std::hex
                              ...
                              << std::hex << _message->get_method() << "] (now have: " << std::dec
                              << other_services_received_notification_[std::make_pair(
                                     _message->get_service(), _message->get_method())]
                              << ")";
            }
                        ...
                        if (!subscribe_on_available_)
                        {
                            ...
                            else if (events_to_subscribe_ > 1)
                            {
                                if (!subscribe_only_one_)
                                {
                                    ...
                                    {
                                        app_->subscribe(
                                            i.service_id, i.instance_id, i.eventgroup_id,
                                            vsomeip::DEFAULT_MAJOR,
                                            static_cast<vsomeip::event_t>(i.event_id + j));
                                    }
                                }
    ...
    bool all_notifications_received()
    {
        return std::all_of(
            ...
            [&](const std::map<std::pair<vsomeip::service_t, vsomeip::method_t>,
                               std::uint32_t>::value_type& v) {
                bool result;
                    ...
                    if (v.second >= initial_event_test::notifications_to_send)
                    {
                        ...
                        if (initial_event_strict_checking_)
                        {
                            ADD_FAILURE()
                                ...
                                << std::setw(4) << std::setfill('0') << std::hex << v.first.first
                                << "." << std::setw(4) << std::setfill('0') << std::hex
                                << v.first.second << " number of received events: " << v.second;
                        }
    ...
    bool all_notifications_received_tcp_and_udp()
    {
        ...
        for (const auto& v : other_services_received_notification_)
        {
            ...
            {
                VSOMEIP_WARNING << "[" << std::setw(4) << std::setfill('0') << std::hex
                                << client_number_ << "] "
                                << " Received multiple initial events from service/instance: "
                                << std::setw(4) << std::setfill('0') << std::hex << v.first.first
                                << "." << std::setw(4) << std::setfill('0') << std::hex
            ...
            && received_normal == ((service_infos_.size() - 1) * events_to_subscribe_) / 2)
        {
            ...
            VSOMEIP_ERROR << "[" << std::setw(4) << std::setfill('0') << std::hex << client_number_
                          << "] "
        ...
        {
            ADD_FAILURE() << "[" << std::setw(4) << std::setfill('0') << std::hex << client_number_
                          << "] "
    ...
    void wait_for_stop()
    {
...

        {
            ...
            VSOMEIP_ERROR << "(" << std::dec << its_call_number << ") [" << std::setw(4)
                          << std::setfill('0') << std::hex << client_number_
...
private:
    ...
    std::map<std::pair<vsomeip::service_t, vsomeip::method_t>, std::uint32_t>
        other_services_received_notification_;
...
TEST(someip_initial_event_test, wait_for_initial_events_of_all_services)
{
    ...
    {
        initial_event_test_client its_sample(
            client_number, service_offered_tcp_and_udp,
            initial_event_test::service_infos_same_service_id, subscribe_on_available,
            subscribe_multiple_events, initial_event_strict_checking, dont_exit, subscribe_only_one,
            reliability_type, client_subscribes_twice);
    }
    else
    {
        initial_event_test_client its_sample(
            client_number, service_offered_tcp_and_udp, initial_event_test::service_infos,
            subscribe_on_available, subscribe_multiple_events, initial_event_strict_checking,
            dont_exit, subscribe_only_one, reliability_type, client_subscribes_twice);
    }
}
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr << "Please specify a client number, like: " << argv[0]
                  << " 2 SUBSCRIBE_BEFORE_START SAME_SERVICE_ID" << std::endl;
        ...
        std::cerr
            << " - SERVICE_OFFERED_TCP_AND_UDP flag. Set this if the service is offered via TCP and UDP"
            << std::endl;
        std::cerr
            << " - Time of subscription, valid values: [SUBSCRIBE_ON_AVAILABILITY, SUBSCRIBE_BEFORE_START], default SUBSCRIBE_BEFORE_START"
            << std::endl;
        std::cerr
            << " - SAME_SERVICE_ID flag. If set the test is run w/ multiple instances of the same service, default false"
            << std::endl;
        std::cerr
            << " - MULTIPLE_EVENTS flag. If set the test will subscribe to multiple events in the eventgroup, default false"
            << std::endl;
        std::cerr
            << " - STRICT_CHECKING flag. If set the test will only successfully finish if exactly the number of initial events were received (and not more). Default false"
            << std::endl;
        std::cerr
            << " - DONT_EXIT flag. If set the test will not exit if all notifications have been received. Default false"
            << std::endl;
        std::cerr
            << " - SUBSCRIBE_ONLY_ONE flag. If set the test will only subscribe to one event even if MULTIPLE_EVENTS is set. Default false"
            << std::endl;
        return 1;
    }
    ...
    if (argc > 2)
    {
        for (int i = 2; i < argc; i++)
        {
            ...
            else if (std::string("CLIENT_SUBSCRIBES_TWICE") == std::string(argv[i]))
            {
                ...
                std::cout
                    << "Testing for initial event after a second subscribe from same client CLIENT_SUBSCRIBES_TWICE"
                    << std::endl;
            }
        }
    }
