...
public:
    initial_event_test_availability_checker(
        int _client_number, std::array<initial_event_test::service_info, 7> _service_infos)
          ...
          stop_thread_(std::bind(&initial_event_test_availability_checker::wait_for_stop, this))
    {
        ...
        app_->register_state_handler(std::bind(&initial_event_test_availability_checker::on_state,
...
TEST(someip_initial_event_test, wait_for_availability_and_exit)
{
    ...
    {
        initial_event_test_availability_checker its_sample(
            client_number, initial_event_test::service_infos_same_service_id);
    }
    else
    {
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr << "Please specify a client number and subscription type, like: " << argv[0]
                  << " 2 SAME_SERVICE_ID" << std::endl;
