...
{
public:
    someip_tp_test_service(struct someip_tp_test::service_info _service_info,
                           someip_tp_test::test_mode_e         _testmode)
    ...
    void start()
    {
        ...
        app_->register_state_handler(
            std::bind(&someip_tp_test_service::on_state, this, std::placeholders::_1));
        ...
        app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE,
                                       service_info_.shutdown_method_id,
                                       std::bind(&someip_tp_test_service::on_shutdown_method_called,
                                                 this, std::placeholders::_1));

        app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE,
                                       service_info_.notify_method_id,
                                       std::bind(&someip_tp_test_service::on_notify_method_called,
                                                 this, std::placeholders::_1));
        app_->register_message_handler(
            vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, service_info_.method_id,
            std::bind(&someip_tp_test_service::on_message, this, std::placeholders::_1));

        app_->register_async_subscription_handler(
            service_info_.service_id, 0x1, service_info_.eventgroup_id,
        ...
        app_->request_event(
            someip_tp_test::service_slave.service_id, someip_tp_test::service_slave.instance_id,
        ...
        app_->register_message_handler(someip_tp_test::service_slave.service_id,
                                       ...
                                       std::bind(&someip_tp_test_service::on_response_from_slave,
                                                 this, std::placeholders::_1));
                         ...
                         bool _available)
    {
        if (_service == someip_tp_test::service_slave.service_id
            && _instance == someip_tp_test::service_slave.instance_id && _available)
        {
            std::lock_guard<std::mutex> its_lock(mutex_);
            wait_for_slave_service_available_ = false;
            condition_.notify_one();
            VSOMEIP_INFO << "Service available Service/Instance [" << std::setw(4)
                         << std::setfill('0') << std::hex << _service << "/" << std::setw(4)
                         << std::setfill('0') << std::hex << _instance << "]";
        }
    }
    ...
    {
        VSOMEIP_INFO << "Received a notification with Client/Session [" << std::setw(4)
                     << std::setfill('0') << std::hex << _message->get_client() << "/"
                     << std::setw(4) << std::setfill('0') << std::hex << _message->get_session()
                     << "] size: " << std::dec << _message->get_payload()->get_length();
        ...
        {
            if (number_notifications_of_slave_ == 0)
            { // ASCENDING with 2nd segment too big
        ...
        EXPECT_EQ(0, std::memcmp(static_cast<void*>(&its_cmp_data[0]),
                                 static_cast<void*>(&its_rcv_data[0]), its_cmp_data.size()));
    ...
    void on_notify_method_called(const std::shared_ptr<vsomeip::message>& _message)
    {
        ...
        app_->notify(service_info_.service_id, service_info_.instance_id, service_info_.event_id,
                     its_payload);
    ...
    {
        VSOMEIP_INFO << "Received a response from the slave with Client/Session [" << std::setw(4)
                     << std::setfill('0') << std::hex << _message->get_client() << "/"
                     << std::setw(4) << std::setfill('0') << std::hex << _message->get_session()
                     << "] size: " << std::dec << _message->get_payload()->get_length();
        EXPECT_EQ(someip_tp_test::service_slave.service_id, _message->get_service());
        ...
        EXPECT_EQ(0, std::memcmp(static_cast<void*>(&its_req_payload[0]),
                                 static_cast<void*>(&its_resp_payload[0]), its_req_payload.size()));
    ...
    void subscription_handler_async(vsomeip::client_t _client, std::uint32_t _uid,
                                    std::uint32_t _gid, bool _subscribed,
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cerr << "Please pass a test mode to this binary like: " << argv[0] << " IN_SEQUENCE"
                  << std::endl;
        std::cerr
            << "Testmodes are [ IN_SEQUENCE, MIXED, INCOMPLETE, DUPLICATE, OVERLAP, OVERLAP_FRONT_BACK ]"
            << std::endl;
        exit(1);
    }
