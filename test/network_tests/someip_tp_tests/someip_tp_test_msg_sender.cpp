...
protected:
    ...
    void call_shutdown_method()
    {
        ...
        std::uint8_t              shutdown_call[] = {0x45, 0x45, 0x45, 0x01, 0x00, 0x00, 0x00, 0x08,
                                        0xDD, 0xDD, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00};
                                                   ...
                                                   std::uint32_t _size, std::uint32_t _segment_size)
    {
        ...
        for (auto current_offset = _data + 16; current_offset < data_end;)
        {
            ...
            // insert tp_header
            const tp_header_t header =
                htonl(static_cast<tp_header_t>((current_offset - VSOMEIP_FULL_HEADER_SIZE - _data))
            ...
            // update length
            const length_t its_length =
                static_cast<length_t>(msg->size() - VSOMEIP_SOMEIP_HEADER_SIZE);
    ...
    void create_fragments(std::uint32_t _count, vsomeip::service_t _service,
                          vsomeip::instance_t _instance, vsomeip::method_t _method,
                          vsomeip::message_type_e _message_type, vsomeip::client_t _client,
                          ...
                          std::uint32_t                               _segment_size)
    {
        ...
        {
            its_payload_data.resize((i * _segment_size) + _segment_size,
                                    static_cast<std::uint8_t>(i));
        }
        std::shared_ptr<vsomeip::payload> payload =
            std::make_shared<vsomeip::payload_impl>(its_payload_data);
        msg.set_payload(payload);
        vsomeip::serializer its_serializer(0);
        msg.serialize(&its_serializer);

        *_target =
            split_message(its_serializer.get_data(), its_serializer.get_size(), _segment_size);
        its_serializer.reset();
    }

    vsomeip::message_buffer_t
    create_full_message(const std::vector<vsomeip::message_buffer_ptr_t>& _fragments)
    {
        ...
        {
            const auto res = its_reassembler->process_tp_message(
                &(*frag)[0], std::uint32_t(frag->size()), address_local_, 12345);
            if (res.first)
            {
    ...
    void increase_segment_front(const vsomeip::message_buffer_ptr_t& _seg, std::uint32_t _amount)
    {
        ...
        const vsomeip::tp::tp_header_t its_tp_header =
            vsomeip::bithelper::read_uint32_be(&(*_seg)[VSOMEIP_TP_HEADER_POS_MIN]);
        std::uint32_t its_offset = vsomeip::tp::tp::get_offset(its_tp_header);
        its_offset -= _amount;
        const vsomeip::tp::tp_header_t its_new_tp_header =
            htonl(static_cast<vsomeip::tp::tp_header_t>(
                its_offset | static_cast<vsomeip::tp::tp_header_t>(its_tp_header & 0x1)));
        *(reinterpret_cast<vsomeip::tp::tp_header_t*>(&((*_seg)[VSOMEIP_TP_HEADER_POS_MIN]))) =
            its_new_tp_header;
    ...
    void decrease_segment_front(const vsomeip::message_buffer_ptr_t& _seg, std::uint32_t _amount)
    {
        ...
        _seg->erase(_seg->begin() + VSOMEIP_TP_PAYLOAD_POS,
                    _seg->begin() + VSOMEIP_TP_PAYLOAD_POS + _amount);
...
TEST_P(someip_tp, send_in_mode)
{
    ...
    boost::asio::ip::udp::socket udp_sd_socket(
        io_, boost::asio::ip::udp::endpoint(boost::asio::ip::udp::v4(), 30490));

    boost::asio::ip::udp::socket udp_client_socket(
        io_, boost::asio::ip::udp::endpoint(boost::asio::ip::udp::v4(), 30001));

    boost::asio::ip::udp::socket udp_server_socket(
        io_, boost::asio::ip::udp::endpoint(boost::asio::ip::udp::v4(), 40001));
        ...
        while (keep_receiving)
        {
            ...
            else
            {
                ...
                vsomeip::service_t    its_service =
                    vsomeip::bithelper::read_uint16_be(&receive_buffer[VSOMEIP_SERVICE_POS_MIN]);
                vsomeip::method_t its_method =
                    vsomeip::bithelper::read_uint16_be(&receive_buffer[VSOMEIP_METHOD_POS_MIN]);
                ...
                if (its_service == vsomeip::sd::service && its_method == vsomeip::sd::method)
                {
                    ...
                    for (const auto& e : sd_msg.get_entries())
                    {
                            ...
                            && !client_subscribed)
                        {
                            ...
                            EXPECT_EQ(vsomeip::sd::entry_type_e::SUBSCRIBE_EVENTGROUP,
                                      e->get_type());
                            ...
                            if (e->get_type() == vsomeip::sd::entry_type_e::SUBSCRIBE_EVENTGROUP)
                            {
                                std::shared_ptr<vsomeip::sd::eventgroupentry_impl>
                                    its_casted_entry =
                                        std::static_pointer_cast<vsomeip::sd::eventgroupentry_impl>(
                                            e);
                                ...
                                if (its_option->get_type()
                                    == vsomeip::sd::option_type_e::IP4_ENDPOINT)
                                {
                                    std::shared_ptr<vsomeip::sd::ipv4_option_impl> its_ipv4_option =
                                        std::dynamic_pointer_cast<vsomeip::sd::ipv4_option_impl>(
                                            its_option);
                                    EXPECT_TRUE(its_ipv4_option > 0);
                                    EXPECT_EQ(vsomeip::sd::layer_four_protocol_e::UDP,
                                              its_ipv4_option->get_layer_four_protocol());
                                ...
                                std::vector<vsomeip::byte_t> its_sub_ack(
                                    &receive_buffer[0], &receive_buffer[0]
                                                            + VSOMEIP_FULL_HEADER_SIZE + 8
                                                            + (sd_msg.get_entries().size() * 16));
                                its_sub_ack[24] = static_cast<vsomeip::byte_t>(
                                    vsomeip::sd::entry_type_e::SUBSCRIBE_EVENTGROUP_ACK);
                                // fix length
                                const std::uint32_t its_length =
                                    htonl(static_cast<std::uint32_t>(its_sub_ack.size())
                                ...
                                boost::asio::ip::udp::socket::endpoint_type target_sd(
                                    address_remote_, 30490);
                                {
                                    std::lock_guard<std::mutex> its_lock(udp_sd_socket_mutex);
                                    udp_sd_socket.send_to(boost::asio::buffer(its_sub_ack),
                                                          target_sd);
                                }
                        ...
                        else if (e->get_type() == vsomeip::sd::entry_type_e::OFFER_SERVICE
                                 && !service_offered)
                        {
    ...
    std::thread send_thread([&]() {
        ...
        try
        {
            ...
            if (std::future_status::timeout
                == offer_received.get_future().wait_for(std::chrono::seconds(10)))
            {
                    ...
                    else
                    {
                        ...
                        while (bytes_transferred > 0)
                        {
                            const std::uint32_t its_message_size =
                                vsomeip::bithelper::read_uint32_be(
                                    &receive_buffer[its_pos + VSOMEIP_LENGTH_POS_MIN])
                                + VSOMEIP_SOMEIP_HEADER_SIZE;

                            std::cout << __LINE__ << ": received response " << its_message_size
                                      << std::endl;

                            vsomeip::deserializer its_deserializer(&receive_buffer[its_pos],
                                                                   its_message_size, 0);
                            vsomeip::service_t    its_service = vsomeip::bithelper::read_uint16_be(
                                &receive_buffer[its_pos + VSOMEIP_SERVICE_POS_MIN]);
                            vsomeip::method_t its_method = vsomeip::bithelper::read_uint16_be(
                                &receive_buffer[its_pos + VSOMEIP_METHOD_POS_MIN]);

                            vsomeip::message_impl msg;
                            EXPECT_TRUE(msg.deserialize(&its_deserializer));
                            if (msg.get_message_type() == vsomeip::message_type_e::MT_RESPONSE)
                            {
                                EXPECT_EQ(vsomeip::message_type_e::MT_RESPONSE,
                                          msg.get_message_type());
                                EXPECT_EQ(someip_tp_test::service.service_id, msg.get_service());
                            }
                            else if (msg.get_message_type()
                                     == vsomeip::message_type_e::MT_NOTIFICATION)
                            {
                                std::cout << __LINE__ << ": received event" << std::endl;
                            }
                            else if (vsomeip::tp::tp::tp_flag_is_set(
                                         receive_buffer[its_pos + VSOMEIP_MESSAGE_TYPE_POS])
                                     && vsomeip::tp::tp::tp_flag_unset(
                                            receive_buffer[its_pos + VSOMEIP_MESSAGE_TYPE_POS])
                                            ...
                                            == vsomeip::message_type_e::MT_NOTIFICATION)
                            {
                                ...
                                if (fragments_event_from_master_.size()
                                    == someip_tp_test::number_of_fragments)
                                {
                                    std::lock_guard<std::mutex> its_lock(
                                        all_fragments_received_mutex_);
                                    ...
                                    std::cout
                                        << __LINE__
                                        << ": received all event fragments as client --> Finished"
                                        << std::endl;
                                    all_fragments_received_cond_.notify_one();
                                    if (received_responses == 2)
                                    {
            ...
            {
                create_fragments(
                    someip_tp_test::number_of_fragments, someip_tp_test::service.service_id,
                    someip_tp_test::service.instance_id, someip_tp_test::service.method_id,
                    vsomeip::message_type_e::MT_REQUEST, vsomeip::ANY_CLIENT, 0xffff,
                ...
                if (mode == order_e::ASCENDING)
                {
                    if (test_mode_ == someip_tp_test::test_mode_e::MIXED)
                    {
                        ...
                        {
                            ADD_FAILURE() << "line: " << __LINE__
                                          << " needs adaption as number_of_fragments changed";
                            return;
                        }
                        else
                        {
                            ...
                            std::cout
                                << __LINE__
                                << ": using following predefined sequence to send request to master: ";
                            ...
                            {
                                udp_client_socket.send_to(
                                    boost::asio::buffer(*fragments_request_to_master_[i]),
                                    target_service);
                            }
                        }
                    }
                    ...
                    else if (test_mode_ == someip_tp_test::test_mode_e::DUPLICATE)
                    {
                        ...
                        {
                            udp_client_socket.send_to(boost::asio::buffer(*(*iter)),
                                                      target_service);
                    ...
                    else
                    {
                        if (test_mode_ == someip_tp_test::test_mode_e::INCOMPLETE)
                        {
                            ...
                            vsomeip::message_buffer_t msg_incomplete(
                                *fragments_request_to_master_[2]);
                            msg_incomplete[VSOMEIP_SESSION_POS_MIN] = 0x33;
                            msg_incomplete[VSOMEIP_SESSION_POS_MAX] = 0x33;
                            udp_client_socket.send_to(boost::asio::buffer(msg_incomplete),
                                                      target_service);
                            ...
                            udp_client_socket2.send_to(boost::asio::buffer(msg_incomplete),
                                                       target_service);
                            boost::system::error_code ec;
                            udp_client_socket2.shutdown(boost::asio::socket_base::shutdown_both,
                                                        ec);
                        ...
                        {
                            udp_client_socket.send_to(boost::asio::buffer(*fragment),
                                                      target_service);
                        }
                    }
                }
                else if (mode == order_e::DESCENDING)
                {
                    if (test_mode_ == someip_tp_test::test_mode_e::MIXED)
                    {
                        std::vector<int> its_indexes =
                            create_shuffled_seqeuence(someip_tp_test::number_of_fragments);
                        std::cout
                            << __LINE__
                            << ": using following random sequence to send request to master: ";
                        for (auto i : its_indexes)
                        {
                            std::cout << i << ", ";
                        }
                    ...
                    else
                    {
                        if (test_mode_ == someip_tp_test::test_mode_e::INCOMPLETE)
                        {
                            ...
                            // send a request fragment with a different session ID first
                            vsomeip::message_buffer_t msg_incomplete(
                                *fragments_request_to_master_[3]);
                ...
                }
                {
                    while (wait_for_all_response_fragments_received_)
                    {
                        ...
                        {
                            EXPECT_EQ(someip_tp_test::number_of_fragments,
                                      fragments_request_to_master_.size());
                            ...
                            else if (test_mode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK)
                            {
                                ...
                                if (mode == ASCENDING)
                                {
                                    decrease_segment_front_back(fragments_request_to_master_[2],
                                                                16);
                                    decrease_segment_front(fragments_request_to_master_[4], 16);
                                }
                                else
                                {
                                    decrease_segment_front_back(fragments_request_to_master_[4],
                                                                16);
                                }
                            }
                            vsomeip::message_buffer_t its_request =
                                create_full_message(fragments_request_to_master_);
                            if (test_mode_ == someip_tp_test::test_mode_e::OVERLAP
                                || test_mode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK)
                            {
                                ...
                                && mode == ASCENDING)
                            {
                                ...
                                {
                                    its_request[VSOMEIP_PAYLOAD_POS
                                                + 2 * (someip_tp_test::max_segment_size - 160)
                                                + i] = 0xff;
                                }
                            }

                            // create complete message from response
                            vsomeip::message_buffer_t its_response =
                                create_full_message(fragments_response_of_master_);
                            if (test_mode_ == someip_tp_test::test_mode_e::OVERLAP
                                || test_mode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK)
                            {
                                EXPECT_EQ(VSOMEIP_FULL_HEADER_SIZE
                            ...
                            its_response[VSOMEIP_MESSAGE_TYPE_POS] =
                                static_cast<vsomeip::byte_t>(vsomeip::message_type_e::MT_REQUEST);
            ...
            {
                if (std::cv_status::timeout
                    == all_fragments_received_cond_.wait_for(its_lock, std::chrono::seconds(20)))
                {
            ...
            // check if received event is correct
            {
                ...
                vsomeip::message_buffer_t its_event =
                    create_full_message(fragments_event_from_master_);
                vsomeip::session_t its_event_session =
                    vsomeip::bithelper::read_uint16_be(&its_event[VSOMEIP_SESSION_POS_MIN]);
                ...
                create_fragments(
                    ...
                    vsomeip::message_type_e::MT_NOTIFICATION, 0x0, its_event_session,
                    &its_cmp_event_fragments,
                    (test_mode_ == someip_tp_test::test_mode_e::OVERLAP) ?
                        vsomeip::tp::tp::tp_max_segment_length_ - 160 :
                        vsomeip::tp::tp::tp_max_segment_length_);
                vsomeip::message_buffer_t its_cmp_event =
                    create_full_message(its_cmp_event_fragments);
                ...
                EXPECT_EQ(0, std::memcmp(static_cast<void*>(&its_cmp_event[0]),
                                         static_cast<void*>(&its_event[0]), its_cmp_event.size()));
            }
            its_lock.unlock();
    ...
    std::unique_lock<std::mutex> all_fragments_received_as_server_lock(
        all_fragments_received_as_server_mutex_);
    ...
    std::thread udp_server_send_thread([&]() {
        ...
        if (std::future_status::timeout
            == remote_client_subscribed.get_future().wait_for(std::chrono::seconds(10)))
        {
        ...
        boost::asio::ip::udp::socket::endpoint_type master_client(address_remote_,
        ...
        for (const order_e mode : {order_e::ASCENDING, order_e::DESCENDING})
        {
            create_fragments(
                ...
                vsomeip::message_type_e::MT_NOTIFICATION, vsomeip::ANY_CLIENT, 0xffff,
            ...
            if (mode == order_e::ASCENDING)
            {
                if (test_mode_ == someip_tp_test::test_mode_e::MIXED)
                {
                    ...
                    else
                    {
                        ...
                        std::cout
                            << __LINE__
                            << ": using following predefined sequence to send event to master: ";
                        for (auto i : its_indexes)
                        {
                            std::cout << i << ", ";
                        }
                        ...
                        for (int i : its_indexes)
                        {
                            udp_server_socket.send_to(
                                boost::asio::buffer(*fragments_event_to_master_[i]), master_client);
                        }
                    }
                }
                else if (test_mode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK)
                ...
                else
                {
                    if (test_mode_ == someip_tp_test::test_mode_e::INCOMPLETE)
                    {
                        ...
                        udp_server_socket.send_to(boost::asio::buffer(msg_incomplete),
                                                  master_client);
                    ...
                    else if (test_mode_ == someip_tp_test::test_mode_e::OVERLAP)
                    {
                        ...
                        create_fragments(someip_tp_test::number_of_fragments + 1,
                                         ...
                                         vsomeip::message_type_e::MT_NOTIFICATION,
                                         vsomeip::ANY_CLIENT, 0xffff, &oversized_event,
                                         vsomeip::tp::tp::tp_max_segment_length_);
                        for (const auto& fragment : oversized_event)
                        {
                            udp_server_socket.send_to(boost::asio::buffer(*fragment),
                                                      master_client);
                        }
                    }
            ...
            else if (mode == order_e::DESCENDING)
            {
                if (test_mode_ == someip_tp_test::test_mode_e::MIXED)
                {
                    std::vector<int> its_indexes =
                        create_shuffled_seqeuence(someip_tp_test::number_of_fragments);
                    std::cout << __LINE__
                              << ": using following random sequence to send event to master: ";
                    for (auto i : its_indexes)
                    {
                        std::cout << i << ", ";
                    }
                ...
                else if (test_mode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK)
                {
                    ...
                    else
                    {
                        ...
                        // update length
                        *(reinterpret_cast<vsomeip::length_t*>(
                            &((*fragments_event_to_master_[4])[VSOMEIP_LENGTH_POS_MIN]))) =
                            htonl(
                                static_cast<vsomeip::length_t>(fragments_event_to_master_[4]->size()
                                                               - VSOMEIP_SOMEIP_HEADER_SIZE));
                        for (int i : its_indexes)
                        {
                            udp_server_socket.send_to(
        ...
        for (const order_e mode : {order_e::ASCENDING, order_e::DESCENDING})
        {
            ...
            {
                if (std::cv_status::timeout
                    == all_fragments_received_as_server_cond_.wait_for(
                        all_fragments_received_as_server_lock, std::chrono::seconds(20)))
                {
                    ADD_FAILURE() << "Didn't receive request from client within time: "
                                  << std::uint32_t(mode);
                    return;
                }
                else
                {
                    EXPECT_EQ(someip_tp_test::number_of_fragments,
                              fragments_received_as_server_.size());
                    // create complete message from request of client
                    vsomeip::message_buffer_t its_request =
                        create_full_message(fragments_received_as_server_);
                    if (test_mode_ == someip_tp_test::test_mode_e::OVERLAP
                        || test_mode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK)
                    {
                        EXPECT_EQ(VSOMEIP_FULL_HEADER_SIZE
                    ...
                    const vsomeip::client_t its_request_client =
                        vsomeip::bithelper::read_uint16_be(&its_request[VSOMEIP_CLIENT_POS_MIN]);
                    ...
                    create_fragments(
                        ...
                        vsomeip::message_type_e::MT_RESPONSE, its_request_client,
                        its_request_session, &fragments_response_to_master_,
                        (test_mode_ == someip_tp_test::test_mode_e::OVERLAP
                         || test_mode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK) ?
                            vsomeip::tp::tp::tp_max_segment_length_ - 160 :
                            vsomeip::tp::tp::tp_max_segment_length_);
                    // create complete message from response
                    vsomeip::message_buffer_t its_response =
                        create_full_message(fragments_response_to_master_);
                    ...
                    boost::asio::ip::udp::socket::endpoint_type master_client(
                    ...
                    if (mode == order_e::ASCENDING)
                    {
                        if (test_mode_ == someip_tp_test::test_mode_e::MIXED)
                        {
                            ...
                            else
                            {
                                ...
                                std::cout
                                    << __LINE__
                                    << ": using following predefined sequence to send back response to master: ";
                                for (auto i : its_indexes)
                                {
                                    std::cout << i << ", ";
                                }
                                ...
                                {
                                    udp_server_socket.send_to(
                                        boost::asio::buffer(*fragments_response_to_master_[i]),
                                        master_client);
                                }
                            }
                        }
                        else if (test_mode_ == someip_tp_test::test_mode_e::OVERLAP_FRONT_BACK)
                        {
                            ...
                            else
                            {
                                ...
                                std::cout
                                    << __LINE__
                                    << ": using following predefined sequence to send response to master: ";
                                for (auto i : its_indexes)
                                {
                                    std::cout << i << ", ";
                                }
                        ...
                        else if (test_mode_ == someip_tp_test::test_mode_e::DUPLICATE)
                        {
                            ...
                            {
                                udp_server_socket.send_to(boost::asio::buffer(*(*iter)),
                                                          master_client);
                        ...
                        else
                        {
                            if (test_mode_ == someip_tp_test::test_mode_e::INCOMPLETE)
                            {
                                ...
                                // send an event fragment with a different session ID first
                                vsomeip::message_buffer_t msg_incomplete(
                                    *fragments_response_to_master_[4]);
                            ...
                            {
                                udp_server_socket.send_to(boost::asio::buffer(*frag),
                                                          master_client);
                            }
                        }
                    }
                    else if (mode == order_e::DESCENDING)
                    {
                        if (test_mode_ == someip_tp_test::test_mode_e::MIXED)
                        {
                            ...
                            std::cout
                                << __LINE__
                                << ": using following random sequence to send back response to master: ";
                            for (auto i : its_indexes)
                            {
                                std::cout << i << ", ";
                            }
                        ...
                        else
                        {
                            if (test_mode_ == someip_tp_test::test_mode_e::INCOMPLETE)
                            {
                                ...
                                // send an event fragment with a different session ID first
                                vsomeip::message_buffer_t msg_incomplete(
                                    *fragments_response_to_master_[3]);
    ...
    std::thread udp_server_receive_thread([&]() {
        ...
        while (keep_receiving)
        {
            ...
            std::size_t bytes_transferred = udp_server_socket.receive_from(
                boost::asio::buffer(receive_buffer, receive_buffer.capacity()), its_remote_endpoint,
                0, error);
            if (error)
            {
                keep_receiving = false;
            ...
            else
            {
                ...
                while (bytes_transferred > 0)
                {
                    ...
                    std::cout << __LINE__ << ": received request from master " << its_message_size
                              << std::endl;
                    ...
                    vsomeip::message_impl msg;
                    EXPECT_TRUE(msg.deserialize(&its_deserializer));
                    if (vsomeip::tp::tp::tp_flag_is_set(
                            receive_buffer[its_pos + VSOMEIP_MESSAGE_TYPE_POS]))
                    {
                        auto its_buffer = std::make_shared<vsomeip::message_buffer_t>(
                            &receive_buffer[its_pos], &receive_buffer[its_pos] + its_message_size);

                        fragments_received_as_server_.push_back(its_buffer);
                        if (fragments_received_as_server_.size()
                            == someip_tp_test::number_of_fragments)
                        {
                            std::lock_guard<std::mutex> its_lock(
                                all_fragments_received_as_server_mutex_);
                            wait_for_all_fragments_received_as_server_ = false;
                            std::cout << __LINE__ << ": received all fragments as server"
                                      << std::endl;
                            ...
                            {
                                std::cout << __LINE__
                                          << ": received all requests as server --> Finished"
                                          << std::endl;
                                keep_receiving = false;
                            }
                        }
...
int main(int argc, char** argv)
{
    ...
    if (argc < 3)
    {
        ...
        std::cerr << "Testmodes are [ IN_SEQUENCE, MIXED, INCOMPLETE, OVERLAP, OVERLAP_FRONT_BACK ]"
                  << std::endl;
    }
    else
    {
