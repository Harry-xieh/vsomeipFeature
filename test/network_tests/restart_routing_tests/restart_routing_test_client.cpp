...
bool routing_restart_test_client::init()
{
    ...
    app_->register_state_handler(
        std::bind(&routing_restart_test_client::on_state, this, std::placeholders::_1));
...
void routing_restart_test_client::run()
{
    ...
    {
        {
            ...
            while (!is_available_)
            {
                if (!condition_.wait_for(its_lock, std::chrono::milliseconds(10000), [this] {
    ...
    if (std::future_status::ready
        == all_responses_received_.get_future().wait_for(std::chrono::milliseconds(10000)))
    {
