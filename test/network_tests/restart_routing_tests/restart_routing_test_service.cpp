...
bool routing_restart_test_service::init()
{
    ...
    app_->register_state_handler(
        std::bind(&routing_restart_test_service::on_state, this, std::placeholders::_1));
    return true;
}
...
void routing_restart_test_service::run()
{
    ...
    init_shutdown_condition_.wait(its_shutdown_lock, [this] {
        return init_shutdown_;
    });
    if (!execute_shutdown_condition_.wait_for(its_shutdown_lock, std::chrono::milliseconds(5000),
                                              [this] {
                                                  return all_received_;
                                              }))
    {
        VSOMEIP_WARNING
