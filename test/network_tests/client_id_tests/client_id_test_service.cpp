...
public:
          ...
          stop_thread_(std::bind(&client_id_test_service::wait_for_stop, this))
    {
        ...
        app_->register_state_handler(
            std::bind(&client_id_test_service::on_state, this, std::placeholders::_1));
    ...
    bool all_responses_and_requests_received()
    {
        const bool responses = std::all_of(
            other_services_received_response_.cbegin(), other_services_received_response_.cend(),
        ...
        const bool requests = std::all_of(
            other_services_received_request_.cbegin(), other_services_received_request_.cend(),
    ...
    void run()
    {
        ...
        for (int var = 0; var < client_id_test::messages_to_send; ++var)
        {
            for (const client_id_test::service_info& i : client_id_test::service_infos)
            {
                ...
                VSOMEIP_DEBUG << "[" << std::setw(4) << std::setfill('0') << std::hex
                              << service_info_.service_id
                              << "] Sending a request to Service/Method [" << std::setw(4)
                              << std::setfill('0') << std::hex << i.service_id << "/"
...
private:
    ...
    std::map<std::pair<vsomeip::service_t, vsomeip::method_t>, std::uint32_t>
                                               other_services_received_response_;
