...
public:
          ...
          client_id_base_(0x0)
    {
        ...
        if (its_plugin)
        {
            ...
            {
                configuration_ =
                    its_config_plugin->get_configuration(APPLICATION_NAME_ROUTING_MANAGER, "");
            }
        }
    }
...
protected:
    virtual void SetUp()
    {
        ...
        CLIENT_ID_ROUTING_MANAGER = static_cast<client_t>(
            (configuration_->get_diagnosis_address() << 8) & configuration_->get_diagnosis_mask());
        ...
        client_id_routing_manager_ = utility::request_client_id(
            ...
            static_cast<client_t>((configuration_->get_diagnosis_address() << 8)
...
public:
    ...
    client_t get_client()
    {
        if (std::future_status::timeout
            == registered_.get_future().wait_for(std::chrono::seconds(11)))
        {
...
TEST_F(client_id_utility_test, ensure_preconfigured_client_ids_not_used_for_autoconfig)
{
    ...
    const std::uint16_t limit = static_cast<std::uint16_t>(
        (APPLICATION_IN_CLIENT_ID & ~diagnosis_mask_) + std::uint16_t(10));
    ...
    for (int var = 0; var < 2; ++var)
    {
        for (std::uint16_t i = 0; i < max_allowed_clients; i++)
        {
            const vsomeip::client_t its_client = vsomeip::utility::request_client_id(
                configuration_, APPLICATION_NAME_NOT_PREDEFINED + std::to_string(i),
...
TEST_F(client_id_utility_test, exhaust_client_id_range_fragmented)
{
    ...
    for (int var = 0; var < 2; ++var)
    {
        ...
        for (std::uint16_t i = 0; i < max_allowed_clients; i++)
        {
            ...
            {
                if ((var == 0 && i > 0)
                    || (var == 1 && i > 1) // special case as in the 1st run the last assigned
        ...
        EXPECT_EQ(VSOMEIP_CLIENT_UNSET,
                  vsomeip::utility::request_client_id(configuration_,
                                                      APPLICATION_NAME_NOT_PREDEFINED + "max2",
                                                      VSOMEIP_CLIENT_UNSET));
...
TEST_F(client_id_utility_test, exhaust_client_id_range_fragmented_extended)
{
    ...
    {
        client_t its_client_id = utility::request_client_id(
            configuration_, APPLICATION_NAME_NOT_PREDEFINED + std::to_string(i) + "intermediate",
            VSOMEIP_CLIENT_UNSET);
        EXPECT_NE(VSOMEIP_CLIENT_UNSET, its_client_id);
        if (its_client_id != VSOMEIP_CLIENT_UNSET)
        {
    ...
    // check limit is reached
    client_t its_illegal_client_id = utility::request_client_id(
        configuration_, APPLICATION_NAME_NOT_PREDEFINED, VSOMEIP_CLIENT_UNSET);
    ...
    its_illegal_client_id = utility::request_client_id(
        configuration_, APPLICATION_NAME_NOT_PREDEFINED, VSOMEIP_CLIENT_UNSET);
    EXPECT_EQ(VSOMEIP_CLIENT_UNSET, its_illegal_client_id);
...
TEST_F(client_id_utility_test, request_released_client_id_after_maximum_client_id_is_assigned)
{
    ...
    client_t its_max_client_id = utility::request_client_id(
        configuration_, APPLICATION_NAME_NOT_PREDEFINED, VSOMEIP_CLIENT_UNSET);
    ...
    its_client_id = utility::request_client_id(configuration_, APPLICATION_NAME_NOT_PREDEFINED,
                                               VSOMEIP_CLIENT_UNSET);
    EXPECT_NE(VSOMEIP_CLIENT_UNSET, its_client_id);
    its_client_ids.push_back(its_client_id);
