...
public:
          ...
          send_thread_(std::bind(&event_test_client::send, this))
    {
        ...
        app_->register_state_handler(
            std::bind(&event_test_client::on_state, this, std::placeholders::_1));
    ...
    void on_notification(const std::shared_ptr<vsomeip::message>& _message)
    {
        ...
        VSOMEIP_DEBUG << "Received a notification with Client/Session [" << std::setw(4)
                      << std::setfill('0') << std::hex << _message->get_client() << "/"
                      << std::setw(4) << std::setfill('0') << std::hex << _message->get_session()
                      << "] from Service/Method [" << std::setw(4) << std::setfill('0') << std::hex
    ...
    {
        {
            ...
            while (wait_until_subscription_accepted_)
            {
                if (std::cv_status::timeout
                    == condition_.wait_for(its_lock, std::chrono::seconds(30)))
                {
...
int main(int argc, char** argv)
{
    ...
    if (argc < 3)
    {
        std::cerr << "Please specify a operation mode, like: " << argv[0] << "PAYLOAD_FIXED TCP"
                  << std::endl;
    ...
    else
    {
        ...
        std::cerr << "Please specify a operation mode, like: " << argv[0] << " PAYLOAD_FIXED"
                  << std::endl;
        std::cerr << "Valid operation modes are PAYLOAD_FIXED and PAYLOAD_DYNAMIC" << std::endl;
        return 1;
    }
