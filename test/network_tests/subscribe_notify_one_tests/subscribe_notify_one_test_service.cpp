...
public:
          ...
          reliability_type_(_reliability_type)
    {
        ...
        app_->register_state_handler(
            std::bind(&subscribe_notify_one_test_service::on_state, this, std::placeholders::_1));

        // offer event
        std::set<vsomeip::eventgroup_t> its_eventgroups;
        its_eventgroups.insert(service_info_.eventgroup_id);
        app_->offer_event(
            service_info_.service_id, service_info_.instance_id, service_info_.event_id,
        ...
        app_->register_message_handler(
            service_info_.service_id, service_info_.instance_id, service_info_.method_id,
        ...
        for (const auto& i : subscribe_notify_one_test::service_infos)
        {
            ...
            app_->register_message_handler(i.service_id, i.instance_id, vsomeip::ANY_METHOD,
                                           std::bind(&subscribe_notify_one_test_service::on_message,
            ...
            app_->request_service(i.service_id, i.instance_id, vsomeip::DEFAULT_MAJOR,
                                  vsomeip::DEFAULT_MINOR);

            auto handler =
                std::bind(&subscribe_notify_one_test_service::on_subscription_state_change, this,
                          std::placeholders::_1, std::placeholders::_2, std::placeholders::_3,
            ...
            app_->register_subscription_status_handler(
                vsomeip::ANY_SERVICE, i.instance_id, i.eventgroup_id, vsomeip::ANY_EVENT, handler);
            app_->register_subscription_status_handler(
                i.service_id, vsomeip::ANY_INSTANCE, i.eventgroup_id, vsomeip::ANY_EVENT, handler);
            app_->register_subscription_status_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE,
            ...
            app_->request_event(i.service_id, i.instance_id, i.event_id, its_eventgroups,
                                vsomeip::event_type_e::ET_SELECTIVE_EVENT, reliability_type_);
    ...
    void stop_offer()
    {
        app_->stop_offer_event(service_info_.service_id, service_info_.instance_id,
            ...
            if (its_service != other_services_available_.end())
            {
                if (its_service->second != _is_available)
                {
                    ...
                    VSOMEIP_DEBUG << "[" << std::setw(4) << std::setfill('0') << std::hex
                                  << service_info_.service_id << "] Service [" << std::setw(4)
    ...
    void on_subscription_state_change(const vsomeip::service_t    _service,
                                      const vsomeip::instance_t   _instance,
                                      const vsomeip::eventgroup_t _eventgroup,
                                      const vsomeip::event_t _event, const uint16_t _error)
    {
        (void)_service;
        (void)_instance;
        (void)_eventgroup;
        ...
        else
        {
            ...
            VSOMEIP_ERROR << std::hex << app_->get_client()
                          << " : on_subscription_state_change: for service " << std::hex << _service
                         ...
                         bool _subscribed)
    {
        ...
        VSOMEIP_DEBUG << "[" << std::setw(4) << std::setfill('0') << std::hex
                      ...
                      << "Client: " << _client << " subscribed, now have " << std::dec
                      << subscribers_.size() << " subscribers. Expecting " << std::dec
    ...
    void on_message(const std::shared_ptr<vsomeip::message>& _message)
    {
        if (_message->get_message_type() == vsomeip::message_type_e::MT_NOTIFICATION)
        {
            ...
            VSOMEIP_DEBUG << "[" << std::setw(4) << std::setfill('0') << std::hex
                          ...
                          << std::setfill('0') << std::hex << _message->get_client() << "/"
                          << std::setw(4) << std::setfill('0') << std::hex
                          << _message->get_session() << "] from Service/Method [" << std::setw(4)
                          << std::setfill('0') << std::hex << _message->get_service() << "/"
                          << std::setw(4) << std::setfill('0') << std::hex << _message->get_method()
                          << "] (now have: " << std::dec
    ...
    {
        return std::all_of(other_services_received_notification_.cbegin(),
    ...
    void run()
    {
        ...
        for (const subscribe_notify_one_test::service_info& i :
             subscribe_notify_one_test::service_infos)
        {
            if ((i.service_id == service_info_.service_id
                 && i.instance_id == service_info_.instance_id)
                || (i.service_id == 0xFFFF && i.instance_id == 0xFFFF))
            ...
            VSOMEIP_DEBUG << "[" << std::hex << service_info_.service_id
                          << "] subscribing to Service/Instance/Eventgroup [" << std::setw(4)
                          << std::setfill('0') << std::hex << i.service_id << "/" << std::setw(4)
                          << std::setfill('0') << std::hex << i.instance_id << "/" << std::setw(4)
                          << std::setfill('0') << std::hex << i.eventgroup_id << "]";
        }
        ...
        {
            VSOMEIP_ERROR << "Subscription state handler check skipped: CallCount=" << std::dec
                          << subscription_state_handler_called_;
        }
    }
    ...
    void notify_one()
    {
        ...
        for (uint32_t i = 0; i < subscribe_notify_one_test::notifications_to_send; i++)
        {
            ...
            {
                VSOMEIP_DEBUG << "[" << std::setw(4) << std::setfill('0') << std::hex
                              << service_info_.service_id << "] Notifying client: " << client
                              << " : " << i + 1;
    ...
    void wait_for_stop()
    {
        ...
        if (app_->is_routing())
        {
            for (const auto& i : subscribe_notify_one_test::service_infos)
            {
                ...
                while (app_->is_available(i.service_id, i.instance_id, vsomeip::ANY_MAJOR,
                                          vsomeip::ANY_MINOR))
                {
...
TEST(someip_subscribe_notify_one_test, send_ten_notifications_to_service)
{
    subscribe_notify_one_test_service its_sample(
        subscribe_notify_one_test::service_infos[service_number], reliability_type);
}
...
int main(int argc, char** argv)
{
    ...
    if (argc < 3)
    {
        std::cerr << "Please specify a service number and event reliability type, like: " << argv[0]
                  << " 2 UDP" << std::endl;
