...
public:
          ...
          offer_thread_(std::bind(&second_address_test_service::run, this))
    {
        ...
        app_->register_state_handler(
            std::bind(&second_address_test_service::on_state, this, std::placeholders::_1));
        ...
        app_->register_message_handler(
            service_info_.service_id, service_info_.instance_id, service_info_.notify_method_id,
        ...
        app_->register_subscription_handler(
            ...
            std::bind(&second_address_test_service::subscription_handler, this,
...
private:
                              ...
                              bool _subscribed)
    {
        (void)_uid;
        (void)_gid;
        VSOMEIP_DEBUG << __func__ << ": client 0x" << std::hex << std::setw(4) << std::setfill('0')
                      << _client << ((_subscribed) ? " subscribed" : "unsubscribed")
                      << " on service.";
        client_subscribed_ = _subscribed;
        return true;
    }

    bool selective_subscription_handler(vsomeip::client_t _client, std::uint32_t _uid,
