...
public:
          ...
          run_thread_(std::bind(&application_test_client_availability::run, this))
    {
        ...
        app_->register_state_handler(std::bind(&application_test_client_availability::on_state,
        ...
        for (std::uint32_t i = 0; i < 16; i++)
        {
            vsomeip::service_t its_service =
                (i & 0x8) ? service_info_.service_id : vsomeip::ANY_SERVICE;
            vsomeip::instance_t its_instance =
                (i & 0x4) ? service_info_.instance_id : vsomeip::ANY_INSTANCE;
            vsomeip::major_version_t its_major =
                (i & 0x2) ? service_info_.major_version : vsomeip::ANY_MAJOR;
            vsomeip::minor_version_t its_minor =
                (i & 0x1) ? service_info_.minor_version : vsomeip::ANY_MINOR;
            app_->register_availability_handler(
                ...
                its_major, its_minor);
            VSOMEIP_DEBUG << "Registering: " << std::setfill('0') << std::hex << std::setw(4)
                          << its_service << "." << std::setw(4) << its_instance << "."
                          << std::setw(2) << (std::uint32_t)its_major << "." << std::setw(4)
                          << its_minor << "." << i;
        }
        app_->register_availability_handler(
            service_info_.service_id, service_info_.instance_id,
            std::bind(&application_test_client_availability::on_availability, this,
                      std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, 16),
            service_info_.major_version, vsomeip::DEFAULT_MINOR);
        VSOMEIP_DEBUG << "Registering: " << std::setw(4) << std::setfill('0') << std::hex
                      << service_info_.service_id << "." << std::setw(4) << std::setfill('0')
                      << std::hex << service_info_.instance_id << "." << std::setw(2)
                      << std::setfill('0') << std::hex << (std::uint32_t)service_info_.service_id
    ...
    void on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance,
                         bool _is_available, std::uint32_t _handler_index)
    {
    ...
    void run()
    {
        ...
        while (!app_->is_available(service_info_.service_id, service_info_.instance_id,
        ...
        for (std::uint32_t i = 0; i < 16; i++)
        {
            ...
            VSOMEIP_DEBUG << "Calling is_available: " << std::setw(4) << std::setfill('0')
                          << std::hex << its_service << "." << std::setw(4) << std::setfill('0')
                          << std::hex << its_instance << "." << std::setw(2) << std::setfill('0')
                          << std::hex << (std::uint32_t)its_major << "." << std::setw(4)
                          << std::setfill('0') << std::hex << its_minor;
            EXPECT_TRUE(app_->is_available(its_service, its_instance, its_major, its_minor));

            VSOMEIP_DEBUG << "Calling are_available: " << std::setw(4) << std::setfill('0')
                          << std::hex << its_service << "." << std::setw(4) << std::setfill('0')
                          << std::hex << its_instance << "." << std::setw(2) << std::setfill('0')
                          << std::hex << (std::uint32_t)its_major << "." << std::setw(4)
                          << std::setfill('0') << std::hex << its_minor;
            vsomeip::application::available_t are_available;
            EXPECT_TRUE(app_->are_available(are_available, its_service, its_instance, its_major,
                                            its_minor));
            bool found(false);
