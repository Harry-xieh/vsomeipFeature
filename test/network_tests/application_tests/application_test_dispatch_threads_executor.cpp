...
protected:
    void SetUp()
    {
        VSOMEIP_INFO << "Setting up test";
    }

    void TearDown()
    {
        VSOMEIP_INFO << "Tearing down";
    }
    ...
    int get_process_pid(std::string process_name)
    {
        ...
        if (file)
        {
            ...
            { // Check that fscanf successfully read one integer
...
TEST_F(someip_application_detach_dispatch_executor, dispatch_thread_detached_forcefully_stopped)
{
    struct shm_remove
    {
        shm_remove()
        {
            boost::interprocess::shared_memory_object::remove("SharedCV");
        }
        ~shm_remove()
        {
            boost::interprocess::shared_memory_object::remove("SharedCV");
    ...
    try
    {
...

        {
            ...
            EXPECT_EQ(data->cv.timed_wait(lock, timeout,
                                          [&] {
                                              return data->status_;
                                          }),
            ...
            { // If the process exists, the command will return 0
