...
public:
    ...
    bool init()
    {
        ...
        app_->register_message_handler(
            service_info_.service_id, service_info_.instance_id, service_info_.get_method_id,
            ...
            {
                {
                    ...
                    VSOMEIP_INFO << "\nSERVICE SIDE -> "
                                 << "Setting event (Length=" << std::dec << its_size << ")."
                                 << std::endl;
                    app_->notify(service_info_.service_id, service_info_.instance_id,
                                 service_info_.event_id, payload_);
                }

                notify_condition_.wait_for(
                    its_lock, std::chrono::seconds(climate_test::OFFER_CYCLE_INTERVAL));
            }
        }
    }
