...
TEST(load, No_element)
{
    ...
    ASSERT_TRUE(full_element.size() > 0)
        << "Full element size is zero after load, should be >0 since policy was loaded";

    // Compare full element greater than element
    ASSERT_GT(full_element.size(), empty_element.size())
        << "Full element is not greater than unit element";
}
...
TEST(load, policy_enabled_and_check_credentials)
{
    ...
    ASSERT_FALSE(its_manager->is_enabled())
        << "policies were loaded. check_credentials should be false";
    ...
    ASSERT_TRUE(its_manager->is_enabled())
        << "policies were loaded. check_credentials should be true";
}
