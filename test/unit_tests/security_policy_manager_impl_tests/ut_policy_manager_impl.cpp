...
{
    // Test pointer.
    std::unique_ptr<vsomeip_v3::policy_manager_impl> its_policy_manager(
...
TEST(security_policy_manager_test, is_client_allowed)
{
    ...
    ASSERT_TRUE(its_policy_manager->is_client_allowed(&its_client_struct, service_number,
                                                      instance_number, method_number, true));
    ...
    ASSERT_FALSE(its_policy_manager->is_client_allowed(nullptr, service_number, instance_number,
                                                       method_number, true));
    ...
    ASSERT_FALSE(its_policy_manager->is_client_allowed(&its_client_struct_default, service_number,
                                                       instance_number, method_number, true));
}
...
TEST(security_policy_manager_test, is_offer_allowed)
{
    ...
    // Test Method. Policy not enabled
    ASSERT_TRUE(its_policy_manager->is_offer_allowed(&its_client_struct, offer_service_number,
                                                     offer_instance_number));
    ...
    // Test Method. client is null. Expect !check_credentials.
    ASSERT_FALSE(
        its_policy_manager->is_offer_allowed(nullptr, offer_service_number, offer_instance_number));
    ...
    // Test method, expect !check_credentials which we set to true, so expect false.
    ASSERT_FALSE(its_policy_manager->is_offer_allowed(&its_client_struct_default,
                                                      offer_service_number, offer_instance_number));
}

TEST(security_policy_manager_test,
     store_sec_client_to_client_mapping_and_get_sec_client_to_clients_mapping)
{
    // Test pointer.
    std::unique_ptr<vsomeip_v3::policy_manager_impl> its_policy_manager(
        new vsomeip_v3::policy_manager_impl());
    ...
    ASSERT_FALSE(
        its_policy_manager->get_sec_client_to_clients_mapping(&its_client_struct, client_set));
    // reset port to 0
    its_client_struct.port = VSOMEIP_SEC_PORT_UNUSED;
...
TEST(security_policy_manager_test, store_client_to_sec_client_mapping)
{
    ...
    // Test method expect false since port != 0.
    ASSERT_FALSE(
        its_policy_manager->store_client_to_sec_client_mapping(client_number, &its_client_struct));
    // reset port to 0
    its_client_struct.port = VSOMEIP_SEC_PORT_UNUSED;
...
TEST(security_policy_manager_test, get_client_to_sec_client_mapping)
{
    ...
    ASSERT_FALSE(
        its_policy_manager->get_client_to_sec_client_mapping(client_number, its_client_struct));
...
// Test method.
#ifndef __QNX__
    ASSERT_TRUE(its_policy_manager->parse_policy(data_ptr_, data_size_, deserialized_uid,
                                                 deserialized_gid, its_policy));
#endif
...
#ifndef __QNX__
    ASSERT_FALSE(its_policy_manager->parse_policy(data_ptr2_, data_size2_, deserialized_uid,
                                                  deserialized_gid, its_policy));
#endif
}
...
// Test method.
#ifndef __QNX__
    ASSERT_TRUE(its_policy_manager->parse_uid_gid(data_ptr_, data_size_, deserialized_uid,
                                                  deserialized_gid));
#endif
...
// Test Method, expect false.
#ifndef __QNX__
    ASSERT_FALSE(its_policy_manager->parse_uid_gid(data_ptr2_, data_size2_, deserialized_uid,
                                                   deserialized_gid));
#endif
}
...
TEST(security_policy_manager_test, get_clients)
{
    ...
    // Add client to manager.
    ASSERT_TRUE(its_policy_manager->store_client_to_sec_client_mapping(client_number + 1,
                                                                       &its_client_struct));
...
TEST(security_policy_manager_test, get_policy_extension_path)
{
    ...
    tree_container_policy_extension.add_child("container",
                                              tree_container_policy_extensions_container);
