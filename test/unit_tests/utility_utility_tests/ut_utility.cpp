...
TEST(utility_test, get_message_size)
{
    ...
    std::array<vsomeip_v3::byte_t, array_size> byte_array_{
        serviceID_byte1, serviceID_byte2, methodID_byte1, methodID_byte2, length_byte1,
        length_byte2,    length_byte3,    length_byte4,   clientID_byte1, clientID_byte2,
        sessionID_byte1, sessionID_byte2, version_byte,   interface_byte, messageType_byte,
        returnCode_byte, payload_byte1,   payload_byte2};

    // Create an array to pass to bithelper.
    std::array<vsomeip_v3::byte_t, 4> uint32_array_{length_byte1, length_byte2, length_byte3,
                                                    length_byte4};

    // Getting size.
    std::uint32_t size_ =
        VSOMEIP_SOMEIP_HEADER_SIZE + bithelper::read_uint32_be(uint32_array_.data());
    ...
    ASSERT_EQ(its_utility->get_message_size(byte_array_.data(), byte_array_.size()),
              VSOMEIP_SOMEIP_HEADER_SIZE);

    // Create an array shorter than the minimum someip header.
    std::array<vsomeip_v3::byte_t, array_size_too_short> byte_array_too_short{serviceID_byte1,
                                                                              serviceID_byte2};
    // Check that 0 is returned for the expected length of the message.
    ASSERT_EQ(
        its_utility->get_message_size(byte_array_too_short.data(), byte_array_too_short.size()), 0);
}
...
TEST(utility_test, get_payload_size)
{
    ...
    // Check if function returns the uint32_t size_ we expect to receive, header + size passed by
    // the 4 length_bytes.
    ASSERT_EQ(its_utility->get_payload_size(byte_array_.data(), byte_array_.size()),
              payload_length);

    // Check if function returns 0, if length field we pass (0x0A) passed is greater than the array
    // size - 8
    ASSERT_EQ(its_utility->get_payload_size(byte_array_.data(),
                                            byte_array_.size() - VSOMEIP_SOMEIP_HEADER_SIZE),
              0);
    ...
    // Check that 0 is returned for the expected length of the message.
    ASSERT_EQ(
        its_utility->get_payload_size(byte_array_too_short.data(), byte_array_too_short.size()), 0);
}
...
TEST(utility_test, request_client_id)
{
    ...
    ASSERT_EQ(its_utility->request_client_id(its_config, client_name_, client_),
    ...
    ASSERT_EQ(its_utility->request_client_id(its_config, client_name2_, client2_),
              VSOMEIP_CLIENT_UNSET);
