...
TEST_F(routing_manager_ut_setup, DISABLED_set_routing_state_RS_SUSPENDED)
{
    ...
    // for this test
    its_manager->add_routing_info(service_, instance_, major_version_, minor_version_, ttl_,
                                  ip_address_, port_reliable, ip_address_, port_unreliable);

    // remote for on avail to be called as part of the del_routing_info call
    its_manager->add_routing_info(service2_, instance2_, major_version_, minor_version_, ttl_,
                                  ip_address_remote_, port_reliable, ip_address_remote_,
                                  port_unreliable);
